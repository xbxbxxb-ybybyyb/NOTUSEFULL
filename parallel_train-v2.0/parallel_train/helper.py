...
TaskMode = Enum('TaskMode', ('Actor', 'Task',))
...
def _validate_task_mode(task_mode):
    ...
    if isinstance(task_mode, str):
        if task_mode not in [t.name for t in TaskMode]:
            raise ValueError(f"task_mode must be in [Task, Actor]")
        task_mode = TaskMode[task_mode]
    else:
        raise ValueError(
            f"`task_mode` must be a str, "
            f"but it was {type(task_mode)}."
            f"\nFIX THIS preferably by passing a `TaskMode` instance as "
            f"the `task_mode` parameter."
        )
    return task_mode
...
def _validate_backend_wrapper(cls, task_mode):
    from .trainer import TaskMode
    from .backend import BackendTaskWrapper, BackendWrapper
    if task_mode == TaskMode.Actor:
        if not issubclass(cls, BackendWrapper):
            raise ValueError(
                f"in `Actor` mode, backend must be a subclass of BackendWrapper, but is was {cls.__name__}."
            )
    elif task_mode == TaskMode.Task:
        if not issubclass(cls, BackendTaskWrapper):
            raise ValueError(
                f"in `Task` mode, backend must be a subclass of BackendTaskWrapper, but is was {cls.__name__}."
            )
    else:
        raise ValueError(
            f"un supported task_mode."
        )
...
def get_backend_config(backend) -> type:
    from .backend import BackendConfigWrapper, BackendWrapper, BackendTaskWrapper
    class ConfigToBackend(BackendConfigWrapper):
        _backend = None
    ...
    if issubclass(backend, BackendWrapper):
        # Actor模式传递用户自定义Backend进BackendExecutor
        config = ConfigToBackend()
        ConfigToBackend._backend = backend
        return config
    elif issubclass(backend, BackendTaskWrapper):
        # Task模式传递模拟基类BackendWrapper进BackendExecutor，不改变源代码情况下，backend_cfg必传
        config = ConfigToBackend()
        ConfigToBackend._backend = BackendWrapper
        return config
