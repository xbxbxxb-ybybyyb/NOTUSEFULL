...
@dataclass
class TensorflowConfig(BackendConfigWrapper):
    @property
    def backend_cls(self):
...
class TensorflowBackend(BackendWrapper):
    def on_start(self, worker_group: WorkerGroup,
    ...
    def handle_failure(self, worker_group: WorkerGroup,
                       failed_worker_indexes: List[int],
                       backend_config: BackendConfigWrapper):
...
@PublicAPI(stability="beta")
@dataclass
class TorchConfig(BackendConfigWrapper):
...
class TorchBackend(BackendWrapper):
