...
from .backend import BackendWrapper, BackendTaskWrapper
...
class TrainerWrapper(Trainer):
    def __init__(
            self,
            backend: Union[str, BackendWrapper, BackendTaskWrapper],
            task_mode: [str],
        ...
        super(TrainerWrapper, self).__init__(backend=self._backend_cfg, num_workers=self._ray_params.num_workers,
    ...
    def start(self, initialization_hook: Optional[Callable[[], None]] = None):
        ...
        if self._task_mode == TaskMode.Actor:
            super(TrainerWrapper, self).start(initialization_hook)

    def shutdown(self):
        if self._task_mode == TaskMode.Actor:
            super(TrainerWrapper, self).shutdown()
    ...
    def parallel_run(self, data_params: Dict, model_params: Dict, callbacks: Optional[List[TrainingCallback]] = None,
        ...
        """
        def init_task_session(world_rank, local_rank, world_size):
        ...
        def train_fun(config):
            ...
            if not actor_mode:
                ...
                init_task_session(config['world_rank'], config['local_rank'], config['world_size'])
            backend.prepare_data(single_data_param)
            backend.train(model_params)

        is_actor_mode = True if self._task_mode == TaskMode.Actor else False
        ...
        if self._task_mode == TaskMode.Actor:
            for data_param in data_params_group:
                yield self.run(train_func=train_fun,
                               config={'backend': self._backend_train_cls,
                                       'single_data_param': data_param,
                                       'is_actor_mode': is_actor_mode},
                               callbacks=callbacks, checkpoint=checkpoint)
        elif self._task_mode == TaskMode.Task:
            ...
            training_futures = [
                ...
                for index, data_param in enumerate(data_params_group)]
