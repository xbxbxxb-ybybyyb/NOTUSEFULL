...
import xfactor.test.symbol_team.DataAPI.GetTradingDay as GetTradingDay

root_013542 = "/data/group/800080/Apollo/AlphaDataBase"
panel_minute_data_path = "/data/group/800080/PanelMinDataForZT"
database_dir = root_013542
barra_dir = "/data/user/666889/Apollo/BarraFactors/"
complete_stock_list_path = root_013542 + "/CompleteStockList.csv"
temp_root = '/data/group/800002/alpha_factor/test/depend_data/fix/excess_return/'
...
def get_panel_min_data(item, start_date, end_date, stock_list=None, dir_path=panel_minute_data_path, forward_adj=True,
    ...
    df = pd.DataFrame()
    import psutil
    the_p = [i for i in psutil.process_iter() if 'python3' in i.name()][0].pid
    p = psutil.Process(the_p)
    data_collect_list = []
    import gc
    gc.disable()
    for i, i_date in enumerate(year_month_list):  # 逐月读取panel data
        ...
        i_df = i_df.reindex(columns = stock_list)
        data_collect_list.append(i_df)
        # print(i)
    gc.enable()
    df = pd.concat(data_collect_list, axis=0, sort=False)
...
def get_factor_excess_ret_kline(start_date,end_date,factor_list):
    ...
    for year_month in year_month_list:
        ...
        try:
            ...
            common_factors = [i for i in factor_list if i in df.columns]
            df_select = df.loc[:,common_factors]
            print('{} not covered in database'.format(miss_factor))
