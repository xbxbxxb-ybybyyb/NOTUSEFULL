...
class CNNBase:
  ...
  def inference(self, input_tensor, train, regularizer):
      ...
      with tf.compat.v1.variable_scope('layer1-conv1'):
         #初始化权重conv1_weights为可保存变量，大小为5x5,3个通道（RGB），数量为32个
          conv1_weights = tf.compat.v1.get_variable("weight",[5,5,c,32],initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.1))
  ...
  #-----------------------第二层----------------------------
      with tf.compat.v1.variable_scope("layer3-conv2"):
          ...
          conv2_weights = tf.compat.v1.get_variable("weight",[5,5,32,128],initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.1))
          conv2_biases = tf.compat.v1.get_variable("bias", [128], initializer=tf.compat.v1.constant_initializer(0.0))
          conv2 = tf.nn.conv2d(input=pool1, filters=conv2_weights, strides=[1, 1, 1, 1], padding='SAME')
          relu3 = tf.nn.relu(tf.nn.bias_add(conv2, conv2_biases))
          print('relu3')
          print(relu3)
  ...
  #-----------------------第三层----------------------------
      ...
      #    conv3_weights = tf.compat.v1.get_variable("weight",[c,c,64,128],initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.1))
  ...
  #-----------------------第四层----------------------------
      ...
      with tf.compat.v1.variable_scope("layer7-conv4"):
          conv4_weights = tf.compat.v1.get_variable("weight",[c,c,128,128],initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.1))
      ...
      with tf.compat.v1.name_scope("layer8-pool4"):
          ...
          p4shape = pool4.get_shape().as_list()
          print(p4shape)
          lsz = 1
          for p in range(1, len(p4shape)):
            lsz = lsz * p4shape[p]
          nodes = lsz
          #nodes = 8192
          #nodes = 2*6*128
  ...
  def Run(self, previous_model):
    ...
    with tf.compat.v1.Session() as sess:
        ...
        if (previous_model == None):
            ...
            previous_id  = int(previous_model.split('-')[-1])
            saver.restore(sess, previous_model)
        ...
        for epoch in range(n_epoch):
            ...
            #保存模型及模型参数   
            saver.save(sess,model_path,global_step=(epoch + previous_id))
