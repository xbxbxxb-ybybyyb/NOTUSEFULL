...
def chose_hdfs():
    try:
        hdfs = pa.hdfs.connect('168.9.65.61', 8020)
        hdfs.ls('/htdata/mdc/histData/')
    except:
...
@ray.remote
def store_data(hdfs, stock, start_date, end_date):
    mdp = MarketData()
    ...
    t1 = time.time()
    try:
        pq.write_to_dataset(table, '/analysis/xquant/013150/mkt_test1', partition_cols=['HTSCSecurityID', 'MDDate'],
                            partition_filename_cb=lambda x: '-'.join(
                                list(map(str, x))) + '.parquet', filesystem = hdfs)
        ...
        pq.write_to_dataset(table, '/analysis/xquant/013150/mkt_test1', partition_cols=['HTSCSecurityID', 'MDDate'],
                            partition_filename_cb=lambda x: '-'.join(
                                list(map(str, x))) + '.parquet', filesystem = hdfs)
    print("write_to_dataset hdfs:", time.time()-t1, df.shape)
...
if __name__ == "__main__":
    for month in range(1,12):
        ...
        hdfs = chose_hdfs()
        tasks = [store_data.remote(hdfs, stock, start_date, end_date) for stock in stock_list]
        ray.get(tasks)
