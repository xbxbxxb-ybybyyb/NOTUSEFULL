...
if xquantEnv == 0:
    ...
    if testEnv == 40:
        ...
        TRANSACTION_CONFIG = {
            "ApplicationConfig": "IStationDubboService",
            "IStation_service_interface": 'com.huatai.fast.business.manager.api.IManagerDubboService',
            "IP_CONFIG": "168.63.65.196:2182"
        }

elif xquantEnv == 1:
    version = "2.0.0"
    DUBBO_CONFIG = {
        "DUBBO_CONFIG_IP": '168.15.225.12:2181, 168.15.225.13:2181, 168.11.225.12:2181, 168.11.225.13:2181, 168.19.225.14:2181',
    ...
    TRANSACTION_CONFIG = {
        "ApplicationConfig": "IStationDubboService",
        "IStation_service_interface": 'com.huatai.fast.business.manager.api.IManagerDubboService',
        "IP_CONFIG": "168.6.5.21:2181,168.6.5.22:2181,168.8.189.50:2181,168.8.189.51:2181"
    }
...
IStationDubboService_dict = {}


def get_jdd_cache():
    if os.path.exists("/etc/.config/octopus/__jurisdictionData_config.py"):
        try:
            p = psutil.Process()
            if 'raylet' in ",".join(p.cmdline()):
                return json.loads(
                    open("/etc/.config/octopus/__jurisdictionData_config.py").readlines()[0].split("=")[1].strip())
        except:
            print("【Waring】__jurisdictionData_config.py error!")


def get_xqconfig_cache():
    if os.path.exists("/etc/.config/octopus/__xquant_config.py"):
        try:
            p = psutil.Process()
            if 'raylet' in ",".join(p.cmdline()):
                return json.loads(open("/etc/.config/octopus/__xquant_config.py").readlines()[0].split("=")[1].strip())
        except:
            print("【Waring】__xquant_config.py error!")
...
def get_userid(key=None):
    ...
    elif sysFlag == "big_data":
        # get xquantConfig
        user_prop = get_xquantConfig()
        user_id = user_prop["userAccount"]
        return user_id
    else:
        #print("【Warning】当前系统没有user_id！")
        return
...
def get_xquantConfig():
    ...
    if sysFlag == "big_data":
        ...
        xquantId = get_userid("xquantId")
        if not xquantId:
            raise Exception("未获取到xquantId!")
        json_str = get_xqconfig_cache()
        if json_str:
            return json_str
        userAccount = get_userid()
        if not userAccount:
        ...
        try:
            if not xquantConfig_user_provider_dict.get(c_name):
                ...
                jurisdictionData_config = ApplicationConfig(DUBBO_APPLICATIONCONFIG_UTILS)
                config_registry = ZookeeperRegistry(DUBBO_CONFIG_IP, jurisdictionData_config)
                xquantConfig_user_provider_dict[c_name] = DubboClient(xquantConfig_service_interface, config_registry,
                                                                      version=version)
            # 调用dubbo接口
            json_dict = {"request": {"userAccount": str(userAccount), "xquantId": xquantId}}
            json_str = json.dumps(json_dict)
...
def set_istationDubboService():
    global IStationDubboService_dict
    c_name = "IStationDubboService"
    if not IStationDubboService_dict:
        if sysFlag == "xquant":
            try:
                IStation_config = ApplicationConfig(TRANSACTION_CONFIG["ApplicationConfig"])
                IStation_service_interface = TRANSACTION_CONFIG["IStation_service_interface"]
                IStation_registry = ZookeeperRegistry(TRANSACTION_CONFIG["IP_CONFIG"], IStation_config)
                IStationDubboService = DubboClient(IStation_service_interface, IStation_registry)
                IStationDubboService_dict[c_name] = IStationDubboService
            except:
                raise Exception("IStationDubboService 调用dubbo接口失败!")
    return IStationDubboService_dict["IStationDubboService"]
        ...
        if sysFlag == "xquant":
            ...
            try:
                ...
                user_provider_create_library_dict[c_name] = DubboClient(service_interface_create_library,
                                                                        factordata_registry, version=version)
            except:
    ...
    if not query_wind_from_oracle_dict:
        if sysFlag == "xquant":
            ...
            try:
                ...
                query_wind_from_oracle_dict[c_name] = DubboClient(service_interface_query_wind, factordata_registry,
                                                                  version=version)
            except:
    ...
    if not query_gogoal_from_oracle_dict:
        if sysFlag == "xquant":
            ...
            try:
                ...
                query_gogoal_from_oracle_dict[c_name] = DubboClient(service_interface_query_gogoal, factordata_registry,
                                                                    version=version)
            except:
        ...
        if sysFlag == "xquant":
            try:
                ...
                operation_registry = ZookeeperRegistry(
                    "168.6.5.21:2181,168.6.5.22:2181,168.8.189.51:2181,168.8.189.50:2181",
                    operation_config)
                ZlGoalDailyTargetDubboService_dict[c_name] = DubboClient(operation_service_interface,
                                                                         operation_registry, version=version)
            except:
