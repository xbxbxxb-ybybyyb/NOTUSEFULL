...
if sysFlag in ['big_data', 'outside']:
    import pymysql1 as mysql
else:
    import MySQLdb1 as mysql
try:
    from .storageConfig import get_sql_config,pool_config
    from .utils import get_engine
except:
    pass
from FactorProvider.conf.DubboConf import set_user_provider_db_pool
import threading
...
def OPMysql(user):
    ...
    try:
        ...
        if platform.system() == 'Windows':
            ...
            raise Exception('please install pycrypto: pip install pycrypto')
...
def get_pool(user):
    ...
    user_provider_db_pool = set_user_provider_db_pool()
    judge_bool = True
    for i in range(10):
        if not (i + 1) % 3:
            print("mysql连接数已达上限，尝试再次创建连接池！")
        if user not in ['xquant_cusdata']:
...
class DML_mysql:
    ...
    def df_to_mysql(self,df,tablename,c_name):
        ...
        try:
            ...
            traceback.print_exc()
            print("【ERROR】sql语句执行失败!若是更新非高频个人因子操作，可能会有数据丢失，请排查错误并重新更新！" )
    ...
    #@utils_set_timeout(90, "sql查询超时！请缩短查询区间！")请勿在ray中使用
...
def sql_connect(user):
    try:
        ...
        if platform.system() == 'Windows':
            ...
            raise Exception(
                'please install pycrypto: pip install pycrypto')
    conn = mysql.connect(host=sql_config['host'], user=sql_config['user'],
