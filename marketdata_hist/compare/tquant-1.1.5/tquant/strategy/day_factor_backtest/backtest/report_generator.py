...
def generate_plot(df, plot_name, x_label, y_label,
                  ...
                  width=img_width * inch, height=img_height * inch,
                  **kwargs):
...
def generate_pdf(pickle_name, corr_information=None, rolling_window=20):
    ...
    g_er_by_month = generate_ts_heatmap_by_month_plot(er_use * 100,
                                                      ...
                                                      height=img_height * 90.0)  #
    seg_ret_use['Avg %d Period' % rolling_window] = seg_ret_use[seg_er_col].rolling(
        rolling_window).mean()  # 滚动平均，seg_er_col最大收益层的超额收益
    g_seg_ret_bar = generate_bar_line_plot(df=seg_ret_use, title='{} Excess Return'.format(seg_er_col),
                                           bar_column=seg_er_col[0],
    ...
    if compare_style_flag:
        ...
        add_avg_str = '\n All History Correlation: %s: %s   %s: %s' % (
    ...
    h_sum = Paragraph('Backtest Information', heading_size)
    ...
    h_seg_ret_table_after_cost = Paragraph('Portfolio Spread Statistics - After Cost%s' % (add_seg_by_ind),
                                           heading_size)

    h_return_by_year = Paragraph('Excess Return by Year', heading_size)
    ...
    h_excess_return_industry_sliced = Paragraph('Segment Excess Return by Industry Performance' + add_t_cost,
                                                heading_size)

    h_sample_ret_stat = Paragraph('Random sampling analysis of top excess return', heading_size)
