...
def sample_data_aux(cur_df, tick_interval_seconds, security_type):
    # Step 1 保留时间信息
    ...
    # step7 重采样
    df_resample = cur_df.resample(rule=str(tick_interval_seconds) + 's', closed='left', label='right').agg(
        how_method_dict)
...
def realtime_data_aux(cur_df, tick_interval_seconds, security_type):
    ...
    cur_df.set_index(['DateTime'], drop=True, inplace=True)
    # Step 1 保留时间信息
    resample_tick_interval_seconds = tick_interval_seconds
    ...
    # how 执行不了，就用agg.()
    try:
        df_resample = cur_df.resample(rule="{}S".format(resample_tick_interval_seconds), closed='right',
...
def sample_transaction_data_aux(df, tick_interval_seconds):
    ...
    how_method_dict = {
        'TradeMoney': ['std', 'mean', 'sum', get_outlier_sum, get_squared_sum, get_quantile_1_8, get_quantile_7_8],
        'TradeQty': ['std', 'mean', 'sum', get_outlier_sum, get_squared_sum, get_quantile_1_8, get_quantile_7_8],
    }
    # Step 1 保留时间信息
    try:
        df_resample = df.resample(rule=str(tick_interval_seconds) + "s", closed='left', label='right').agg(how_method_dict)
    except:
        df_resample = df.resample(rule=str(tick_interval_seconds) + "s", how=how_method_dict, closed='left', label='right')
    if df_resample.empty:
        return df_resample
...
def sample_transaction_data(df, tick_interval_seconds):
    ...
    df_buy_deal = sample_transaction_data_aux(df_buy_deal, tick_interval_seconds)
    df_buy_deal.rename(columns={"TradeMoney": "BuyMoney", "TradeQty": "BuyQty"}, inplace=True)
    if not df_buy_deal.empty:
        df_buy_deal.drop(columns=["MDDate", "MDTime"], inplace=True)

    df_sell_deal = sample_transaction_data_aux(df_sell_deal, tick_interval_seconds)
    df_sell_deal.rename(columns={"TradeMoney": "SellMoney", "TradeQty": "SellQty"}, inplace=True)
