...
class OnRecvMarkertData(OnRecvMarkertDataBase):
    ...
    def __socket_recv(self, client_sock, client_addr):
        ...
        if actor_name not in self.actor_socket:
            ...
            self.socket_actor[client_sock] = actor_name
            ...
            for stock in self.actor_stock[actor_name]:
                ...
                for socket_client in self.stock_dict[stock]:
                    if actor_name != self.socket_actor[socket_client]:
...
def get_stream_data(user_account, data_input_mode = ["TICK_ROW"], security_type = "stock", playback_or_realtime ="playback", playback_date = "20201221", security_list=None, verbose = 0, auto_exit=True):
    """
