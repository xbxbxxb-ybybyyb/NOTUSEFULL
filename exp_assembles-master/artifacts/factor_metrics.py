...
from artifacts.factor_metrics_toolbox import *
try:
...
class FactorBacktest(object):
    ...
    def prepare_factor_label(self, dp, label="Label", dataset_type="train", use_normalized=False):
        if dataset_type == "train":
            dataset = dp.train_dataset
        elif dataset_type == "valid":
            dataset = dp.valid_dataset
        else:
            dataset = dp.test_dataset
        assert len(dataset.feature_df_list) == len(dataset.label_df_list)

        factor_label_list = []
        for i, (feature, label) in enumerate(zip(dataset.normalized_data_list, dataset.label_df_list)):
