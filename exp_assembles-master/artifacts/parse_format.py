...
def parse_online_trade_records(trade_records):
    ...
    trade_records_df["orderStatus"] = trade_records_df["orderStatus"].apply(lambda x: order_status_map[str(x)])
    trade_records_df["side"] = trade_records_df["side"].apply(lambda x: side_map[int(x)])
...
def parse_ats_trade_records(df_order):
    ...
    :return:
    """
    sh_order_status_map = {"CANCELLED": "已撤", "DONE": "已成", "FILLING": "部撤","PARTIALLY_CANCELLED": "部撤", "REJECTED":"已拒"}
    sh_side = {"BID": "买", "ASK": "卖"}
    ...
    if df_order.empty:
        df_order = pd.DataFrame(columns = ["交易标的", "委托时间", "买卖方向", "委托状态", "委托价格","委托数量", "成交均价", "成交数量", "成交金额", "成交时间"])
...
def parse_signal_txt(signal_file):
    ...
    df_signal["PERIOD_BEGIN"] = pd.to_datetime(df_signal["PERIOD_BEGIN"])
    df_signal["DATETIME"] = df_signal["PERIOD_BEGIN"]
    df_signal["PERIOD_END"] = pd.to_datetime(df_signal["PERIOD_END"])
    ...
    if "PREDICT" not in df_signal.columns:
