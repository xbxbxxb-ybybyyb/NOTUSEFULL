...
class ExpArtifacts:
    ...
    def __init__(self, exp_name, exp_base = "/dfs/group/800657/exp_results/"):
        """
        :param exp_name:
        """
        ...
        if xquantEnv == 0:
            ...
            self.exp_path = os.path.join(exp_base, self.exp_name)
        ...
        self.reserve_and_long_value = None
        self.db = None
        self.fa = None
        self.ma = None

    def __get_mongodb(self):
        if not self.db:
            self.db = MongoDB()
        return self.db

    def __get_factordata(self):
        if not self.fa:
            self.fa = FactorData()
        return self.fa

    def __get_marketdata(self):
        if not self.ma:
            self.ma = MarketData()
        return self.ma

    def path_of_exp_version(self, version_alias = None):
        if not version_alias:
            ...
            self.__make_dirs(os.path.join(self.exp_path, version_alias))
            return os.path.join(self.exp_path, version_alias)
    ...
    def path_of_signal_process_save(self, evaluate_type="single_label_th_classify", version_alias=None, **kwargs):
        ...
        if evaluate_type == "single_label_th_classify":
            try:
                if not hasattr(self, 'signal_process_save_path_single_label_th_classify'):
                    self.signal_process_save_path_single_label_th_classify = "f\"" + os.path.join(self.exp_path,
                                                                                      version_alias,
                                                                                      "{label_name}-{symbol}",
                                                                                      "label_th@{'%.1f'%label_th1}-probs_up@{'%.2f'%probs_up}-probs_dw@{'%.2f'%probs_dw}/",
                                                                                      "signal_files_processed") + "\""
                return eval(self.signal_process_save_path_single_label_th_classify)
            except:
                print(traceback.print_exc())
                raise Exception("路径信息缺少初始化变量！")
        elif evaluate_type == "long_short_pred_th_classify":
            try:
                if not hasattr(self, 'signal_process_save_path_single_label_th_classify'):
                    self.signal_process_save_path_long_short_pred_th_classify = "f\"" + os.path.join(self.exp_path,
                                                                                                     version_alias,
                                                                                                     "{label_name}-{symbol}",
                                                                                                     "pred_th_up@{'%.2f'%pred_th_up}-pred_th_dw@{'%.2f'%abs(pred_th_dw)}/",
                                                                                                     "signal_files_processed") + "\""
                return eval(self.signal_process_save_path_long_short_pred_th_classify)
            except:
                print(traceback.print_exc())
                raise Exception("路径信息缺少初始化变量！请在path_of_signal_process_save方法中传入该参数！")
        else:
            raise Exception("evaluate_type仅支持以下范围：[single_label_th_classify, long_short_pred_th_classify]")
    ...
    def activate_version_to_save(self, model_config = None, version_alias = None, msg_info=None):
        ...
        # self.__check_and_save_config_version(model_config, version_alias, reg_type="model", msg_info=msg_info)
        ...
        self.signal_process_base_dir_long_short_pred_th_classify = os.path.join(self.exp_version_path,
                                                                             "{label_name}-{symbol}",
                                                                             "pred_th_up@{'%.2f'%pred_th_up}-pred_th_dw@{'%.2f'%abs(pred_th_dw)}/")

        self.signal_process_save_path_long_short_pred_th_classify = "f\"" + os.path.join(
            self.signal_process_base_dir_long_short_pred_th_classify,
            "signal_files_processed") + "\""
    ...
    def regist_config(self, config, config_alias, reg_type="strategy", msg_info=None):
        # 参数描述，中文英文或为None
        assert reg_type in ["strategy", "model"], "参数类型必须为strategy或model"
        config_c = deepcopy(config)
        version = self.__check_and_save_config_version(config_c, config_alias, reg_type="strategy", msg_info=msg_info)
        return version
        ...
        if "params_jsonstr_extra" in df_v.columns.tolist():
            ...
            if params_jsonstr_extra:
                ...
                for key in list(params_extra.keys()):
                    ...
                    # 剔除较长字段时只遍历了两层
                    if len(key_lst) == 1:
                        params[key_lst[0]] = params_extra[key]
                    elif len(key_lst) == 2:
    ...
    def __remove_reserve_and_long_value(self, ddict, limit_len=10):
        # 预设参数key不参与版本号计算
        reserve_fields = ["factor_name_list", "tagger_name_list", "sysbol_list"]
        # 暂时只校验字典的前两层，如果为列表的值内有字典元素则不做剔除
        reserve_and_long_dct = {}
        for key1 in list(ddict.keys()):
            if key1 in reserve_fields:
                reserve_and_long_dct[key1] = ddict[key1]
                del ddict[key1]
            elif isinstance(ddict[key1], dict):
                for key2 in list(ddict[key1].keys()):
                    if key2 in reserve_fields:
                        uk = key1 + "|" + key2
                        reserve_and_long_dct[uk] = ddict[key1][key2]
                        del ddict[key1][key2]
                    # 如果值类型为float或int等则len()会报错
                    # 如果值的长度大于给定值，则把字段中的该键值对删除，添加到long_value_d
                    elif isinstance(ddict[key1][key2], list):
                        # 如果为列表的值内有字典元素则不做剔除
                ...
                if len(ddict[key1]) >= limit_len:
                    reserve_and_long_dct[key1] = ddict[key1]
                    del ddict[key1]
        return ddict, reserve_and_long_dct
    ...
    def __save_params_json(self, save_path):
        ...
        if self.reserve_and_long_value:
            with open(os.path.join(save_path, "params_jsonstr_extra.json"), "w") as fp:
                json.dump(self.reserve_and_long_value, fp)

    def __check_and_save_config_version(self, config_dict, version_alias, reg_type="model", msg_info=None):
        """
        :param config_dict:
        :return:
        """
        ...
        config_dict_v, reserve_and_long_value = self.__remove_reserve_and_long_value(config_dict_c, 10)
        if reg_type == "model":
            self.reserve_and_long_value = reserve_and_long_value
        version_new = self.__get_version_by_config(config_dict_v.copy())
        json_df = self.__get_mongodb().load_configs()
        if json_df.empty:
            version_alias_list = []
            version_list = []
        else:
        ...
        if version_alias in version_alias_list:
            ...
            if reserve_and_long_value:
                params_jsonstr_extra = json.dumps(reserve_and_long_value, sort_keys=True, separators=(",", ":"))
            else:
            ...
            self.__get_mongodb().exp_params_todb(exp_id=self.exp_name, version_alias=version_alias,
                                    hash256=version_new, info=msg_info,
                                    exp_type=exp_type,
    ...
    def model_factorlist_save(self, overwrite=False):
        factor_name_list = self.get_target_key(self.model_config_dict, "factor_name_list")
        if not factor_name_list:
            raise Exception("model_config中因子列表为空，请给model_config赋值factor_name_list！")
        model_save_path = self.model_save_path
        self.__make_dirs(model_save_path)
        if not os.path.exists(os.path.join(model_save_path, "factors.csv")):
            model_save_and_evaluate.save_factor_list(factor_name_list, model_save_path)
        else:
            if not overwrite:
                raise Exception("factors.csv已存在！{}".format(model_save_path))
            else:
                date = time.strftime("%Y%m%d%H%M%S")
                bak_path = os.path.join(model_save_path, "{}_factors.csv".format(date))
                print(f"将重写因子文件，原本的因子列表已备份为：{bak_path}。")
                model_save_and_evaluate.save_factor_list(factor_name_list, model_save_path)


    def model_file_save(self, model_obj, mode, overwrite=False, extra_name = ""):
        """
        :param model_obj:
        :param mode:
        :param overwrite:
        ...
        if not exists_flag:
            ...
            model_save_and_evaluate.save_model_and_factor_list(factor_name_list, model_obj, mode, model_save_path, WSIZE,FNUM)
        else:
            if overwrite == True:
        ...
        self.__get_mongodb().user_event_todb(exp_id=self.exp_name, version_alias=self.version_alias,
    ...
    def model_signal_save(self, label_name, symbol, tm_values, yhat_values, y_values,
        ...
        sig_df = model_save_and_evaluate.generate_signal_without_class(symbol_name = symbol,
                                                                       t_values=tm_values, yhat_values=yhat_values,
                                                                       y_values=y_values,
    ...
    def model_signal_process_single_label_th_classify(self, label_name, symbol, label_th1, probs_up, probs_dw,
        ...
        # 五分类转换
        sig_df, table1_json, table2_json = model_save_and_evaluate.model_signal_process_single_label_th_classify(signal_df,
                                                                                                           label_th1,
                                                                                                           label_th2,
                                                                                                           probs_up,
                                                                                                           probs_dw,
    ...
    def model_signal_process_long_short_pred_th_classify(self, label_name, symbol, pred_th_up, pred_th_dw, njobs=None):
        assert self.is_activated, "要存储数据必须先调用activate_version_to_save方法，指定实验版本号！"
        signal_df = self.model_signal_load(self.version_alias, label_name, symbol)
        signal_process_base_dir = eval(self.signal_process_save_path_long_short_pred_th_classify)
        self.__make_dirs(signal_process_base_dir)
        # 五分类转换
        sig_df = model_save_and_evaluate.model_signal_process_long_short_pred_th_classify(
            signal_df,
            pred_th_up,
            pred_th_dw,
            symbol=symbol,
        ...
        print(
            "########### pred_th_dw:{}  pred_th_dw:{} ###############".format(pred_th_up, pred_th_dw))
        print("signal_process_base_dir:", signal_process_base_dir)
        self.__get_mongodb().user_event_todb(exp_id=self.exp_name, version_alias=self.version_alias,
                                event_type=inspect.currentframe().f_code.co_name,
                                collection="-", operate="insert",
                                params={"label_name": label_name, "symbol": symbol, "pred_th_up": pred_th_up,
                                        "pred_th_dw": pred_th_dw})
        return True

    def model_signal_evaluation_single_label_th_classify(self, label_name, symbol, label_th=1.1,
        ...
        df_th1, df_th2 = model_save_and_evaluate.model_signal_evaluation_acc_table_daily(Y_test, Y_test_pred, T_test,
                                                                                  label_th=label_th,
                                                                                  start_date=start_date,
                                                                                  end_date=end_date,
                                                                                  pred_th_abs_limits=pred_th_abs_limits)
        # 信号评价数据入mongoDB
        self.__get_mongodb().signal_evaluation_data_todb(exp_id=self.exp_name, version_alias=self.version_alias,
    ...
    def backtest_trade_file_save(self, strategy_name, strategy_config_alias, signal_process_base_dir, trade_records_df):
        ...
        signal_process_path_split = signal_process_base_dir.split("/")
        exp_version_path_split = self.exp_version_path.split("/")
        label_name = signal_process_path_split[len(exp_version_path_split)].split("-")[0]
        if not trade_records_df.empty:
            dates = sorted(set(trade_records_df["tradeDate"].tolist()))
            for date in dates:
                df_date = trade_records_df[trade_records_df["tradeDate"] == date]
                df_date.to_parquet(os.path.join(trade_save_path, f"{date}_trade_records.parquet"))
                df_date["strategy_name"] = strategy_name
                df_date["strategy_config_alias"] = strategy_config_alias
                df_date["label_name"] = label_name
                self.__get_mongodb().online_trasaction_data_todb(df_date)
            return True
    ...
    def backtest_trade_file_load(self, strategy_name, strategy_config_alias, signal_process_base_dir, start_date,
        ...
        print("trade_records_save_path:", trade_save_path)
        self.__make_dirs(trade_save_path)
        dates = self.__get_factordata().tradingday(str(start_date).replace("-", ""), str(end_date).replace("-", ""))
        result_list = []
        for date in dates:
    ...
    def backtest_trade_evaluation(self, strategy_name, strategy_config_alias, signal_process_base_dir,
                                  trade_records_df, is_operation=False):
        assert self.is_activated, "要存储数据必须先调用activate_version_to_save方法，指定实验版本号！"
        trade_save_path = signal_process_base_dir.replace("signal_files_processed",
                                                          f"{strategy_name}-{strategy_config_alias}")
        ...
        if not trade_records_df.empty:
            ...
            result_list = []
            for date in dates:
                trade_summary = backtest_save_and_evaluate.backtest_trade_evaluation(
            ...
            stock = result_df.iloc[0]["symbol"]
            date_ = dt.datetime.strptime(date, "%Y-%m-%d").strftime("%Y%m%d")
            daily_market_df = self.__get_marketdata().get_data_by_date("Stock", stock, date_)
            max_sell_volume = result_df.iloc[0]["sell_qty"]
            initial_price = daily_market_df["PreClosePx"].iloc[0]
            bottom_position_max = max_sell_volume * initial_price
            result_df_db = result_df.copy()
            result_df_db["yield"] = result_df_db["pure_pnl_no_pos"] - result_df_db["pnl_only_pos"]
            result_df_db["bottom_position_max"] = abs(bottom_position_max)

            self.__get_mongodb().backtest_evaluation_data_todb(exp_id=self.exp_name, version_alias=self.version_alias,
                                                  symbol=symbol, strategy_name=strategy_name,
                                                  strategy_alias=strategy_config_alias,
                                                  backtest_evaluation_type="T0",
                                                  condition=condition,
                                                  metrics=result_df_db,
    ...
    def backtest_plot_signal_only(self, signal_df_day, plot_save_dir="./"):
        ...
        :return:
        """
        fig_signal1 = backtest_save_and_evaluate.backtest_plot_signal_only(signal_df_day)
    ...
    def backtest_plot_signal_trade(self, signal_df_day, trade_records_df_day, ma_df_day=None, plot_save_dir="./"):
        ...
        if not isinstance(ma_df_day, pd.DataFrame):
            ma_df_day = self.__get_marketdata().get_data_by_date("STOCK", symbol, date.replace("-", ""))
        if ma_df_day.empty:
            raise Exception("ma_df_day为空DataFrame.")
        self.__make_dirs(plot_save_dir)
        fig_signal1, fig_signal2 = backtest_save_and_evaluate.backtest_plot_signal_trade(signal_df_day, trade_records_df_day,
        ...
        with open(os.path.join(plot_save_dir, "plot_signal_trade_{}.html".format(date)), "a") as f:
            ...
            print("save signal&trade plot file ok! {}".format(os.path.join(plot_save_dir, "plot_signal_trade_{}.html".format(date))))
        self.__get_mongodb().user_event_todb(exp_id=self.exp_name, version_alias=self.version_alias,
                                event_type=inspect.currentframe().f_code.co_name,
                                collection="-", operate="plot",
                                params={"-": "plot"})
    ...
    def operation_backtest_plot_signal_trade(self, strategy_name, strategy_config_alias,
                                             signal_df_day, trade_records_df_day,
                                             ma_df_day=None, plot_save_dir="./",
                                             is_operation=False):
        date = str(signal_df_day["PERIOD_BEGIN"].iloc[0])[:10]
        symbol = signal_df_day["SYMBOL"].iloc[0]
        ...
        if ma_df_day.empty:
            raise Exception("ma_df_day为空DataFrame.")
        self.__make_dirs(plot_save_dir)
        fig_signal1 = backtest_save_and_evaluate.backtest_plot_signal_trade_db(signal_df_day, trade_records_df_day,
                                                                                         ma_df_day, plot=False,
...
#             print("save signal&trade plot file ok! {}".format(os.path.join(plot_save_dir, "plot_signal_trade_{}.html".format(date))))
        ...
        file_name = "plot_signal_trade_{}.html".format(date)
        plot_save_path = os.path.join(plot_save_dir, "plot_signal_trade_{}.html".format(date))
        res = self.__get_mongodb().UploadFile(file_name, plot_save_path)
        if is_operation:
            data_type = "online"
        else:
            data_type = "offline"
        self.__get_mongodb().plot_uniquekey_todb(stock=symbol, date=date.replace("-", ""), strategy_name=strategy_name,
                                 strategy_config_alias=strategy_config_alias, data_type=data_type,
                                 uniquekey=res.text, source='trade')
        self.__get_mongodb().user_event_todb(exp_id=self.exp_name, version_alias=self.version_alias,
                                event_type=inspect.currentframe().f_code.co_name,
                                collection="-", operate="plot",
                                params={"-": "plot"})
    ...
    def load_exp_params(self, version_alias=None, hash256=None):
        df = self.__get_mongodb().load_configs(version_alias, hash256, params_jsonstr=None, params_jsonstr_extra=None)
        return df

    def load_user_event(self, exp_id=None, version_alias=None, operate=None, event_type=None, collection=None):
        df = self.__get_mongodb().load_user_event(exp_id, version_alias, operate, event_type, collection)
        return df

    def load_signal_evaluation_data(self, exp_id=None, symbol=None, version_alias=None, evaluation_type=None):
        df = self.__get_mongodb().load_signal_evaluation_data(exp_id, symbol, version_alias, evaluation_type)
        return df
    ...
    def load_backtest_evaluation_data(self, exp_id=None, symbol=None, version_alias=None,
        ...
        df = self.__get_mongodb().load_backtest_evaluation_data(exp_id, symbol, version_alias, backtest_evaluation_type, is_operation)
        return df
