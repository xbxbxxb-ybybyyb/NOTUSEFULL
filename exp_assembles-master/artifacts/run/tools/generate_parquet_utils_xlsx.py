...
def start_generate_percent_signal(exp_list, symbol_list, start_date, end_date, l3_flag=False):
    ...
    for label_name, exp_name, version_alias in exp_list:
        ...
        try:
                ...
                for path in excel_path_list:
                    ...
                    for win_limit in win_limits[1:]:
                        for loss_limit in loss_limits:
                            ...
                            #     tasks.append(task)
                            remote_func = ray.remote(calc_save_winloss_func_pred00)
                            task = remote_func.remote(symbol_name, source_signal_df, pred00_file_path,
                                                      local_mode=True, target_type=target_type,
                                                      t_sta="09:33:00",
                                                      win_limits=[win_limit],
                                                      loss_limits=[loss_limit])
            ...
            #     pattern = "\d{6}\.SZ|\d{6}\.SH"
            #     res = re.findall(pattern, task_id)
            #     if res:
            #         symbol_name = res[0]
            #     else:
            #         print(f"Warning：{task_id}没有匹配到股票代码，未存储数据。")
            #         continue
            #     if not results_df.empty:
            #         save_and_append_parquet(symbol_name, results_df, task_id, overwrite_col="DATE")
    ...
    for label_name, exp_name, version_alias in exp_list:
        ...
        try:
            ...
            for task_id in tasks.keys():
                ...
                save_winloss_data_xlsx(results_df, sign_col="标的", output_path=res_out_path, overwrite_col="日期")
        except Exception as e:
            import traceback
            print(traceback.print_exc())
            print(e)
            pass
        print(f"{version_alias}-{start_date}-{end_date}-{len(symbol_list)}支标的，阈值百分比：{percent_list} 止盈止损存储耗时：{time.time() - t0} s")
        t2 = time.time()
        # TODO 计算固定阈值的止盈止损
        for th in th_list:
            try:
