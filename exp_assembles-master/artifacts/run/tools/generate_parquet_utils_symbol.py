...
from artifacts.utils import save_backtest_result, save_backtest_result_nolock
...
winloss_field_pattern = ["模型名称","日期", "标的", "止盈线", "止损线", "预测阈值", "信号质量加权", "总信号个数", "总止盈率", "总止损率", "总平率",
...
# def inner_func(model_name, symbol_name, pred00_df, pred_th, date_list,
#                win_limit, loss_limit, file_path):
#     # print(f"开始计算：{symbol_name}-{pred_th}-{win_limit}-{date_list}")
#     df_list = []
#     for date in date_list:
#         df_pred00_p = pred00_df.filter(pl.col("DATE") == date)
#         if df_pred00_p.shape[0] == 0:
#             continue
#         df_p = find_stop_new(df_pred00_p, float(win_limit), float(loss_limit), pred_th,
#                              model_name=model_name, symbol_name=symbol_name, date=date)
#         df_list.append(df_p)
#     if df_list:
#         res_df = pd.concat(df_list)
#         # print(res_df)
#         save_backtest_result(res_df, symbol_name, overwrite_col="index", output_path=file_path)
...
def inner_func0(model_name, symbol_name, pred00_df, percent, date_list,
    ...
    if df_calc_list:
        ...
        save_backtest_result(df_calc, symbol_name, overwrite_col="日期",
                             output_path=file_path)

def start_generate_and_evaluate_signal(exp_list, symbol_list, start_date, end_date, model_config = model_config,
                                       data_type="tick_l2p", model_file_type = "pkl", l3_flag = False,
                                       winloss_save_path=None, flag_generate_parquet = True,
                                       flag_winloss = True, factor_type = 'factor'):
    ...
    if not l3_flag:
        ...
        excel_path_list = ["signal_winloss", "signal_sample_winloss"] # mid_signal_winloss_win1.5_loss2_pred1.0.xlsx # signal_sample_new.xlsx signal_all_new.xlsx
    ...
    for label_name, exp_name, version_alias in exp_list:
        ...
        if flag_generate_parquet:
            tasks = [
                remote_func.remote(label_name, symbol, exp_version_path, start_date, end_date, data_type, model_config, factor_type=factor_type, model_file_type = model_file_type)
                    ...
                    for path in excel_path_list:
                        ...
                        for win_limit in win_limits[1:]:
                            for loss_limit in loss_limits:
                                ...
                                df_pred00, no_cache_dates = get_cache_data_and_lack_param(file_path=pred00_file_path,
                                                                                          increase_col="DATE",
                                ...
                                if no_cache_dates:
                                    ...
                                    no_cache_dates.sort()
                                    calc_df = calc_save_winloss_func_pred00(nocache_signal_df,
                                                                            local_mode=True, target_type=target_type,
                                                                            t_sta="09:33:00",
                                                                            win_limits=[win_limit],
                                                                            loss_limits=[loss_limit])
                                    if not calc_df.empty:
                                        save_and_append_parquet(symbol_name, calc_df, pred00_file_path,
                                                                overwrite_col="DATE")
                                        if df_pred00.empty:
                                            df_pred00 = calc_df
                                        else:
                                            df_pred00 = pd.concat([df_pred00, calc_df])
                                df_pred00_pl = pl.from_pandas(df_pred00)
                                # TODO 计算阈值百分比的止盈止损
                                for percent in percent_list:
                                    percent_winloss_file = f"{symbol_name}_{target_type}_{path}_win{float(win_limit * 1000)}_loss{float(loss_limit * 1000)}_pred_percent{int(percent)}.xlsx"
                                    if winloss_save_path == None:
                                        ...
                                        winloss_save_path_v = os.path.dirname(exp_version_path)
                                        excel_name = f"{target_type}_{path}_win{float(win_limit * 1000)}_loss{float(loss_limit * 1000)}_pred_percent{int(percent)}"
                                        symbol_path = os.path.join(exp_version_path, excel_name)
                                        os.makedirs(symbol_path, exist_ok=True)
                                        percent_winloss_file_path = os.path.join(symbol_path, percent_winloss_file)
                                    else:
                                        ...
                                        os.makedirs(os.path.join(winloss_save_path, version_alias), exist_ok=True)
                                        excel_name = f"{target_type}_{path}_win{float(win_limit * 1000)}_loss{float(loss_limit * 1000)}_pred_percent{int(percent)}"
                                        symbol_path = os.path.join(winloss_save_path_v, version_alias, excel_name)
                                        os.makedirs(symbol_path, exist_ok=True)
                                        percent_winloss_file_path = os.path.join(symbol_path, percent_winloss_file)
                                    task = inner_func0.remote(version_alias, symbol_name, df_pred00_pl, percent,
                                                              signal_dates,
                                                              win_limit, loss_limit, percent_winloss_file_path)
                                    tasks.append(task)
                ray.get(tasks)
                print("阈值百分比计算止盈止损耗时：{} s".format(time.time()-t0))
            except Exception as e:
                import traceback
                ...
                for win_limit in win_limits[1:]:
                    for loss_limit in loss_limits:
                        for percent in percent_list:
                            ...
                            files = os.listdir(symbol_path)
                            print(f"开始汇总：{symbol_path}的文件")
                            for file in files:
                                symbol = file.split("_")[0]
                                file_path = os.path.join(symbol_path, file)
                                try:
                                    df_s = pd.read_excel(file_path, engine="openpyxl")
                                except Exception as e:
                                    print(f"读取{file_path}失败：{e}")
                                    continue
                                if not df_s.empty:
                                    save_backtest_result(df_s, sheet_name=symbol, overwrite_col="index",
                                                         output_path=os.path.join(winloss_save_path_v,
                                                                                  version_alias,
                                                                                  excel_name + ".xlsx"))
            print("阈值百分比汇总excel文件耗时：{} s".format(time.time() - t1))
            tt0 = time.time()
            for th in th_list:
                try:
                        ...
                        for path in excel_path_list:
                            ...
                            for win_limit in win_limits[1:]:
                                for loss_limit in loss_limits:
                                    long_pred_th = th
                                    short_pred_th = -th
                                    file_name = f"{symbol_name}_{target_type}_{path}_win{win_limit * 1000}_loss{loss_limit * 1000}_pred{th}.xlsx"
                                    if winloss_save_path == None:
                                        # winloss_save_path_v = os.path.dirname(exp_version_path)
                                        # excel_path = os.path.join(exp_version_path, file_name)
                                        winloss_save_path_v = os.path.dirname(exp_version_path)
                                        excel_name = f"{target_type}_{path}_win{win_limit * 1000}_loss{loss_limit * 1000}_pred{th}"
                                        symbol_path = os.path.join(exp_version_path, excel_name)
                                        os.makedirs(symbol_path, exist_ok=True)
                                        excel_path = os.path.join(symbol_path, file_name)
                                    else:
                                        # winloss_save_path_v = winloss_save_path
                                        # os.makedirs(os.path.join(winloss_save_path, version_alias), exist_ok=True)
                                    ...
                                    # tasks_dict[excel_path + "+" + symbol_name].append(task)
                    ...
                    for task_id in tasks_dict.keys():
                        ...
                        xls_name = "_".join(filename.split("_")[1:])
                        out_file = "agg_" + xls_name
                        out_path = os.path.join(winloss_save_path_v, version_alias, out_file)
                        save_backtest_result(result_agg_df, symbol, output_path=out_path)
                except Exception as e:
                    import traceback
                    print(traceback.print_exc())
                    print(e)
                    pass
            print("固定阈值计算并存储止盈止损耗时：{} s".format(time.time() - tt0))
            tt1 = time.time()
            for th in th_list:
                for path in excel_path_list:
                    for win_limit in win_limits[1:]:
                        for loss_limit in loss_limits:
                            ...
                            for file in files:
                                ...
                                if not df_s.empty:
                                    save_backtest_result(df_s, sheet_name=symbol, overwrite_col="index",
                                                         output_path=os.path.join(winloss_save_path_v, version_alias,
                                                                                  excel_name + ".xlsx"))
            print(f"固定阈值excel汇总文件耗时：{time.time() - tt1} s")
            # # 把所有的mid_winloss_detail数据合并为一个pkl文件
            # file_pattern = f"mid_signal_winloss_win(\d+(\.\d+)?)_loss(\d+(\.\d+)?)_pred(\d+(\.\d+)?)" + ".xlsx"
            # cache_file_name = os.path.join(exp_version_path, "winloss_detail_all.pkl")
            # res_df_list = []
            #
            # for f in os.listdir(exp_version_path):
            #     match = re.match(file_pattern, f)
            #     if match:
            #         winloss_file = os.path.join(exp_version_path, f)
            #         win, loss, pred = float(match.group(1)), float(match.group(3)), float(match.group(5))
            #         try:
            #             stock_set = fastexcel.read_excel(winloss_file).sheet_names
            #             res_df_dict = pl.read_excel(winloss_file, sheet_name=stock_set, engine="calamine")
            #         except:
            #             print("文件读取失败：{}".format(winloss_file))
            #             continue
            #         for stock in stock_set:
            #             try:
            #                 res_df = res_df_dict[stock].to_pandas()
            #                 res_df["模型名称"] = version_alias
            #                 res_df["预测阈值"] = pred
            #                 res_df["止盈线"] = res_df["止盈线"] * 1000
            #                 res_df["止损线"] = res_df["止损线"] * 1000
            #                 res_df = res_df.rename(columns={"index": "日期"})  # 格式2019-07-01
            #                 res_df = res_df.reindex(columns=winloss_field_pattern)
            #                 res_df_list.append(res_df)
            #                 print(f"load_winloss_detail_data shape:  {stock} {res_df.shape} {version_alias} {f}")
            #             except Exception as e:
            #                 print(f"load_winloss_detail_data error: {version_alias} {f} {stock} {e}")
            #                 continue
            # res_df = pd.concat(res_df_list) if len(res_df_list) else pd.DataFrame()
            # if len(res_df) > 0:
            #     res_df.to_pickle(cache_file_name)
...
def calc_save_winloss_func_pred00(source_signal_df, use_self_prob=False,
