        ...
        if flag_winloss:
            ...
            try:
                    ...
                    for path in excel_path_list:
                        ...
                        for win_limit in win_limits[1:]:
                            for loss_limit in loss_limits:
                                ...
                                if no_cache_dates:
                                    print(f"{symbol_name}-{no_cache_dates}开始计算0阈值信号文件")
                                    # 把未缓存00阈值数据的日期，计算并缓存
                                ...
                                for percent in percent_list:
                                    percent_winloss_file = f"{target_type}_{path}_win{float(win_limit * 1000)}_loss{float(loss_limit * 1000)}_pred_percent{int(percent)}.xlsx"
                                    if winloss_save_path == None:
                                    ...
                                    task = inner_func0.remote(version_alias, symbol_name, df_pred00_pl, percent,
                                                              signal_dates, win_limit, loss_limit)
                ...
                for task_id in tasks.keys():
                    ...
                    save_winloss_data_xlsx(results_df, sign_col="标的", output_path=res_out_path,
                                           overwrite_col="日期")
                print(f"{version_alias}-{start_date}-{end_date}-{len(symbol_list)}支标的，阈值百分比：{percent_list} 止盈止损存储耗时：{time.time() - t4} s")
            except Exception as e:
                import traceback
            ...
            for th in th_list:
                try:
                        ...
                        for path in excel_path_list:
                            ...
                            for win_limit in win_limits[1:]:
                                for loss_limit in loss_limits:
                                    long_pred_th = th
                                    short_pred_th = -th
                                    file_name = f"{target_type}_{path}_win{win_limit * 1000}_loss{loss_limit * 1000}_pred{th}.xlsx"
                                    if winloss_save_path == None:
                    ...
                    for task_id in tasks_dict.keys():
                        ...
                        res_out_path = os.path.join(winloss_save_path_v, version_alias, file_name)
                        save_winloss_data_xlsx(result_agg_df, sign_col="标的", output_path=agg_out_path)
                        save_winloss_data_xlsx(result_res_df, sign_col="标的", output_path=res_out_path,
                                               overwrite_col="日期")
                    print(f"{version_alias}-{start_date}-{end_date}-{len(symbol_list)}支标的，固定阈值：{th} 止盈止损存储耗时：{time.time() - t3} s")
                except Exception as e:
                    import traceback
                    print(traceback.print_exc())
                    print(e)
                    pass
            print(f"{version_alias}-{start_date}-{end_date}-{len(symbol_list)}支标的 固定阈值计算止盈止损并存储，耗时：{time.time() - t2} s")
            print(f"{version_alias}-{start_date}-{end_date}-{len(symbol_list)}支标的 flag_winloss 耗时:{time.time() - tt2} s")
            ...
            cache_file_name = os.path.join(model_path, "winloss_detail_all.parquet")
            res_df_list = []
            ...
            print("汇总固定阈值止盈止损数据到parquet，耗时：{} s".format(time.time() - t5))

        print(f"{version_alias}-{start_date}-{end_date}-{len(symbol_list)}支标的总耗时：{time.time()-tt0} s")
    ray.shutdown()
