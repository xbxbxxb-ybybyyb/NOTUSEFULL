...
def main(exp_list, start_date, end_date, flag_generate_parquet, flag_winloss, update_ret_flag, factor_type='factor', winloss_save_path=None):
    model_type = "l2p"
    ################################################################################
    first_point = False
    if first_point:
    ...
    for label_name, exp_name, version_alias, symbol_list, data_type in exp_list:
        if "l3" in exp_name and not "noflying" in version_alias:
            model_type = "l3"
        if version_alias not in ['HS_tick2_add', 'HS_tick2_new', 'HS_l2p2', 'HS_l2p']:
            continue
...
if __name__ == "__main__":
    ...
    end_date = "2024-11-25"
    ray.init(num_cpus=40, local_mode=False, log_to_driver=True)

    flag_generate_parquet = True
    flag_winloss = True
    update_ret_flag = False
    factor_type = 'factor'
    # winloss_save_path = '/dfs/group/800657/exp_results/KG101_model/debug_symbol'
    # exp_config_list = load_model_config(file_path="/data/user/013150/exp_assembles-master/artifacts/run/tools/model_config.json")
    ...
    main(exp_config_list, start_date, end_date, flag_generate_parquet= flag_generate_parquet,
         ...
         factor_type=factor_type)# , winloss_save_path=winloss_save_path
