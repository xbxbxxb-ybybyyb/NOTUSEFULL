...
class StrategySignalT0Simple(StrategyBase):  # 策略类需继承 StrategyBase 基类
        ...
        if self.sellable_position - size >= 0:
            if net_position > 0:
                if abs(net_position - size - self.sellsize_onway) <= self.hold_position_limit:
                    sell_order = super(StrategySignalT0Simple, self).sell(price=price, size=size,
        ...
        if self.sellable_position - net_position - size - self.buysize_onway >= 0:
            if net_position >= 0:
                if abs(net_position) + abs(size) + self.buysize_onway <= self.hold_position_limit:
                    buy_order = super(StrategySignalT0Simple, self).buy(price=price, size=int(size),
                                                                        exectype=xb.Order.Limit)
    ...
    def process_init_openning(self, stock, side, last_price_df):
        ...
        if side == Order.Buy:
            ...
            # size = int(self.ema_vol[0]*0.2/100+1)*100
            # open_size = 200 * (1 + 8 * __cal_vol_ratio(signal_long, self.init_open_long_th, self.open_long_times))
            # size = min(size, open_size)
            ...
            # size = int(self.ema_vol[0]*0.2/100+1)*100
            # open_size = 200*(1+8*__cal_vol_ratio(signal_short, self.init_open_short_th, self.open_short_times))
            # size = min(size, open_size)
            # print("size:", size, "open_size:", open_size)
            size = self.p.tradesize
    ...
    def next(self):
        ...
        last_datetime = xb.num2date(self.datas[0].datetime[-1])
        lasttime = last_datetime.time().strftime('%H:%M:%S')
        # 传入标的查询行情数据
        price_df = self.get_past_feed_price_df(datanames=stock, lagged_bar=1)
        last_price_df = price_df[stock].iloc[-1]


        sub_signal_df = self.signal_df[(last_datetime<self.signal_df["PERIOD_BEGIN"]) & (self.signal_df["PERIOD_BEGIN"]<=now_datetime)]
        # 查询信号数据
