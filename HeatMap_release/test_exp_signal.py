...
strategy_name = "exp_l3_kc50_th10_60s"
label_name = "LabelFirstPeak_th10_60s"

os.system("rm -rf {}".format(symbol))

#signal_df = pd.read_excel("/data/user/013150/trade_data/mm_log/predictJson_{}.xlsx".format(date1), sheet_name = symbol)
base_dir = "/dfs/group/800657/exp_results/{}/xgboost_base/{}-{}/pred_th_up@1.50-pred_th_dw@1.50/signal_files_processed".format(strategy_name,label_name, symbol)
signal_df = parse_format.parse_signal_txt(os.path.join(base_dir, f"{date1}.txt"))
...
signal_df["cls"] = signal_df["PROBABILITY"].apply(lambda x:np.argmax(x))
print(signal_df[signal_df["cls"]!=0])
...
def func_inner(iidx, i):
    ...
    fig.add_trace(go.Scatter(name='BuySignal',
                                 x=buy_group['MDTime'], y=buy_group["TARGET_VALUE"],
                                 marker=dict(size=7, opacity=1, symbol="triangle-up"),
                                 mode='markers', line_color="red",
                                 ), secondary_y=False, row=1, col=1)

    fig.add_trace(go.Scatter(name='SellSignal',
                                 x=sell_group['MDTime'], y=sell_group["TARGET_VALUE"],
                                 marker=dict(size=7, opacity=1, symbol="triangle-down"),
    ...
    with open(f'./{symbol}/{symbol}_{date}_{time_stamps_str[i]}.html', "w") as f:
