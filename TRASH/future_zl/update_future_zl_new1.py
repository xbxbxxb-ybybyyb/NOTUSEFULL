...
dates = fa.tradingday("20201015", "20201105")
...
for date in dates:
                ...
                if future_type == 'Financial':
                    ...
                    if not __is_change_month_date(new_zl00_id , date):
                        ...
                        for alter_code in contract_f_c[1:]:
                            ...
                            old_code_number = old_contract_id[len(old_contract_id):len(old_contract_id) + __code_len(old_contract_id)]
                            if new_code_number > old_code_number:
                                xq_logger.info(
    ...
    first_pre_zl_pair, second_pre_zl_pair = get_pre_zlcode(old_contract_ids, old_contract_dates)#获取前一个主力合约的换仓日
    ...
    try:
        cur_close, adj, total_volume = df_value.loc[new_zl00_id, "ClosePx"], np.nan, df_value.loc[
            new_zl00_id, "TotalVolumeTrade"]
    except:
