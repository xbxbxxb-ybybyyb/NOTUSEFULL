...
def get_pre_zlcode(new_zl00_id, symbol, contract_type, old_contract_ids, old_contract_dates):
    ...
    if len(old_contract_ids) == 0:
        ...
        cur_start_date, pre_zl_mapping, pre_zl_close = np.nan, np.nan, np.nan
        return cur_start_date, pre_zl_mapping, pre_zl_close
    ...
    if pre_zl_contract_id == -1:
        ...
        cur_zl_date = old_contract_dates[len(old_contract_ids) - 1 - pre_zl_contract_id + 1]
        cur_start_date = df_zl[(df_zl["CODE"] == symbol) & (df_zl["ZL_CODE"] == symbol + contract_type) & (
                df_zl["TDATE"] == cur_zl_date)].iloc[0]["ZL_STARTDATE"]
        pre_zl_mapping, pre_zl_close = np.nan, np.nan
    else:
        ...
        pre_zl_close = df_zl[(df_zl["CODE"] == symbol) & (df_zl["ZL_CODE"] == symbol + contract_type) & (
                df_zl["TDATE"] == startdate)].iloc[0]["ZL_CUR_CLOSE"]
        pre_zl_mapping = df_zl[(df_zl["CODE"] == symbol) & (df_zl["ZL_CODE"] == symbol + contract_type) & (
                df_zl["TDATE"] == startdate)].iloc[0]["ZL_MAPPINGCODE"]
        if pre_zl_contract_id == 0:
            xq_logger.info("找到上一份不同的主力合约ID，日期为上一个交易日{}。".format(old_contract_dates[len(old_contract_ids) - 1 - pre_zl_contract_id]))
            cur_start_date = np.nan
        else:
            cur_zl_date = old_contract_dates[len(old_contract_ids) - 1 - pre_zl_contract_id+1]
            cur_start_date = df_zl[(df_zl["CODE"] == symbol) & (df_zl["ZL_CODE"] == symbol + contract_type) & (
                    df_zl["TDATE"] == cur_zl_date)].iloc[0]["ZL_STARTDATE"]
            xq_logger.info(df_zl[(df_zl["CODE"] == symbol) & (df_zl["ZL_CODE"] == symbol + contract_type) & (
                    df_zl["TDATE"] == cur_zl_date)].iloc[0])
            xq_logger.info("找到上一份相同的主力合约ID，日期为{}， 合约开始日期为{}。".format(cur_zl_date, cur_start_date))
    # print(pre_zl_value)
    return cur_start_date, pre_zl_mapping, pre_zl_close
...
date = "20201014"
contract_type = "ZL00"
future_type="Financial"  #'Financial' #"Commodity":
...
for date in dates:
            ...
            if old_rank!=0:
                ...
                for i in range(0, min(old_rank,len(contract_f_c))):
                    ...
                    if __is_change_future(old_contract_id, first_contract):
                        xq_logger.info("Financial prepare change ZL code, from {} to {}:".format(old_contract_id, first_contract))
                        change_store0 = True
                ...
                if future_type == 'Financial':
                    ...
                    if not __is_change_month_date(new_zl00_id , date):
                        ...
                        for alter_code in contract_f_c[1:]:
                            if __is_change_future(old_contract_id, alter_code):
                                xq_logger.info(
                                    "Commodity prepare change ZL code, from {} to {}:".format(old_contract_id, alter_code))
                                change_store0 = True
                                new_zl00_id = alter_code
                                break
                            else:
                                xq_logger.info(
                                    "failed change ZL00 code, from {} to {}:".format(old_contract_id, alter_code))
                        if change_store0 == False:
                            # 应该不会出现这种情况:
                            new_zl00_id = old_contract_id
    ...
    cur_start_date, pre_zl_mapping, pre_zl_close = get_pre_zlcode(new_zl00_id, symbol, contract_type, old_contract_ids, old_contract_dates)#获取前一个主力合约的换仓日
    startdate, last_close = cur_start_date, np.nan
    ...
    if new_zl00_id != None:
        ...
        mappingcode = pre_zl_mapping

    if change_store0:
        mappingcode = new_zl00_id
        startdate = date
        last_close = pre_zl_close
