...
"""
1. top_k% Sign-accuracy
2. MSE
3. MAE
4. correlation between labels and predictions
"""
...
def get_validation_predicts_stat(labels=None, predicts=None):
    ...
    labels_ranges = [concat_infer[int(i * concat_infer.shape[0] / 10)][1] for i in range(10)] + [concat_infer[int(concat_infer.shape[0] - 1)][1]]
    
    predicts_str = "Prediction range: \n"
    for predicts_range in predicts_ranges:
        predicts_str += "{0: .6f};  ".format(predicts_range)
    predicts_str += "\n Ground truth range: \n"
    for labels_range in labels_ranges:
        predicts_str += "{0: .6f};  ".format(labels_range)
...
def get_validation_win_return(labels=None, predicts=None, buy=True):
    ...
    for quantile in quantile_list:
        ...
        #
        negative_num = (dir_rst[:, 1] < 1.2).sum()  # 如果是<1.2 就是考虑交易手续费，赚的大于手续费才算胜利
