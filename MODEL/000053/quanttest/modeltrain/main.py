...
nnTrainDataPath = "/data/user/000053/quanttest/saved/nntrainfactors.npy"
...
for tag_index in range(6):
    ...
    # 封装多目标（多输出）回归,边训边测
    multi_xgb.fit(x_train, y_train, eval_set=[(x_valid, y_valid)], eval_metric="rmse", early_stopping_rounds=40)
    print("End training ====================================\n", showTime())
    
    # Evaluate on Train Set
    print('Evaluate Train Set ================================', showTime())
    mse = mean_squared_error(y_train, multi_xgb.predict(x_train))
    print('(Lon and Lat) Train MSE: %.5f' % mse, 'Train RMSE: %.5f' % np.sqrt(mse))

    # Save xgb
    with open('/data/user/000053/quanttest/saved/bigxgb_tag_{}.pkl'.format(tag_index), 'wb') as handle:
