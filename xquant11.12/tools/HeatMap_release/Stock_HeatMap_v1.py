...
def l2p_data_prepare(l2p_data, time_mode='S', num_mode=20):
    ...
    l2p_data['M_SellPrice'] = l2p_data['M_SellPrice'].apply(lambda x: x[:num_mode])
    l2p_data['M_SellOrderQty'] = l2p_data['M_SellOrderQty'].apply(lambda x: x[:num_mode])
    l2p_data['M_BuyPrice'] = l2p_data['M_BuyPrice'].apply(lambda x: x[:num_mode])
    l2p_data['M_BuyOrderQty'] = l2p_data['M_BuyOrderQty'].apply(lambda x: x[:num_mode])
    
#     l2p_data['M_SellPrice'] = l2p_data['M_SellPrice'].apply(lambda x: list(pd.DataFrame(x)[0].replace(0,np.nan).ffill().values))
#     l2p_data['M_SellOrderQty'] = l2p_data['M_SellOrderQty'].apply(lambda x: list(pd.DataFrame(x)[0].replace(0,np.nan).ffill().values))
#     l2p_data['M_BuyPrice'] = l2p_data['M_BuyPrice'].apply(lambda x: list(pd.DataFrame(x)[0].replace(0,np.nan).ffill().values))
#     l2p_data['M_BuyOrderQty'] = l2p_data['M_BuyOrderQty'].apply(lambda x: list(pd.DataFrame(x)[0].replace(0,np.nan).ffill().values))
...
def order_data_prepare(l2p_data):
    ...
    order_b_px['M_BuyOrderQty'] = order_b_qty['M_BuyOrderQty'].replace(0,np.nan).ffill()
    order_b_px['M_BuyPrice'] = order_b_px['M_BuyPrice'].replace(0,np.nan).ffill()
    ...
    order_s_px['M_SellOrderQty'] = order_s_qty['M_SellOrderQty'].replace(0,np.nan).ffill()
    order_s_px['M_SellPrice'] = order_s_px['M_SellPrice'].replace(0,np.nan).ffill()
...
def heatmap_size_prepare(df):
    ...
    df['sum'] = df['sum'].apply(lambda x: max((x/(x+min_set)-0.05),0.1) if x > 0 else min((x/(-x+min_set)+0.05),-0.1))
...
#             xgap=8,
    ...
    fig.update_layout(title='Buy&Sell',
...
def Stock_HeatMap(SYMBOL="000977.SZ", trade_date = "20231215", start_time = "093000", end_time = "093100", time_mode = 'ms',num_mode=20, base_dir="./"):
    ...
    l2p_data = l2p_data_prepare(l2p_data, time_mode=time_mode,num_mode=num_mode)
