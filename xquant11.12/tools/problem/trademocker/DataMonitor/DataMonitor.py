from DataInterface.Config import FLAG_COLUMNS
...
class DataMonitor(object):
    def __init__(self, lib_name, code):
        ...
        self.code = code
        self.daily_monitor = dict()
        self.minute_monitor = dict()
        self.tick_monitor = dict()
        self.tran_monitor = dict()
        self.order_monitor = dict()
    ...
    def run_monitor(self):
        ...
        if self.tran_monitor:
            tran_flag = self.__dict2df(self.tran_monitor)
            self.save_tick_monitor(tran_flag, TRANSACTION_MONITOR_HBASE_COLUMNS)

        if self.order_monitor:
            order_flag = self.__dict2df(self.order_monitor)
            self.save_tick_monitor(order_flag, ORDER_MONITOR_HBASE_COLUMNS)
    ...
    def update_transaction_monitor(self, tran_monitor):
        self.tran_monitor.update(tran_monitor)

    def update_order_monitor(self, order_monitor):
        self.order_monitor.update(order_monitor)
