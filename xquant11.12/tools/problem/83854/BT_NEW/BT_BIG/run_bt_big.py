...
def run_bt_big(is_cp_signal2share, is_bt_production_signals):
    ...
    if is_cp_signal2share:
        ...
        copy_signals_to_share(upload_date, symbols, src_path, dst_path, end_date, is_big=True)
    ...
    for portfolio in trade_portfolio:
        ...
        signal_csv_dir = '666888/ModelProduction/' + model_prefix + '/universe/'
        ...
        if is_bt_production_signals:
            ...
            symbols, init_quantities = hf.get_symbols_quantities_from_json(
                bt_dir + '{}_quantity.json'.format(portfolio))
            # TODO 改路径
            signal_csv_dir = '666888/ProductionLogSignals/universe_new/'
            trigger_json_dir = '666888/production_triggers/{}/'.format(portfolio)
            with py.open('production_triggers/{}/come_from.json'.format(portfolio), 'rb') as f:
                name = f.read()
                name = json.loads(name)
...
if __name__ == '__main__':
    run_bt_big(is_cp_signal2share=True, is_bt_production_signals=True)
