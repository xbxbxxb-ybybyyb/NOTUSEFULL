...
import numpy as np
import json
from ModelSystem.getAveTickVol_zrz import StockVolMean
from multiprocessing import Pool
from tqdm import tqdm
import traceback

StockVolMean_inst = StockVolMean()
def getMaxVolumePerOrder(symbol, back_test_dates, dayNum):
    ...
    for cur_date in back_test_dates:
        ...
        if not cur_date in maxVolumePerOrder:
            volumePerOrder = StockVolMean_inst.getVolMean(symbol, int(pre_date), dayNum)
            maxVolumePerOrder[cur_date] = volumePerOrder
    
    return maxVolumePerOrder    
                
def get_order_capacity(signal_dates):
    ...
    for i in tqdm(range(stock_num)):
        ...
        # if True: 
            ...
            maxVolumePerOrders = getMaxVolumePerOrder(stock_code, signal_dates, maxVolumePerOrderDayNum)
...
def get_order_capacity_paral(signal_dates):
    ...
    for index in range(processNum):
        if index < remains:
            ...
            edx = idx + baseNum
        pool.apply_async(get_order_capacity2, (signal_dates, idx, edx))
        idx = edx
    pool.close()
    pool.join()
