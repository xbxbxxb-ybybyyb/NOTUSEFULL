...
def get_bt_data(target_code, volume, para_split, combine_path, start_date, end_date):
    ...
    for index in para_split:
        ...
        for d in capacity["OrderCapacity"]:
            if d >= start_date and d <= end_date and d != '20190112' and d != '20190405':
                dates.append(d)
        if len(dates) == 0:
...
def combine_trade_and_capacity(portfolio, start_date, end_date, curr_date):
    ...
    file_name = "20190627_{}.xlsx".format(portfolio)
    ftp.downloadFile("666888/20190627/"+file_name, "/app/data/666888/ftp_uploads/"+file_name)
...
def main():
    ...
    py.upload(
        '{}/{}-{}_{}/{}/'.format(folder_temp_name, start_date, end_date, curr_date, portfolio),
        '/app/data/666888/BT_Trade_OrderCapacity/{}-{}_{}/{}/'.format(start_date, end_date, curr_date, portfolio)
    )
    py.copyToShare(
        '$21/ModelProduction/20190101_48_end/bt_info/{}-{}_{}/{}/'.format(start_date, end_date, curr_date, portfolio),
        '{}/{}-{}_{}/{}/'.format(folder_temp_name, start_date, end_date, curr_date, portfolio)
    )
    py.delete(folder_temp_name, recursive=True)
