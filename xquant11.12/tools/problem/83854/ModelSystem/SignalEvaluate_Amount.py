...
class SignalEvaluate:
    ...
    def __init__(self, symbol, signal_data: pd.DataFrame, stock_data_path, mockTradePara, executorStr, output_path):
        # 定义一些常量
        ...
        self.__short_symbol = symbol[: -3]
        self.__stock_data_path = stock_data_path
        self.__output_path = output_path

        self.__tick, self.__transaction, self.__data_dict = self.__get_data()
        self.__signal_data: pd.DataFrame = signal_data
        if self.__signal_data.columns[0] != 'Timestamp':
            raise Exception('The first column is not Timestamp!')
        elif len(self.__signal_data.columns) != 3:
    ...
    def __get_data(self):
        with open(self.__stock_data_path, "rb") as f:
            tickData, transactionData = pickle.load(f)
        data_converter = SimpleDataConverter(tickData)
        data_dict = data_converter.get_data_dict()
        print('Finished loading data.')
        return tickData, transactionData, data_dict

    def evaluate(self, show=False):
        rollingTradingOrder = []
        tempCombineTradingOrder = self.runBackTest()
        self.TradingEvaluate(tempCombineTradingOrder, show)
        if show:
        ...
        print('Output to the spreadsheet.')
        OutputSpreadSheet(detailedOrders, tempCombineTradingOrder, self.__output_path, self.__funStr)
        print('Finished back-testing.')
        return rollingTradingOrder
    ...
    def runBackTest(self):
        ...
        self.__outputMgr = OutputManager(self.__COST_RATE, True)
        self.__signalExecutor.generateTriggerRatio(self.__symbol, {}, self.__tick)
        print('Finished generating trigger ratios.')
        print('Doing back-testing.')
        for index, row in self.__signal_data.iterrows():
            timestamp = row[0]
