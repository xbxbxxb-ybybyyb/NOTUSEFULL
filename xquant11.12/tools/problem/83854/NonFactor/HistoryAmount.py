...
class HistoryAmount(Factor):  # 非因子类，利用率较高的中间变量
    def __init__(self, para, factorManagement):
        ...
        self.__amount = self.getFactorData({"name": "amount", "className": "Amount",
        ...
        if not self.__isHistoryLoaded:
            ...
            if slice_data != None:
                for i in range(len(slice_data)):
                    self.addData(slice_data[i].amount, slice_data[i].timeStamp)
                self.__isHistoryLoaded = True
        else:
            self.addData(self.__amount.getLastContent(), self.__data.getLastTimeStamp())
