...
def main(is_get_trade=True, is_combine_trade_capacity=False, is_copy_trade_2_share=False):
    ...
    is_get_trade = is_get_trade
    is_get_capacity = False
    
    # 固定组合数据生成
    is_combine_trade_and_capacity = is_combine_trade_capacity
    is_cp_trade2_share = is_copy_trade_2_share
    
   
    
    signal_dates = []  # empty means all; or e.g. ['20180901']
    signal_dates.append(config.today.strftime("%Y%m%d"))
    #signal_dates = ["20190315", "20190318","20190319", "20190320", "20190321","20190322", "20190325"]
    # config.codes = ["000725.SZ"]
     #  获取行情用户撮合       
    # if is_get_trade:
    ...
    if True:
        ...
        portfolios = ["BigOld", "BigNew"]
        portfolios = ["BigOld"]

        today_str = "20190430"
        model_prefix = "20190101_end"
        for portfolio in portfolios:
            ...
            py.copyToShare('$21/ModelProduction/'+model_prefix+'/bt_info/{}-{}/{}/'.format(today_str, today_str, portfolio),
...
if __name__ == '__main__':
    main(is_get_trade=False, is_combine_trade_capacity=False, is_copy_trade_2_share=True)
