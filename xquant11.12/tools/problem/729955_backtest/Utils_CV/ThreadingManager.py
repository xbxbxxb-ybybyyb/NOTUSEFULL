...
def func_dump_symbol_params(context, task_meta):
    ...
    # rp.print(all_param_dict)
    # rp.print(input_manager.output_dir + 'TEMP/' + symbol + '.json')
    py = Pyfile()
...
def func_run_seperate_params(context, task_meta):
    ...
    for i in range(len(symbols)):
        ...
        if input_manager.is_cross_validation():
            results_first = []
            results_second = []
            if re.search("Easy", input_all.executor_str):
                input_first.update_executor_str(input_all.executor_str + "BT")
                input_second.update_executor_str(input_all.executor_str + "BT")
            for trigger_dict in params:
                # rp.print(symbol, trigger_dict)
                if trigger_dict[0] is not None:
                    se_first = SignalEvaluate(input_first, [(input_first.signal_data, trigger_dict[0])])
                    result_first = se_first.evaluate()
                    results_first.append(result_first)
                if trigger_dict[1] is not None:
                    se_second = SignalEvaluate(input_second, [(input_second.signal_data, trigger_dict[1])])
            ...
            results = []
            if re.search("Easy", input_all.executor_str):
                input_all.update_executor_str(input_all.executor_str + "BT")
            for trigger_dict in params:
...
def func_run_output_results(context, task_meta):
    ...
    if re.search("Easy", input_all.executor_str):
        input_all.update_executor_str(input_all.executor_str + "BT")
        input_first.update_executor_str(input_first.executor_str + "BT")
        input_second.update_executor_str(input_second.executor_str + "BT")
        # input_all.update_executor_str(input_all.executor_str + "CV")
        # input_first.update_executor_str(input_first.executor_str + "CV")
        # input_second.update_executor_str(input_second.executor_str + "CV")
...
class ThreadingManager:
    ...
    def pre_process_params(self):
        ...
        for file in files:
            ...
            for trigger_ratio in data[0]:
                total_list.append((symbol, [trigger_ratio, None]))
            for trigger_ratio in data[1]:
                total_list.append((symbol, [None, trigger_ratio]))
