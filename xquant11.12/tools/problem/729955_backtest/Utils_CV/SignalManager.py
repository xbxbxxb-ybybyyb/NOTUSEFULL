...
class SignalManager:
    ...
    def __re_split_data(self) -> None:
        if len(self.__valid_dates) == 0:
            return
        if self.__first_half_data is None or self.__second_half_data is None:
            self.__valid_dates.sort()
            length = len(self.__valid_dates)
            half = int(length / 2)
            self.__first_half_dates = self.__valid_dates[: half]
            self.__second_half_dates = self.__valid_dates[half:]
            # rp.print(self.__first_half_dates, self.__second_half_dates)
            self.__pre_process_first_half()
            self.__pre_process_second_half()
    ...
    def __pre_process_single(self, filename: str) -> pd.DataFrame:
        ...
        # if True:
            ...
            self.__valid_dates.append(filename[10:18])
        except:
            rp.print (self.__symbol, filename[10:18], " no data")
            return None
