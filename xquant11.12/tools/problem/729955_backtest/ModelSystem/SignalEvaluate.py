...
class SignalEvaluate:
    ...
    def __init__(self, inputs: 'Input', signal_trigger_pairs: List[Tuple[pd.DataFrame, Dict[str, float]]]):
        ...
        if self.__LAST_OPEN_TIME is None:
            self.__LAST_OPEN_TIME = dt.time(14, 55, 0)  # 若外部参数没有设置，则默认值为 14:57:00起禁止再开仓
    ...
    def __valid_slice_data(self, slice_data):
    ...
    def __calCloseQty(self, symbol, price):
        ...
        if symbol in self.__exePriceQty and "volume" in self.__exePriceQty.get(symbol):
            ...
            if self.__riskMgr.isInDanger(symbol):
                # TODO
                return min(netPosition, int(self.__maxTurnoverPerOrder / price))
            else:
                if volume > netPosition:
                    ...
                    exposure = min(exposure, int(maxExposure / price), int(self.__maxTurnoverPerOrder / price),
    ...
    def __reset_init_amount(self, signals: pd.DataFrame):
        ...
        if self.__initAmount is None:
            raise Exception('Initialize amount failed! The first ' + str(try_times) + ' timestamps are not in the Data.pickle' + self.__symbol)
    ...
    def __calMarketCloseData(self, netPosition, last_slice_data):
        ...
        if accVolume < absPosition:
            ...
            if netPosition > 0:
                return round(math.floor(price * 1000) / 1000, 3), accAmount
            else:
                return round(math.ceil(price * 1000) / 1000, 3), accAmount
