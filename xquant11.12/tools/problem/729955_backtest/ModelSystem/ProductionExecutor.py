...
class ProductionExecutor(SignalExecutorBase):
    def __init__(self, positionMgr, riskMgr):
        ...
        self.__is_second_up = False
        self.__is_second_down = False
...
#        self.STOP_PROFIT_LOSS = 15
#        self.STOP_PROFIT_LOSS = 10
#        self.STOP_PROFIT_LOSS = 25
        self.DROP_LOSS = 5
    ...
    def set_high_vol(self, param):
        self.__HIGH_VOL = 3.37
        self.__HIGH_VOL = 0.0
        # self.__HIGH_VOL = param.get("HighVol")
    ...
    def __check_second_price_limit(self, slice_data):
        # if self.__symbol[-2:] == "SZ":
        #     return False
        price_ratio = 1000 * (slice_data.lastPrice / slice_data.previousClosingPrice - 1)
    ...
    def __get_open_long_offset(self, log_vol_ma_200, cur_return):
        ...
        # loss_ratio = min(self.__max_long_return_rate - cur_return, 3.0) / 10
        # vol_ratio = min((self.__HIGH_VOL - log_vol_ma_200) / 2, 1.0)
    ...
    def updatePredictInfo(self, predictions, slice_data):
        ...
        self.__check_second_price_limit(slice_data)
        ...
        if self._positionMgr.isPositionClosed(self.__symbol):
            ...
            if (predictions[0] > self.__init_open_long_threshold) or (
                    self.__signal_sorted(self.__bid_predictions[-5:]) and predictions[
                0] > self.__init_open_long_threshold - 0.3):
                if predictions[1] < self.__init_close_long_threshold:
                    self.__pre_net_position = self._positionMgr.getNetPosition(self.__symbol)
                    return
                if self.__openFilterCondition():
...
#                close_decrease_interval = 100
                close_decrease_interval = 20
                if self.__close_long_threshold is None:
                    loss_ratio = (self.__max_long_return_rate - curr_return) / 10
                    self.__init_close_long_threshold += int(
                        self._cum_open_tick_count2 / close_decrease_interval) * loss_ratio
                ...
                elif ((self.__is_first_up or self.__is_first_down) and not self.__is_first_position_cleared) or \
                        ((self.__is_second_up or self.__is_second_down) and not self.__is_second_position_cleared):
                    self.__process_price_limit_clear(self.__symbol, slice_data)
                elif (bid0 / ask0 - 1) * 1000 > self.__UNALLOWED_OPENLOSS_RATIO and predictions[
                    0] > self.__cal_open_trigger():
                    if self.__openFilterCondition():
                        self.__process_multi_open(predictions, slice_data, OrderSide.Buy)
                # elif self.__is_stop_profit(slice_data):
                # self.__process_stop_profit(slice_data, OrderSide.Sell)
    ...
    def isOpenLong(self, predictions, slice_data):
        if "OpenLong" not in self.__order:
            ...
            newQty = self.__checkCJZB(volume, "B", slice_data)
            newQty = self.__checkCJJE(price, newQty, "B", slice_data)
            if newQty < 10:
                return False
            else:
                return {"price": price, "volume": newQty}
...
#        open_long_coef = 1.0002
    ...
    def __cal_multi_dynamic_open_quantity(self, side, price, prediction, delta, slice_data):
        ...
        ema = self.__ema_volume()
        # tick_time = dt.datetime.fromtimestamp(slice_data.timeStamp).time()
        # if tick_time >= self.__vol_limit_start_time:
        #     limit_vol = 5 * self.__ema_volume_new()
        #     volume = min(volume, limit_vol)
...
#        close_long_coef = 0.9998
        ...
        if side == OrderSide.Sell:
            ...
            if predictions[1] < self.__init_close_long_risk_threshold:
...
#        close_long_coef = 0.9998
        ...
        if self._positionMgr.isPositionPositive(self.__symbol):
            if ask < self.__init_open_short_threshold:
                ...
                # price = max(ask0 - 0.1, bid0 * close_long_coef, ask0 * close_long_limited_coef)
...
#            close_long_limited_coef = 0.98
    ...
    def __cal_vol_ratio(self, delta, prediction, base, times):
        ...
        # def __is_stop_profit(self, slice_data):
        # cur_pos = int(self._positionMgr.getNetPosition(self.__symbol) / 10) * 10
        # finishedOrders = self._positionMgr.getFinishedOrders(self.__symbol)
        # long_order_price = []
        # last_update_time = None
        # if cur_pos < 10:
        # return False

        # for order in reversed(finishedOrders):
        # if order.BSFlag == 'B':
        # if order.volume == 0:
        # continue
        # if cur_pos < 10:
        # break
        # cur_pos -= order.volume
        # long_order_price.append(order.price())
        # last_update_time = order.lastUpdateTime.timestamp()

        # max_open_price = max(long_order_price)
        # # try:
        # # max_open_price = max(long_order_price)
        # # except:
        # # rp.print("cur_pos: ", cur_pos)
        # # for order in reversed(finishedOrders):
        # # rp.print("finishedOrders: ", order.volume, order.price(), order.BSFlag)
        # cur_return = 1000 * (slice_data.bidPrice[0] - max_open_price) / max_open_price
        # start_index = 0
        # for t in reversed(self.__timestamp_today):
        # if t < last_update_time:
        # break
        # start_index += 1
        # bid_price_list = self.__bid0_price_today[-start_index:]
        # timestamp_price_list = self.__timestamp_today[-start_index:]
        # max_bid_p = max(bid_price_list)
        # drop_since_max = 1000 * (max_bid_p - slice_data.bidPrice[0]) / max_bid_p
        # if cur_return > self.STOP_PROFIT_LOSS and drop_since_max > self.DROP_LOSS:
        # return True

        # return False

    # # 处理平仓信号
    # def __process_stop_profit(self, slice_data, side):
    # close_long_coef = 0.9998
    # close_long_coef = 0.9995
    # close_long_limited_coef = 0.995
    # bid0 = slice_data.bidPrice[0]
    ...
    def __process_afternoon_close(self, symbol, slice_data, predictions):
        ...
        if netPosition > 0:
            if isCloseAtEase:
                # if (predictions[0] > self.__init_open_long_threshold) or (self.__signal_sorted(self.__bid_predictions[-5:]) and predictions[0] > self.__init_open_long_threshold - 0.3):
    ...
    def __process_price_limit_clear(self, symbol, slice_data):
        if self._positionMgr.isPositionClosed(symbol):
            return
        ...
        price_limit_clear_aggressive_coef = 0.999
        price_limit_clear_passive_coef = 0.995
        price = max(bid0 * price_limit_clear_aggressive_coef, ask0 * price_limit_clear_passive_coef)

        self.__order.update({"CloseLong": (price, positionQty)})

    def __checkCJZB(self, qty, side, slice_data):
        upperLimit = sum(self.__volume_today[-60:]) * 0.2
    ...
    def __openFilterCondition(self):
        """开仓过滤，"""
        bid0_price_list = list(filter(lambda x: x is not None, self.__bid0_price_today))
        # 过去1min的涨跌幅
        select_bid0_list = bid0_price_list[-60:]
        pct_1min = select_bid0_list[-1] / np.nanmax(select_bid0_list) - 1 if np.nanmax(select_bid0_list) != 0 else 0
        if pct_1min < - 0.01:
            # 当前位置在当天的分位数
