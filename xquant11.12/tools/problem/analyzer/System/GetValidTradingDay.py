...
def getValidTradingDayList(stockCode, startDate, endDate, printNotTradeDay=False):
    ...
    keyDateList = list(map(str, tradingDayList))
    df1 = fd.get_factor_value("Basic_factor", [stockCode], keyDateList, ["trade_status"])
    df1 = df1.reset_index()
    df2 = df1[df1["trade_status"] != "停牌"]
    df2 = df2[df2["trade_status"] != "待核查"]
    dateList = df2["mddate"].tolist()
    dateList.sort()
    validTradingDayList = list(map(int, dateList))
    
    if len(validTradingDayList) <= 0:
