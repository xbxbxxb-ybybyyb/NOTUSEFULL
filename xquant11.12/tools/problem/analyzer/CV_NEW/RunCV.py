...
def run_cv():
    ...
    for portfolio in trade_portfolio:
        ...
        if portfolio == "easy":
            fa = FactorData()
            hs300 = fa.hset("INDEX", "20210326", "HS300")["stock"].tolist()
            zz500 = fa.hset("INDEX", "20210326", "ZZ500")["stock"].tolist()
            target_stock_list = sorted(hs300 + zz500)
            for symbol, quantity in zip(symbols, init_quantities):
                if symbol in target_stock_list:
                    init_quantities[symbols.index(symbol)] = 0.5 * quantity 
