...
from INFER_SIGNAL_SMALL_UNIVERSE.SignalConfig import InferSignalConfig
from INFER_SIGNAL_SMALL_UNIVERSE.InferSignal import infer_signal
...
if len(target_code_list) > 0:
    if not is_multiprocess:
        ...
        for code in target_code_list:
            pool.apply_async(infer_signal, (signalPath, logPath, modelPath, libraryName, signalLibrary, factorNames, tagNames, tagDict,
                                            test_start_date, test_end_date, [code], pid, ))
        pool.close()
        pool.join()
