...
from INFER_SIGNAL_FEATURE.ModelInferHDFS import ModelCNNLSTMLongShort


def infer_signal(signalPath, logPath, absolutePath, libraryName, signalLibrary, factorNames, factorIndex, tagNames, tagDict, codeList, pid):
    ### 模型参数
    sliceLags = [160, 200, 240]
    open_tick_num = 21
    factorIndex = factorIndex    # 设置需要训练的因子Index，默认是全部因子
    ...
    for stock, date in codeList:
        ...
        log_file = log_file_path + "/" + stock_code + date + "-" + date + ".txt"
        ...
        try:
            ...
            if not test_factor_data or not test_tag_data or not test_timestamp:
                log_fd.logger.debug('No Data between {} and {}, {}, {}'
                                    .format(date, date, stock_code, pid))
                continue
...
def main():
    from INFER_SIGNAL_FEATURE_CHECK.SignalConfig import InferSignalConfig
    ...
    if is_multiprocess:
        ...
        process_num = min(len(codeList), 20)
        assert multicore_init() == True
        pool = mp.Pool(processes=process_num)
        for code in codeList:
            pool.apply_async(infer_signal, (signalPath, logPath, absolutePath, libraryName, signalLibrary, factorNames, factorIndex, tagNames, tagDict, [code], pid, ))
        pool.close()
        pool.join()

    else:
        infer_signal(signalPath, logPath, absolutePath, libraryName, signalLibrary, factorNames, factorIndex, tagNames, tagDict, codeList, pid)
