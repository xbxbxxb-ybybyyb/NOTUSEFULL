...
class SignalManager:
    ...
    def __init__(self, symbol: str, root: str='./stock_data/', bt_dir: str='./stock_data/'):
        ...
        self.__prediction_names = [
            ...
            "prediction5minLong", "prediction10minLong", "prediction1minShort", "prediction2minShort",
            "prediction5minShort", "prediction10minShort"
        ]
        self.__rename_dict = {
            "timestamp": "Timestamp",
            "ticktime": "Ticktime",
            ...
            "prediction10minLong": "10minLong",
            "prediction10minShort": "10minShort"
        }
    ...
    def __pre_process_single(self, filename: str) -> pd.DataFrame:
        ...
        # if True:
            ...
            if self.__is_executor:
                remote_print(self.__symbol, filename[10:18], " no signal data")
            else:
                print(self.__symbol, filename[10:18], " no signal data")
            return None

        # column_long = data['1minLong'] + data['2minLong'] + data['5minLong']
        # column_long /= 3
        column_long = data['5minLong'] + data['10minLong']
        column_long /= 2
        data = data.assign(ave_long=column_long)

        # column_short = data['1minShort'] + data['2minShort'] + data['5minShort']
        # column_short /= 3
        column_short = data['5minShort'] + data['10minShort']
        column_short /= 2
        data = data.assign(ave_short=column_short)

        data['Ticktime'] = data['Ticktime'].apply(lambda x: dt.datetime.strptime(x, '%H:%M:%S').time())
        data = SignalManager.average_special_time(data, dt.time(11, 25, 0), dt.time(11, 20, 0), dt.time(11, 30, 0))
        data = SignalManager.average_special_time(data, dt.time(14, 52, 0), dt.time(14, 47, 0), dt.time(14, 57, 0))
        return data
    ...
    def average_special_time(data: pd.DataFrame, time1: dt.time, time2: dt.time, time3: dt.time) -> pd.DataFrame:
        ...
        ave_long2 = data.loc[index2, '5minLong']
        ave_short2 = data.loc[index2, '5minShort']
