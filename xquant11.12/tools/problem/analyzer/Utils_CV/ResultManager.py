...
class ResultManager:
    def __init__(self, method='default', default_keys=None):
    ...
    def __output_param_mat(self, symbol: str, best_param: Dict[str, float], output_dir, suffix):
        ...
        if hf.exists(xls_dir):
            hf.delete(xls_dir)
        with hf.open(xls_dir, 'wb') as f:
        ...
        if 'condition' in best_param:
            ...
            if hf.exists(file_name):
                hf.delete(file_name)
            with hf.open(file_name, 'wb') as f:
    ...
    def __my_best_param_solution(self, symbol: str) -> Dict[str, float]:            
        ...
        day_winning_rate_threshold = 0.45
        ...
        if position_time_select.shape[0] == 0:
            ...
            elif position_time_threshold > 35:
                position_time_threshold = 35
            ...
            if position_time_threshold < 12:
                ...
                position_time_threshold = 35
            annual_return_mask = annual_return > 0
            position_time_condition = position_time <= position_time_threshold
            position_time_condition = position_time_condition & annual_return_mask
            win_rate_condition = win_rate >= win_rate_threshold
            win_rate_condition = win_rate_condition & annual_return_mask
            day_winning_rate_condition = day_winning_rate >= day_winning_rate_threshold
            day_winning_rate_condition = day_winning_rate_condition & annual_return_mask
            times_daily_condition = times_daily >= times_daily_threshold
            times_daily_condition = times_daily_condition & annual_return_mask
        ...
        if np.nanmax(np.abs(average_return)) > 0:
            ...
            times_daily_weight = times_daily / np.nanmax(times_daily)
