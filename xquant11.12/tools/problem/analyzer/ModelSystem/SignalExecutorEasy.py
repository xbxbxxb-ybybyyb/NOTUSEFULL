...
class SignalExecutorEasy(SignalExecutorBase):
    def __init__(self, positionMgr, riskMgr):
        ...
        self.__hold_cost_long = None
        self.__hold_cost_short = None
        self.__max_bid = None
        self.__min_ask = None
        self.__max_long_return_rate = None
        self.__tick_count_since_init_open_long = None
        self.__tick_count_since_init_open_short = None
        self.__amt_today = []
    ...
    def updatePredictInfo(self, outSamplePredict, slice_data):
        ...
        if self._positionMgr.isPositionClosed(self.__symbol):
            ...
            self.__max_bid = outSamplePredict[0]
            self.__min_ask = outSamplePredict[1]
            ...
            if self._positionMgr.isPositionNegative(self.__symbol):
                ...
                self.__min_ask = min(self.__min_ask, outSamplePredict[1])
                self.__max_bid = outSamplePredict[0]
                ...
                self.__tick_count_since_init_open_long = None
                self.__tick_count_since_init_open_short += 1
                ...
                self.__max_bid = max(outSamplePredict[0], self.__max_bid)
                self.__min_ask = outSamplePredict[1]
    ...
    def __get_close_short_offset(self):
        if self.__min_ask is None:
            return 0
            
        return max(0, self.__init_open_short_threshold - self.__min_ask) / 2.0   
    ...
    def __get_open_long_offset(self, log_vol_ma_200, cur_return):
        loss_ratio = min(self.__max_long_return_rate - cur_return, 3.0) / 6  
    ...
    def __initial_open(self, outSamplePredict, slice_data, side):
        ...
        if side == OrderSide.Buy:
            ...
            self.__tick_count_since_init_open_short = 0
    ...
    def __cal_dynamic_open_quantity(self, price, side, delta, prediction, slice_data):
        ...
        if side == OrderSide.Buy:
            ...
            total_market_amt_since_open = sum(self.__amt_today[-self.__tick_count_since_init_open_long - 1:])
            ...
            short_percent = self.__get_position_percent_in_market(self.__tick_count_since_init_open_short - 1)
            total_market_amt_since_open = sum(self.__amt_today[-self.__tick_count_since_init_open_short - 1:])
            cur_position = abs(self._positionMgr.getNetPosition(self.__symbol)) 
            if price == 0:
                limit_vol = 0
            else:    
        ...
        if side == OrderSide.Buy:
            ...
            # 平仓且反向开多    
                ...
                if outSamplePredict[0] > self.__init_close_short_risk_threshold:
                    ...
                    close_price = ask0
                # volume = int(volume_close / 100) * 100
