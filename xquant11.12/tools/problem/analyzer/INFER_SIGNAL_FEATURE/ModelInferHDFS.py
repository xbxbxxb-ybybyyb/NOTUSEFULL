...
from INFER_SIGNAL.DeepNNDataSetLongShort import DeepNNDataSet
from INFER_SIGNAL_FEATURE.FeatureNames import single_feature_names, feature_name_list
...
class ModelCNNLSTMLongShort:
    ...
    def infer(self):
        ...
        for d in timestamp["timestamp"].keys():
            ...
            predict_daily = None
            label_daily = None
            extract_features_daily = None

            for index, tag_name_list in enumerate(self.tagNames):
                for tag_name in tag_name_list:
                    model_path = self.model_path + "{}{}{}/".format(self.model_prefix, tag_name, self.sliceLags[index])
                    ...
                    predictNNData = preprocess_test_data(factor_data[d][:, tag_factor_index], mean_load, std_load, up_load, down_load)
                    ...
                    with tf.Session(config=config) as sess:
                        ...
                        extract_name_list = ["ensemble_sub_0/Reshape", "ensemble_sub_0/lstm/lstm/multi_rnn_cell/cell_4_13/dropout/mul",
                                             "ensemble_sub_0/dnn/dropout/mul"]
                        feature_output_list = []
                        for name in extract_name_list:
                        ...
                        cnn_features, lstm_features, dnn_features = sess.run(feature_output_list, {x: predict_factor_data,
                                                                                rnn_keepProb: 1.0, dnn_keepProb: 1.0})
                    cnn_features = cnn_features[:, -1, :]
                    extract_features = np.concatenate((cnn_features, lstm_features, dnn_features), axis=1)
                    ...
                    if extract_features_daily is None:
                        extract_features_daily = extract_features
                    else:
                        extract_features_daily = np.concatenate((extract_features_daily, extract_features), axis=1)
            ...
            for i in range(len(feature_name_list)):
                daily_prediction[cvt_d_key]["signals"][feature_name_list[i]] = extract_features_daily[:, i].tolist()
