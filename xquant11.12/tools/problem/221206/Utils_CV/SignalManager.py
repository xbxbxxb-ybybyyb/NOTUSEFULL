...
class SignalManager:
    def __init__(self, symbol, root, bt_dir, param_dir):
        self.__py = Pyfile()
        self.__factor_data = FactorData()
        # self.__signals_dict = signals_dict
        self.__root = root
        self.__bt_dir = bt_dir
        self.__param_dir = param_dir
        ...
        self.__prediction_names = ["timestamp", "ticktime", "prediction1minLong", "prediction2minLong",
    ...
    def __pre_process_single(self, date):
        try:
            data = self.__factor_data.get_factor_value(self.__root, self.__symbol, date, self.__prediction_names)
            data = data.rename(columns=self.__rename_dict)
        except:
            return None
        # data = self.__signals_dict[date]
        ...
        data = self.average_special_time(data, dt.time(11, 25, 0), dt.time(11, 28, 0), dt.time(11, 30, 0))
        data = self.average_special_time(data, dt.time(14, 52, 0), dt.time(14, 55, 0), dt.time(14, 57, 0))
        return data
    ...
    def __load_order_capacity(self):
        with self.__py.open(self.__param_dir + "/" + self.__symbol + ".json", 'rb') as f:
            data = f.read()
