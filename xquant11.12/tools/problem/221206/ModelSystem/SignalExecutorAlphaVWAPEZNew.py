...
class SignalExecutorAlphaVWAPEZNew(SignalExecutorBase):
    def __init__(self, position_manager):
        ...
        self._MARKET_CLOSE_START_TIME_AFTERNOON = dt.time(14, 56, 00)
    ...
    def _cal_aggressive_trigger(self):
        if self._side == OrderSide.Buy.name:
            if self._long_aggressive_trigger is None:
                return self._long_aggressive_ratio
            else:
                return self._long_aggressive_trigger
        else:
            if self._short_aggressive_trigger is None:
                return self._short_aggressive_ratio
            else:
                return self._short_aggressive_trigger

    # def _cal_passive_trigger(self):
    # if self._side == OrderSide.Buy.name:
    # if self._long_passive_trigger is None:
    # return self._long_passive_ratio
    # else:
    # return min(self._long_passive_trigger, self._long_aggressive_ratio)
    # else:
    # if self._short_passive_trigger is None:
    # return self._short_passive_ratio
    # else:
    # return max(self._short_passive_trigger, self._short_aggressive_ratio)
    ...
    def _passive_open(self, slice_data, unfinished_order):
        ...
        if self._side == OrderSide.Buy.name:
            ...
            if bid0 >= self._price_cut_off:
                price = max(min(ask0 * (1 - self._PRICE_MULTI), ask0 - 0.01), ask0 - 0.02, bid0, slice_data.minPrice)
            else:
                price = max(min(max(ask0 - 0.02, bid0) + 0.01, ask0), slice_data.minPrice)
            price = round(price, 2)
