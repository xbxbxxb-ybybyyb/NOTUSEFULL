...
import json
import InferSignal.SIGNAL_CONFIG as SIGNAL_CONFIG
...
def generate_signals():
    ...
    config = SIGNAL_CONFIG.InferSignalConfig()
    if config.is_use_config_codes:
        codes = config.codes
    else:
        codes = get_complete_stock_list()
    number_stock = len(codes)

    print("Generating signals of {} stocks".format(number_stock))
    ...
    AIMR.runTasks("/InferSignal/gpu_work_infer_signal.py", json.dumps(params))
