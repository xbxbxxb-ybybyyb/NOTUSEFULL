...
def combine(target_code, volume, start_date, end_date, combine_name):
    combine_path = root_path + "/" + start_date + "-" + end_date + "_universal/"
...
def combine_trade_and_capacity(portfolio, start_date, end_date):
    df = pd.read_excel("/data/user/666888/Apollo/portfolios/CompleteStockList/CompleteStockList_20191002.xlsx")
...
def main():
    ...
    combine_trade_and_capacity(portfolio, start_date, end_date)

    py = Pyfile()
    src_path = "{}/{}-{}_universal/".format(root_path, start_date, end_date)
    dst_path = "Apollo/cv_info/{}-{}_universal/".format(start_date, end_date)
