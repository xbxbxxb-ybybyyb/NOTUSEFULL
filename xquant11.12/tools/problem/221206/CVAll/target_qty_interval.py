...
def generate_target_qty_interval(cv_dir, output_dir, json_filename, s_date, e_date, period):
    py = Pyfile()
    with py.open(cv_dir + json_filename) as f:
...
def main():
    ...
    cv_dir = "Apollo/cv_info/{}-{}_universal/".format(s_date, e_date)
    output_dir = "Apollo/cv_params/{}-{}_universal/{}/".format(s_date, e_date, portfolio)
    json_filename = "{}_quantity.json".format(portfolio)

    generate_target_qty_interval(cv_dir, output_dir, json_filename, s_date, e_date, period)
