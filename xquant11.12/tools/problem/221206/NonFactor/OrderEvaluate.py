# -*- coding: utf-8 -*-
"""
Created on Wed Aug 16 17:52:29 2017

@author: 006688
"""
# 根据相邻两个切片盘口还原出委托
# 整体逻辑思路（以卖盘为例）：以两个卖盘共同挂单价（较小卖一价至较小卖十价）上挂单数量差进行判断，考察新卖一价位置，其下方为主买，上方为撤单或补单
            ...
            if len(AskPriceAll) > 0:  # 涨停不进行比较
                ...
                for j in range(startid + 1, len(AskVolChange)):  # 分析主动买单上方部分
                    if AskVolChange[j] > 0:
                        ...
                        OrderBookTmp.append([self.__data.getLastTimeStamp(), AskPriceAll[j], -AskVolChange[j], 'C'])
            ...
            if len(BidPriceAll) > 0:  # 跌停不进行比较
                ...
                for j in range(startid + 1, len(BidVolChange)):  # 分析主动卖单下方部分
                    if BidVolChange[j] > 0:
                        ...
                        OrderBookTmp.append([self.__data.getLastTimeStamp(), BidPriceAll[j], -BidVolChange[j], 'C'])
        ...
        for k in range(len(OrderBookTmp)):
            if OrderBookTmp[k][3] == 'B':
                OrderAmountBuy += (OrderBookTmp[k][1] * OrderBookTmp[k][2]) / (
                    1 + math.exp(-1000 * (OrderBookTmp[k][1] / self.__midPrice.getContent()[-2] - 1)))
