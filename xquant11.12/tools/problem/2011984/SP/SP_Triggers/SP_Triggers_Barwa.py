...
def main():
    ...
    # trade_date = '20220301'
    user_id = '011668'
    code_signal_dict = load_json_file(f'/data/user/666888/barwa/stockList/{trade_date}/stockList{trade_date}.json')

    total_signal_lib, total_count = 0, 0
    for (signal_lib, code_list) in code_signal_dict.items():
        total_signal_lib += 1
        total_count += len(code_list)
        code_select = code_list.copy()
        save_path = f'/data/user/{user_id}/CVTriggers/SPTriggers/Barwa/{trade_date}/{signal_lib}'
        time_list = ['10:00:00']
        while len(code_select) > 0:
            print(f'{signal_lib} - {len(code_select)}只标的需要计算')
            Task(code_select, trade_date, trade_date, 'Albest', signal_lib, save_path, time_list=time_list).start(mode=mode)
            code_select = list(sorted(set(code_select) - set([x[:-5] for x in file_list_dir(save_path)])))
    LinkMessage().sendMessage("011668", f"{trade_date} Barwa 实盘阈值已生成， {total_signal_lib}个信号库合计{total_count}只标的")
    LinkMessage().sendMessage("015390", f"{trade_date} Barwa 实盘阈值已生成， {total_signal_lib}个信号库合计{total_count}只标的")
