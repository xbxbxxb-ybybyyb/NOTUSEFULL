...
def main():
    ...
    signal_lib = 'Everest20211001_20210515'
    save_path = f'/data/user/{user_id}/CVTriggers/SPTriggers/Everest/{trade_date}/{signal_lib}'
    while len(code_select) > 0:
        print(f'{len(code_select)}只标的需要计算')
        Task(code_select, trade_date, trade_date, 'Everest', signal_lib, save_path).start(mode=mode)
        code_select = list(sorted(set(code_select) - set([x[:-5] for x in file_list_dir(save_path)])))


class SignalCV:
    def __init__(self, st_date, ed_date, code, strategy, signal_lib, save_path, vt_name='8min', dfs=None):
    ...
    def select_best_triggers(self, signal_data_df_select, dir_):
        ...
        params = {'min_triggers': 5, 'open_p': 5, 'open_coef': -1.0, 'close_p1': 10, 'close_p2': 20, 'close_coef': 0.0, 'close_slip': 0}
        if self.strategy == 'Albest':
            params.update({'close_slip': 0.2})
        if self.strategy == 'Everest':
            params.update({'close_slip': -0.1})
...
class Task:
    def __init__(self, code_list, st_date, ed_date, strategy, signal_lib, save_path, vt_name='8min'):
    ...
    def single_task_spark(self, context, code):
        ...
        signal_cv = SignalCV(self.st_date, self.ed_date, code, self.strategy, self.signal_lib, save_path_spark, vt_name=self.vt_name, dfs=dfs)
        signal_cv.start()
