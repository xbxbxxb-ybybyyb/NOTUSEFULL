...
def main():
    ...
    freq = "1"
    signal_lib = 'Albest20211101Order{}Signals'.format(freq)
    ...
    while len(code_select) > 0:
        ...
        Task(code_select, trade_date, trade_date, strategy, signal_lib, save_path, time_list=time_list,
             tag_lib=tag_lib, mock_lib=mock_lib, is_add_mock=False, overwrite_params={'min_triggers': 15}).start(mode=mode)  # local, multi_processing, spark
...
class SignalCV:
    def __init__(self, st_date, ed_date, code, strategy, signal_lib, save_path='lib', time_list=None, vt_name='8min', tag_lib=None, mock_lib=None,
        ...
        if save_path != 'lib':
            ...
            self.fa = FactorData()
        self.__trigger_save_lib = 'DynamicTriggers1000'
    ...
    def start(self):
        ...
        for trade_date in valid_date_list:
            ...
            if signal_data_df_select.empty:
                ...
                long_trigger_ratio = self.select_best_triggers(signal_data_df_select, dir_='long', min_triggers=self._params['min_triggers'])
                short_trigger_ratio = self.select_best_triggers(signal_data_df_select, dir_='short', min_triggers=self._params['min_triggers'])
    ...
    def select_best_triggers(self, signal_data_df_select, dir_, min_triggers=1):
        ...
        # 寻找开仓阈值
        signal_index_df_open = signal_data[(signal_data[:, 0] >= min_triggers) & (signal_data[:, 1] < self._params['open_p'])]
    ...
    def get_date_list(self):
        ...
        date_list_p = FactorData().tradingday(FactorData().tradingday(self.st_date, -100)[0], self.ed_date)
...
class Task:
    def __init__(self, code_list, st_date, ed_date, strategy, signal_lib,
                 save_path='lib', time_list=None, vt_name='8min', tag_lib=None, mock_lib=None, is_add_mock=False, overwrite_params=None):
    ...
    def single_task_spark(self, context, code):
        try:
            ...
            signal_cv = SignalCV(self.st_date, self.ed_date, code, self.strategy, self.signal_lib, save_path_spark,
                                 time_list=self.time_list, vt_name=self.vt_name,
                                 tag_lib=self.tag_lib, mock_lib=self.mock_lib, is_add_mock=self.is_add_mock,
                                 overwrite_params=self.overwrite_params, dfs=dfs)
            signal_cv.start()
        except:
...
class SignalManager:
    def __init__(self, code, date_list, signal_lib, tag_lib=None, mock_lib=None, vt_name='8min', signal_col_names=None,
                 is_add_tag=False, is_get_valid=False, is_add_mock=True, is_add_pct=False):
            ...
            try:
                ...
                if not self.__is_add_mock:  # 剔除补的数据
                    ...
                    if self.__is_add_pct:
                        ...
                        data2 = data2[['T_IsMock', 'pct', 'T_LastPrice']]
