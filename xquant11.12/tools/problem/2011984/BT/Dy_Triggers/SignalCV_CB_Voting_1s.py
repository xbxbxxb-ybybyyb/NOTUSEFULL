...
class SignalCV:
    def __init__(self, st_date, ed_date, code, signal_tag_lib_list, save_path='lib', is_add_mock=True, dfs=None):
        self.st_date = st_date
        self.ed_date = ed_date
        self.code = code
        self.signal_tag_lib_list = signal_tag_lib_list
    ...
    def select_best_triggers(signal_data_df_select, tag_name, dir_):
        ...
        # 寻找开仓阈值
        signal_index_df_open = signal_data[(signal_data[:, 0] >= 15) & (signal_data[:, 1] < 10)]  # 日均最小触发5次，小于5%分位数
    ...
    def get_signal_data(self):
        ...
        signal_data_list = []
        for [signal_lib, tag_lib] in self.signal_tag_lib_list:
            signal_data = SignalManager(self.code, date_list, signal_lib, tag_lib=tag_lib, is_add_tag=True, is_get_valid=True,
            ...
            signal_data_list.append(signal_data_df)

        signal_data_df_combine = pd.concat(signal_data_list).sort_values(by='Timestamp')

        signal_dict = get_voting_dict([0.25, 0.5, 1, 2, 5])
        for tag_name in self.tag_list:
            if tag_name not in [0.25, 0.5, 1, 2, 5]:
                signal_data_df_combine[f'{tag_name}minLong'] = signal_data_df_combine[[f'{t}minLong' for t in signal_dict[tag_name]]].mean(axis=1)
                signal_data_df_combine[f'{tag_name}minShort'] = signal_data_df_combine[[f'{t}minShort' for t in signal_dict[tag_name]]].mean(axis=1)
                signal_data_df_combine[f'tag{tag_name}minLong'] = signal_data_df_combine[[f'tag{t}minLong' for t in signal_dict[tag_name]]].mean(axis=1)
                signal_data_df_combine[f'tag{tag_name}minShort'] = signal_data_df_combine[[f'tag{t}minShort' for t in signal_dict[tag_name]]].mean(axis=1)
        date_list = list(sorted(set(date_list).intersection(set([x_[0] for x_ in signal_data_df_combine.index]))))
        return signal_data_df_combine, date_list
...
class Task:
    def __init__(self, code_list, st_date, ed_date, signal_tag_lib_list, save_path, is_add_mock):
    ...
    def single_task(self, code_list):
        for code in code_list:
            signal_cv = SignalCV(self.st_date, self.ed_date, code, self.signal_tag_lib_list, self.save_path, self.is_add_mock)
            signal_cv.start()

    def single_task_spark(self, context, code):
        dfs = context.get_hdfs()
        save_path_spark = self.save_path.replace('/data/user/', '')
        signal_cv = SignalCV(self.st_date, self.ed_date, code, self.signal_tag_lib_list, save_path_spark, self.is_add_mock, dfs=dfs)
        signal_cv.start()
