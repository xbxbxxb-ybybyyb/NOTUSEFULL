"""Everest 1秒频策略（股票T+0策略）下单Executor——update @2022.4.15"""
...
class ExecutorEverestSP1S(SignalExecutorBase):
    ...
    def on_new_day(self, trade_date):
        ...
        if str(trade_date) in self._param['order_capacity'].keys():  # 没有值的话就不更新，沿用前一天的值
            ...
            try:
                self.__HIGH_VOL = self._param['high_vol'][str(trade_date)]
            except:
                raise ValueError('HighVol Error', self._param['code'], trade_date)
    ...
    def update_predict_info(self, predictions, slice_data):
        ...
        if self._positionMgr.is_position_closed():  # 没有头寸
            ...
            if predictions[0] > self.__cal_close_trigger() or \
                    (self.__signal_sorted(self._bid_predictions[-5:]) and (predictions[0] > self.__init_open_long_threshold - 0.3)):
    ...
    def __process_close_signal(self, predictions, slice_data, side):
        ...
        if side == OrderSide.Buy:
            if (predictions[0] > self.__init_open_long_threshold) \
                    or (self.__signal_sorted(self._bid_predictions[-5:]) and (predictions[0] > self.__init_open_long_threshold - 0.3)):  # 平仓且反向开多
    ...
    def __process_stop_loss(self, predictions, slice_data, side):
        ...
        vol_ema = self.__ema_volume()
        net_position = self._positionMgr.get_net_position()
        volume = min(vol_ema * 2.5, self.__MAX_QTY_PER_ORDER * 5, abs(net_position))
    ...
    def __cal_open_quantity(self, side, prediction, slice_data, is_init_open=True):
        ...
        if side == OrderSide.Buy:
            oppo_price, oppo_vol = slice_data.askPrice[0], slice_data.askVolume[0]
            delta = self._bid_predictions[-1] - self._bid_predictions[-2] if len(self._bid_predictions) > 1 else 0
            vol_ratio = self.__cal_vol_ratio(prediction, self.triggersDict['longTriggerRatio'], delta, self.__open_long_times)
            trade_percent = self.__get_position_percent_in_market(max(1, int(self.__tick_count_since_init_open_long)) - 1)
            total_market_amt_since_open = sum(self._amt_today[- max(1, int(self.__tick_count_since_init_open_long)) - 1:])
        else:  # side == OrderSide.Sell
            oppo_price, oppo_vol = slice_data.bidPrice[0], slice_data.bidVolume[0]
            delta = self._ask_predictions[-1] - self._ask_predictions[-2] if len(self._ask_predictions) > 1 else 0
            vol_ratio = self.__cal_vol_ratio(prediction, self.triggersDict['shortTriggerRatio'], delta, self.__open_short_times)
            trade_percent = self.__get_position_percent_in_market(max(1, int(self.__tick_count_since_init_open_short)) - 1)
            total_market_amt_since_open = sum(self._amt_today[- max(1, int(self.__tick_count_since_init_open_short)) - 1:])

        open_vol = (1 + 9 * min(1, vol_ratio)) * self.__MAX_QTY_PER_ORDER
        limit_vol = max(0, total_market_amt_since_open * trade_percent / oppo_price - abs_net_position) if oppo_price != 0 else 0
        available_vol = self._positionMgr.get_avail_qty()
        quantity = min(open_vol, max(oppo_vol, 0.5 * vol_ema), 1.5 * vol_ema, limit_vol, available_vol)

        if not is_init_open:
            limit_total_vol = min(max(vol_ema * 4, self.__MAX_QTY_PER_ORDER * 5), self.__MAX_QTY_PER_ORDER * 20)
            if abs_net_position + int(quantity / 100) * 100 > limit_total_vol:  # 累计下单量超过上限
    ...
    def __cal_open_trigger(self):
        ...
        if self._positionMgr.is_position_positive():
            if self.__open_long_threshold is None:
                ...
                delta_bid_ma = 0.1 * self.__cal_ema_delta(self.__bid_predictions_delta)
                open_th = self.__open_long_threshold + delta_bid_ma
            ...
            if self.__open_short_threshold is None:
                ...
                delta_ask_ma = 0.1 * self.__cal_ema_delta(self.__ask_predictions_delta)
                open_th = self.__open_short_threshold - delta_ask_ma
        return open_th
    ...
    def __cal_close_trigger(self):
        ...
        if self._positionMgr.is_position_negative():
            if self.__close_short_threshold is None:
                ...
                delta_bid_ma = 0.1 * self.__cal_ema_delta(self.__bid_predictions_delta)
                close_th = self.__close_short_threshold + delta_bid_ma
            ...
            if self.__close_long_threshold is None:
                ...
                delta_ask_ma = 0.1 * self.__cal_ema_delta(self.__ask_predictions_delta)
                close_th = self.__close_long_threshold - delta_ask_ma
    ...
    def __process_market_closed(self, slice_data):
        ...
        if self._param['close_time_morning'] <= tick_time <= '11:30:00' or self._param['close_time_afternoon'] <= tick_time <= '14:59:59':
            ...
            net_position = self._positionMgr.get_net_position()
            vol_limit = abs(net_position) / 5
            vol_ema = self.__ema_volume()
            quantity = min(max(vol_ema * 5, vol_limit), abs(net_position))
    ...
    def __pre_processing(self):
        ...
        self.__ask_predictions_delta.append(abs(self._ask_predictions[-1] - self._ask_predictions[-2]) if len(self._ask_predictions) > 1 else 0)
        self.__bid_predictions_delta.append(abs(self._bid_predictions[-1] - self._bid_predictions[-2]) if len(self._bid_predictions) > 1 else 0)
        ...
        if self._positionMgr.is_position_closed():
            ...
            if time.strftime('%H:%M:%S', time.localtime(slice_data.timeStamp)) >= self._param['high_vol_start_time']:
                ...
                if log_vol_ma_200 <= self.__HIGH_VOL:
                    loss_ratio = min(self.__max_long_return_rate - curr_return, 3.0) / 6
            ...
            self.__min_ask = min(self.__min_ask, predictions[1])
            self.__max_bid = predictions[0]
            short_close_threshold_offset = max(0, self.__init_open_short_threshold - self.__min_ask) / 2.0
            ...
            self.__init_short_risk_threshold += short_close_threshold_offset
    ...
    # 风控指标
    @staticmethod
    def __get_position_percent_in_market(tick_num):
    ...
    def __log_vol_ma_200(self):
        if self._volume_today is None or len(self._volume_today) == 0:
            return 0.0
        return np.mean(np.log(np.array(self._volume_today[-200:]) + 100.))

    @staticmethod
    def __cal_ema_delta(delta_list):
        delta = 0
    ...
    def is_cancel(self, predictions, slice_data):
        ...
        if self.__tick_count_since_init_open_long is not None:
            self.__tick_count_since_init_open_long += 1

        if self.__tick_count_since_init_open_short is not None:
            self.__tick_count_since_init_open_short += 1
