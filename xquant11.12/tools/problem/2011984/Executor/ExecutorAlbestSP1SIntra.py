...
class ExecutorAlbestSP1SIntra(SignalExecutorBase):
    def __init__(self, position_manager, param_manager, total_position_manager):
        ...
        self.time_list = self._param['time_list']
        self.trigger_intra = dict([(x, None) for x in self.time_list])
        # 初始开仓阈值参数
    ...
    def on_new_day(self, trade_date):
        if 'triggers_by_date' not in self._param.keys():  # 所有日期都是一套阈值
            ...
            for time_single in self.time_list:
                self.trigger_intra.update({time_single: self._param['triggers_by_date'][f'{trade_date}_{time_single}']})
        self.__pre_net_position = 0
    ...
    def update_predict_info(self, predictions, slice_data):
        for time_single in self.time_list[::-1]:
            if slice_data.time > int(time_single.replace(':', '')) * 1000:
                self.triggersDict = self.trigger_intra[time_single]
                break
