...
class ExecutorAlbestSP1SVolRatio(SignalExecutorBase):
    ...
    def __cal_open_quantity(self, side, prediction, slice_data, is_init_open=True):
        ...
        if not np.isnan(self._param['best_qty']):
            quantity = max(quantity * self._param['init_qty'] / self._param['best_qty'], 100)
        return quantity
