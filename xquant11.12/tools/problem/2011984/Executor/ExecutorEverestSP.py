"""Everest策略（股票T+0策略）下单Executor——update @2021.7.25"""
...
class ExecutorEverestSP(SignalExecutorBase):
    ...
    def __cal_open_quantity(self, side, prediction, slice_data, is_init_open=True):
        ...
        if side == OrderSide.Buy:
            ...
            trade_percent = self.__get_position_percent_in_market(self.__tick_count_since_init_open_long - 1)
            total_market_amt_since_open = sum(self._amt_today[-self.__tick_count_since_init_open_long - 1:])
            ...
            trade_percent = self.__get_position_percent_in_market(self.__tick_count_since_init_open_short - 1)
            total_market_amt_since_open = sum(self._amt_today[-self.__tick_count_since_init_open_short - 1:])
    ...
    def __cal_open_trigger(self):
        ...
        if self._positionMgr.is_position_positive():
            if self.__open_long_threshold is None:
                ...
                delta_bid_ma = 0.1 * self.__cal_ema_delta(self._bid_predictions[-5:])
            ...
            if self.__open_short_threshold is None:
                ...
                delta_ask_ma = 0.1 * self.__cal_ema_delta(self._ask_predictions[-5:])
    ...
    @staticmethod
    def __cal_ema_delta(predictions_list):
        delta = 0
        if len(predictions_list) < 2:
            return delta
        for i in range(len(predictions_list) - 1):
            delta = 0.75 * delta + 0.25 * abs(predictions_list[i + 1] - predictions_list[i])
        return delta
