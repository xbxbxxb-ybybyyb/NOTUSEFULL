"""Albest策略（股票T+0策略）下单Executor——update @2021.7.25"""
...
class ExecutorAlbestSPIntra(SignalExecutorBase):
    def __init__(self, position_manager, param_manager):
        ...
        self.triggers_am = None
        self.triggers_pm = None
    ...
    def on_new_day(self, trade_date):
        if 'triggers_by_date' not in self._param.keys():  # 所有日期都是一套阈值
            ...
            self.triggers_am = self._param['triggers_by_date'][f'{trade_date}_09:30:00']
            self.triggers_pm = self._param['triggers_by_date'][f'{trade_date}_10:00:00']
        self.__pre_net_position = 0
    ...
    def update_predict_info(self, predictions, slice_data):
        if slice_data.time > 100000000:
            self.__init_open_long_threshold = self.triggers_pm['longTriggerRatio']
            self.__init_close_long_threshold = self.triggers_pm['longCloseRatio']
            self.__init_long_risk_threshold = self.triggers_pm['longRiskRatio']
            self.__init_open_short_threshold = self.triggers_pm['shortTriggerRatio']
            self.__init_close_short_threshold = self.triggers_pm['shortCloseRatio']
            self.__init_short_risk_threshold = self.triggers_pm['shortRiskRatio']
        else:
            self.__init_open_long_threshold = self.triggers_am['longTriggerRatio']
            self.__init_close_long_threshold = self.triggers_am['longCloseRatio']
            self.__init_long_risk_threshold = self.triggers_am['longRiskRatio']
            self.__init_open_short_threshold = self.triggers_am['shortTriggerRatio']
            self.__init_close_short_threshold = self.triggers_am['shortCloseRatio']
            self.__init_short_risk_threshold = self.triggers_am['shortRiskRatio']
    ...
    def __cal_open_quantity(self, side, prediction, slice_data, is_init_open=True):
        vol_ema = self.__ema_volume()
        if slice_data.time > 100000000:
            trigger_dict = self.triggers_pm
        else:
            trigger_dict = self.triggers_am
        if side == OrderSide.Buy:
            oppo_vol = slice_data.askVolume[0]
            delta = self._bid_predictions[-1] - self.__exponential_predict(np.array(self._bid_predictions[-5:]))
            vol_ratio = self.__cal_vol_ratio(prediction, trigger_dict['longTriggerRatio'], delta, self.__open_long_times)
            ...
            vol_ratio = self.__cal_vol_ratio(prediction, trigger_dict['shortTriggerRatio'], delta, self.__open_short_times)
    ...
    def __pre_processing(self):
        ...
        elif curr_net_position < 0 <= last_net_position:
