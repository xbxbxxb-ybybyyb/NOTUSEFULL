"""Kunlun投票策略（转债策略）下单Executor——update @2021.8.4"""
...
class ExecutorKunlunSPVoting(SignalExecutorBase):
    def __init__(self, position_manager, param_manager, get_total_net_position, get_total_finished_orders):
        SignalExecutorBase.__init__(self, position_manager, param_manager)
        self.__getTotalNetPosition = get_total_net_position
        self.__getTotalFinishedOrders = get_total_finished_orders
    ...
    # onNewTick
    def update_predict_info(self, predictions, slice_data, vt_name=None):
    ...
    def __process_market_closed(self, slice_data, predictions):
        ...
        if self._param['close_time_morning'] <= tick_time <= '11:30:00' or self._param['close_time_afternoon'] <= tick_time <= '14:59:59':
            if self._positionMgr.is_position_closed():
                return True
            net_position = self.__getTotalNetPosition()
            ...
            if net_position > 0:
                if self._param['close_time_morning'] <= tick_time < self._param['easy_close_time_morning']:  # 上午第一阶段平仓
                    ...
                    price = bid0
                self.__order.update({"MarketCloseLong": (price, quantity)})
            return True
        return False
    ...
    def __check_trade_vol_percentage(self, qty, side, slice_data):
        ...
        traded_volume = 0
        finished_orders = self.__getTotalFinishedOrders()
        for order in finished_orders:
            if "Self" in order.orderType:
                continue
    ...
    def get_ema_volume(self):
        return self.__ema_volume()
    ...
    def is_market_close_long(self):
        if "MarketCloseLong" in self.__order:
            price, volume = self.__order["MarketCloseLong"]
            return {"price": price, "volume": volume}
        return False
