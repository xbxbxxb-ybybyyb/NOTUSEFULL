...
class FactorAskDisMaxMinSum(Factor):
    ...
    def calculate(self):
        ...
        if ask_price_max > 1e-6 and ask_price_min > 1e-6:
            ask_max_value = (ask_price_max / ask_price_list[-1] - 1) * 1000
            ask_min_value = (ask_price_min / ask_price_list[-1] - 1) * 1000
            ask_max_min_value = ask_max_value + ask_min_value
        else:
            ask_max_min_value = 0

        ask_max_list = self.getIntermediate("ask_max")
        ask_max_list.append(ask_max_min_value)
