...
class FactorAskDealWaitingTime(Factor):
    def __init__(self, config, factorManager):
        super().__init__(config, factorManager)
        self.__lag = self._getParameter("Lag")

        self._addIntermediate("AskDealTime", [])
        self._early_order_dict = None
        self._noon_order_dict = None

    def calculate(self):

        ask_deal_time = self.getIntermediate("AskDealTime")
        ...
        if trans is not None:
            ...
            this_trans_index = list(self._getTransactionData("AskOrder", trans))
            this_trans_time = list(self._getTransactionData("Timestamp", trans))
            this_trans_time_dict = dict(zip(this_trans_index, this_trans_time))  # 成交时间，对于重复的AskOrder，取最后一笔时间
        ...
        ask_deal_time.append(this_deal_time)

        if np.any(~np.isnan(ask_deal_time[-self.__lag:])):
            factorValue = np.nanmean(ask_deal_time[-self.__lag:]) / 10
        else:
            factorValue = 0.
