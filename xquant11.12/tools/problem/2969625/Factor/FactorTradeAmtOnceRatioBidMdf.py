...
class FactorTradeAmtOnceRatioBidMdf(Factor):
    ...
    def calculate(self):
        ...
        if len(trade_matrix_select) > 0:
            ...
            vol_once_sum = np.nansum(vol_array[(dir_array == 1.0) & (count_array == 1)] * price_array[(dir_array == 1.0) & (count_array == 1)])
            vol_total_sum = np.nansum(vol_array[dir_array == 1.0] * price_array[dir_array == 1.0])
