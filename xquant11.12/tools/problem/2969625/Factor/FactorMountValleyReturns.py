...
class FactorMountValleyReturns(Factor):
    ...
    def calculate(self):
        ...
        if len(mvloc) > 0:
            ...
            if mvloc[-1] < 0:
                itmp = np.nanargmax(sub_midp)
            else:
                itmp = np.nanargmin(sub_midp)
            if (itmp > len(sub_midp) - self.__wd) and (itmp < len(sub_midp) - 2):
                factorValue = (midpw[-1] / sub_midp[itmp] - 1) * 1e2
            else:
                factorValue = (midpw[-1] / mvv[-1] - 1) * 1e2
        else:
            factorValue = 0
