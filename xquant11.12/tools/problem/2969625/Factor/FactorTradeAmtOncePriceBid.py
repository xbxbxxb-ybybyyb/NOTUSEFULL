...
class FactorTradeAmtOncePriceBid(Factor):
    def __init__(self, config, factorManager):
        super().__init__(config, factorManager)
        self.__lag = self._getParameter("Lag")

    def calculate(self):
        bid1_price = self._getLastTickData("BidPrice")[0]
        if bid1_price < 1e-4:
            bid1_price = self._getLastTickData("MinPrice")
        ...
        if len(trade_matrix_select) > 0:
            ...
            vol_once_sum = np.nansum(vol_array[(price_array >= bid1_price) & (count_array == 1)] *
                                     price_array[(price_array >= bid1_price) & (count_array == 1)])
            vol_total_sum = np.nansum(vol_array[price_array >= bid1_price] * price_array[price_array >= bid1_price])
