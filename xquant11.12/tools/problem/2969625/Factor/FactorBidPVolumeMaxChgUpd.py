...
class FactorBidPVolumeMaxChgUpd(Factor):
    ...
    def calculate(self):
        ...
        if len(tick_time_crt) < self.__mlag * 20:
            bidp_dict = self.__update_price_dict(bidp_dict, dict(zip(bidp[-1], bidv[-1])))
        else:
            bidp_dict = self.__update_price_dict(bidp_dict, dict(zip(bidp[-1], bidv[-1])), dict(zip(bidp[0], bidv[0])))
