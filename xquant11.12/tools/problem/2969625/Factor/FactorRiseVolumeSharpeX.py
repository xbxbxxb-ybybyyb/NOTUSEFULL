...
class FactorRiseVolumeSharpeX(Factor):
    ...
    def calculate(self):
        ...
        riseRatioSlice = np.array(riseRatioList[-self.__window:])

        if len(riseRatioSlice) > 5 and np.nanstd(riseRatioSlice) > 1e-6:
            factorValue = np.nanmean(riseRatioSlice) / np.nanstd(riseRatioSlice)
        else:
