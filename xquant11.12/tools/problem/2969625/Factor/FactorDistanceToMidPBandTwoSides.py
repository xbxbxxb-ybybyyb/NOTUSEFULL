...
class FactorDistanceToMidPBandTwoSides(Factor):
    ...
    def calculate(self):
        ...
        mvolatility_up = np.nanstd(mrtns[mrtns > 0])
        mvolatility_down = np.nanstd(mrtns[mrtns < 0])
        expt_high = mid_price_pre * (1 + mvolatility_up * self.__vscale)
        expt_low = mid_price_pre * (1 - mvolatility_down * self.__vscale)
