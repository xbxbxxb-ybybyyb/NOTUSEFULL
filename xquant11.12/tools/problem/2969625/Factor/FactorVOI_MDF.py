...
class FactorVOI_MDF(Factor):
    ...
    def calculate(self):
        bidPrice = copy.deepcopy(self._getLastNTickData("BidPrice", 2))
        askPrice = copy.deepcopy(self._getLastNTickData("AskPrice", 2))
        ...
        if len(bidPrice) <= 1:
            ...
            for i in range(2):
                askPrice[i][askPrice[i] == 0] = maxPrice
                bidPrice[i][bidPrice[i] == 0] = minPrice
            ...
            orderImbalance = (deltaVB - deltaVA) / deltaSum if deltaSum != 0. else 0.
