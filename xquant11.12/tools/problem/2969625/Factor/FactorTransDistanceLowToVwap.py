...
class FactorTransDistanceLowToVwap(Factor):
    def __init__(self, config, factorManager):
        ...
        self.__transLowp = self._getFactor(
            {
                "ClassName": "TransLowPrice",
                "Parameters": {
                    "Direction": "Both",
                }
            }
    ...
    def calculate(self):
        ...
        tlowp = self.__transLowp.getLastFactorValue()

        filter_llts = list(filter(lambda x: x is not None, llts))
        if not np.isnan(tvwap):
            dist = (np.divide(tvwap, tlowp) - 1) * 1e3
            dists.append(dist)
