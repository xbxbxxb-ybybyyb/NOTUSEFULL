...
class FactorVolumeTopTailVwapRatio_MDF(Factor):
    ...
    def calculate(self):
        ...
        if len(volumeList) == 0:
            self.__hisVolumeList = self._getAllHistoricalTickData("Volume").tolist()
            self.__hisAmountList = self._getAllHistoricalTickData("Amount").tolist()

        amountList.append(amount)
        volumeList.append(volume)

        volumeAll = self.__hisVolumeList + volumeList
        amountAll = self.__hisAmountList + amountList
        volumeSlice = np.array(volumeAll[-self.__window:])
        amountSlice = np.array(amountAll[-self.__window:])
