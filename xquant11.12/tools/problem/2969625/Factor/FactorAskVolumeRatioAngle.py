...
class FactorAskVolumeRatioAngle(Factor):
    ...
    def calculate(self):
        ...
        askVolumeRatioSlice = np.array(filterAskVolumeRatioList)
        if len(askVolumeRatioSlice) > 3:
            lookBack = self.__lag if self.__lag < len(askVolumeRatioSlice) else 3
