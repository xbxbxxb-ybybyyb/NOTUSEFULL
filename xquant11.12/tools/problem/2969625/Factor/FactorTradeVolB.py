...
class FactorTradeVolB(Factor):
    ...
    def calculate(self):
        ...
        if len(transaction_array) > 0:
            ...
            buy_vol_sum = np.nansum(trade_vol[(trade_price <= bid0 + 0.01) & (trade_dir == 1)])
            sell_vol_sum = np.nansum(trade_vol[(trade_price >= ask0 - 0.01) & (trade_dir == 1)])
