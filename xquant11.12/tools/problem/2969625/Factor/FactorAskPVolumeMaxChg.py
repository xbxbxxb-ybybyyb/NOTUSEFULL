...
class FactorAskPVolumeMaxChg(Factor):
    ...
    def calculate(self):
        ...
        if len(tick_time_crt) < self.__mlag * 20:
            askp_dict.update(self.__update_price_dict(askp_dict, dict(zip(askp[-1], askv[-1]))))
        else:
            askp_dict.update(self.__update_price_dict(askp_dict, dict(zip(askp[-1], askv[-1])), dict(zip(askp[0], askv[0]))))
