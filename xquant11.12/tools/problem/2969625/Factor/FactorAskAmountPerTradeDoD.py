...
class FactorAskAmountPerTradeDoD(Factor):
    ...
    def calculate(self):
        ...
        if self.__amountPerTradeHistorical is not None:
            ...
            if current_time in self.__amountPerTradeHistorical and (self.__amountPerTradeHistorical.loc[current_time] > 1e-6):
                factorValue = np.nanmean(ask_deal_amount_list[-self.__lag:]) / self.__amountPerTradeHistorical.loc[current_time]
            elif np.nanmean(self.__amountPerTradeHistorical) > 1e-6:  # 如果没有当前Minute的用全天均值代替
                factorValue = np.nanmean(ask_deal_amount_list[-self.__lag:]) / np.nanmean(self.__amountPerTradeHistorical)
            else:
                factorValue = 0.
        else:
            factorValue = 0.
