...
def main(mdLibrary, tickLibrary, tranOrderLibrary, l2pTickLibrary, l2pTranOrderLibrary, infLibrary, tickCleanMode,
    ...
    if checkMode in ["Local", "Spark"]:
        tickFactorNameList, taskList = check_completeness(
            ...
            str(eDate), taskList, tickLibrary)
        if len(taskList) > 0:
...
if __name__ == "__main__":
    ...
    tickLibrary_SH = "ZeusNewDataLib"#"Channel036STickDataLib"
    tickLibrary_SZ = "Channel036STickDataLib"
    tranOrderLibrary = "OrderTradeDataSH2023IT"
    l2pTickLibrary = ""
    l2pTranOrderLibrary = ""
    INFLibrary = "INFactor"

    tickOutputLibrary = "Factor_Zeus_Plus_20230725"
    l2pTickOutputLibrary = ""
    minuteOutputLibrary = ""

    tickCleanMode_SH = "StartEnd"  # ["StartEnd", "StartNotEnd", "NotClean"]
    ...
    stockList_SH = [i for i in stockList if i[-1] == 'H']
    stockList_SZ = [i for i in stockList if i[-1] == 'Z']
    stockList_SH += ['689009.SH']
    # stockList_SH = ['603586.SH', '688266.SH', '688512.SH', '603657.SH', '688196.SH']
    # stockList_SH = ['603586.SH']
    ...
    # stockList_SH = stockList_SH[0:1]
    print(stockList_SH)
    #股票因子
    import pandas as pd
    df_factor = pd.read_excel('/data/user/018106/TrendFactor/factor_pass/final_0907_1.xlsx')
    factorList_old = df_factor['oldNotOrder'][~df_factor['oldNotOrder'].isnull()].tolist() + df_factor['oldOrder'][~df_factor['oldOrder'].isnull()].tolist() + df_factor['SPNotOrder'][~df_factor['SPNotOrder'].isnull()].tolist() + df_factor['SPOrder'][~df_factor['SPOrder'].isnull()].tolist() + \
...
#    barwa = eval(factor_data[0])
    ...
    # factorList = ['factorTransBuyVolumeDistance5_40', 'factorTransSellVolumeDistance5_10', 'factorTransVolumeWeightedSwing5_10', 'factorTransVolumeWeightedSwing5_40', 'factortradeAskBidVolume', 'factorActiveBuyVolGrowth40']
...
#    factorConfig = TICK_FACTOR_CONFIG_1 + TICK_FACTOR_CONFIG_2 + TICK_TAG_CONFIG
    ...
    cm = CalculationMonitor(assetType, "Daily", startDate, endDate, stockList_SH, tickOutputLibrary, TICK_FACTOR_CONFIG,
...
#        while not flag_1 or not flag_2:
        ...
        while not flag_1:
            ...
            flag_1 = main(marketDataLibrary, tickLibrary_SH, tranOrderLibrary, l2pTickLibrary, l2pTranOrderLibrary, INFLibrary,
                        ...
                        startDate, endDate, stockList_SH, stockL2PList, mode, dayUnit, stockUnit, maxExecutorNum, stockGroupNum,
