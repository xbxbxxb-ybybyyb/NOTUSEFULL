...
class ETFDataManager:
    ...
    def __init__(self, libraryName, assetType, underlyingAssetType, startDate, endDate, stockSetTS, stockSetCS,
                 stockSetAll, configAnalyzer):
        self.__PRICE_COLUMN_NAMES_1 = ["PreviousClose", "LastPrice", "OpenPrice", "HighPrice", "LowPrice", "MaxPrice", "MinPrice"]
        ...
        self.__tickColumnNames1 = ETF_TICK_COLUMN_NAMES1
        self.__tickColumnNames2 = ETF_TICK_COLUMN_NAMES2
        self.__minuteColumnNames = ETF_MINUTE_COLUMN_NAMES
        self.__dailyColumnNames = ETF_DAILY_COLUMN_NAMES
        self.__tickColumnIndexDict1 = ETF_TICK_COLUMN_INDEX_DICT1
        self.__transactionColumnIndexDict = ETF_TRANSACTION_COLUMN_INDEX_DICT
        self.__priceColumnIndex3 = ETF_TRANSACTION_COLUMN_INDEX_DICT["Price"]
    ...
    def __loadStockHFData(self):
        self.__HFDataDict = {stock: HFData(self.__library, stock) for stock in self.__stockSetAll}
    ...
    def __loadIndexHFData(self):
        ...
        self.__HFDataDictForIndex = {index: HFData(self.__library, index) for index in self.__indexSet}
    ...
    def __loadUAHFData(self):
        ...
        self.__HFDataDictForUA = {ua: HFData(self.__library, ua) for ua in self.__UASet}
    ...
    def __loadTickDataFirstTime(self):
        for stock in self.__stockSetAll:
            ...
            if startDate > endDate:  # 不需要Tick数据
                ...
                self.__tickDataDict1[stock], self.__tickDataDict2[stock] = self.__preprocessTickData(stock, startDate, endDate)
                if self.__splitAdjustedNeeded:
                    self.__tickDataDict1SplitAdjusted[stock], self.__tickDataDict2SplitAdjusted[stock] = \
                        self.__preprocessTickData(stock, startDate, endDate)
    ...
    def __updateTickData(self, date):
        for stock in self.__stockSetAll:
            ...
            data1, data2 = self.__preprocessTickData(stock, updateDate, updateDate)
    ...
    def __updateUATickData(self, date):
        ...
        for ua in self.__UASet:
            data1 = self.__preprocessUATickData(ua, updateDate, updateDate)
