...
from System.COLUMN_CONFIG import (CINDEX_TICK_COLUMN_NAMES1, CINDEX_MINUTE_COLUMN_NAMES,
                                  CINDEX_DAILY_COLUMN_NAMES, CINDEX_TICK_COLUMN_INDEX_DICT1)
...
class IndexDataManager:
    ...
    def __init__(self, libraryName, assetType, underlyingAssetType, startDate, endDate, stockSetTS, stockSetAll,
                 configAnalyzer):
        self.__PRICE_COLUMN_NAMES_1 = ["PreviousClose", "LastPrice", "OpenPrice", "HighPrice", "LowPrice"]
        ...
        self.__tickColumnNames1 = CINDEX_TICK_COLUMN_NAMES1
        self.__minuteColumnNames = CINDEX_MINUTE_COLUMN_NAMES
        self.__dailyColumnNames = CINDEX_DAILY_COLUMN_NAMES
        self.__tickColumnIndexDict1 = CINDEX_TICK_COLUMN_INDEX_DICT1

        self.__tickColumnNames1UA = INDEX_TICK_COLUMN_NAMES
        self.__minuteColumnNamesUA = INDEX_MINUTE_COLUMN_NAMES
        self.__dailyColumnNamesUA = INDEX_DAILY_COLUMN_NAMES
        self.__tickColumnIndexDict1UA = INDEX_TICK_COLUMN_INDEX_DICT
        ...
        for date in getTradingDay(startDate, endDate):
            for stock in self.__stockSetTS:
                underlyingAsset = configAnalyzer.getUnderlyingAsset(stock, date)
                if underlyingAsset is not None:
                    self.__UASet.add(underlyingAsset)
        self.__HFDataDictForUA = {ua: HFData(libraryName, ua) for ua in self.__UASet}
    ...
    def __loadTickDataFirstTime(self):
        for stock in self.__stockSetAll:
            ...
            self.__tickDataDict1[stock] = self.__preprocessTickData(
                stock, startDate, endDate
            )

            if self.__splitAdjustedNeeded:
                self.__tickDataDict1SplitAdjusted[stock] = self.__preprocessTickData(
                    stock, startDate, endDate
                )
    ...
    def __loadUAMinuteData(self):
        for ua in self.__UASet:
            if self.__historicalMinuteDataLengthUA > 0:
                startDate = getNDaysOff(self.__startDate, self.__historicalMinuteDataLengthUA)
                if "Minute" in self.__dataTypeUA:
                    endDate = self.__endDate
                else:
                    endDate = getNDaysOff(self.__endDate, 1)
                self.__minuteDataDict[ua] = self.__preprocessUAMinuteData(ua, startDate, endDate)
            else:
                if "Minute" in self.__dataTypeUA:
                    startDate = self.__startDate
                    endDate = self.__endDate
    ...
    def __loadUATickDataFirstTime(self):
        for ua in self.__UASet:
            if self.__historicalTickDataLengthUA > 0:
                startDate = getNDaysOff(self.__startDate, self.__historicalTickDataLengthUA)
                if "Tick" in self.__dataTypeUA:
                    endDate = self.__startDate
                else:
                    endDate = getNDaysOff(self.__startDate, 1)
                self.__tickDataDict1[ua] = self.__preprocessUATickData(ua, startDate, endDate)
            else:
                if "Tick" in self.__dataTypeUA:
                    startDate = self.__startDate
                    endDate = self.__startDate
    ...
    def __updateTickData(self, date):
        for stock in self.__stockSetAll:
            ...
            data1 = self.__preprocessTickData(stock, startDate, endDate)
    ...
    def __updateUATickData(self, date):
        ...
        elif self.__historicalTickDataLengthUA > 0:
            startDate = getNDaysOff(date, 1)
            endDate = getNDaysOff(date, 1)
            realStartDate = getNDaysOff(date, self.__historicalTickDataLengthUA)
        else:
        ...
        for ua in self.__UASet:
            data1 = self.__preprocessUATickData(ua, startDate, endDate)
    ...
    def __preprocessTickData(self, stock, startDate, endDate):
        ...
        return data[self.__tickColumnNames1].astype(np.float64)
    ...
    def __preprocessUATickData(self, stock, startDate, endDate):
        data = self.__HFDataDictForUA[stock].get_tick_data(startDate, endDate)
        data = data.loc[:, self.__tickColumnNames1UA].astype(np.float64)
    ...
    @staticmethod
    def __getFillTickData(invalidTradingDaySet, tickColumnNames1):
        invalidTradingDayList = list(invalidTradingDaySet)
        invalidTradingDayListStr = list(map(str, invalidTradingDayList))
        ...
        fillTickData = pd.DataFrame(index=dateList, columns=tickColumnNames1)
