...
class CalculationScheduler:
    def __init__(self):
        ...
        self.__tickLibrary = None
        self.__tranOrderLibrary = None
        self.__l2pTickLibrary = None
        self.__l2pTranOrderLibrary = None
        self.__infLibrary = None
        self.__tickFactorLibrary = None
        self.__l2pTickFactorLibrary = None
        self.__minuteFactorLibrary = None
    ...
    def setTickLibrary(self, mdLibrary):
        self.__tickLibrary = mdLibrary if mdLibrary != "" else None

    def setTranOrderLibrary(self, mdLibrary):
        self.__tranOrderLibrary = mdLibrary if mdLibrary != "" else None

    def setL2PTickLibrary(self, mdLibrary):
        self.__l2pTickLibrary = mdLibrary if mdLibrary != "" else None

    def setL2PTranOrderLibrary(self, mdLibrary):
        self.__l2pTranOrderLibrary = mdLibrary if mdLibrary != "" else None

    def setINFLibrary(self, infLibrary):
        self.__infLibrary = infLibrary

    def setTickCleanMode(self, tickCleanMode):
        self.__tickCleanMode = tickCleanMode

    def setFactorLibrary(self, tickFactorLibrary, l2pTickFactorLibrary, minuteFactorLibrary):
    ...
    def setStockL2PList(self, stockList):
        self.__stockSetL2P = set(stockList)
    ...
    def setTaskList(self, taskList):
        if taskList is not None:
            for stock, date in taskList:
                self.__taskList.append(TaskMeta(int(date), int(date), {stock}))
    ...
    def __initConfigAnalyzer(self):
        self.__configAnalyzer = ConfigAnalyzer(self.__mdLibrary, self.__assetType, self.__underlyingAssetType, self.__factorConfig)
    ...
    def __runTasksLocal(self):
        ...
        for taskMeta in self.__taskList:
            ...
            if self.__assetType == "Stock":
                dataManager = StockDataManager(self.__mdLibrary, self.__tickLibrary, self.__tranOrderLibrary,
                                               self.__l2pTickLibrary, self.__l2pTranOrderLibrary, self.__infLibrary, self.__tickCleanMode,
                                               self.__assetType, self.__underlyingAssetType, sDate, eDate, stockSetTS,
                                               stockSetCS, stockSetAll, stockSetTS.intersection(self.__stockSetL2P),
                                               self.__configAnalyzer)
            elif self.__assetType == "CB":
                dataManager = CBDataManager(self.__mdLibrary, self.__tickLibrary, self.__tranOrderLibrary,
                                            self.__l2pTickLibrary, self.__l2pTranOrderLibrary, self.__tickCleanMode, self.__assetType,
                                            self.__underlyingAssetType, sDate, eDate, stockSetTS, stockSetCS,
                                            stockSetAll, stockSetTS.intersection(self.__stockSetL2P),
                                            self.__configAnalyzer)
            elif self.__assetType == "ETF":
                dataManager = ETFDataManager(self.__mdLibrary, self.__assetType, self.__underlyingAssetType, sDate,
                                             eDate, stockSetTS, stockSetCS, stockSetAll, self.__configAnalyzer)
                ...
                dataManager = INFDataManager(self.__mdLibrary, self.__assetType, self.__underlyingAssetType, sDate,
                                             eDate, stockSetTS, stockSetAll, self.__configAnalyzer)
            elif self.__assetType == "StockAfternoon":
                dataManager = StockDataManagerAfternoon(self.__mdLibrary, self.__infLibrary, self.__assetType,
                                                        self.__underlyingAssetType, sDate, eDate, stockSetTS,
                                                        stockSetAll, self.__configAnalyzer)
            else:
            ...
            for stock in stockSetTS:
                ...
                factorManagerDict[stock] = FactorManager(self.__assetType, stock, self.__factorConfig, sDate, eDate,
                                                         stockSetAllSingle, self.__tickFactorLibrary,
                                                         self.__l2pTickFactorLibrary, self.__minuteFactorLibrary,
                                                         dataManager)
    ...
    def __runTasksSpark(self):
        ...
        for i, taskMeta in enumerate(self.__taskList):
            ...
            taskList.append(
                SparkTaskMeta(self.__mdLibrary, self.__tickLibrary, self.__tranOrderLibrary, self.__l2pTickLibrary,
                              self.__l2pTranOrderLibrary, self.__infLibrary, self.__tickCleanMode, self.__tickFactorLibrary,
                              self.__l2pTickFactorLibrary, self.__minuteFactorLibrary, self.__assetType,
                              self.__underlyingAssetType, self.__factorConfig, sDate, eDate, stockSetTS,
                              stockSetTS.intersection(self.__stockSetL2P), self.__sparkLogFilePath, i)
            )
        ...
        endDatetime = dt.datetime.now()
        print(f"INFO: Time cost for factor calculation: {round((endDatetime - startDatetime).total_seconds() / 60, 2)} min")

    def __runTasksRay(self):
        startDatetime = dt.datetime.now()

        rayLauncher = RayLauncher(self.__mdLibrary, self.__tickFactorLibrary, self.__factorConfig)
    ...
    def __runTasksRayUsingAIMR(self):
        ...
        parallelList = ["{}-{}-{}-{}-{}-{}".format(self.__mdLibrary, self.__tickFactorLibrary, tradingDayList[sIndex],
