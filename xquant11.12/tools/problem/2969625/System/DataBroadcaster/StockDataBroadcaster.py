...
class StockDataBroadcaster:
    ...
    def __init__(self, symbol, stockSet, dataManager):
        ...
        self.__earlyDataMorning = None
        self.__earlyDataAfternoon = None
        self.__minuteData = None
        self.__dailyData = None
        self.__tickData1SA = None
        self.__tickData2SA = None
        self.__earlyDataMorningSA = None
        self.__earlyDataAfternoonSA = None
        ...
        self.__currIndexTick = None
        self.__currIndexMinute = None
    ...
    def __loadData(self):
        ...
        self.__earlyDataMorning = self.__dataManager.getEarlyDataMorning(self.__stockIndexINFSet, False)
        self.__earlyDataAfternoon = self.__dataManager.getEarlyDataAfternoon(self.__stockIndexINFSet, False)
        ...
        if self.__splitAdjustedNeeded:
            ...
            self.__earlyDataMorningSA = self.__dataManager.getEarlyDataMorning(self.__stockIndexINFSet, True)
            self.__earlyDataAfternoonSA = self.__dataManager.getEarlyDataAfternoon(self.__stockIndexINFSet, True)
    ...
    def __generateStartEndIndexAllDict(self):
        ...
        for stock in self.__stockIndexINFSet:
            if stock in self.__indexSet:
                ...
                isL2PIndex = self.__tickColumnIndexDictIndex["IsL2P"]
                ...
                isL2PIndex = self.__tickColumnIndexDictINF1[stockTmp]["IsL2P"]
                ...
                isL2PIndex = self.__tickColumnIndexDictINF1[self.__infMap[stock]]["IsL2P"]
            else:
                ...
                isL2PIndex = self.__tickColumnIndexDict1["IsL2P"]
            ...
            self.__isL2PDict[stock] = tickData[:, isL2PIndex]
    ...
    def __generateStartEndIndexDict(self, date):
        ...
        self.__currIndexTick = self.__tickStartIndexDict[self.__symbol]
        self.__currIndexMinute = self.__minuteStartIndexDict[self.__symbol]

    def broadcast(self):
        nextTickTimestamp = self.__tickTimestampDict[self.__symbol][self.__currIndexTick] if self.__currIndexTick < self.__tickEndIndexDict[self.__symbol] else None
        nextMinuteTimestamp = self.__minuteTimestampDict[self.__symbol][self.__currIndexMinute] if self.__currIndexMinute < self.__minuteEndIndexDict[self.__symbol] else None

        if nextMinuteTimestamp is None:
            if nextTickTimestamp is None:
                return None, None
            else:
                self.__currIndexTick += 1
                thisTimestamp = self.__tickTimestampDict[self.__symbol][self.__currIndexTick - 1]
                dataType = 2 if self.__isL2PDict[self.__symbol][self.__currIndexTick - 1] else 0
        else:
            if nextTickTimestamp is None:
                self.__currIndexMinute += 1
                thisTimestamp = self.__minuteTimestampDict[self.__symbol][self.__currIndexMinute - 1]
                dataType = 1
            else:
                if nextMinuteTimestamp <= nextTickTimestamp:
                    self.__currIndexMinute += 1
                    thisTimestamp = self.__minuteTimestampDict[self.__symbol][self.__currIndexMinute - 1]
                    dataType = 1
                else:
        ...
        if thisTimestamp > self.__broadcastingEndTimestamp:
            return None, None

        self.__tickCurrIndexDict[self.__symbol] = self.__currIndexTick
        self.__minuteCurrIndexDict[self.__symbol] = self.__currIndexMinute
        ...
        for symbol in self.__stockIndexINFSet2:
            ...
            while (tickCurrIndex < tickTimestampArray.shape[0]
                   and tickTimestampArray[tickCurrIndex] <= thisTimestamp - self.__TICK_PLAY_LAG):
                tickCurrIndex += 1
            ...
            while (minuteCurrIndex < minuteTimestampArray.shape[0]
                   and minuteTimestampArray[minuteCurrIndex] <= thisTimestamp - self.__MINUTE_PLAY_LAG):
                minuteCurrIndex += 1
            self.__minuteCurrIndexDict[symbol] = minuteCurrIndex

        return thisTimestamp, dataType

    def getEarlyDataMorning(self, field, date, stock, splitAdjusted):
        if splitAdjusted:
            return self.__earlyDataMorningSA[stock][date][field]
        else:
            return self.__earlyDataMorning[stock][date][field]

    def getEarlyDataAfternoon(self, field, date, stock, splitAdjusted):
        currTickDate, currTickSec = divmod(self.__tickTimestampDict[self.__symbol][self.__currIndexTick - 1] + 28800, 86400)
        currDate, _ = divmod(self.__broadcastingEndTimestamp + 28800, 86400)

        if currTickSec < 43200 or currTickDate < currDate:
            raise Exception("getEarlyDataAfternoon is called before afternoon begins")
        if splitAdjusted:
            return self.__earlyDataAfternoonSA[stock][date][field]
        else:
            return self.__earlyDataAfternoon[stock][date][field]
    ...
    def getLastNTickData(self, field, startDate, stock, splitAdjusted, mode, n=None):
        tickData, tickColumnIndexDict, isl2PArray = self.__getTickData(field, splitAdjusted, stock)
        ...
        mask = isl2PArray[tickStartIndex:tickEndIndex]

        mask = mask == 0
        if mask.shape[0] > 0 and mode != 2:
            mask[-1] = True
        res = res[mask]
            ...
            if splitAdjusted:
                if field in self.__tickColumnIndexDict1:
                    if isGetAll:
                        ...
                        return self.__tickData1SA[stock], self.__tickColumnIndexDict1, self.__tickData1[stock][:, self.__tickColumnIndexDict1["IsL2P"]]
                else:
                    if isGetAll:
                        return self.__tickData2SA, self.__tickColumnIndexDict2, None
                    else:
                        return self.__tickData2SA[stock], self.__tickColumnIndexDict2, self.__tickData1[stock][:, self.__tickColumnIndexDict1["IsL2P"]]
            else:
                if field in self.__tickColumnIndexDict1:
                    if isGetAll:
                        return self.__tickData1, self.__tickColumnIndexDict1, np.nan
                    else:
                        return self.__tickData1[stock], self.__tickColumnIndexDict1, self.__tickData1[stock][:, self.__tickColumnIndexDict1["IsL2P"]]
                else:
                    if isGetAll:
                        return self.__tickData2, self.__tickColumnIndexDict2, None
                    else:
                        return self.__tickData2[stock], self.__tickColumnIndexDict2, self.__tickData1[stock][:, self.__tickColumnIndexDict1["IsL2P"]]
