...
class INFDataBroadcaster:
    ...
    def __init__(self, symbol, stockSet, dataManager):
        ...
        self.__stockIndexSet = self.__stockSet.union(self.__indexSet)
        self.__stockIndexSet2 = self.__stockSet2.union(self.__indexSet)
    ...
    def __loadData(self):
        self.__tickData1, self.__tickData2 = self.__dataManager.getTickData(self.__stockIndexSet, False)
        self.__minuteData = self.__dataManager.getMinuteData(self.__stockIndexSet, False)
        self.__dailyData = self.__dataManager.getDailyData(self.__stockIndexSet, False)

        if self.__splitAdjustedNeeded:
            self.__tickData1SA, self.__tickData2SA = self.__dataManager.getTickData(self.__stockIndexSet, True)
            self.__minuteDataSA = self.__dataManager.getMinuteData(self.__stockIndexSet, True)
            self.__dailyDataSA = self.__dataManager.getDailyData(self.__stockIndexSet, True)

    def __generateStartEndIndexAllDict(self):
        # Generate StartIndexAllDict & EndIndexAllDict according to the data
        for stock in self.__stockIndexSet:
    ...
    def __generateStartEndIndexDict(self, date):
        for symbol in self.__stockIndexSet:
    ...
    def broadcast(self):
        ...
        for symbol in self.__stockIndexSet2:
