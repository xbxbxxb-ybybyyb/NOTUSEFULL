...
from System.COLUMN_CONFIG import (CINDEX_TICK_COLUMN_INDEX_DICT1, CINDEX_MINUTE_COLUMN_INDEX_DICT,
                                  CINDEX_DAILY_COLUMN_INDEX_DICT)
...
class IndexDataBroadcaster:
    ...
    def __init__(self, symbol, stockSet, dataManager):
        ...
        self.__minuteColumnIndexDict = CINDEX_MINUTE_COLUMN_INDEX_DICT
        self.__dailyColumnIndexDict = CINDEX_DAILY_COLUMN_INDEX_DICT
        ...
        self.__tickColumnIndexDictUA = INDEX_TICK_COLUMN_INDEX_DICT
        self.__minuteColumnIndexDictUA = INDEX_MINUTE_COLUMN_INDEX_DICT
        self.__dailyColumnIndexDictUA = INDEX_DAILY_COLUMN_INDEX_DICT
    ...
    def __loadData(self):
        self.__tickData1 = self.__dataManager.getTickData(self.__stockIndexUASet, False)
        ...
        if self.__splitAdjustedNeeded:
            self.__tickData1SA = self.__dataManager.getTickData(self.__stockIndexUASet, True)
    ...
    def __generateStartEndIndexAllDict(self):
        ...
        for stock in self.__stockIndexUASet:
            if stock in self.__indexSet:
                ...
                timestampIndex = self.__tickColumnIndexDictUA["Timestamp"]
                dateIndex = self.__tickColumnIndexDictUA["Date"]
            else:
    ...
    def __getTickData(self, field, splitAdjusted, stock=None, isGetAll=False):
        if stock in self.__indexSet:
            ...
            if splitAdjusted:
                return self.__tickData1SA[stock], self.__tickColumnIndexDictUA
            else:
                return self.__tickData1[stock], self.__tickColumnIndexDictUA
        else:
            if splitAdjusted:
