...
if __name__ == "__main__":
    ...
    tickLibrary = "Channel258STickDataLib"
    tranOrderLibrary = "OrderTradeDataSZ2023"
    l2pTickLibrary = ""
    l2pTranOrderLibrary = ""
    INFLibrary = "INFactor"

    tickOutputLibrary = "Factor_258_Channel_SZ"
...
#    data = pd.read_excel(
#        '/data/user/018106/TrendFactor/FactorList/036147factorList/Everest_1S_Factor_Info_20220524.xlsx')
#    everestFactorList = sorted(set(data["Exist"].dropna().to_list() + data["New"].dropna().to_list()))
#    with open("/data/user/018106/TrendFactor/FactorList/036147factorList/KunlunFactorList1S_20220701.pickle",
#              "rb") as f:
...
#    factorList_old_1 = df_factor['oldNotOrder'][~df_factor['oldNotOrder'].isnull()].tolist() + df_factor['oldOrder'][
#        ~df_factor['oldOrder'].isnull()].tolist() + df_factor['SPNotOrder'][
#                           ~df_factor['SPNotOrder'].isnull()].tolist() + df_factor['SPOrder'][
#                           ~df_factor['SPOrder'].isnull()].tolist() + \
#                       df_factor['CB'][~df_factor['CB'].isnull()].tolist()
#    factorList_new = df_factor['newNotOrder'][~df_factor['newNotOrder'].isnull()].tolist() + df_factor['newOrder'][
#        ~df_factor['newOrder'].isnull()].tolist()
