...
class Tag20minLongBothSides(Factor):
    ...
    def calculate(self):
        ...
        while self.__getTimeDiff(timestampList[i], thisTimestamp) >= 1202.5:
            bidPrice1ListTmp = [bidPrice1List[-1 - j] for j, tsp in enumerate(timestampList[-6:][::-1])
                                if self.__getTimeDiff(tsp, thisTimestamp) < 3]
            askPrice1ListTmp = [askPrice1List[i + j] for j, tsp in enumerate(timestampList[i: (i+6)])
    ...
    def _onDayEnd(self):
        ...
        for i in range(unfinishedStartIndex, len(timestampList)):
            ...
            if len(timestampListTmp) > 0:
                ...
                askPrice1ListTmp = [askPrice1List[i]]  # 最后一个
            if len(bidPrice1ListTmp) > 0 and np.any(~np.isnan(askPrice1ListTmp)):
                factorValue = np.mean(bidPrice1ListTmp) / np.nanmean(askPrice1ListTmp) - 1
            else:
                factorValue = 0
