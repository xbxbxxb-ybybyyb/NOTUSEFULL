...
class Tag10minShortTwap(Factor):
    ...
    def calculate(self):
        ...
        for i in range(unfinishedMinuteStartIndex, len(minuteTimestampList)):
            ...
            if self.__getTimeDiff(minuteTimestampList[i], thisMinuteTimestamp) > 660 - 0.5:
                ...
                askPrice1ListTmp = self.__collectLastNMinPrice(2, askPrice1List, thisMinuteTimestamp)
                unfinishedTickStartIndex = self.__updateUnfinishedTickStartIndex()
                bidPrice1ListTmp = self.__collectNextNMinPrice(2, bidPrice1List, unfinishedTickStartIndex)
    ...
    def _onDayEnd(self):
        ...
        for i in range(unfinishedMinuteStartIndex, len(minuteTimestampList)):
            ...
            if self.__getTimeDiff(minuteTimestampList[unfinishedMinuteStartIndex], minuteTimestampList[-1]) > 4 * 60 + 0.5:
                ...
                askPrice1ListTmp = self.__collectLastNMinPrice(2, askPrice1List, minuteTimestampList[-1])
                unfinishedTickStartIndex = self.__updateUnfinishedTickStartIndex()
                bidPrice1ListTmp = self.__collectNextNMinPrice(2, bidPrice1List, unfinishedTickStartIndex)
