...
class Tag5minShortTwap(Factor):
    ...
    def calculate(self):
        ...
        askPrice1List = [each if each > 0 else midPriceList[i] for i, each in enumerate(self.__ask1Price.getFactorValueList())]
        bidPrice1List = [each if each > 0 else np.nan for each in self.__bid1Price.getFactorValueList()]

        # 标签计算
        for i in range(unfinishedMinuteStartIndex, len(minuteTimestampList)):

            if self.__getTimeDiff(minuteTimestampList[i], thisMinuteTimestamp) > 360 - 0.5:

                askPrice1ListTmp = self.__collectLastNMinPrice(1, askPrice1List, thisMinuteTimestamp)
                unfinishedTickStartIndex = self.__updateUnfinishedTickStartIndex()
                bidPrice1ListTmp = self.__collectNextNMinPrice(1, bidPrice1List, unfinishedTickStartIndex)
    ...
    def _onDayEnd(self):
        ...
        for i in range(unfinishedMinuteStartIndex, len(minuteTimestampList)):
            ...
            if self.__getTimeDiff(minuteTimestampList[unfinishedMinuteStartIndex], minuteTimestampList[-1]) > 2 * 60 + 0.5:

                askPrice1ListTmp = self.__collectLastNMinPrice(1, askPrice1List, minuteTimestampList[-1])
                unfinishedTickStartIndex = self.__updateUnfinishedTickStartIndex()
                bidPrice1ListTmp = self.__collectNextNMinPrice(1, bidPrice1List, unfinishedTickStartIndex)
                ...
                bidPrice1ListTmp = bidPrice1List[unfinishedTickStartIndex: (unfinishedTickStartIndex + leftn // 2)]
                askPrice1ListTmp = askPrice1List[(unfinishedTickStartIndex + leftn // 2):]
    ...
    def __collectLastNMinPrice(self, n, priceList, thisMinuteTimestamp):
        timestampList = self._getAllTodayTickData("Timestamp")
        PriceListTmp = []
        for j, timestamp in enumerate(timestampList[::-1]):  # 左闭右开，例如交易的时间区间为[9:40, 9:41)
            if 0 < self.__getTimeDiff(timestamp, thisMinuteTimestamp) < n * 60 + 0.5:  # 交易1min
    ...
    def __collectNextNMinPrice(self, n, priceList, unfinishedTickStartIndex):
        ...
        PriceListTmp = []
        for j in range(unfinishedTickStartIndex, len(timestampList)):  # 左闭右开，例如交易的时间区间为[9:40, 9:41)
