...
class Tag10minMidPriceReturn(Factor):
    ...
    def calculate(self):
        ...
        for i in range(unfinishedStartIndex, len(timestampList)):
            if self.__getTimeDiff(timestampList[i], thisTimestamp) >= 606:
                midPriceListTmp = [midPriceList[-1 - j] for j in range(min(5, len(timestampList)))
                                   if timestampList[-1 - j] > timestampList[i]]
                if len(midPriceListTmp) > 0 and midPriceList[i] > 0:
                    factorValue = np.mean(midPriceListTmp) / midPriceList[i] - 1
                else:
                    factorValue = 0
