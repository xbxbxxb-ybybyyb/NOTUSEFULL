...
class Tag20minShortTwap(Factor):
    ...
    def calculate(self):
        ...
        for i in range(unfinishedMinuteStartIndex, len(minuteTimestampList)):
            ...
            if self.__getTimeDiff(minuteTimestampList[i], thisMinuteTimestamp) > 1260 - 0.5:

                askPrice1ListTmp = self.__collectLastNMinPrice(4, askPrice1List, thisMinuteTimestamp)
                unfinishedTickStartIndex = self.__updateUnfinishedTickStartIndex()
                bidPrice1ListTmp = self.__collectNextNMinPrice(4, bidPrice1List, unfinishedTickStartIndex)
    ...
    def _onDayEnd(self):
        ...
        for i in range(unfinishedMinuteStartIndex, len(minuteTimestampList)):
            ...
            if self.__getTimeDiff(minuteTimestampList[unfinishedMinuteStartIndex], minuteTimestampList[-1]) > 8 * 60 + 0.5:

                askPrice1ListTmp = self.__collectLastNMinPrice(4, askPrice1List, minuteTimestampList[-1])
                unfinishedTickStartIndex = self.__updateUnfinishedTickStartIndex()
                bidPrice1ListTmp = self.__collectNextNMinPrice(4, bidPrice1List, unfinishedTickStartIndex)
