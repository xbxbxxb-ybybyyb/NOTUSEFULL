...
class Tag5minLongSpeed(Factor):
    ...
    def calculate(self):
        ...
        for i in range(unfinishedStartIndex, len(timestampList)):
            if self.__getTimeDiff(timestampList[i], thisTimestamp) >= 300:
                askPriceTmp = askPrice1List[i]
                if askPriceTmp <= 0:
                    factorValue = 0
                else:
                    returnSpeedListTmp = [(bidPrice1List[j] / askPriceTmp - 1) / (j - i)
                                          for j in range(i + 1, len(timestampList))
                                          if timestampList[j] > timestampList[i]]
                    if len(returnSpeedListTmp) > 0:
                        factorValue = np.max(returnSpeedListTmp)
                    else:
                        factorValue = 0
    ...
    def _onDayEnd(self):
        ...
        for i in range(unfinishedStartIndex, len(timestampList)):
            ...
            if askPriceTmp <= 0:
                ...
                returnSpeedListTmp = [(bidPrice1List[j] / askPriceTmp - 1) / (j - i)
                                      for j in range(i + 1, len(timestampList)) if timestampList[j] > timestampList[i]]
                if len(returnSpeedListTmp) > 0:
                    factorValue = np.max(returnSpeedListTmp)
                else:
                    factorValue = 0
