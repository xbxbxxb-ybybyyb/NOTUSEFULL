...
class Tag30minShortTwap(Factor):
    ...
    def calculate(self):
        ...
        for i in range(unfinishedMinuteStartIndex, len(minuteTimestampList)):
            ...
            if self.__getTimeDiff(minuteTimestampList[i], thisMinuteTimestamp) > 1860 - 0.5:

                askPrice1ListTmp = self.__collectLastNMinPrice(5, askPrice1List, thisMinuteTimestamp)
                unfinishedTickStartIndex = self.__updateUnfinishedTickStartIndex()
                bidPrice1ListTmp = self.__collectNextNMinPrice(5, bidPrice1List, unfinishedTickStartIndex)
    ...
    def _onDayEnd(self):
        ...
        for i in range(unfinishedMinuteStartIndex, len(minuteTimestampList)):
            ...
            if self.__getTimeDiff(minuteTimestampList[unfinishedMinuteStartIndex], minuteTimestampList[-1]) > 10 * 60 + 0.5:

                askPrice1ListTmp = self.__collectLastNMinPrice(5, askPrice1List, minuteTimestampList[-1])
                unfinishedTickStartIndex = self.__updateUnfinishedTickStartIndex()
                bidPrice1ListTmp = self.__collectNextNMinPrice(5, bidPrice1List, unfinishedTickStartIndex)
