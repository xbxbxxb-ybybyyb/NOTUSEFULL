...
class FactorLongRetWithHighVolM(Factor):
    ...
    def calculate(self):
        ...
        if len(ret) >= self.__window:
            res = sum(np.array(ret[-self.__window:])[np.array(ret_vol[-self.__window:]) > 1])
        else:
            res = sum(np.array(ret[-self.__window:])[np.array(ret_vol[-self.__window:]) > 1]) * (self.__window/len(ret))#
