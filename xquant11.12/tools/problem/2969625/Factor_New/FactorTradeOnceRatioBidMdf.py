...
class FactorTradeOnceRatioBidMdf(Factor):
    ...
    def calculate(self):
        ...
        if len(trade_matrix_select) > 0:
            ...
            if len(dir_array) > 3:
                vol_once_sum = np.nansum(vol_array[(dir_array == 2.0) & (count_array == 1)])
                vol_total_sum = np.nansum(vol_array[dir_array == 2.0])
                factor_value = vol_once_sum / vol_total_sum if vol_total_sum > 1e-6 else 0
