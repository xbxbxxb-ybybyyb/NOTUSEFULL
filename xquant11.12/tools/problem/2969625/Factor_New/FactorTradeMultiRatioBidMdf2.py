...
class FactorTradeMultiRatioBidMdf2(Factor):
    ...
    def calculate(self):
        ...
        if len(trade_matrix_select) > 0:
            ...
            ask_order = self._getTransactionData("BidOrder", trade_matrix_select)
            ...
            if len(dir_array) > 3:
                vol_once_sum = np.nansum(vol_array[(dir_array == 2.0) & (count_array > 5)])
