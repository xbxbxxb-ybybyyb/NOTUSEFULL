...
class FactorTradeBidSuccessRatio(Factor):
    def __init__(self, config, factorManager):
        super().__init__(config, factorManager)
        self.__lag = self._getParameter("Lag")

    def calculate(self):
        volumeArray = self._getLastNTickData("AskVolume", 2)[0]
        ...
        if transaction is not None:
            ...
            tradeVolume = np.nansum(volume[bsFlag == 1])

        factorValue = tradeVolume / gdVolume if gdVolume > 1e-4 else 0
