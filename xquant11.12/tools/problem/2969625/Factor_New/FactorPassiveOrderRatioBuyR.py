...
class FactorPassiveOrderRatioBuyR(Factor):
    ...
    def calculate(self):
        buyQty = self.passiveOrderNum.getLastFactorValue()[0]
        ...
        askQty = np.sum(askV[:5]) / np.sum(askN[:5]) if np.sum(askN[:5]) > 0 else np.sum(bidV[:5]) / np.sum(bidN[:5])

        ratio = self.getIntermediate("ratio")
        ratio.append(buyQty / askQty)
