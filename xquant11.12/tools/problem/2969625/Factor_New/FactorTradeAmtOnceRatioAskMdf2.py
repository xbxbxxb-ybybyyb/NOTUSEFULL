...
class FactorTradeAmtOnceRatioAskMdf2(Factor):
    ...
    def calculate(self):
        ...
        if len(trade_matrix_select) > 0:
            ...
            vol_once_sum = np.nansum(vol_array[(dir_array == 2.0) & (count_array == 1)] * price_array[(dir_array == 2.0) & (count_array == 1)])
            vol_total_sum = np.nansum(vol_array[dir_array == 2.0] * price_array[dir_array == 2.0])
            factor_value = vol_once_sum / vol_total_sum if vol_total_sum > 1e-4 else 0
        else:
            factor_value = 0
        factor_value = max(0.1, min(0.9, factor_value))
