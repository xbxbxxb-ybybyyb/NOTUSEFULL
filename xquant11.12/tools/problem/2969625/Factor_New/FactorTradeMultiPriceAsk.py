...
class FactorTradeMultiPriceAsk(Factor):
    ...
    def calculate(self):
        ...
        if len(trade_matrix_select) > 0:
            ...
            vol_once_sum = np.nansum(vol_array[(price_array <= ask1_price) & (count_array > 5)])
            vol_total_sum = np.nansum(vol_array[price_array <= ask1_price])
