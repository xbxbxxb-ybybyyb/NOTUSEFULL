...
def save_factor_analysis(start_date, end_date, stock, factor_label_df, label_name, factor_list=None, data_type='enhanced_tick'):
    ...
    #                                                       data_type=data_type)
    analysis_res =  factor_eval_save_to_dolphindb(factor_label_df, label=label_name, factor_list=factor_list,
                                           start_date=start_date, end_date=end_date)
    analysis_res['stock'] = stock
    factor_name_list = factor_list
    analysis_res.to_parquet("/data/user/013150/exp_result/ALL_SYMBOL/{}_zt/{}_all_factor_stats_df.parquet".format(data_type, stock))
...
def main():
    ...
    factor_list = [tag for tag in library_info["Factor_036_Channel_SH"] if tag.startswith("factor")]
    label_list = [tag for tag in library_info["Factor_036_Channel_SH"] if tag.startswith("tag")]
    print("factor_list:", len(factor_list))
    print("label_list:", len(label_list))

    days = fa.tradingday("20210101", "20231025")
    stocks = [
        # "688012.SH",
        # "688008.SH",
        # "688036.SH",
    ...
    error_date_dict = defaultdict(list)
    columns = copy.deepcopy(factor_list)
    columns.extend(label_list)
    columns.extend(["timestamp"])
    print("columns:", len(columns))

    for stock in stocks:
        high_df_list = []
        if False:#os.path.exists(f"feature_label_df_{stock}.parquet"):
            factor_label_df = pd.read_pickle(f"feature_label_df_{stock}.parquet").set_index("timestamp")
        else:
            for day in days:
                try:
                    if stock.endswith("SH"):
                        high_df = fa.get_factor_value('Factor_036_Channel_SH', stock, day, columns)
                    else:
                        high_df = fa.get_factor_value('Factor_036_Channel_SZ', stock, day, columns)
                except:
                    print("data error: {} {}".format(stock, day))
                    error_date_dict[stock].append(day)
                    continue
                high_df_list.append(high_df)
            factor_label_df = pd.concat(high_df_list, axis=0)
            factor_label_df["timestamp"] = factor_label_df["timestamp"].apply(lambda x: datetime.datetime.fromtimestamp(x))
            factor_label_df = factor_label_df.set_index("timestamp")
            # factor_label_df.to_pickle(f"feature_label_df_{stock}-{day}.parquet")
        print("factor_label_df shape:", factor_label_df)

        factor_list = [col for col in factor_label_df.columns if col.startswith("factor")]
        label_list = [col for col in factor_label_df.columns if col.startswith("tag")]

        data_type = 'enhanced_tick'
        start_date, end_date = factor_label_df.index[0].strftime("%Y%m%d"),factor_label_df.index[-1].strftime("%Y%m%d")

        fp = FactorProvider('016869')
        label_df_all = fp.load_public_data_from_dfs(symbol=[stock], factor_list=["LabelFirstPeak_th10_120s"], start_time=start_date,
                                                    end_time=end_date, factor_type='label', data_type="enhanced_tick")
        label_df_all = label_df_all.set_index('timestamp')
        factor_label_df = pd.merge(factor_label_df, label_df_all, left_index=True, right_index=True)

        # for label_name in tqdm(label_list):
        for label_name in tqdm(["LabelFirstPeak_th10_120s"]):
            print("label_name", label_name)
            res = save_factor_analysis(start_date, end_date, stock, factor_label_df, label_name, factor_list=factor_list,
                                 data_type=data_type)
            print(res)
