...
def upload_tick3s_data(s, data, cover=False):
    ...
    df['AfterHoursNumTrades'] = 0
    df['AfterHoursTotalVolumeTrade'] = 0
    df['AfterHoursTotalValueTrade'] = 0
    df['ChannelNo'] = 0
 
    # 用xquant接口取出来的数据比从hadoop下载的数据字段多，提取部分
    need_cols = ['MDDate', 'MDTime', 'SecurityType', 'SecuritySubType', 'SecurityID', 'SecurityIDSource', 'Symbol', 'TradingPhaseCode', 
    'PreClosePx', 'NumTrades', 'NumBidOrders','NumOfferOrders','TotalVolumeTrade', 'TotalValueTrade', 'LastPx', 'OpenPx', 'ClosePx', 'HighPx', 'LowPx', 'DiffPx1', 'DiffPx2',
     'MaxPx', 'MinPx', 'TotalBidQty', 'TotalOfferQty', 'WeightedAvgBidPx', 'WeightedAvgOfferPx', 'AfterHoursNumTrades', 'AfterHoursTotalVolumeTrade', 
    'AfterHoursTotalValueTrade', 'Buy1Price', 'Buy1OrderQty', 'Buy1NumOrders', 'Buy1NoOrders', 'Buy1OrderDetail', 'Sell1Price', 'Sell1OrderQty',
     'Sell1NumOrders', 'Sell1NoOrders', 'Sell1OrderDetail', 'Buy2Price', 'Buy2OrderQty', 'Buy2NumOrders', 'Sell2Price', 'Sell2OrderQty', 'Sell2NumOrders',
     'Buy3Price', 'Buy3OrderQty', 'Buy3NumOrders', 'Sell3Price', 'Sell3OrderQty', 'Sell3NumOrders', 'Buy4Price', 'Buy4OrderQty', 'Buy4NumOrders', 
    'Sell4Price', 'Sell4OrderQty', 'Sell4NumOrders', 'Buy5Price', 'Buy5OrderQty', 'Buy5NumOrders', 'Sell5Price', 'Sell5OrderQty', 'Sell5NumOrders', 
    'Buy6Price', 'Buy6OrderQty', 'Buy6NumOrders', 'Sell6Price', 'Sell6OrderQty', 'Sell6NumOrders', 'Buy7Price', 'Buy7OrderQty', 'Buy7NumOrders', 
    'Sell7Price', 'Sell7OrderQty', 'Sell7NumOrders', 'Buy8Price', 'Buy8OrderQty', 'Buy8NumOrders', 'Sell8Price', 'Sell8OrderQty', 'Sell8NumOrders', 
    'Buy9Price', 'Buy9OrderQty', 'Buy9NumOrders', 'Sell9Price', 'Sell9OrderQty', 'Sell9NumOrders', 'Buy10Price', 'Buy10OrderQty', 'Buy10NumOrders', 
    'Sell10Price', 'Sell10OrderQty', 'Sell10NumOrders', 'HTSCSecurityID',"WithdrawBuyNumber","WithdrawBuyAmount","WithdrawBuyMoney",
"WithdrawSellNumber","WithdrawSellAmount","WithdrawSellMoney", 'TotalBidNumber', 'TotalOfferNumber', 'BidTradeMaxDuration', 'OfferTradeMaxDuration', 'ReceiveDateTime', 'ChannelNo']
    df = df[need_cols]
    df.loc[df['HTSCSecurityID'].str.endswith('SH'),['SecurityType']] = 2
    df['SecurityIDSource'] = df['SecurityIDSource'].str.replace('XSHG', '101').astype(int)
...
def main(start_date, end_date, stock, security_type='stock'):
    ...
    if security_type=='stock':
        tick_data = md.get_data_by_date("Stock",stock, start_date)
    else:
        tick_data = fnd.get_fund_data(stock,start_date+" 090000000",start_date+" 153000000","TICK")

    if tick_data.empty:
        return
 
    try:
        tick_data['SecurityType'] = tick_data['SecurityType'].astype(int)
    except:
        tick_data['SecurityType'] = 1
    if security_type=='stock':        
        transaction_data = md.get_data_by_date("Transaction",stock, start_date)
    else:
        transaction_data = fnd.get_fund_data(stock,start_date+" 090000000",start_date+" 153000000","TRANSACTION")
...
if __name__ == "__main__":
    ...
    for date in tradingday:
        main(start_date=date, end_date=date, stock='510500.SH', security_type='fund') 
