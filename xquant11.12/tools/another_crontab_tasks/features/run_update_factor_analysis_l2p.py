...
def calc_factor_analysis(start_date, end_date, stock, label_name, factor_list=None, data_type='enhanced_tick'):
    ...
    factor_list = list(set(factor_list_all).intersection(set(factor_list)))
    print("start analysis: ", stock, factor_list)
...
if __name__ == "__main__":
    ...
    data_type = 'tick_l2p'
    stocks = pd.read_csv("kc50.csv", header=None)[0].tolist()
    stocks = stocks
    factor_list = ['FactorCGWFactorPanFlowK', 'FactorCGWOrderSellBidNumRatio', 'FactorCGWOrderSellNumTrend', 'FactorCGWSellAggrMildOrderVRatio', 'FactorCGWSellDealMaxRatio', 'FactorLuAskBidMildOrderRatio', 'FactorLuMaPriceBollingUp', 'FactorLuOrdBidNumQuantile1', '.ipynb_checkpo', 'FactorCGWBuyDisDetrend', 'FactorCGWLongStrength', 'FactorLuBidOrderK', 'FactorLuBidOrderK1', 'FactorLuBidIncremtActiveVolume2', 'FactorNSWBigTradeSkew', 'FactorNSWMonmentumLessTrade', 'FactorNSWNegSkewTrend', 'FactorNSWReverseLessTrade', 'FactorNSWSellTradeComOpen', 'FactorNSWAvgTradeOutflowRatioRelatively', 'FactorNSWAvgTradeOutflowRatio','FactorLuVwapPriceGapPctE', 'FactorLuVOI', 'FactorLuBuySideGap', 'FactorLuSellSideGap', 'FactorLuSellTopGap', 'FactorLuBOVwapGap', 'FactorLuBuyTopGap', 'FactorLuBOPriceGap']
    # stocks = [
    #     "688012.SH",
    #     "688008.SH",
    #     "688036.SH",
    ...
    # ]
    for stock in stocks:
        res = save_factor_analysis(start_date, end_date, stock, label_name, factor_list, data_type)
        print(res)
