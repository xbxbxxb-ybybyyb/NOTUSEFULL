...
from DecimalUtil import isEqual, greaterThan, lessThan, equalGreaterThan, equalLessThan

class EventTwoSideVI(FactorBase):
                ...
                if isEqual(tickDataIndex, orderIndex):
                    if isEqual(orderBSFlag, 1):
                        if isEqual(currentTickAskP0, lastTickAskP0) \
                                ...
                                and greaterThan(currentTickBidV0, lastTickBidV0):
                        ...
                        if isEqual(currentTickAskP0, lastTickAskP0) \
                                and isEqual(currentTickBidP0, lastTickBidP0) \
                                and greaterThan(currentTickAskV0, lastTickAskV0):
                            if self.is_event_triggered(tickAskP0List, tickAskV0List, tickBidP0List, tickBidV0List,
                                                       "Sell"):
                                factorValue = -1
        ...
        if side == "Buy":
            ...
            for i in list(range(L-1))[::-1]:
                ...
                if isEqual(currentAskP0, localAskP0) \
                    ...
                    if equalGreaterThan((currentBidV0 - localBidV0) / currentBidV0, self.__ratio):
                        return True
                    if CONTINUOUS_CALC_FLAG:
                        if lessThan(localBidV0, lastBidV0):
                            CONTINUOUS_NUM += 1
                            lastBidV0 = localBidV0
                            if CONTINUOUS_NUM >= self.__trigger_n:
                                return True
                        elif greaterThan(localAskV0, lastAskV0):
                            lastAskV0 = localAskV0
                        elif greaterThan(localBidV0, lastBidV0) or lessThan(localAskV0, lastAskV0):
            ...
            for i in list(range(L-1))[::-1]:
                ...
                if isEqual(currentAskP0, localAskP0) \
                        and isEqual(currentBidP0, localBidP0):
                    if equalGreaterThan((currentAskV0 - localAskV0) / currentAskV0, self.__ratio):
                        return True
                    if CONTINUOUS_CALC_FLAG:
                        if lessThan(localAskV0, lastAskV0):
                            CONTINUOUS_NUM += 1
                            lastAskV0 = localAskV0
                            if CONTINUOUS_NUM >= self.__trigger_n:
                                return True
                        elif greaterThan(localBidV0, lastBidV0):
                            lastBidV0 = localBidV0
                        elif greaterThan(localAskV0, lastAskV0) or lessThan(localBidV0, lastBidV0):
                            CONTINUOUS_CALC_FLAG = False
                else:
                    return False
        return False
