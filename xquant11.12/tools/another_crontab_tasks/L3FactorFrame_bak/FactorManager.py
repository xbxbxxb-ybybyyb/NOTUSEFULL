...
class FactorManager:
    ...
    def register_factor(self, config, factor_path = None, event_path = None):
        ...
        for factor_class_name in config:
            if factor_class_name=="SeqNo":
                ...
                if event_path:
                    # 行数不一样，需要两份SeqNo
                    factor_instance = factor_class({}, self.marketDataManager)
            ...
            if factor_class_name in event_list_all:
                ...
                for pidx, param_dict in enumerate(sub_config):
                    try:
                        factor_instance = factor_class(param_dict, self.marketDataManager)
    ...
    def calc_loop(self, mode = "FULL"):
        ...
        if mode.upper() == "FULL":
            ...
            while self.marketDataManager.idx < len(self.marketDataManager.tick_array)-1:
                ...
                if self.verbose > 0:
                    t0 = time.time()
                for fac in self.event_list+self.factor_list:
                    if self.verbose >0 :
                        t1 = time.time()
                    calc_factor(fac)
                    if self.verbose >0 :
            ...
            # 只有在事件值非零时计算
            while self.marketDataManager.idx <len(self.marketDataManager.tick_array)-1:
                # if self.marketDataManager.idx >20000:
                #     break
                self.marketDataManager.broadcast()
                flag = False
                if self.verbose > 0:
            ...
            raise Exception("计算模式仅支持FULL(全量)和ESAMPLE(事件触发)两种计算模式！")
