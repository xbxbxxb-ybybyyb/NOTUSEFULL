...
def resample_edf_1s(edf_ms, event_list = []):
    """
    ...
    ###################对1s内的数据求和#############################
    how_method_dict = {}
    # if not flying_factors:
    #     target_columns = edf.columns
    # else:
    #     target_columns = list(set(edf.columns).intersection(set(flying_factors)))
    for col in edf_ms.columns:
...
## 盘口即时特征删选时间段
def load_flying_factors(symbol, dates=None, use_pandas=False, model_config = None, flying_factors = None,
    ...
    def resample_flying_factors(edf, use_pandas=True):
        if use_pandas:
            ...
            edf_resample["open_flying"] = (abs(edf_resample["PriceSpread"]) >= 0.2) | (
                        abs(edf_resample["BreakingP0NumOrders"]) >= 0.2) | (abs(edf_resample["OneBigOrder"]) >= 0.2) | (
                                                      abs(edf_resample["CumOrdersNetVolOverV0"]) >= 0.2
            ...
            if len(edf) == 0:
                return pl.DataFrame()
            event_list = ['BreakingP0NumOrders', 'OneBigOrder', 'CumOrdersNetVolOverV0', 'PriceSpread', 'OneBigOrderExtend', 'LevelOneChange']
            event_list = [ee for ee in event_list if ee in edf.columns]

            # 去除前50秒的数据和最后三分钟的数据
            edf_resample = resample_edf_1s(edf, event_list)
            ...
            #     .with_columns(
            #     PriceSpread=0,
            #     BreakingP0NumOrders=0,
            #     OneBigOrder=0,
            #     CumOrdersNetVolOverV0=0
            # )
    ...
    if use_pandas:
        for date in dates:
            ...
            try:
                ...
                print(e, "resample_flying_factors error: ", symbol, " ", date)
        ...
        for date in dates:
            try:
                ...
                if not len(sub_edf) == 0:
                    flying_valid_num = sub_edf_resample.select(pl.col('open_flying')).sum()[0, 0]
                    # if 100<= flying_valid_num <=2400:
                    data_list.append(sub_edf_resample)
                # if not len(sub_edf) == 0:
                #     data_list.append(sub_edf_resample)
            except Exception as e:
