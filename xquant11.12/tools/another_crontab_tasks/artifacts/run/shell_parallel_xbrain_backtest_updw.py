...
def get_backtest_path(stock,  process_singal = True, online_path = False,  **kwargs):
...
if __name__ == '__main__':
    ...
    # 运行示例：
    #python3 shell_parallel_xbrain_backtest_updw.py --symbol 688981.SH --pred_up 1.2 --pred_dw -1.2 --exp_name exp_l2p_688981.SH --version xgboost_base --strategy StrategySignalT0SwiftProfit --mdtype EnhancedTICK --parallel --artifacts
    ...
    parser.add_argument('-m', '--mdtype', type=str, default="EnhancedTICK", help='marketdata type, Tick or EnhancedTICK.')
    parser.add_argument('-x', '--strategy', type=str, default="StrategySignalT0Simple", help='xbrain strategy name')
    parser.add_argument('-e', '--exp_name', type=str, help='Description for exp_name parameter')
    ...
    # 默认为False，传参--parallel解析值才为True
    parser.add_argument('-p', '--parallel', default = False, action='store_true', help='Enable parallel mode')
    # # 默认为False，传参--process_singal解析值才为True
    # parser.add_argument('-p', '--process_singal', default = False, action='store_true', help='Enable parallel mode')
    # 默认为False，传参--artifacts解析值才为True
    parser.add_argument('-f','--artifacts', default = False, action='store_true', help='Use artifacts format')
    parser.add_argument('-sd', '--start_date', type=str, default="20231215", help='Start date, eg. 20231206.')
    parser.add_argument('-ed', '--end_date', type=str, default="20240210", help='End date, eg 20240123.')

    # 解析策略参数
    args = parser.parse_args()
    symbol_list = []
    symbol_list.append((args.symbol, args.pred_up, args.pred_dw))
    market_type = args.mdtype
    exp_name = args.exp_name
    version_alias = args.version
    local_mode = not args.parallel
    ...
    strategy_params["MARKET_TYPE"] = market_type
    ...
    t1  = time.time()
    if local_mode:
        backtest_date = "20231128" # "20231128"
        stock, pred_th_up, pred_th_dw  = "000977.SZ",1.8, -1.6
        ...
        for stock, pred_th_up, pred_th_dw in symbol_list:
            ...
            if artifacts_format:
                signal_process_dir = get_backtest_path(stock, process_singal = True, online_path=False,
                                                       exp_name=exp_name, version_alias = version_alias,
                ...
                signal_process_dir = get_backtest_path(stock, online_path=True, process_singal= True,
                                                       ...
                                                       pred_th_up=pred_th_up, pred_th_dw=pred_th_dw,
                                                       label_name=label_name)
                plot_save_dir = os.path.join(signal_process_dir, "..", strategy_name_version)

            print("plot_save_dir:", plot_save_dir)
            parallel_run(strategy_class, signal_process_dir, plot_save_dir, stock, start_date=start_date,end_date=end_date,
