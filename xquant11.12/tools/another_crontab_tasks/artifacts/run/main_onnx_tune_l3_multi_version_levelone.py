...
class L2PXGBoostRegPack:
    ...
    def prepare_data(self, data_params):
        ...
        use_pandas = False
        factor_descibe_dict = {}
        label_list = self.model_config["tagger_name_list"]
        for symbol in model_config["symbol_list"]:
            ...
            if os.path.exists(source_data_path) and not use_pandas:
                ...
                if not os.path.exists(os.path.join(exp_path, "dataset/{}_flying_factor.parquet".format(symbol))):
                    ...
                    print("edf_resample_df_all:", edf_resample_df_all)
                    print("factor_label_df_all:", factor_label_df_all)

                    open_flying_df = factor_label_df_all.join(edf_resample_df_all, on="DateTime").filter((pl.col("open_flying") != 0) | (pl.col("close_flying") != 0))
    ...
    def train_loop(self, model_params):
        ...
        for symbol in symbol_list:
            ...
            ############## 数据采样，保留所有的openFlying，并采样30%的close_flying#############
            open_flying_df_source = open_flying_df
            open_flying_df_close = open_flying_df.filter(
                (pl.col("close_flying") != 0) & (pl.col("open_flying") == 0))
            open_flying_df_open = open_flying_df.filter(pl.col("open_flying") != 0)
            open_flying_df_close = sample_by_sort(open_flying_df_close, label_list[0], group_num=10,
                                                       sample_ratio=0.3)
            open_flying_df = pl.concat([open_flying_df_open, open_flying_df_close]).sort(by="DateTime")
            print(f"all_source shape: {open_flying_df_source.shape}, all shape: {open_flying_df.shape}, all_open shape: {open_flying_df_open.shape}, all_close shape: {open_flying_df_close.shape}")
        ...
        xgb_regressor = XGBRegressor(**self.model_config["xgb_config"], n_jobs=35)
    ...
    def predict_signal(self, xgb_regressor):
        ...
        for symbol in model_config["symbol_list"]:
            ...
            open_flying_df, open_flying_label_df, open_flying_flag_df, close_flying_flag_df = merge_norm_flying_factor(
            ...
            signal_df = model_save_and_evaluate.generate_signal_without_class(symbol, T_test, Y_test_pred, Y_test,
                                                                              ...
                                                                              target_type="mid")
            signal_df["flying_flag"] = open_flying_flag_df
            signal_df["close_flying_flag"] = close_flying_flag_df
    ...
    def analysis_signal_winloss(self, start_date="2023-12-06", end_date="2024-02-06"):
        ...
        for path in ["signal_sample.xlsx", "signal_all.xlsx"]:
            ...
            for symbol_name in self.model_config["symbol_list"]:
                if not symbol_name.startswith("688"):
                    continue
                try:
                    ...
                    tasks = []
                    if target_type == "mid":
                        th_list = [1.0, 1.1, 1.2, 1.4, 1.6, 1.8, 2.0, 2.2]
                    else:
                        th_list = [0.8, 0.9, 1.0, 1.1, 1.2, 1.4, 1.6, 1.8]
                    for th in th_list:
                        for win_limit in [0.001, 0.0015]:
                            ...
                            tasks.append(inner_func.remote(symbol_name, source_signal_df, long_pred_th,
...
if __name__ == "__main__":
    ...
    model_config_source["train_start_time"] = "20230106"
    model_config_source["train_end_time"] = "20240430"
    model_config_source["valid_start_time"] = "20240501"
    model_config_source["valid_end_time"] = "20240607"
    model_config_source["test_start_time"] = "20240610"
    model_config_source["test_end_time"] = "20240710"
    ...
    if stock_set == "zz500":
        ...
        flying_factor = ["PriceSpread", "OneBigOrder", "OneBigOrderExtend", "CumOrdersNetVolOverV0", "BreakingP0NumOrders"]
    ...
    elif stock_set=="hs300":
        symbols = pd.read_csv("hs300_select.csv", header=None)[0].tolist()
        SYMBOL_LIST = symbols
        cached_norm_dataset = "/dfs/group/800657/exp_results/zz500_dataset"
        select_factors = pd.read_csv(os.path.join(cached_norm_dataset, "factors_drop.csv"), header=None)[0].tolist()
        flying_factor = pd.read_csv("extra_factors.csv", header=None)[0].tolist()
        flying_factor = ["PriceSpread", "OneBigOrder", "CumOrdersNetVolOverV0", "BreakingP0NumOrders"]#"OneBigOrderExtend",
        # flying_factor = ["FacPriceSpread", "FacOneBigOrder", "FacCumOrdersNetVolOverV0", "FacBreakingP0NumOrders"]
        # last_idx = SYMBOL_LIST.index("603589.SH")
        # SYMBOL_LIST = SYMBOL_LIST[last_idx:]
    else:
    ...
    for label_name, exp_name, version_alias in [
        ##################20240714##################
        # ("LabelFirstPeak_th12_60s", "exp_l3_zzkc_flying4_sample", 'tmp_tmp'),
        ...
        # ("LabelFirstPeak_th10_60s", "exp_l3_zzkc_flying4_levelone", 'LabelFirstPeak_th10_60s_factor98_levelone_amp_log2'),
        # ("LabelFirstPeak_th10_60s", "exp_l3_zzkc_flying4_levelone", 'LabelFirstPeak_th10_60s_factor98_levelone'),
        # ("LabelFirstPeakAdjust0_th10_60s", "exp_l3_zzkc_flying4_levelone", 'LabelFirstPeakAdjust0_th10_60s_factor98_levelone_log2'),
        # ("LabelFirstPeakAdjust0_th12_60s", "exp_l3_zzkc_flying4_levelone", 'LabelFirstPeakAdjust0_th12_60s_factor98_levelone_log2'),
        ##################20240728##################
        ("LabelFirstPeak_th10_60s", "exp_l3_hs300_flying4_levelone", 'LabelFirstPeak_th10_60s_factor98_levelone_log2'),
    ]:
        ...
        if "amp" in version_alias:
            ...
            flying_factor = ["FacPriceSpread", "FacOneBigOrder","FacOneBigOrderExtend","FacCumOrdersNetVolOverV0", "FacBreakingP0NumOrders"]
