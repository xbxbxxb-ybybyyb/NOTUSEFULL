...
################################################################
def main_generate_parquet(symbol, exp_version_path):
    try:
        xgb_regressor = None
        ...
        for date in dates:
            ...
            flying_factor_df = load_flying_factors(symbol, dates = [date], use_pandas = False, flying_base_dir = "/dfs/group/800657/library/l3_event/event_data")
...
#         factor_config = pd.read_json("/dfs/group/800657/exp_results/kc_dataset/{}_factor_config.json".format(symbol))
            ...
            if os.path.exists(save_parquet_path):
                try:
                    ...
                    if "close_flying" in source_signal_df.columns:
                        signal_df["close_flying_flag"] = merge_df["close_flying"]  # .astype(int)
...
if __name__=="__main__":
    ...
    symbol_list = [symbol for symbol in symbol_list if symbol.startswith("688")]
    ...
    symbol_list = ["688037.SH", "688506", "688072.SH", "688536.SH", "688318.SH"]
    symbol_list = ["688271.SH", "688052.SH", "688012.SH", "688981.SH", "688017.SH", "688256.SH", "688047.SH", "688041.SH", "688498.SH"]
    ...
    for label_name, exp_name, version_alias in [
        ...
        if flag_generate_parquet:
            ...
            tasks = [remote_func.remote(symbol, exp_version_path) for symbol in symbol_list]
            ray.get(tasks)
        ############################(3)计算止盈止损率指标##############################
        if flag_winloss:
            result_list = []
            for symbol_name in symbol_list:
                try:
                    tasks_dict = {}
                    for path in ["signal_sample_new.xlsx", "signal_all_new.xlsx"][:1]:
                        ...
                        try:
                            ...
                            source_signal_df = source_signal_df[(source_signal_df["DATE"]>=sdate) & (source_signal_df["DATE"]<=edate)]
                        ...
                        if target_type == "mid":
                            ...
                            th_list = [0.9, 1.0, 1.2, 1.4, 1.6, 1.8]
                        for th in th_list:
