...
from artifacts.dataload_utils import load_polars_label
...
class L2PXGBoostRegPack:
        ...
        for symbol in model_config["symbol_list"]:
            ...
            if os.path.exists(source_data_path) and not use_pandas:
                ...
                if not os.path.exists(os.path.join(exp_path, "dataset/{}_flying_factor.parquet".format(symbol))):
                    ...
                    label_df_all = load_polars_label(symbol, model_config["train_start_time"], model_config["test_end_time"], label_list[0])
                    if len(label_df_all)==0:
                        continue
                    # ############## 去除上市首月的数据, 避免极端行情的影响
                    label_df_all = label_df_all.select(["DateTime", label_list[0]]).filter(pl.col(label_list[0])<=model_config["data_config"]["tagger_limit"])[14200*22:]
    ...
    def train_loop(self, model_params):
        ...
        for symbol in symbol_list:
            ...
            open_flying_label_df = pl.read_parquet(
                os.path.join(exp_path, "dataset/{}_flying_label.parquet".format(symbol)))
            if label_name.startswith("LabelShort") or "short_ret" in label_name:
                top = open_flying_label_df.select(label_name).quantile(quantile=0.90)[0, 0]
                open_flying_label_df_pos = open_flying_label_df.filter(pl.col(label_name) > 0)
                open_flying_label_df_neg = open_flying_label_df.filter(pl.col(label_name) <= 0)
                ttl_num = open_flying_label_df.shape[0]
                ######### 理想的跌、涨、大涨比例 ##########
                target_fraction = [0.5, 0.4, 0.1]
                neg_fraction = min(len(open_flying_label_df_neg), ttl_num*0.5)/len(open_flying_label_df_neg)
                pos1_fraction = min(ttl_num-len(open_flying_label_df_neg), ttl_num*0.4, len(open_flying_label_df_neg)*0.8)/len(open_flying_label_df_pos)
                pos2_fraction = min(ttl_num-len(open_flying_label_df_neg), ttl_num*0.1, len(open_flying_label_df_neg)*0.2)/len(open_flying_label_df_pos)

                # open_flying_label_df_neg = open_flying_label_df_neg.sample(fraction=neg_fraction)
                open_flying_label_df_pos2 = open_flying_label_df.filter(
                    (pl.col(label_name) > 0) & (pl.col(label_name) < top)). \
                    sample(fraction=pos2_fraction, seed=2024)  # 正负样本比越0.6/0.4
                open_flying_label_df_pos1 = open_flying_label_df.filter(pl.col(label_name) >= top).sample(fraction=pos1_fraction,
                                                                                                          seed=2024)
                open_flying_label_df = pl.concat(
                    [open_flying_label_df_neg, open_flying_label_df_pos1, open_flying_label_df_pos2])
                ttl_num = open_flying_label_df.shape[0]
                print(f"[{label_name}] fraction: neg: {len(open_flying_label_df_neg) / ttl_num}, pos1: {len( open_flying_label_df_pos1) / ttl_num}, pos2: {len(open_flying_label_df_pos2) / ttl_num}")
            elif label_name.startswith("LabelLong")  or "long_ret" in label_name:
                bottom = open_flying_label_df.select(label_name).quantile(quantile=0.1)[0, 0]
                open_flying_label_df_pos = open_flying_label_df.filter(pl.col(label_name) >= 0)  # 正负样本比越0.6/0.4
                open_flying_label_df_neg = open_flying_label_df.filter(pl.col(label_name) < 0)  # 正负样本比越0.6/0.4

                ttl_num = open_flying_label_df.shape[0]
                ######### 理想的涨、跌、大跌比例 ##########
                target_fraction = [0.5, 0.4, 0.1]
                pos_fraction = min(len(open_flying_label_df_pos), ttl_num*0.5)/len(open_flying_label_df_pos)
                neg1_fraction = min(ttl_num-len(open_flying_label_df_pos), ttl_num*0.4, len(open_flying_label_df_pos)*0.8*1.1)/len(open_flying_label_df_neg)
                neg2_fraction = min(ttl_num-len(open_flying_label_df_pos), ttl_num*0.1,  len(open_flying_label_df_pos)*0.2*1.1)/len(open_flying_label_df_neg)

                open_flying_label_df_neg2 = open_flying_label_df.filter(pl.col(label_name) <= bottom).sample(
                    fraction=neg2_fraction, seed=2024)
                open_flying_label_df_neg1 = open_flying_label_df.filter(
                    (pl.col(label_name) < 0) & (pl.col(label_name) > bottom)).sample(fraction=neg1_fraction, seed=2024)  # 正负样本比越0.6/0.4
                open_flying_label_df = pl.concat([open_flying_label_df_neg1, open_flying_label_df_neg2, open_flying_label_df_pos])
                ttl_num = open_flying_label_df.shape[0]
                print(f"[{label_name}] fraction: pos: {len(open_flying_label_df_pos) / ttl_num}, neg1: {len(open_flying_label_df_neg1) / ttl_num}, neg2: {len(open_flying_label_df_neg2) / ttl_num}")


            open_flying_df = open_flying_factor_df.join(open_flying_label_df, on="DateTime")
            # 将采样完的样本仍然按之前的时间排列
            open_flying_df = open_flying_df.sort(by = "DateTime")
            ########################划分训练测试集################################
            ...
            if model_config["data_config"]["tagger_trim"]:
                Y_train[(Y_train<model_config["data_config"]["tagger_trim"]) & (Y_train>-model_config["data_config"]["tagger_trim"])]=0
                Y_valid[(Y_valid<model_config["data_config"]["tagger_trim"]) & (Y_valid>-model_config["data_config"]["tagger_trim"])]=0
    ...
    def predict_signal(self, xgb_regressor):
        ...
        for symbol in model_config["symbol_list"]:
            ...
            if model_config["data_config"]["tagger_trim"]:
                Y_test[Y_test<model_config["data_config"]["tagger_trim"]]=0
    ...
    def analysis_signal_winloss(self, start_date="2023-12-06", end_date="2024-02-06"):
        ...
        for path in ["signal_sample.xlsx", "signal_all.xlsx"][:1]:
...
if __name__ == "__main__":
    ...
    for label_name, exp_name, version_alias in [
        # 从polars加载标签数据
        # #######################20240630##################
        # ("vwap01_long_ret_60s", "exp_l3_zzkc_flying4", 'vwap01_long_ret_60s_factor98_log2_huber1'), # 没有对数据采样，效果不好
        # ("vwap01_short_ret_60s", "exp_l3_zzkc_flying4", 'vwap01_short_ret_60s_factor98_log2_huber1'),
        # ("smooth_long_ret_60s", "exp_l3_zzkc_flying4", 'smooth_long_ret_60s_factor98_log2_huber1'),
        # ("smooth_short_ret_60s", "exp_l3_zzkc_flying4", 'smooth_short_ret_60s_factor98_log2_huber1'),
        # ("dol_long_ret_60s", "exp_l3_zzkc_flying4", 'dol_long_ret_60s_factor98_log2_huber1'),
        # ("dol_short_ret_60s", "exp_l3_zzkc_flying4", 'dol_short_ret_60s_60s_factor98_log2_huber1'),
        # #######################20240703##################
        ...
        # #######################20240710##################
        # ("vwap01_long_ret_120s", "exp_l3_zzkc_flying4", 'vwap01_long_ret_120s_factor98_sample_log2'),  # 样本进行不均匀采样后不适合用log2，完全破坏分布
        # ("vwap01_short_ret_120s", "exp_l3_zzkc_flying4", 'vwap01_short_ret_120s_factor98_sample_log2'),
        # #######################20240710##################
        ("vwap01_long_ret_60s", "exp_l3_zzkc_flying4", 'vwap01_long_ret_60s_factor98_sample_trim'),#截取0.5以下的标签
        ("vwap01_short_ret_60s", "exp_l3_zzkc_flying4", 'vwap01_short_ret_60s_factor98_sample_trim'),
        ("smooth_long_ret_60s", "exp_l3_zzkc_flying4", 'smooth_long_ret_60s_factor98_sample_trim'),
        ("smooth_short_ret_60s", "exp_l3_zzkc_flying4", 'smooth_short_ret_60s_factor98_sample_trim'),
        ("dol_long_ret_60s", "exp_l3_zzkc_flying4", 'dol_long_ret_60s_factor98_sample_trim'),
        ("dol_short_ret_60s", "exp_l3_zzkc_flying4", 'dol_short_ret_60s_60s_factor98_sample_trim'),
    ]:
        ...
        if "log2" in version_alias:
            ...
            model_config["data_config"]["tagger_trim"] = 0.4 #截取0.5以下的标签
