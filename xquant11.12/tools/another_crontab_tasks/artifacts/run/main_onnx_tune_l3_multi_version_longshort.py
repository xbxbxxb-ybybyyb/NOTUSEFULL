...
class L2PXGBoostRegPack:
    ...
    def train_loop(self, model_params):
        ...
        for symbol in symbol_list:
            ...
            if label_name.startswith("LabelShort"):
                top = open_flying_label_df.select(label_name).quantile(quantile=0.9)[0, 0]
                open_flying_label_df_neg = open_flying_label_df.filter(pl.col(label_name) <= 0)
                open_flying_label_df_pos1 = open_flying_label_df.filter(
                    (pl.col(label_name) > 0) & (pl.col(label_name) < top)). \
                    sample(fraction=0.5, seed=2024)  # 正负样本比越0.6/0.4
                open_flying_label_df_pos2 = open_flying_label_df.filter(pl.col(label_name) >= top).sample(fraction=0.8,
                                                                                                          seed=2024)
            ...
            elif label_name.startswith("LabelLong"):
                bottom = open_flying_label_df.select(label_name).quantile(quantile=0.1)[0, 0]
                open_flying_label_df_neg1 = open_flying_label_df.filter(pl.col(label_name) <= bottom).sample(
                    fraction=0.8, seed=2024)
                open_flying_label_df_neg2 = open_flying_label_df.filter(
                    (pl.col(label_name) < 0) & (pl.col(label_name) > bottom)). \
                    sample(fraction=0.5, seed=2024)  # 正负样本比越0.6/0.4
                open_flying_label_df_pos = open_flying_label_df.filter(pl.col(label_name) >= 0)  # 正负样本比越0.6/0.4
                open_flying_label_df = pl.concat(
                    [open_flying_label_df_neg1, open_flying_label_df_neg2, open_flying_label_df_pos])
                ttl_num = open_flying_label_df.shape[0]
            ...
            if model_config["data_config"]["tagger_trim"]:
                Y_train[(Y_train < model_config["data_config"]["tagger_trim"]) & (
                            Y_train > -model_config["data_config"]["tagger_trim"])] = 0
                Y_valid[(Y_valid < model_config["data_config"]["tagger_trim"]) & (
                            Y_valid > -model_config["data_config"]["tagger_trim"])] = 0
...
if __name__ == "__main__":
    ...
    for label_name, exp_name, version_alias in [
        ...
        # ##################20240627##################
        # ("LabelLongTwoMin", "exp_l3_zzkc_flying4", 'LabelLongTwoMin_factor98_sample_log2'),
        # ("LabelShortTwoMin", "exp_l3_zzkc_flying4", 'LabelShortTwoMin_factor98_sample_log2'),
        # ("LabelLongOneMin", "exp_l3_zzkc_flying4", 'LabelLongOneMin_factor98_sample_log2'),
        # ("LabelShortOneMin", "exp_l3_zzkc_flying4", 'LabelShortOneMin_factor98_sample_log2'),
    ]:
