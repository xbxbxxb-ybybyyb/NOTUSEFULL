...
class StrategyTimer(StrategyBase): # 策略类需继承 StrategyBase 基类
    ...
    def __init__(self):
        ...
        self.add_timer(name='1s', when=xb.timer.SESSION_START, offset=datetime.timedelta(seconds=0),
                       repeat=datetime.timedelta(seconds=1))
        # self.add_timer(name='2s', when=xb.timer.SESSION_START, offset=datetime.timedelta(seconds=2),
        #               repeat=datetime.timedelta(seconds=2))
    ...
    def notify_timer(self, timer, when, *args, **kwargs):
        ...
        if kwargs.get('name') == '1s':
            ...
            self.log('notify_timer 1s: when {}, open: {} close: {}, high: {}, low: {},  volume: {}'.format(when,
                opent, close, high, low, volume
            ))
        else:
            self.log('notify_timer 2s: when {}, open: {} close: {}, high: {}, low: {},  volume: {}'.format(when,
...
if __name__ == '__main__':
    # 创建回测框架 XBrain 实例
    brain = XBrain(start_date="20190401 093000000", end_date="20190401 095959000", live=False)
    ####################################################################
