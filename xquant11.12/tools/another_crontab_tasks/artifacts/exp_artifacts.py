...
class ExpArtifacts:
    ...
    def backtest_trade_file_save(self, strategy_name, strategy_config_alias, signal_process_base_dir, trade_records_df):
        ...
        if not trade_records_df.empty:
            ...
            for date in dates:
                ...
                self.db.online_trasaction_data_todb(df_date)
            return True
    ...
    def backtest_trade_evaluation(self, strategy_name, strategy_config_alias, signal_process_base_dir,
        ...
        if not trade_records_df.empty:
            ...
            daily_market_df = self.ma.get_data_by_date("Stock", stock, date_)
            ...
            self.db.backtest_evaluation_data_todb(exp_id=self.exp_name, version_alias=self.version_alias,
    ...
    def backtest_plot_signal_trade(self, signal_df_day, trade_records_df_day, ma_df_day=None, plot_save_dir="./"):
        ...
        if not isinstance(ma_df_day, pd.DataFrame):
            ma_df_day = self.ma.get_data_by_date("STOCK", symbol, date.replace("-", ""))
        if ma_df_day.empty:
...
#             print("save signal&trade plot file ok! {}".format(os.path.join(plot_save_dir, "plot_signal_trade_{}.html".format(date))))
        ...
        res = self.db.UploadFile(file_name, plot_save_path)
        if is_operation:
            data_type = "online"
        else:
            data_type = "offline"
        self.db.plot_uniquekey_todb(stock=symbol, date=date.replace("-", ""), strategy_name=strategy_name,
    ...
    def load_backtest_evaluation_data(self, exp_id=None, symbol=None, version_alias=None,
        ...
        df = self.db.load_backtest_evaluation_data(exp_id, symbol, version_alias, backtest_evaluation_type, is_operation)
        return df
