    ...
    for idx,row in valid_df.iterrows():
        if row["cls"] != 2:
            ...
            if row["cls"] > 2:  # 看涨
                ...
                res = judge_long(list(tmp[up_end_price_col]), profit, loss)
                #print(res)
                if res == 2:
                    info_dic["up_up"] += 1
                if res == 1:
                ...
                res = judge_short(list(tmp[dw_end_price_col]), profit, loss)
                if res == 2:
                    info_dic["dw_dw"] += 1
                if res == 1:
                    info_dic["dw_0"] += 1
...
def winloss_stop_table_daily(signal_df, long_pred_th = 1.2, short_pred_th = -1.2, target_type = "mid", win_limits = [0.0015], loss_limits = [0.002], t_sta="09:33:00", use_self_prob = False, local_mode = False):
    """
...
def winloss_func_agg(res_df, verbose = 1):
    ...
    try:
        ...
        print("'short_pred_th' referenced before assignment: ", e)
        pass
        symbol_name = ""
...
def winloss_func(symbol_name, source_signal_df, long_pred_th, short_pred_th, start_date="2023-12-06", end_date="2024-02-06", use_self_prob = False,
    ...
    print("source_signal_df: ", source_signal_df)
