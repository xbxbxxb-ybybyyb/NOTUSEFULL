...
if __name__=="__main__":
    ######################注意设置标的和日期####################
    check_data_dict = json.load(open("/data/user/013150/tmp/factor_result.json", "r"))
    fa = FactorData()
    symbol_flag = "HS300"
    symbols_all = get_stock_pool(["HS300", "HS_TICK2_new", "HS_TICK2_add"])
    symbols = symbols_all
    ...
    try:
        ...
        dates = fa.tradingday("20240901", "20241210")
    ...
    print("symbols_all:", len(symbols_all))
    print(dates)
    ######################注意修改存储路径####################
    updateL3Data = False
    updateVolQuantile = False
    ...
    if updateVolQuantile:
        ...
        tasks = [remote_func.remote(code, dates, lookback_days=lookback_days) for code in symbols]
    ...
    if updateFlyingFactorFull:
        for task_func in [UpdateFlyingFactorFull]:
            ...
            for symbol in symbols[::-1]:
                for date in dates[::-1]:
                    if date in check_data_dict and symbol in check_data_dict[date]:
