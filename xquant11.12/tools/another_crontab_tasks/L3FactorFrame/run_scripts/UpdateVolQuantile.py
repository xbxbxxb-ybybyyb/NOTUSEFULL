...
def get_daily_vol_df(symbol, date, use_pandas = False):
    try:
        local_vol_df, _, _, _ = get_l3_trade_order_data(symbol, date, use_pandas = use_pandas)
...
def UpdateVolQuantile(code, dates, lookback_days = 5):
    try:
        ...
        print("ERROR: {} {}".format(code, dates), traceback.print_exc())
