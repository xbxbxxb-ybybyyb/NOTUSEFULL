...
# label_name, exp_name,version_alias = ("LabelFirstPeak_th12_60s", "exp_l3_zzkc_flying4", 'LabelFirstPeak_th12_60s')
...
start_date, end_date = "20220511", "20220511"
...
winloss_result_list = []
for symbol in ["002432.SZ"]:  # symbol_list[:1]:
    ...
    for date in dates:
        ...
        signal_df["flying_flag"] = merge_df["open_flying"]
        signal_df["PREDICTED"] = signal_df["PREDICTED"] * signal_df["flying_flag"].astype(int)
        signal_df["PREDICT"] = signal_df["PREDICT"] * signal_df["flying_flag"].astype(int)
...
#         signal_df_day = online_model.generate_probs_v3(signal_df["PREDICT"], long_pred_th, short_pred_th, amp=6, period=120, target_value=signal_df["TARGET_VALUE"].values, target_type="mid")
