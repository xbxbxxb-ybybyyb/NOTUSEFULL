...
class FacActivePVBuy(FactorBase):
    def __init__(self, config, factorManager, marketDataManager):
        ...
        self.active_trade_price = config.get("active_trade_price", 0.0008)
        ...
        if len(asks_price) < 2:
            ...
            if tickDataIndex == tradeIndex:
                ...
                if (currentTickAskP0-0.01-(currentTickBidP0+0.01))/ mid_price > self.price_spread:
                    ...
                    active_buy_volume =  np.sum(trade_volume[(trade_bs_flag == 1) & (trade_price >= bid_base_price*(1+self.active_trade_price))])
                    active_sell_volume =  np.sum(trade_volume[(trade_bs_flag == 2) & (trade_price <= ask_base_price*(1-self.active_trade_price))])
                    # 条件3：主动买大于某个值
                    if active_buy_volume > self.active_volume:
                        factor_value = active_buy_volume/(active_sell_volume+active_buy_volume)
