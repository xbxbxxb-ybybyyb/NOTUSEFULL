...
def pd_hdf5_writer(pd_factor, hdf5, dataset=None, override=None, append=None,append_all=None,
                   min_itemsize=None,from_scratch=True,date_column=None,override_overlap=True):
    '''
    --- DESCRIPTION ---
...
def automatic_date(trading_days):
    ...
    # min_trading_day = min(trading_days)
    # max_trading_day = max(trading_days)
    # if isinstance(min_trading_day,pd.Timestamp) and isinstance(max_trading_day,pd.Timestamp):
    #     start_date = dt.datetime.strftime(min_trading_day,"%Y%m%d")
    #     end_date = dt.datetime.strftime(max_trading_day,"%Y%m%d")
    # else:
    ...
    with pd.HDFStore(hdf5) as h5_store:
        ...
        for file in file_list:
            ...
            if date_ticker in dt_lst:
                ...
                record_num = h5_store.remove(dataset, 'dt=date_ticker')
                logger.info('%d records deleted at %s %s' % (record_num, date_ticker.isoformat(), dataset))
                # logger.info('%s already exists in database, skipping...' % date_ticker.isoformat())
