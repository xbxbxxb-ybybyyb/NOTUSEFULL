...
class UpdateBarraRisk:
    def __init__(self, sdate, edate, operation='append', universe='alpha_universe', benchmark = 'zz500', 
        ...
        print("self.sdate_prev:>",self.sdate_prev)
    ...
    def load_basic_data(self):
        ...
        alt_mdstock = "/app/data/wdb_h5/WIND/MD_CHINA_STOCK_DAILY_WIND/MD_CHINA_STOCK_DAILY_WIND.h5"
        h5_data = read_data([sdate_prev, edate], columns=md_list,alt=alt_mdstock)
        # modify total_shares 测试库为空暂无法update 手动填充值
        h5_data['total_shares'] = 214934.4971
        ...
        alt_univ = "/app/data/wdb_h5/WIND/UNIV_CHINA_STOCK_DAILY_OPTM/UNIV_CHINA_STOCK_DAILY_OPTM.h5"
        ...
        alt_industry = "/app/data/wdb_h5/WIND/INDUSTRY_CHINA_STOCK_DAILY_WIND/INDUSTRY_CHINA_STOCK_DAILY_WIND.h5"
        ...
        alt_mdindex = "/app/data/wdb_h5/WIND/MD_CHINA_INDEX_DAILY_WIND/MD_CHINA_INDEX_DAILY_WIND.h5"
        h5_index = read_data([self.sdate_prev, self.edate], 'close', alt=alt_mdindex)
        # modify 测试库内没有值，暂时填充固定值
        h5_index['close'] = 1478.0754
        ...
        print("len(dat['excess_return']):>1",len(dat['excess_return']))
        ...
        print("self.data:>",len(self.data["excess_return"]))
    ...
    def load_fdd_data(self):
        ...
        # modify
        alt_AShareDividend = "/app/data/wdb_h5/WIND/AShareDividend/AShareDividend.h5"
        ...
        alt_AShareEODDerivativeIndicator = "/app/data/wdb_h5/WIND/AShareEODDerivativeIndicator/AShareEODDerivativeIndicator.h5"
        data_eod_derive = read_data([self.sdate, self.edate], columns=eod_derive_list,alt=alt_AShareEODDerivativeIndicator)
        alt_AShareBalanceSheet = "/app/data/wdb_h5/WIND/AShareBalanceSheet/AShareBalanceSheet.h5"
        data_balance_full = read_data([sdate_qtr, edate_qtr], columns=balance_list+['STATEMENT_TYPE'],alt=alt_AShareBalanceSheet)
        # modify 测试库没有数据 暂时填充固定值
        data_balance_full.fillna(1602000.4, inplace=True)
        ...
        alt_AShareIncome = "/app/data/wdb_h5/WIND/AShareIncome/AShareIncome.h5"
        data_income_full = read_data([sdate_qtr, edate_qtr], columns=income_list+['STATEMENT_TYPE'], alt=alt_AShareIncome)
        # modify 测试库数据缺失 暂填充固定值
        data_income_full['NET_INT_INC'].fillna(3299257000, inplace=True)
        ...
        alt_AShareFinancialIndicator =  "/app/data/wdb_h5/WIND/AShareFinancialIndicator/AShareFinancialIndicator.h5"
        data_fin_ind = read_data([sdate_qtr, edate_qtr], columns=ind_list, alt=alt_AShareFinancialIndicator)
        alt_AShareTTMHis = "/app/data/wdb_h5/WIND/AShareTTMHis/AShareTTMHis.h5"
        data_ttm = read_data([sdate_qtr, edate_qtr], columns=ttm_list, alt=alt_AShareTTMHis)

        alt_AShareCashFlow = "/app/data/wdb_h5/WIND/AShareCashFlow/AShareCashFlow.h5"
    ...
    # def risk_beta(self, half_life=25, look_back_days=30, smooth_num=4, ma_alpha=11, lag_alpha=11):
        logger.info('Update Risk Factor: Market Beta & Residual Volatility')
        take_idx = self.iter_num + look_back_days + ma_alpha + lag_alpha + 1
        # 测试数据量少  暂注释
        print("self.data['excess_return']:>",len(self.data['excess_return']))
    ...
    def risk_liquidity(self, window=2, look_back_terms=12):
...
#    # modify 暂固定sdate,edate,cdate_list
#    # sdate,edate,cdate_list = check_update_date()
#    sdate = 20131129
#    edate = 20131129
#    cdate_list = [20131129]
#    #operation = 'append'
#    self = UpdateBarraRisk(sdate, edate)
#    self.load_data()
#    #self.read_data()
#    self.launch()
