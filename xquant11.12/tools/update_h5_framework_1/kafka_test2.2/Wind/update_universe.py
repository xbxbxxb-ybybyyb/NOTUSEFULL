...
def get_hset(date,index_stock,sql_config):
    ...
    sql_use = "select stock_code as symbol, stock_name as name, weight from qd_index_weights_view where tradingcode = %s%s%s and tradingday = %s " % ("'", index_stock, "'", str(20180522))
    logger.info(sql_use)
...
class unv_factor(object):
    ...
    def csv_to_database(self):
        ...
        # data_universe.reset_index(['dt','Ticker'],inplace=True)
        return data_universe
        # for dataset_name in dataset_names:
        #     df_universe = data_universe.loc[:,dataset_name]
        #     pd_hdf5_writer(df_universe, h5_path_destination, dataset_name, append=True)
