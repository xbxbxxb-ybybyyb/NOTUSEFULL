...
def hdf5_writer(xml_path,cdate_list,sql_config,supplement_h5_name,supplement_factors,unable_write_list):
    """
    ...
    for h5_name,source_data in factor_dict.items():
        ...
        if supplement_h5_name and h5_name not in supplement_h5_name:
            continue
        if unable_write_list and h5_name in unable_write_list:
            continue
        ...
        for type in source_data.keys():
            ...
            if date_type == "Quarterly" and source_data[type]['freq'] != "Quarterly":
                continue
            elif date_type == "Daily" and source_data[type]['freq'] == "Quarterly":
                continue
                ...
                if "dt_field" in wind_table[table]:
                    if h5_name in ['i_organ_score', 'gg_org_list', 'i_report_type', 'author_core_type','t_great_author',
                        ...
                        sql_use = "select *" + " from " + table + " where " + wind_table[table]["dt_field"] + "<=" + "to_date(" + str(date) + ",'YYYYMMDD')" + \
                                  " and " + wind_table[table]["dt_field"] + ">=" + "to_date(" + str(sdate) + ",'YYYYMMDD')"
                    # elif h5_name in ['author_pj']:
                    #     sql_use = "select *" + " from " + table + " where " + 'Rpt_Date' + "=" + str(date)[:4]
                    else:
                        sql_use = "select *" + " from " + table + " where " + wind_table[table]["dt_field"] + "<=" + str(date) + " and " + wind_table[table]["dt_field"] + ">=" + str(sdate)
                else:
                    sql_use = "select *" + " from " + table
                logger.info(sql_use)
                df = sql_parser(queryUserTableData(sql_use, sql_config))
                ...
                if 'dt' in df.columns and 'Ticker' in df.columns:
                    ...
                    if "dt_field" in wind_table[table]:
                        if h5_name == "FCD_CHINA_STOCK_DAILY_SUNTIME":
                            pd_hdf5_writer(df, store_h5_path + h5_name + ".h5", h5_name, append_all=True,min_itemsize=Indefinite_length)
                            ...
                            pd_hdf5_writer(df, store_h5_path + h5_name + ".h5", real_table, append_all=True,min_itemsize=Indefinite_length)
                    else:
                        pd_hdf5_writer(df, store_h5_path + h5_name + ".h5", real_table, override=True)
                    logger.info("The h5 - %s write success!" % h5_name)
                else:
...
def execute(start_date,end_date,xml_path,sql_config,supplement_h5_name,supplement_factors,unable_write_list):
    # sdate, edate, cdate_list = check_update_date(start_date, end_date)
    # for his_enddate in cdate_list:
    start = time.time()
    his_sdate, his_edate, his_datelst = check_update_date(start_date, end_date)
    ...
    while not csv_exist:
        ...
        if not os.path.exists(csv_name):
            ...
            time.sleep(10)
    # 开始解析配置项并写入h5文件
    hdf5_writer(xml_path,his_datelst,sql_config,supplement_h5_name,supplement_factors,unable_write_list)
...
def parse_argv():
    ...
    if len(sys.argv) == 5:
        ...
        start_date = 20080331 #20131118
        end_date = 20191231 # 20080930
    ...
    return start_date,end_date,supplement_h5_name,supplement_factors,unable_write_list
...
if __name__ == "__main__":
    ...
    start_date,end_date,supplement_h5_name,supplement_factors,unable_write_list = parse_argv()
    if supplement_factors and not supplement_h5_name:
        raise Exception("supplement data must appoint the h5 name,aborting...")
    execute(start_date, end_date, xml_path, sql_config, supplement_h5_name, supplement_factors,unable_write_list)
