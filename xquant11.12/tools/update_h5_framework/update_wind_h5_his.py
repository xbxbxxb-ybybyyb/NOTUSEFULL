...
def hdf5_writer(xml_path,cdate_list,sql_config,supplement_h5_name,supplement_factors):
    ...
    for h5_name,source_data in factor_dict.items():
        ...
        #     logger.info("time sleep 2 hours...")
        #     time.sleep(7200)
        #     logger.info("Continue...")
        time3 = datetime.datetime.strptime(str(datetime.datetime.now().date()) + '23:00', '%Y-%m-%d%H:%M')
        time4 = datetime.datetime.strptime(str(datetime.datetime.now().date()) + '23:59', '%Y-%m-%d%H:%M')
        now2 = datetime.datetime.now()
        if now2 > time3 and now2 <= time4:
            logger.info("time sleep 10 hours...")
            time.sleep(36000)
            logger.info("Continue...")
...
def parse_argv():
    ...
    if len(sys.argv) == 5:
        ...
        start_date = 20200506 #20131118
        end_date = 20200506 # 20080930
        # 补某h5的历史数据，supplement_factors为空则补该h5的所有因子
        #supplement_h5_name = ['AShareEODDerivativeIndicator','con_forecast_schedule','FCD_CHINA_STOCK_DAILY_SUNTIME','stock_report_number','con_forecast_c2_stk','con_forecast_c3_stk','cmb_report_adjust','stock_order3','stock_order2','stock_report_adjustment2','stock_concern_level','con_stock_deviation3']
        supplement_h5_name = ['con_forecast_schedule','stock_order3','stock_report_number','stock_order2','stock_report_adjustment2','stock_concern_level','con_stock_deviation3']
        supplement_factors = []
    else:
