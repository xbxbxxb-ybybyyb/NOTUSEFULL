...
def path_assembler(mkttype, dtype, ftype, dfreq, dsource, alt, h5root):
    ...
    if alt is not None:
        ...
        # else:
        #     abs_h5_path = '/app/data/wdb_h5/WIND/' + names[0] + '/' + names[0] + '.h5'
    else:
...
def get_Lasttradingday(date,sql_config):
    ...
    :return:
    """
    sql_use = "select lasttradingday from center_read.qd_tradingdays where tradingday =%s"%str(date)
...
def Judge_Tradingday(date,sql_config):
    sql_tradingday = "select istradingday from center_read.qd_tradingdays where tradingday = %s" % str(date)
...
def get_Tradingday_list(sql_config,start_date,end_date):
    ...
    :return:
    """
    sql_use = "select tradingday from center_read.qd_tradingdays where (tradingday >= {0} and tradingday <= {1}) and istradingday=1".format(start_date,end_date)
...
def judge_Quarterly(date,sql_config):
    ...
    is_tradingday = sql_parser(queryUserTableData(sql_tradingday,sql_config))
    if is_tradingday.empty:
        logger.info("Warning: date %s not in center_read.qd_tradingdays"%str(date))
        return "No_tradingday"
    else:
        is_trading = int(is_tradingday.iloc[0][0])
...
def get_trading_day_offset(start_date, offset_list, dfreq=DFreq.DAILY, dtype=DType.STOCK,
    ...
    alt = "/app/data/wdb_h5/WIND/MD_CHINA_INDEX_DAILY_WIND/MD_CHINA_INDEX_DAILY_WIND.h5"
...
def get_prev_sdate(sdate,day_num,sql_config):
    sql_use = "select * from (select a.* from center_read.qd_tradingdays a where tradingday <= %s and istradingday=1 order by tradingday desc) where rownum <= %d" % (
    str(sdate), day_num)
