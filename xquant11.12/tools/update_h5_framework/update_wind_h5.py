...
def hdf5_writer(xml_path, cdate_list, sql_config, fail_h5_list=None, supplement_h5_name=None, supplement_reverse=False):
    """
                ...
                if type == 'type_0':
                    ...
                    for table in wind_table.keys():
                        ...
                        for factor_name in factor_list_1:
                            ...
                            params = {'method': key, 'taskNames': h5_name + '_' + factor_name, 'execDate': execDate,
                    ...
                    if source_data[type]["return_df"] == "all_factor":
                        try:
                            ...
                            produceTime11 = dt.datetime.strftime(dt.datetime.now(), '%Y-%m-%d %H:%M:%S')
                            logger.error("h5name is %s  Error occurred: %s" % (h5_name, e))
                            params_e = {'method': 'stop', 'taskNames': h5_name, 'execDate': execDate, 'taskStatus': 3,
                                        'isH5': '0', 'produceTime': produceTime11}
                            push_data(params_e)
                            params_message_e = json.dumps(params_e)
                            logger.info("kafka_message" + params_message_e)
                            continue
                    elif source_data[type]["return_df"] == "Internal_write":
                        try:
                            f(date, sql_config)
                            produceTime10 = dt.datetime.strftime(dt.datetime.now(), '%Y-%m-%d %H:%M:%S')
                            params = {'method': 'stop', 'taskNames': h5_name, 'execDate': execDate, 'taskStatus': 2,
                                      'isH5': '0', 'produceTime': produceTime10}
                            push_data(params)
                            params_message = json.dumps(params)
                            logger.info("kafka_message" + params_message)
                            logger.info("The h5 - %s write success!" % h5_name)
                            ...
                            produceTime12 = dt.datetime.strftime(dt.datetime.now(), '%Y-%m-%d %H:%M:%S')
                            logger.error("h5name is %s  Error occurred: %s" % (h5_name, e))
                            params_e = {'method': 'stop', 'taskNames': h5_name, 'execDate': execDate, 'taskStatus': 3,
                                        'isH5': '0', 'produceTime': produceTime12}
                            push_data(params_e)
                            params_message_e = json.dumps(params_e)
                            logger.info("kafka_message" + params_message_e)
                            continue
                    for factor_name in factor_list:
                        ...
                        params = {'method': key, 'taskNames': h5_name + '_' + factor_name, 'execDate': execDate,
                                  'isH5': '0', 'produceTime': produceTime5}
                        push_data(params)
                        params_message = json.dumps(params)
                        logger.info("kafka_message" + params_message)
                        if source_data[type]["return_df"] == "single_factor":
                            try:
                                ...
                                produceTime13 = dt.datetime.strftime(dt.datetime.now(), '%Y-%m-%d %H:%M:%S')
                                logger.error(
                                    "h5name is %s factor_name is %s Error occurred: %s" % (h5_name, factor_name, e))
                                params_e = {'method': 'stop', 'taskNames': h5_name + '_' + factor_name,
                                            'execDate': execDate, 'taskStatus': 3, 'isH5': '0',
                                            'produceTime': produceTime13}
                                push_data(params_e)
                                params_message_e = json.dumps(params_e)
                                logger.info("kafka_message" + params_message_e)
                                continue
                        if df is None or df.empty:
                            ...
                            params1 = {'method': 'stop', 'taskNames': h5_name + '_' + factor_name, 'execDate': execDate,
                                       'taskStatus': 4, 'isH5': '0', 'produceTime': produceTime6}
                            push_data(params1)
                            params_message1 = json.dumps(params1)
                            logger.info("kafka_message" + params_message1)
                    ...
                    params = {'method': 'stop', 'taskNames': h5_name, 'execDate': execDate, 'taskStatus': 2,
                              'isH5': '0', 'produceTime': produceTime7}
                    push_data(params)
                    params_message = json.dumps(params)
                    logger.info("kafka_message" + params_message)
                    logger.info("The h5 - %s write success!" % h5_name)
                    ...
                    if "dt_field" in wind_table[table]:
                        ...
                        elif h5_name in ['cmb_report_subtable', 'der_report_subtable', 'cmb_report_adjust',
                                         'report_author', 'cmb_report_score_adjust', 'cmb_report_research',
                                         'der_report_research']:
                    ...
                    if df is None or df.empty:
                        ...
                        params = {'method': 'stop', 'taskNames': h5_name, 'execDate': execDate, 'taskStatus': 4,
                                  'isH5': '0', 'produceTime': produceTime8}
                        push_data(params)
                        params_message = json.dumps(params)
                        logger.info("kafka_message" + params_message)
                    ...
                    params = {'method': key, 'taskNames': h5_name, 'execDate': execDate, 'isH5': '0',
                              'produceTime': produceTime9}
                    push_data(params)
                    params_message = json.dumps(params)
                    logger.info("kafka_message" + params_message)
                        ...
                        if "dt_field" in wind_table[table]:
                            if h5_name == "FCD_CHINA_STOCK_DAILY_SUNTIME":
                                pd_hdf5_writer(df, store_h5_path + h5_name + ".h5", h5_name, append=op_type,
                                               min_itemsize=Indefinite_length, date=cdate)
                            ...
                            pd_hdf5_writer(df, store_h5_path + h5_name + ".h5", real_table, override=True, date=cdate)
                        logger.info("The h5 - %s write success!" % h5_name)
                    else:
...
def execute2(xml_path, sql_config, cdate_list, supplement_h5_name=None, supplement_reverse=False):
    fail_list = hdf5_writer(xml_path, cdate_list, sql_config, supplement_h5_name=supplement_h5_name,
                            supplement_reverse=supplement_reverse)
...
def execute(start_date, end_date, xml_path, sql_config):
    ...
    wind_stock_path = "/app/data/wdb_h5/WIND/universe_complete/stocklst/"
    ...
    while not csv_exist:
        ...
        if not os.path.exists(csv_name):
            logger.info(
                "Write the csv %s %s times,it has not written,the progrem sleep 10s" % (str(edate) + '.csv', str(i)))
            time.sleep(10)
    # 开始解析配置项并写入h5文件
    # 每天17点开始更新指定的几个H5文件
    crontab_17_start = '16:55:00'
    crontab_17_end = '17:05:00'
    now_time = dt.datetime.now().time()
    logger.info("Now the time is %s ..." % str(now_time))
    # 17点单独更新的表
    supplement_h5_name_1 = ['AShareEODDerivativeIndicator']
    if now_time >= dt.datetime.strptime(crontab_17_start, '%H:%M:%S').time() and now_time <= dt.datetime.strptime(
            crontab_17_end, '%H:%M:%S').time():
        hdf5_writer(xml_path, cdate_list, sql_config, supplement_h5_name=supplement_h5_name_1)
        logger.info("Special 17:00 tasks have been completed!")
    else:
        p1 = multiprocessing.Process(target=execute2, args=(xml_path, sql_config, cdate_list, supplement_h5_name_1))
        p2 = multiprocessing.Process(target=execute2,
                                     args=(xml_path, sql_config, cdate_list, supplement_h5_name_1, True))
