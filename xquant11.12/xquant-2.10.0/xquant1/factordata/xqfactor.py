...
from xquant1.factordata.factorenum import *
from xquant1.factordata.source_table_config import *
import pandas as pd
import numpy as np
import time
...
def hset(plateType, dateTime, plateID, weightType=0, use_prev_name=True):
    ...
    if plateType == "INDEX":
        ...
        if weightType == 0:
            ...
            sql3 = "select " \
                   "(case exchangecode when '101' then concat(constituentcode,'.SH') when '105' then concat(constituentcode,'.SZ') end) as 'STOCK_CODE'," \
                   "round(weight,2) as weight from news_csinextdayweight where effectivedate = {0} and indexcode = '{1}' and isvalid = 1".format(
                dateTime, icode)
            df3 = dml2.getAllByPandas(c_name, sql3)
