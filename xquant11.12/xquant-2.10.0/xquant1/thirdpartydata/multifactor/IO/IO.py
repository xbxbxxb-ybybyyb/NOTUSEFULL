...
def automatic_date(trading_days):
    #[19980101, 20200101]
    start_date = pd.Timestamp(str(min(trading_days)))
    end_date = pd.Timestamp(str(max(trading_days)))

    datestart = pd.to_datetime(start_date)
    dateend = pd.to_datetime(end_date)
    date_list = []
    date_list.append(datestart)
    while datestart < dateend:
        datestart += dt.timedelta(days=1)
        date_list.append(datestart)
    # df = pd.DataFrame({'a': date_list, 'b': range(len(date_list))})
    # df.set_index('a', inplace=True)
    df = pd.DataFrame({'dt': date_list}).set_index('dt')
    return df

@statisticLog('thirdpartydata', 'multifactor.IO')
def read_data(trading_days, columns=None, universe=None, mkttype=MktType.CHINA, dtype=DType.STOCK,
              ftype=FType.MD, dfreq=DFreq.DAILY, dsource=DSource.HTSC, alt=None, h5root="/data/group/wdb_h5/WIND/", max_workers=1):
    '''
    --- DESCRIPTION ---
    ...
    with pd.HDFStore(abs_h5_path, 'r') as h5_store:
        ...
        sc_date = 'dt >= %r & dt <= %r' % (start_date, end_date) if start_date is not None else 'dt in %r' % trading_days
...
def path_assembler(mkttype, dtype, ftype, dfreq, dsource, alt, h5root):
    ...
    if alt is not None:
        ...
        names = alt.split('/')
        if len(names) > 1:
            abs_h5_path = '/data/group/wdb_h5/WIND/' + names[0].upper() + '/' + names[1] + '/' + names[1] + '.h5'
        else:
            abs_h5_path = '/data/group/wdb_h5/WIND/' + names[0] + '/' + names[0] + '.h5'
    else:
...
# modify 修改h5root='S:/Quant/data' 路径
def get_available_cols(mkttype=MktType.CHINA, dtype=DType.STOCK, ftype=FType.MD, dfreq=DFreq.DAILY, dsource=DSource.HTSC, alt=None, h5root='/data/group/wdb_h5/WIND/'):
    abs_h5_path = path_assembler(mkttype=mkttype, dtype=dtype, ftype=ftype, dfreq=dfreq, dsource=dsource, alt=alt, h5root=h5root)
    with pd.HDFStore(abs_h5_path, 'r') as h5_store:
        h5_root_keys = get_root_keys(h5_store)
        if len(h5_root_keys) == 1:
