...
from xquant.xqutils.utils import get_jurisdictionData,get_xquantConfig,statisticLog
...
DUBBO_APPLICATIONCONFIG = DUBBO_CONFIG.get("DUBBO_CONFIG")
DUBBO_CONFIG_IP = DUBBO_CONFIG.get("DUBBO_CONFIG_IP")

if os.environ.get('ENV_VERSION',False):
    factordata_ApplicationConfig = ApplicationConfig(DUBBO_APPLICATIONCONFIG)
    factordata_registry = ZookeeperRegistry(DUBBO_CONFIG_IP, factordata_ApplicationConfig)
else:
    factordata_ApplicationConfig = None
    factordata_registry = None

service_interface_create_library = None
user_provider_create_library = None
service_interface_add_factor = None
user_provider_add_factor = None
service_interface_remove_factor = None
user_provider_remove_factor = None

#调用服务
service_interface_query_wind = None
query_wind_from_oracle = None

service_interface_query_gogoal = None
query_gogoal_from_oracle = None


if os.environ.get('ENV_VERSION',False):
    try:
        service_interface_create_library = 'com.htsc.xquant.factor.manager.server.python.FactorMetaResourcesStoredService'
        user_provider_create_library = DubboClient(service_interface_create_library, factordata_registry, version="1.0.0")

        service_interface_add_factor = 'com.htsc.xquant.factor.manager.server.python.FactorMetaResourcesStoredService'
        user_provider_add_factor = DubboClient(service_interface_add_factor, factordata_registry, version="1.0.0")

        service_interface_remove_factor = 'com.htsc.xquant.factor.manager.server.python.FactorMetaResourcesStoredService'
        user_provider_remove_factor = DubboClient(service_interface_remove_factor, factordata_registry, version="1.0.0")
    except:
        pass
...
if os.environ.get('ENV_VERSION',False):
    try:
        ...
        query_wind_from_oracle = DubboClient(service_interface_query_wind, factordata_registry, version="1.0.0")

        service_interface_query_gogoal = 'com.htsc.xquant.dataagent.service.dubbo.GogoalOrGogoalNewInfoService'
        query_gogoal_from_oracle = DubboClient(service_interface_query_gogoal, factordata_registry, version="1.0.0")
    except:
        pass
...
if os.environ.get('ENV_VERSION',False):
    try:
        ...
        operation_registry = ZookeeperRegistry(
            "168.6.5.21:2181,168.6.5.22:2181,168.8.189.51:2181,168.8.189.50:2181,168.160.36.11:2181", operation_config)
        ZlGoalDailyTargetDubboService = DubboClient(operation_service_interface, operation_registry, version="1.0.0")
    except:
        pass
else:
    operation_config = None
    operation_service_interface = None
    operation_registry = None
    ZlGoalDailyTargetDubboService = None
...
class FactorData():
    ...
    def __init__(self):
        ...
        self.bd = DataProvider(mod)
        self.jurisdictionData_dict = get_jurisdictionData()
    ...
    def __check_permission(self, library_name, factor_names=None, readable=False):
        ...
        :return: bool
        '''
        permission_info = self.jurisdictionData_dict.get("因子权限信息")
        factor_data = self.jurisdictionData_dict.get("因子信息").get("jurisdictionData").get(library_name).get("factorInfo")
        catalogId = self.jurisdictionData_dict.get("因子信息").get("jurisdictionData").get(library_name).get("catalogId")
    ...
    def __insertLibraryInfo(self, library_name, library_type, ordertpye):
        ...
        try:
            ...
            get_result = user_provider_create_library.insertLibraryInfo(json_str)
    ...
    def create_factor_library(self, library_name, library_type):
        ...
        # 得到所有库名
        DB_names = list(self.jurisdictionData_dict["因子信息"]["jurisdictionData"].keys())
        ...
        try:
            ...
            json_str = json.dumps(json_dict)
            get_result_insertAfterStepsInfo = user_provider_create_library.insertAfterStepsInfo(json_str)
        ...
        # 更新权限文件
        self.jurisdictionData_dict["因子信息"]["jurisdictionData"][library_name] = {
            "factorInfo": {},
            "libraryId": library_id,
            "libraryType": catalog_type,
            "catalogId": catalog_id
        }
        self.jurisdictionData_dict["因子权限信息"]["目录可读写"].append(catalog_id)
        return True
    ...
    def __get_dubbo_add_factor(self, library_name, factor_name):
        ...
        try:
            ...
            json_str = json.dumps(json_dict)
            get_result = user_provider_add_factor.addFactor(json_str)
            get_result = json.loads(get_result)
            return get_result, 200
        except Exception as e:
            return str(e), 500
    ...
    def add_factor(self, library_name, factor_names):
        ...
        catalog_id_dict = self.jurisdictionData_dict["因子信息"]["jurisdictionData"][library_name]
        # 得到该库所有因子名
        ...
        for factor_name in factor_names:
            ...
            # 更新权限文件
            self.jurisdictionData_dict["因子信息"]["jurisdictionData"][library_name]["factorInfo"][factor_name] = {
                "column": owner_meta_col_name,
                "idInfo": [factor_id, catalog_id],
                "table": owner_meta_table_name,
                'statementTypeMap': {}
            }
            # self.jurisdictionData_dict["因子权限信息"]["因子可读写"].append(factor_id)
        return True
    ...
    def __remove_factor(self, libraryId, factorId):
        ...
        try:
            ...
            json_str = json.dumps(json_dict)
            get_result = user_provider_remove_factor.delFactor(json_str)
            get_result = json.loads(get_result)
            return get_result, 200
        except Exception as e:
            return str(e), 500
    ...
    def remove_factor(self, library_name, factor_names):
        ...
        for factor_name in factor_names:
            factor_id = \
            self.jurisdictionData_dict["因子信息"]["jurisdictionData"][library_name]['factorInfo'][factor_name]['idInfo'][0]
            ...
            # 更新权限文件
            self.jurisdictionData_dict["因子信息"]["jurisdictionData"][library_name]["factorInfo"].pop(factor_name)
            # self.jurisdictionData_dict["因子权限信息"]["因子可读写"].remove(factor_id)
        return True
    ...
    @statisticLog('factordata', 'FactorData')
    def update_factor_value(self, library_name, factor_values, stock=None, mddate=None):
        """
        更新指定因子库中的因子值
        :param library_name: 因子库名
        ...
        high_list = []
        for i in range(1, 10):
            low_list.append("low" + str(i))
            high_list.append("high" + str(i))
        d1 = {
            "time": ["20190328"],
        }
        d2 = {
            "stock": ["sz001", "sz001", "sz001", "sz002", "sz002", "sz002"],
            "mddate": ["20190403", "20190404", "20190405", "20190406", "20190407", "20190408"],
        }
        for i in high_list:
            d1[i] = [1]
        for i in low_list:
            d2[i] = [123, 345, 567, 456, 567, 890]
        df1 = pd.DataFrame(d1)
        df2 = pd.DataFrame(d2)
        df1.set_index("time", inplace=True)
        df2.set_index(["stock", "mddate"], inplace=True)
        s.update_factor_value("xx_low_22", df2)
        s.update_factor_value("xx_high_23",df1, "SH001", "20190325")
        #返回：
        True
        """
        # 查找所有库名，判断用户输入的库名是否存在
        ...
        if catalog_id_dict["libraryType"] == 1:
                ...
                if total_ <= 20000:
                    ...
                    for number in range(0, total_, 20000):
                        ...
                        try:
                            ...
                            self.dml_xquant_cusdata.rollback(conn_name)
                            raise Exception("low_frequency_update_factor error!")
            ...
            try:
                ...
                self.dml_xquant_cusdata.rollback(conn_name)
                self.dml_xquant_cusdata.close(conn_name)
                raise Exception("high_frequency_update_factor error!")
        else:
            raise Exception("librarytype error!")

    def __low_frequency_update_factor(self, table_name, columns, values, conn_name):
        '''
        非高频更新factor内部函数
        :param table_name: 表名
        :param columns: 列名
        ...
        logger.debug(low_frequency_sql)
    ...
    def __get_basic_factor(self, symbols, dates, factor_names, statement_type, stock_type, block_type, fill_na):
        ...
        :return:
        """
        basic_info_dict = self.jurisdictionData_dict["因子信息"]["jurisdictionData"]["Basic_factor"]["factorInfo"]
        basic_factor_dict = {
            2: [],
            3: [],
            4: [],
    ...
    def __get_operation_data(self, stock, mddate, factor_name, fill_na):
        ...
        try:
            ...
            json_str = json.dumps(json_dict)
            get_result = ZlGoalDailyTargetDubboService.getFactorData(json_str)
            get_result = json.loads(get_result)
    ...
    def __get_wind_source(self, library_name, **kwargs):
        ...
        if library_name[:4] == "WIND":
            judge_json = query_wind_from_oracle.queryTableAccount(json_data)
        else:
            judge_json = query_gogoal_from_oracle.queryTableAccount(json_data)
        ...
        if library_name[:4] == "WIND":
            result_json = query_wind_from_oracle.queryWindInfo(json_data)
        else:
            result_json = query_gogoal_from_oracle.queryGogoalOrGogoalNewInfo(json_data)
    ...
    def __get_stock(self, c_name):
        '查询全量股'
        sql_use = "select a.S_INFO_WINDCODE from asharedescription a where a.S_INFO_LISTBOARDNAME in ('主板', '创业板', '中小企业板')"
    ...
    def get_factor_value(self, library_name, stock=None, mddate=None, factor_names=None, statement_type=STYPE.COMBINED,
                         stock_type=1, block_type=4, fill_na=False, use_mysql=False, **kwargs):
        """
        查询指定因子库中的因子值
        ...
        from xquant.factordata import FactorData
        s = FactorData()
        low_df1 = s.get_factor_value("xx_low_22", ["sz001", "sz002"], low_list, ["20190404", "20190405"])
        low_df2 = s.get_factor_value("Basic_factor",['000630.SZ', '000023.SZ'],["pre_close",'close','high','low'],["20180331", "20190405"])
        high_df = s.get_factor_value("xx_high_23", "SH001", high_list, "20190325")
        print(low_df1)
        print(low_df2)
        print(high_df)
        #返回：
        ...
        mddate   stock
        20190404 sz001     345.00  345.00  345.00  345.00  345.00  345.00  345.00
                 sz002        NaN     NaN     NaN     NaN     NaN     NaN     NaN
        20190405 sz001     567.00  567.00  567.00  567.00  567.00  567.00  567.00
                 sz002        NaN     NaN     NaN     NaN     NaN     NaN     NaN

                             low8    low9
        mddate   stock
        20190404 sz001     345.00  345.00
                 sz002        NaN     NaN
        20190405 sz001     567.00  567.00
                 sz002        NaN     NaN
                           pre_close close high  low
        mddate   stock
        20180331 000023.SZ       NaN   NaN  NaN  NaN
                 000630.SZ       NaN   NaN  NaN  NaN
        20190405 000023.SZ       NaN   NaN  NaN  NaN
                 000630.SZ       NaN   NaN  NaN  NaN
                  high1  high2  high3  high4  high5  high6  high7  high8  high9
        time
        20190325   9895   9895   9895   9895   9895   9895   9895   9895   9895

        # 查万得源表：
        from xquant.factordata import FactorData
        s = FactorData()
        df1 = s.get_factor_value("WIND_AIndexEODPrices",factors=["S_INFO_WINDCODE","TRADE_DT","S_DQ_PRECLOSE","S_DQ_OPEN"],trade_dt=['>20180701','<20181129'])
        print(df1.head())
        # 返回：
               S_INFO_WINDCODE  TRADE_DT  S_DQ_PRECLOSE  S_DQ_OPEN
        0           h20754.CSI  20180702      1637.2878        NaN
        1           h20754.CSI  20180703      1595.4304        NaN
        2           h20754.CSI  20180704      1592.4745        NaN
        3           h20754.CSI  20180705      1575.4330        NaN
        4           h20754.CSI  20180706      1536.1493        NaN

        df2 = s.get_factor_value("WIND_AIndexEODPrices",factors=["S_INFO_WINDCODE","TRADE_DT","S_DQ_PRECLOSE","S_DQ_OPEN"],trade_dt=['20180702','20180703','20180704'],S_INFO_WINDCODE=['931151.CSI','931118.CSI','h01010.CSI'])
        print(df2)
        # 返回：
              S_INFO_WINDCODE  TRADE_DT  S_DQ_PRECLOSE S_DQ_OPEN
        0      931118.CSI  20180702      1017.0033       NaN
        1      931118.CSI  20180703      1017.1629       NaN
        2      931118.CSI  20180704      1009.2815       NaN
        3      931151.CSI  20180702      1528.1997       NaN
        4      931151.CSI  20180703      1489.4293       NaN
        5      931151.CSI  20180704      1503.6361       NaN
        6      h01010.CSI  20180702        91.6253       NaN
        7      h01010.CSI  20180703        91.6494       NaN
        8      h01010.CSI  20180704        91.6873       NaN

        df3 = s.get_factor_value("WIND_ASHAREPREVIOUSNAME",factors=['S_INfo_WINDCODE','begindate','enddate'],opdate=['>=20090525','<=20090530'],enddate='is not null')
        print(df3.head())
        # 返回：
                S_INFO_WINDCODE BEGINDATE   ENDDATE
        0         600673.SH  20061009  20080429
        1         600678.SH  20061009  20090629
        2         600679.SH  20061009  20070114
        3         600680.SH  20061009  20061119
        4         600683.SH  20061009  20090719
        ...
        if library_name[:4] == "WIND":
            ...
            if not stock:
                stock = self.__get_stock(conn_name)
            DB_names = list(self.jurisdictionData_dict["因子信息"]["jurisdictionData"].keys())
            if not library_name in DB_names:
                raise Exception("library_name doesn't exist: %s因子库不存在！" % library_name)
            catalog_id_dict = self.jurisdictionData_dict["因子信息"]["jurisdictionData"][library_name]
            ...
            if catalog_id_dict["libraryType"] == 1:
                ...
                if library_name == self.basic_factor:
                    basic_df = self.__get_basic_factor(stock, new_dates, factor_names, statement_type, stock_type,
                                                       block_type, fill_na)
                    # basic_df = basic_df[factor_names]
                    return basic_df
                # 针对运营数据操作
    ...
    def get_library_info(self):
        ...
        lib_info = self.jurisdictionData_dict.get("因子信息").get("jurisdictionData")
