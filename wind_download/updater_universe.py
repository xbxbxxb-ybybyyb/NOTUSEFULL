...
def create_universe_v2(sdate, edate, h5_path):
    ...
    if (date_check >= [1000, 1000, 1000, 1000, 1000, 1000]).sum() < 6:
        logger.info('Warning: universe raw data error')
        logger.info(date_check)
        raise AssertionError

        # check mkt_cap_ard
    mkt_cap_MI = IO.read_data([sdate_prev, edate], columns=['mkt_cap_ard'],
    ...
    # risk / alpha 
    data_MI['risk_universe'] = data_MI['listing_1yr'] * data_MI['STPT'] * data_MI['over_half_for_half_year'] * data_MI[
        'mkt_cap_5pct']
    ...
    data_config = {'index_name': ['dt', 'Ticker'],
                   ...
                   'type_bool': ['OPENDOWNLIMIT', 'OPENUPLIMIT', 'SSO', 'STPT', 'SUSPEND',
                                 'risk_universe', 'alpha_universe', 'alla', 'index_300', 'index_500', 'index_50',
                                 'index_1000',
...
def get_ST(date):
    h5_path2 = os.path.join(settings.BASE_DIR, 'DATABASE/WIND/')
    table_name = 'AShareST'
...
def stock_filter_v2(cdate_list, filter_type):
    store_path = os.path.join(settings.BASE_DIR, "LOCAL_DATA/CSV/stock_universe/")
...
class UnvFactor(object):
    def __init__(self, start_date, end_date):
        ...
        self.h5_path_destination = IO.path_assembler(mkttype=MktType.CHINA, dtype=DType.STOCK, ftype=FType.UNIV,
                                                     dfreq=DFreq.DAILY, dsource=DSource.OPTM, alt=None,
                                                     h5root=self.h5_root)
    ...
    def csv_to_database(self):
        ...
        h5_path_source = os.path.join(settings.BASE_DIR, "LOCAL_DATA/CSV/stock_universe/universe_complete.h5")

        utils.repack_file(h5_path_source)

        factor_list = ['alla', 'index_50', 'index_300', 'index_500', 'index_1000', 'Listing_date', 'OPENDOWNLIMIT',
                       'OPENUPLIMIT', 'SSO', 'STPT', 'SUSPEND']
        logger.info("开始更新：file={}, path={}".format(os.path.basename(h5_path_source), h5_path_source))
        update_universe_raw(self.cdate_list, csv_path, h5_path_source, factor_list, operation='append')
        logger.info("完成更新：file={}, path={}".format(os.path.basename(h5_path_source), h5_path_source))
        """Create Universe"""
        ...
        IO.pd_hdf5_writer(data_universe, self.h5_path_destination, dataset_name, append=True)

    def cronb(self):
        utils.repack_file(self.h5_path_destination)
        logger.info(
            "开始更新：name={}, path={}".format(os.path.basename(self.h5_path_destination), self.h5_path_destination))
        self.retriever()  # 生成csv
        self.csv_to_database()  # csv to h5
        logger.info(
            "完成更新：name={}, path={}".format(os.path.basename(self.h5_path_destination), self.h5_path_destination))


def updater_universe(sdate, edate):
    sdate, edate, cdate_list = check_update_date(sdate, edate)
    UnvFactor(sdate, edate).cronb()
