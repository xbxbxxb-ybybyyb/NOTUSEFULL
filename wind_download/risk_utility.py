...
h5_path_listing_delisting = os.path.join(settings.BASE_DIR, 'ETC/CHINA_STOCK/WIND/ETC_CHINA_STOCK_WIND.h5')

path_dict = {'wind': os.path.join(settings.BASE_DIR, 'DATABASE/WIND'),
             'derived': os.path.join(settings.BASE_DIR, 'DATABASE'),
             'suntime': os.path.join(settings.BASE_DIR, 'DATABASE/SUNTIME')
             }
flag_path_data = os.path.join(settings.BASE_DIR, 'LOCAL_DATA/FLAG')
...
def regression_ols(y, x, weight=None):
    ...
    for date_idx in range(date_num):
        ...
        try:
            if weight is None:
                res_mat[date_idx, :], r2_mat[date_idx], beta_mat[date_idx, :], tstats_mat[date_idx,
                                                                               :] = stats_model_ols(y_mat[date_idx, :],
                                                                                                    _x)
            else:
                res_mat[date_idx, :], r2_mat[date_idx], beta_mat[date_idx, :], tstats_mat[date_idx,
                                                                               :] = stats_model_ols(y_mat[date_idx, :],
...
def stats_model_ols(y, x, weight=None, min_percentage=10):
    ...
    if weight is None:
        reg = resider(x[mask], y[mask], method='sm.OLS', add_const=True, mean_only=False, r_square=False,
                      return_sm=True)
    else:
        reg = resider(x[mask], y[mask], weight[mask], method='sm.WLS', add_const=True, mean_only=False, r_square=False,
                      return_sm=True)
    res[mask] = reg.resid
...
def EWMA(df, roll_num, half_life, ufunc='mean', min_pct=0.5,
         min_weight=0.5):  # 计算A 前n 期样本加权平均值权重为0.9i，(i 表示样本距离当前时点的间隔)
...
def get_backfill_prep(sdate, edate, announce_date=None):
    ...
    if announce_date is not None:
        ...
        issuing_date_ps = \
            IO.read_data([qtr_list[0], qtr_list[-1]], ftype=FType.FDD, dsource=DSource.WIND, dfreq=DFreq.QUARTERLY,
                         columns=['stm_issuingdate'])['stm_issuingdate']
        issuing_date_ps = issuing_date_checker(issuing_date_ps)
...
def backfill_master(fac_qtr, sdate, edate, prep_data=None):
    ...
    for fac in fac_list:
        fac_daily_mi = backfill(sdate, edate, fac_qtr_mi[[fac]], issuing_date_ps, trading_date_list, True,
                                listing_delisting_filter)
...
def multiprocess_wrapper(func, iter_list=None, logger=None,
    ...
    with Pool(max_process) as executor:
        ...
        if iter_list is not None:
            for itr in iter_list:
                ...
                try:
                    ...
                    logger.error("任务初始化失败: msg={}".format(e))
        ...
        for f in as_completed(future_tasks):
            ...
            try:
                ...
                logger.error("任务异常退出：msg={}".format(e))
                f.cancel()
            if done_ind:
                try:
                    ts = f.result()
                    ...
                    logger.error("获取任务结果失败：msg={}".format(e))
                    done_ind = None
...
def calc_dividend_yield(sdate, edate, dividend_data, alpha_universe, mkt_cap_ard):
    ...
    dividend_payout_ex = calc_df_ts_roll(calc_by_count, div_widow, div_dat_ex, dividend_count_guess_ex, min_pct,
                                         handle_nan=False)

    div_dat_pa = easy_fill(dividend_data[['dividend_amount', 'S_DIV_PRELANDATE']], sdate_prev, edate,
                           foward_date='S_DIV_PRELANDATE', fill_method=None)
    ...
    dividend_payout_pa = calc_df_ts_roll(calc_by_count, div_widow, div_dat_pa, dividend_count_guess_pa, min_pct,
                                         handle_nan=False)
...
def calc_df_ts_roll(func, roll_num, y_df, x_df=None, min_pct=0.5, handle_nan=True):
