...
def fdd_qtr_retriever_withcal(date, dataset_name):
    ...
    if dataset_name == 'ebitdatosales':
        ...
        def _help(pdf):
            a = pdf['TOT_SHRHLDR_EQY_INCL_MIN_INT'] + pdf['LT_BORROW'] + pdf['BONDS_PAYABLE'] + pdf['LT_PAYABLE'] + pdf[
                'SPECIFIC_ITEM_PAYABLE']
            b = pdf['FIX_ASSETS'] + pdf['FIN_ASSETS_AVAIL_FOR_SALE'] + pdf['HELD_TO_MTY_INVEST'] + pdf[
                'LONG_TERM_EQY_INVEST']
            if b == 0:
                return np.nan
            return 100 * a / b

        df[dataset_name] = df.apply(lambda x: _help(x), axis=1)
        ...
        def _help(pdf):
            a = pdf['TOT_CUR_LIAB']
            b = pdf['TOT_SHRHLDR_EQY_EXCL_MIN_INT']
            return 100 * a / b
        ...
        def _help(pdf):
            a = pdf['LT_BORROW'] + pdf['BONDS_PAYABLE'] + pdf['LT_PAYABLE'] + pdf['SPECIFIC_ITEM_PAYABLE']
            b = pdf['TOT_LIAB']
            if b == 0:
                return np.nan
            return 100 * a / b
        ...
        def _help(pdf):
            a = pdf['S_FA_EBIT_TTM_INVERSE']
            b = pdf['S_FA_ASSET_MRQ']
            return 100 * a / b
        ...
        if date % 10000 == 331:
            ...
            def _help(pdf):
                a = pdf['S_FA_GROSSMARGIN']
                b = pdf['S_FA_GROSSMARGIN2']
                return a - b
        ...
        def _help(pdf):
            a = pdf['NET_INCR_CASH_CASH_EQU']
            b = pdf['NET_INCR_CASH_CASH_EQU2']
            return 100 * (b - a) / abs(a)
        ...
        def _help(pdf):
            a = pdf['TOT_OPER_REV']
            b = (pdf['TOT_CUR_ASSETS'] - pdf['TOT_CUR_LIAB'] + pdf['TOT_CUR_ASSETS2'] - pdf['TOT_CUR_LIAB2'])
            return 2 * a / b
        ...
        def _help(pdf):
            a = pdf['LESS_OPER_COST']
            b = (pdf['ACCOUNTS_PAYABLE'] + pdf['ACCOUNTS_PAYABLE2'])
            return 2 * a / b
        ...
        def _help(pdf):
            if np.isnan(pdf['TOT_ASSETS12']):
                if np.isnan(pdf['TOT_ASSETS']):
                    a = pdf['TOT_ASSETS3']
                else:
                    a = pdf['TOT_ASSETS']
            else:
                a = pdf['TOT_ASSETS12']
            if np.isnan(pdf['TOT_ASSETS2']):
                b = pdf['TOT_ASSETS4']
            else:
                b = pdf['TOT_ASSETS2']
            return 100 * (b - a) / abs(a)
        ...
        factor = ['MONETARY_CAP', 'ST_BORROW', 'ACCOUNTS_PAYABLE', 'TRADABLE_FIN_LIAB', 'NON_CUR_LIAB_DUE_WITHIN_1Y',
                  'STATEMENT_TYPE']
        table = 'AShareBalanceSheet'
        table_path = h5_path + table + '/' + table + '.h5'
        df = IO.read_data(date, columns=factor, alt=table_path)
        ...
        def _help(pdf):
            a = pdf['MONETARY_CAP']
            b = (pdf['ST_BORROW'] + pdf['ACCOUNTS_PAYABLE'] + pdf['TRADABLE_FIN_LIAB'] + pdf[
                'NON_CUR_LIAB_DUE_WITHIN_1Y'])
            return a / b

        df[dataset_name] = df.apply(lambda x: _help(x), axis=1)
        df.drop(['MONETARY_CAP', 'ST_BORROW', 'ACCOUNTS_PAYABLE', 'TRADABLE_FIN_LIAB', 'NON_CUR_LIAB_DUE_WITHIN_1Y'],
                axis=1, inplace=True)
        ...
        def _help(pdf):
            a = pdf['NET_PROFIT_INCL_MIN_INT_INC']
            b = pdf['NET_PROFIT_INCL_MIN_INT_INC2']
            return 100 * (b - a) / abs(a)
        ...
        def _help(pdf):
            a = pdf['S_FA_CFPS'] * pdf['TOT_SHR']
            b = pdf['S_FA_CFPS2'] * pdf['TOT_SHR2']
            return 100 * (b - a) / abs(a)
