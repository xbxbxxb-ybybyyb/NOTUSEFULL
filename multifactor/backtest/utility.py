...
def segment_test(factor_pd, stock_close_pd, holding_period, benchmark_price_ps, segment_num,
    ...
    bucket_ics[:],easy_seg_return_mat[:] = np.nan,np.nan
...
def segment_test_by_industry(factor_pd, stock_close_pd, holding_period, benchmark_price_ps, segment_num,
    ...
    for idx in pd.Series(np.unique(stock_industry)).dropna():
        ...
        _industry_hpr_daily['Industry'] = idx
        industry_hpr_daily.append(_industry_hpr_daily)
        
    industry_hpr_daily = pd.concat(industry_hpr_daily, axis=0).reset_index().set_index(['dt','Industry'])  
    ...
    if return_industry:
        seg_return_combined[max_quantile + '-' + 'Index'] = seg_return_combined[max_quantile] - industry_hpr_daily['total_return']
        if transaction_cost is None:
            return res,seg_return_combined
        else:
            seg_return_combined_after_cost[max_quantile + '-' + 'Index'] = seg_return_combined_after_cost[max_quantile] - industry_hpr_daily['total_return']
...
def calc_annualized_return(return_ps, interest_type='SIMPLE'):
    ...
    if interest_type == 'SIMPLE':
        ...
        res = calc_cum_return_ts(return_ps, interest_type=interest_type).iloc[-1] ** (year_date_num / len(return_ps)) - 1
    return res
...
def segment_performance_measure(seg_return,interest_type='SIMPLE'):
    ...
    _segment_performance_measure = pd.concat([ret_annual, vol_annual, sharpe_ratio,mdd,hit_rate,
                                              ret_excess_annual,tracking_error,info_ratio, mdd_er,hit_rate_er], axis=1)
        ...
        if w is not None:
            ...
            if handle_nan:
                ...
                if np.count_nonzero(mask) >= min_num:
                    ...
                    res_mat[i] = func(xr,yr,wr,**kwargs) # put into row
            else:
                res_mat[i] = func(xr,yr,wr,**kwargs) # put into row
        else:
            if handle_nan:
...
def calc_ic_all(factor_pd,stock_close,holding_period,weight=None,fac_name=None,
    ...
    for ic_type in ic_type_list:
        _ic_ts = calc_ic(factor_pd,stock_close,holding_period=holding_period,ic_type=ic_type,weight=weight,q=q,sign_correct=sign_correct)
        ic_ts_list.append(_ic_ts)
    ic_ts = pd.concat(ic_ts_list,axis=1)
    if fac_name is not None:
        ic_ts = ic_ts.reset_index()
...
def find_er_ls_col(seg_ret):
    ...
    top_q,bottom_q = ret.argmax(),ret.argmin()
    #ls_col = '%s-%s'%(top_q,bottom_q)
