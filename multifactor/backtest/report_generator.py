...
import numpy as np
import warnings
from multifactor.backtest.utility import pprint, max_drawdown_ts, calc_cum_return_ts, \
                                         calc_year_date_num, calc_annualized_return, calc_percentile, \
                                         calc_hhi,calc_filter_correlation,find_er_ls_col,segment_performance_measure
...
def segment_helper(ax, easy_seg_return, title=False, title_text='', interest_type='SIMPLE'):
    ...
    easy_seg_return.is_copy = False
    max_q = easy_seg_return.iloc[:, [0, -3]].mean().idxmax()
    easy_seg_return.loc[:, max_q + '-index'] = easy_seg_return[max_q] - easy_seg_return['Index']
    easy_seg_return_cum = calc_cum_return_ts(easy_seg_return, interest_type)
    easy_seg_return_cum.loc[:, 'MDD_L-index'] = max_drawdown_ts(easy_seg_return_cum[max_q + '-index'], interest_type=interest_type)
    mdd_l_index = easy_seg_return_cum['MDD_L-index'].min()
    _ = easy_seg_return_cum[max_q + '-index']
    _.name = 'Excess Return - ' + title_text
    plt1 = _.plot(ax=ax, figsize=(fig_width, fig_height), fontsize=font_size_axis, legend=False)
    _ = easy_seg_return_cum['MDD_L-index']
    _.name = 'MDD - ' + title_text
    plt2 = _.plot(ax=ax, figsize=(fig_width, fig_height), secondary_y=True,
                  fontsize=font_size_axis, style='--', legend=False)
    plt.legend(*legend_helper(), loc=legend_loc, fontsize=font_size_legend)
    plt.ylim([0, mdd_l_index - 0.1])
...
def generate_stacked_bar_plot(df,x_label, y_label,freq='m',title=None,rot=None,
    ...
    df_mean.plot(kind='bar',stacked=True,figsize=(fig_width,fig_height),
                 legend=legend_on, fontsize=font_size_axis, rot=rot,title=title)
    if legend_on:
        if legend_outside:
            plt.legend(loc='center left', bbox_to_anchor=(1, 0.5), fontsize=font_size_legend)
        else:
...
def generate_pdf(pickle_name,rolling_window=20):
    #pickle_name = r'S:\Quant\backtest\backtest_output\pfcc_htuniv\FactorBacktest_pfcc_htuniv.pkl'
    ...
    #seg_ret_30_stat = output_dict['seg_return_30_stat']
    seg_return_30_stat_year = output_dict['seg_return_30_stat_year']
    ...
    bucket_ic_30_roll = bucket_ic_30.rolling(rolling_window).mean()
    ...
    if seg_by_industry_flag:
        t_excess_return_industry_sliced = generate_table(excess_return_industry_sliced[tr_list], col_type_tr_list, axis=1, reformat_type=True)
    ...
    ic_ts_combined_roll = ic_ts_combined_show.rolling(rolling_window).mean()
    ...
    col_name_roll = 'Avg %d Period' %(rolling_window)
    ...
    g_market_rank = generate_bar_line_plot(df=market_rank_df, title='Top Segment Total Return Market Rank', bar_column='market_rank',
                                           line_column=col_name_roll,show_stats=True,show_format='pct2')
    
    g_ic_ts_combined_roll = generate_plot_with_secondary(ic_ts_combined_roll, bucket_ic_30_roll,
                         'IC (Rolling %d day Average) \nCurrent %s Percentile %.2f' %(rolling_window,show_percentile,ic_orig_percentile),
    ...
    g_return_by_year = generate_plot(seg_return_by_year, plot_name=seg_er_col[0] + ' Return by Year', x_label='Day in Year',
    ...
    g_seg_sharpe_year = generate_plot(seg_sharpe_year, plot_name='Sharpe', x_label='', y_label='Sharpe',
                                      plot_type='bar', legend_outside=True, rot=45)
    #g_seg_ret_30_annret = generate_plot(seg_ret_30_annret, plot_name='Segment Return - All History', x_label='',
    #                                    y_label='Return (Ann.)', plot_type='bar', bottom=0.2)
    ...
    g_seg_return_30_stat_year = generate_segment_ret_year_plot(seg_return_30_stat_year, col_name='Return(Ann.)')
    ...
    show_dict_stat = {i:segment_performance_measure(show_dict[i]) for i in show_dict} 
    ...
    er_use = seg_ret_use[er_col_use]
    g_er_by_month = generate_ts_heatmap_by_month_plot(er_use*100, title_text='Excess Return(%) by Month',
                                                      vmin=-10,vmax=10,fmt='.2f',agg_method='sum')
    seg_ret_use['Avg %d Period' % rolling_window] = seg_ret_use[seg_er_col].rolling(rolling_window).mean()
    g_seg_ret_bar = generate_bar_line_plot(df=seg_ret_use, title='Excess Return', bar_column=seg_er_col[0],
                                           line_column='Avg %d Period' %(rolling_window),show_stats=True,show_format='pct2')

    #generate_stacked_bar_plot(df,x_label, y_label,freq='m',title=None,rot=None)
                          
    if compare_style_flag:
        seg_ret_use_fac = seg_return_after_cost_fac if t_cost_flag else seg_return_fac
        er_col_use_fac,l_col_use_fac,s_col_use_fac  = find_er_ls_col(seg_ret_use_fac)
    
        #ls_ret_use = seg_ret_use[l_col_use] - seg_ret_use[s_col_use]
        ls_ret_use_fac = seg_ret_use_fac[l_col_use_fac] - seg_ret_use_fac[s_col_use_fac]
        ...
        er_ls_corr = seg_ret_use[er_col_use].rolling(corr_win).corr(ls_ret_use_fac)
        all_history_corr = seg_ret_use[er_col_use].corr(ls_ret_use_fac)
        ...
        er_ls_corr_remove_middle = calc_filter_correlation(corr_use,corr_win,filter_x_middle=corr_remove_mid_pct,min_pct=0.8)
        ...
        ls_corr_name = 'Correlation with %s (Rolling %d Days)'%(compare_style,corr_win)
        add_avg_str = '\n All History Correlation: %s: %s   %s: %s'%(corr_df.columns[0],num2str(all_history_corr,'pct2'),
                                                                         corr_df.columns[1],num2str(all_history_remove_mid_corr,'pct2'))
        plt_name = 'Correlation with %s (Rolling %d Days)\n(Factor Excess Return vs Style Long Short Return)%s'%(compare_style,corr_win,add_avg_str)
        g_ls_corr_show = generate_plot(corr_df, plot_name=plt_name, x_label='',y_label='Correlation',
                                       show_stats=True,show_format='pct2')
    ...
    if not easy_test:
        g_Tstat = generate_bar_line_plot(df=Tstat_ts, title='T-Statistics', bar_column='T-stat',
                                         line_column='Avg %d Period' % rolling_window,show_stats=True,show_format='dcm2')
        ...
        g_ic = generate_bar_line_plot(df=ic_neu, title='IC', bar_column='IC Neutralized', line_column='Avg %d Period' % rolling_window,
    ...
    if seg_by_industry_flag:
        ...
        g_seg_ind_by_month = generate_ts_by_type_plot(seg_by_industry, by='Month', x_label='Excess Return(%)',ann_scale=True,vmin=-10,vmax=10)
        g_seg_ind_by_year = generate_ts_by_type_plot(seg_by_industry, by='Year', x_label='Excess Return(%)',ann_scale=True,vmin=-50,vmax=50)
        g_industry_concentration = generate_plot(industry_concentration, plot_name = 'Excess Return - Industry Concentration', 
                                                 x_label='', y_label='Concentration', plot_type='line',show_stats=True)#, rot=45)        
    plt.ioff()

    pprint('Generating headers')
    add_t_cost = ' After Cost' if t_cost_flag else ' Before Cost'
    add_seg_by_ind = ' with Industry Neutral' if seg_by_industry_flag else ''

    h_sum = Paragraph('Factor Information', heading_size)
    h_factor_stat = Paragraph('Factor Distribution', heading_size)
    h_seg_ret = Paragraph('Segment Return Performance', heading_size)
    h_seg_ret_table = Paragraph('Portfolio Spread Statistics - %s%s'%('Before Cost',add_seg_by_ind), heading_size)
    h_excess_ret_stat_use = Paragraph('Excess Return Statistics - %s%s'%(add_t_cost,add_seg_by_ind), heading_size)
    h_seg_ret_table_after_cost = Paragraph('Portfolio Spread Statistics - After Cost%s'%(add_seg_by_ind), heading_size)

    h_return_by_year = Paragraph('Excess Return', heading_size)
    h_seg_year = Paragraph('Annual Return by Year', heading_size)
    h_seg_cumret = Paragraph('Portfolio Spread - No Cost', heading_size)
    h_ic_decay = Paragraph('Alpha Decay', heading_size)
    h_factor_auto_correlation_rolling = Paragraph('Factor Autocorrelation', heading_size)
    h_seg_ret_30_annret = Paragraph('30 Segment', heading_size)
    h_seg_ret_30_year = Paragraph('Portfolio Spread by Year %s No Industry Neutral (30 Segment)'%(add_t_cost), heading_size)
    h_ic_stats = Paragraph('IC Statistics', heading_size)
    h_market_rank = Paragraph('Top Bucket Total Return Market Rank', heading_size)
    h_ic_by_month = Paragraph(ic_type+' by Month', heading_size)
    h_er_by_month = Paragraph('Excess Return by Month', heading_size)

    h_factor_ts = Paragraph('Factor Coverage', heading_size)
    h_seg_return_new = Paragraph('Excess Return Performance - %d Segment' % seg_num, heading_size)
    current_time = datetime.datetime.today()
    pdf_report_date = 'Test Date:  ' + current_time.strftime('%Y-%m-%d %H:%M:%S')
    h_version = Paragraph('Version: %s'%(version_number), pdf_styles['Normal'])
    h_date = Paragraph(pdf_report_date, pdf_styles['Normal'])
    h_ic_style_corr = Paragraph('Style Correlation - IC Time Series', heading_size)
    if compare_style_flag:
        h_ls_corr_show = Paragraph(ls_corr_name,heading_size)
    ...
    if easy_test == False:
        ...
        h_ic_by_industry = Paragraph('IC by Industry', heading_size)
        h_style_corr = Paragraph('Style Correlation Time Series - Rolling %d Days' % rolling_window, heading_size)
        h_seg_by_industry = Paragraph('Excess Return by Industry'+add_t_cost, heading_size)

    h_excess_return_industry_sliced = Paragraph('Segment Excess Return by Industry Performance'+add_t_cost, heading_size)
