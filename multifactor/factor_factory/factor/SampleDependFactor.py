...
class SampleDependFactor(BaseFactor):
    #  定义因子参数

    # 因子频率，默认为日频因子， 可不设置
    factor_type = "DAY"
    # 依赖的平台原始数据，包括FactorData和MarketData接口中的数据。 默认为空，必须设置
    depend_data = ["FactorData.Basic_factor.volume", "FactorData.Basic_factor.close"]
    ...
    def calc_single(self, database):
        ...
        samplefactor = database.depend_factors["SampleFactor"]
        ans = (close - close.shift(self.lag-1)) / (close.shift(self.lag-1)) * volume
        ans = ans + samplefactor
