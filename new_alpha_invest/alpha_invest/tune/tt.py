...
def parallel_train():
    ...
    trainer = ParallelTrainer(backend=XGboostPack, task_mode='LocalTrain',
...
def dist_train():
    ...
    # step2: 实例化Trainer，设置Trainer入参，已继承并实现的Backend类，任务类型，以及ray_params参数类型
    trainer = ParallelTrainer(backend=DistXGboostPack, task_mode='DistTrain',
                              ray_params=RayParams(cpus_per_worker=1, gpus_per_worker=0.5))
...
def parallel_tune():
    ...
    config = {
        ...
        "num_features": 100,#tune.grid_search([100, 50]),
        "tag_name": "Tag1minRet",#tune.grid_search(["Tag1minRet", "Tag2minRet"]),
        # tune.choice(['tag2minShort', 'tag2minShort', 'tag1minShort', 'tag5minLong', 'tag2minLong', 'tag1minLong'])
        'n_estimators': 350,
        'max_depth': 10,#tune.randint(3, 10),
        'learning_rate': 0.001,#tune.uniform(0.001, 0.02),
        'subsample': 0.50,
        'colsample_bytree': 0.50,
        'gamma': 0,
        'tree_method': 'hist',
    ...
    tune.run(objective, config=config,
             num_samples=1, resources_per_trial={'gpu': 0},
             name='xgboost_T0_600519', local_dir='/data/user/quanttest007/alpha_invest/ray_results')
...
if __name__ == "__main__":
    if False:
        ...
        # time.sleep(60)
        # 启动分布式ray集群
        redis_address = start_ray_cluster(cpus_per_node=2, gpus_per_node=0, mem_per_node=24, maxWorkers=10,
                                          minWorkers=10,
