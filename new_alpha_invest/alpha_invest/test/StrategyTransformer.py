...
class StrategyBase:
    ...
    def prepare_data(self, data_params):
        ...
        end_date = data_params["data_params"]["date_range"][1]
        #选择部分因子，减少计算量
        ...
        if not data_params["data_cache"]:
            ...
            pickle.dump(self.original_datax, open("./original_datax.pkl", "wb"))
            pickle.dump(self.original_datay, open("./original_datay.pkl", "wb"))
            print("finish prepare data!")
        else:
            alpha_logger.warning("use cache data...")
            self.original_datax = pd.read_pickle("./original_datax.pkl")
            self.original_datay = pd.read_pickle("./original_datay.pkl")
    ...
    def train_loop(self, model_params):
        ...
        if model_params["train_mode"]:
            ...
            self.train_dataset, self.test_dataset = self.model_mgr.split_torch_dataset(
                                                        self.tds_ds_train, model_params["train_split_ratio"])
