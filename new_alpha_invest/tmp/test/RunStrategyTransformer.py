...
from xquant.model.tracking import parse_params, log_params, log_metrics
from xquant.model.parallel_train.backend import LocalTrainBackend
from xquant.model.parallel_train.trainer import ParallelTrainer, SplitTaskParam, RayParams
...
class StrategyTransformer(LocalTrainBackend):
    def __init__(self, **kwargs):
        self.project_config = parse_params("model_config_transformer.yaml")
    ...
    def run(self):
        ...
        """
        # STEP1: 设置运行参数
        self.dataset_config["date_range"] = (str(self.date_range['start_date']), str(self.date_range['end_date']))
        self.dataset_config["data_cache"] = self.task_config["data_cache"]

        self.model_config["model_params"]["train_mode"] = self.task_config["train_mode"]
        self.model_config["model_params"]["d_feat"] = 10
        self.model_config["model_params"]["n_epochs"] = 1
        self.dataset_config["data_params"]['d_feat'] = self.model_config["model_params"]["d_feat"]

        # 上传工程配置参数
        log_yaml_params("model_config_transformer.yaml")

        # STEP2: 加载原始因子数据,数据预处理&特征工程
        self.prepare_data(self.dataset_config)

        # STEP3: 初始化模型，并整理、分割数据集; 模型训练&预测
        self.train(self.model_config["model_params"])
...
def main():
    instance = StrategyTransformer()
    # STEP1: 设置运行参数
    instance.model_config["model_params"]["train_mode"] = instance.task_config["train_mode"]
    # instance.model_config["model_params"]["d_feat"] = 10
    # instance.model_config["model_params"]["n_epochs"] = 1

    instance.dataset_config["data_cache"] = False#instance.task_config["data_cache"]
    instance.dataset_config["data_params"]['d_feat'] = instance.model_config["model_params"]["d_feat"]
    instance.dataset_config["date_range"] = SplitTaskParam([("20180630", "20190630"), ("20190630", "20200630")])

    # 上传工程配置参数
    log_params(instance.dataset_config)
    log_params(instance.model_config["model_params"])

    # step2: 实例化Trainer，设置Trainer入参，已继承并实现的Backend类，任务类型（DistTrain or LocalTrain），以及ray_params参数类型
    trainer = ParallelTrainer(backend=StrategyTransformer,
    ...
    callback_iterator = trainer.parallel_run(model_params = instance.model_config["model_params"], data_params = instance.dataset_config)
