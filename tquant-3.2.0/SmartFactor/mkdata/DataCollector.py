...
def collect_market_data(security_code, start_date, end_date, collect_mode=[DataCollectMode.TICK_RAW],
    ...
    for mode in collect_mode:
        ...
        if mode == DataCollectMode.TICK_RAW:
            ...
            df_dict["tick"] = get_market_data(security_code, start_datetime, end_datetime, bar_size=bar_size, security_type=security_type)
            ...
            df_tick = get_market_data(security_code, start_datetime, end_datetime, bar_size=bar_size, security_type=security_type)
            ...
            if df_dict.get("transaction"):
                raise Exception("DataCollectMode中存在重复的行情数据类型{}，请重新传入！".format(mode))
            df_dict["transaction"] = get_market_data(security_code, start_datetime, end_datetime, bar_size="TRANSACTION", security_type=security_type)
            ...
            if df_dict.get("order"):
                raise Exception("DataCollectMode中存在重复的行情数据类型{}，请重新传入！".format(mode))
            df_dict["order"] = get_market_data(security_code, start_datetime, end_datetime, bar_size="ORDER", security_type=security_type)
...
def get_market_data(security_code, start_datetime, end_datetime, bar_size, security_type=None):
    """
    获取 tick级行情数据
