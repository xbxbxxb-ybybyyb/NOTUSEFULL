...
class Factor(object):
    ...
    def get_factor_data(self, quarterlag_dt_list, start_date, end_date, securities_list):
        ...
        # res_dict: {'BasicDayFactor.open':multiindexDF,'BasicDayFactor.close':multiindexDF}
        ...
        for i in trange(loop_time, desc='loading depend factor data'):
            ...
            if depend_factor_type in ["financialanalysis", "financialreport"]:
                temp_data = sd.get_tfactor_value(trading_codes=securities_list,
                                                 date_list=quarterlag_dt_list,
                                                 factor_list=depend_factor_list,
                                                 factor_lib=depend_factor_type,
                                                 fill_na=True)
            ...
            elif depend_factor_type in ["market", "valuation", "riskanalysis", "newmsgindex", "alpha",
                                        "barra", "technicalanalysis", "dividend", "emotion", "momentum", "barrarisk6"]:
                temp_data = sd.get_tfactor_value(trading_codes=securities_list,
                                                 date_list=date_list,
                                                 factor_list=depend_factor_list,
                                                 factor_lib=depend_factor_type,
            ...
            elif depend_factor_type in ["consensus"]:
                raise Exception("一致预期类型的因子暂不支持作为依赖因子传入！")
            elif depend_factor_type == 'SharedFactor':
                shared_factor_data = sd.get_factor_characteristic(factor_list=depend_factor_list, date_list=date_list,
                                                                  trading_codes=securities_list)
                for depend_factor_name in depend_factor_list:
                    new_depend_factor_describe = 'SharedFactor.' + depend_factor_name
                    res_dict[new_depend_factor_describe] = shared_factor_data[depend_factor_name]
            else:
                temp_data = tps.get_factor_value(library_name=depend_factor_type,
                                                 mddate_list=date_list,
                                                 security_list=securities_list,
                                                 factor_list=depend_factor_list,
                                                 sort=False)
                for depend_factor_name in depend_factor_list:
                    new_depend_factor_describe = 'ReleaseFactor.' + depend_factor_name
                    if int(self.calc.__code__.co_argcount) == 3:
                        res_dict[new_depend_factor_describe] = temp_data[depend_factor_name]
                    else:
                        res_dict[new_depend_factor_describe] = temp_data[depend_factor_name].unstack()
    ...
    # 用户调用低频因子的调试方法，
    @event_trace
    def run_day_factor_value(self, start_date, end_date, dynamic_load_attr=True):
