...
def setup_logging(logger_name='', dirPath="/tmp"):
    ...
    thread_id = datetime.datetime.now().strftime("%Y%m%d_%H%M%S%f")
    dirPath = os.path.join(dirPath, thread_id.split("_")[0]+"/"+thread_id.split("_")[1]+"_"+logger_name)
    ...
    if logger_name:
        if logger_name not in ["quant_handle", "quant_info", "quant_debug", "pika_starter", "ray_starter"]:
...
if __name__ == "__main__":
    ...
    logger_info = setup_logging("quant_info1")
