...
from SmartFactor.calculation.DataCalculation import get_share_data
class LowFactorShare(Factor):
    print(get_share_data())
    """
    (-1 * DELTA((((CLOSE - LOW) -  (HIGH - CLOSE)) / (HIGH - LOW)), 1))
    """
    factor_type = "DAY"         # 因子类型 ： 日频(DAY)
    factor_name = 'LowFactorShare'  # 设置因子名称，因子名称只包含大小写字母，数字或下划线。
