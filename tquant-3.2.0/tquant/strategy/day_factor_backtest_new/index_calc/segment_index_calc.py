...
from tquant.strategy.day_factor_backtest_new.index_calc.return_index_calc import max_drawdown, cal_hpr_daily, \
    calc_cum_return_ts, calc_annualized_return
from tquant.strategy.day_factor_backtest_new.index_calc.statistical_index_calc import winsorized_mean
...
def segment_test_daily(factor_score, daily_return, segment_num,
    ...
    if handle_return_outlier:
        for i in idx_seperators:
            ll = factor_ret_sorted[i:i + num_per_quantile, 1]
            if len(ll) > 0 and np.isnan(ll).sum() != len(ll):
                seg_ret_reversed.append(winsorized_mean(ll))
            else:
                seg_ret_reversed.append(np.nan)
        # seg_ret_reversed = [winsorized_mean(factor_ret_sorted[i:i + num_per_quantile, 1]) for i in idx_seperators]
    else:
        for i in idx_seperators:
            ll = factor_ret_sorted[i:i + num_per_quantile, 1]
            if len(ll) > 0 and np.isnan(ll).sum() != len(ll):
                seg_ret_reversed.append(np.nanmean(ll))
            else:
                seg_ret_reversed.append(np.nan)
        # seg_ret_reversed = [np.nanmean(factor_ret_sorted[i:i + num_per_quantile, 1]) for i in idx_seperators]
    # calculate bucket IC
    if return_bucket_ic:
        bucket_avg_factor_score = []
        for i in idx_seperators:
            ll = factor_ret_sorted[i:i + num_per_quantile, 0]
            if len(ll) > 0 and np.isnan(ll).sum() != len(ll):
                bucket_avg_factor_score.append(np.nanmean(ll))
            else:
                bucket_avg_factor_score.append(np.nan)
...
def segment_test(factor_pd, stock_close_pd, holding_period, benchmark_price_ps, segment_num,
    ...
    if transaction_cost is not None:
        ...
        # SJL 这里的逻辑是Q1到Q10选最大收益，在report_generate中换成了10层中的最大层作为收益。
        _ = easy_seg_return_after_cost[name_pool].mean()
        if not _.isnull().all():
    ...
    # SJL 这里的逻辑是Q1到Q10选最大收益，在report_generate中换成了10层中的最大层作为收益。
    _ = easy_seg_return[name_pool].mean()
    if not _.isnull().all():
        max_quantile = _.idxmax() if max_quantile is None else max_quantile
    else:
        max_quantile = 'Q1'
...
def segment_test_by_industry(factor_pd, stock_close_pd, holding_period, benchmark_price_ps, segment_num,
    ...
    industry_hpr_daily = pd.concat(industry_hpr_daily, axis=0).reset_index().set_index(
        ['dt', 'Industry'])  # 行业每天的收益率，行业中每只票加权求和，算作行业收益率
    industry_ret_weighted = industry_hpr_daily.multiply(industry_weight, axis=0).groupby('dt').sum()
    seg_return_combined = pd.concat(seg_return_by_industry, axis=0).reset_index().set_index(
        ['dt', 'Industry'])  # 行业每天的分层收益
    res = seg_return_combined.multiply(industry_weight, axis=0).groupby('dt').sum()
    res['Index'] = industry_ret_weighted  # seg_return['Index']
    _ = res[name_pool].mean()
    # SJL 这里的逻辑是Q1到Q10选最大收益，在report_generate中换成了10层中的最大层作为收益。
    if not _.isnull().all():
        max_quantile = _.idxmax() if max_quantile is None else max_quantile
    else:
    ...
    if return_industry:
        ...
        if transaction_cost is None:
            ...
            seg_return_combined_after_cost[max_quantile + '-' + 'Index'] = seg_return_combined_after_cost[
                                                                               ...
                                                                               'total_return']
            # res是按行业权重加权后的分层收益，combined 是包含行业明细的收益率值，索引为dt，industry双索引
...
def calc_segment_rank(seg_return, holding_period_ret, holding_period=1, segment_number=10):
    """
    全市场股票收益率优于最顶层的分层收益率的比率
    """
    holding_period_ret_daily = (holding_period_ret + 1) ** (1 / holding_period) - 1
    name_pool = ['Q' + str(segment_number - i) for i in range(int(segment_number))]
    _ = seg_return[[name_pool[0], name_pool[-1]]].mean()
    max_quantile, min_quantile = _.idxmax(), _.idxmin()
    market_rank = calc_market_rank(seg_return.dropna()[max_quantile], holding_period_ret_daily)
    return market_rank


def compute_top_excess_return(factor_data, neutralized_data=None, standardized_data=None, stock_close_pd=None,
                              benchmark_price_ps=None, holding_period=1,  transaction_cost=0,
                              robust_segment=False, group=5):
    """"""
    # 分层后选取收益最大层的收益
    weight = np.arange(group, 0, -1)
    weight = weight / np.sum(weight)

    if neutralized_data is not None:
        factor_data = neutralized_data
    elif standardized_data is not None:
        factor_data = standardized_data
    else:
        factor_data = factor_data.copy()

    factor_top = factor_data[factor_data.rank(pct=True, ascending=False, axis=1) < (1. / group)]  #### top 20% stock

    _ = segment_test(factor_pd=factor_top, stock_close_pd=stock_close_pd, holding_period=holding_period,
                     benchmark_price_ps=benchmark_price_ps,segment_num=group,
                     handle_return_outlier=robust_segment, transaction_cost=transaction_cost)
