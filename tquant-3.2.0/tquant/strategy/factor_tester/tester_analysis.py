...
def day_factor_test(start_date, end_date, factor_result, price_type, rho, top_range, neutralize,
    ...
    if isinstance(factor_value.index, pd.MultiIndex):
        assert len(factor_value.columns) == 1, "仅支持对单个因子进行检测！"
        factor_value = factor_value.unstack()[factor_name]
        factor_value = factor_value[
            (factor_value.index >= str(start_date)) & (factor_value.index <= str(end_date))]
    ...
    try:
        ...
        return ret, test_result['stat_result']['statu']
    except Exception:
        traceback.print_exc()
        return False, 3
