...
def generate_plot(df, plot_name, x_label, y_label,
    ...
    df_plot = df.plot(kind=plot_type, figsize=(fig_width, fig_height),
                      legend=legend_on, fontsize=font_size_axis, rot=rot, colormap = 'Spectral', **kwargs)
    if legend_on:
        if legend_outside:
...
def generate_segment_ret_plot(seg_dict, interest_type):
    ...
    imgdata.seek(0)
    plt.close()
    return Image(imgdata, width=img_width * inch, height=img_height * inch*1.3)
...
def segment_helper(ax, easy_seg_return, title=False, title_text='', interest_type='SIMPLE'):
    ...
    max_q = easy_seg_return.iloc[:, 0:9].mean().idxmax()
...
def generate_pdf(pickle_name, corr_information=None, rolling_window=20,factor_data_num=0):
    ...
    g_seg_ret_bar = generate_bar_line_plot(df=seg_ret_use, title='{} Excess Return'.format(seg_er_col[0]),
