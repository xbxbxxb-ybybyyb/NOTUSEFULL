...
def get_trading_day_offset(start_date, offset_list, dfreq=DFreq.DAILY, dtype=DType.STOCK, mkttype=MktType.CHINA, dsource=DSource.HTSC):
    ...
    if dfreq == DFreq.DAILY:
        ...
        temp_data = read_data([start_date+pd.tseries.offsets.DateOffset(days=min(int(min(offset_list)*7-10), -20)),
...
def get_financial_date_range(start_date, end_date, mkttype=MktType.CHINA, dtype=DType.STOCK):
    ...
    if mkttype == MktType.CHINA and dtype == DType.STOCK:
        trading_days = get_trading_date_range(start_date, end_date, dfreq=DFreq.DAILY, dtype=dtype, mkttype=mkttype, dsource=DSource.HTSC)
