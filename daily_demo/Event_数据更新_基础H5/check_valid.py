...
def univ_flag_check(date):
    path = 'Z:\\warehouse\\prod\\LOCAL_DATA\\FLAG\\' + str(date) + '\\' + str(date) + '_' + 'UNIV.success'
...
def check_valid(date):
    root_path = 'Z:\\warehouse\\prod\\LOCAL_DATA\\CSV\\WIND\\MINUTE\\'
    ...
    for date in rst:
        ...
        df_index.to_csv('Z:\\warehouse\\test\\LOG\\index_minute\\' + str(date) + '.csv')
    ...
    for date in rst:
        df_suspend = IO.read_data(date, columns=['SUSPEND'], alt = r'Z:\warehouse\prod\LOCAL_DATA\CSV\stock_universe\universe_complete.h5')
...
#        print(df)
        df.to_csv('Z:\\warehouse\\test\\LOG\\stock_minute\\' + str(date) + '_new.csv')
        flag_path = 'Z:\\warehouse\\prod\\LOCAL_DATA\\CSV\\WIND\\MINUTE\\flag\\' + str(date) + '_' + 'minute_stock.success'
        if len(df) == 0:
...
#        print(stk_df)
    end_time = time.time()
    print("update by date time_cost="+str(end_time-start_time))
