...
class WIND_DATABASE:
            ...
            for fname in update_list:
                ...
                if len(dat1) <= min_size:
                    ...
                    if self.table_name == 'WIND_AShareProfitNotice':
                        ...
                        dat['S_INFO_COMPCODE'] = dat['S_INFO_COMPCODE'].astype(str)
                        dat.drop(['MEMO', "TOT_OPER_COST2", "AMODCOST_FIN_ASSETS", "TOT_OPT_COST_DIF", "TOT_OPT_COST_DIF_MEMO", 
                        "TOT_OPT_INC_DIF", "TOT_OPT_INC_DIF_MEMO"], axis=1, inplace=True)
                        # print(dat.dtypes)
                        h5_store.append(self.table_name, dat, data_columns=True, min_itemsize={'S_INFO_COMPCODE': 20,'OBJECT_ID':47})
                        ...
                        h5_store.append(self.table_name, dat, data_columns=True,
                                        min_itemsize={'S_INFO_MANAGER_NAME': 100, 'MANID': 35, 'S_INFO_MANAGER_POST':180})
                        ...
                        dat = dat[['OBJECT_ID', 'WIND_CODE', 'S_DIV_PROGRESS', 'STK_DVD_PER_SH', 'CASH_DVD_PER_SH_PRE_TAX', 'CASH_DVD_PER_SH_AFTER_TAX', 'EQY_RECORD_DT', 'EX_DT', 'DVD_PAYOUT_DT', 'LISTING_DT_OF_DVD_SHR', 'S_DIV_PRELANDATE', 'S_DIV_SMTGDATE', 'DVD_ANN_DT', 'S_DIV_BASEDATE', 'S_DIV_BASESHARE', 'CRNCY_CODE', 'ANN_DT', 'IS_CHANGED', 'S_DIV_CHANGE', 'S_DIV_BONUSRATE', 'S_DIV_CONVERSEDRATE', 'MEMO', 'S_DIV_PREANNDT', 'S_DIV_OBJECT', 'IS_TRANSFER', 'OPDATE', 'OPMODE']]
                        ...
                        def help(df):
                            ...
                            with open('/data/group/800080/warehouse/prod/LOCAL_DATA/CSV/WIND/comp_dict.pickle',
                                      'rb') as file:
    ...
    def get_df(self, date):
        ...
        if self.table_name in ['WIND_AShareFinancialIndicator', 'WIND_AShareProfitExpress', 'WIND_AShareTTMHis',
                               'WIND_AShareBalanceSheet',  'WIND_AShareIncome',
        ...
        elif self.table_name in ['WIND_AShareCashFlow']:
            df = s.get_factor_value(self.table_name, report_period=[str(date)])
            # Wind废弃该字段了
            df['OTHERS'] = np.NaN
            df = df[['OBJECT_ID', 'NET_INCR_DEP_COB', 'NET_INCR_LOANS_CENTRAL_BANK', 'NET_INCR_FUND_BORR_OFI', 'CASH_RECP_PREM_ORIG_INCO', 'NET_INCR_INSURED_DEP', 'NET_CASH_RECEIVED_REINSU_BUS', 'NET_INCR_DISP_TFA', 'NET_INCR_INT_HANDLING_CHRG', 'NET_INCR_DISP_FAAS', 'NET_INCR_LOANS_OTHER_BANK', 'NET_INCR_REPURCH_BUS_FUND', 'OTHER_CASH_RECP_RAL_OPER_ACT', 'STOT_CASH_INFLOWS_OPER_ACT', 'CASH_PAY_GOODS_PURCH_SERV_REC', 'CASH_PAY_BEH_EMPL', 'PAY_ALL_TYP_TAX', 'NET_INCR_CLIENTS_LOAN_ADV', 'NET_INCR_DEP_CBOB', 'CASH_PAY_CLAIMS_ORIG_INCO', 'HANDLING_CHRG_PAID', 'COMM_INSUR_PLCY_PAID', 'OTHER_CASH_PAY_RAL_OPER_ACT', 'STOT_CASH_OUTFLOWS_OPER_ACT', 'NET_CASH_FLOWS_OPER_ACT', 'CASH_RECP_DISP_WITHDRWL_INVEST', 'CASH_RECP_RETURN_INVEST', 'NET_CASH_RECP_DISP_FIOLTA', 'NET_CASH_RECP_DISP_SOBU', 'OTHER_CASH_RECP_RAL_INV_ACT', 'STOT_CASH_INFLOWS_INV_ACT', 'CASH_PAY_ACQ_CONST_FIOLTA', 'CASH_PAID_INVEST', 'NET_CASH_PAY_AQUIS_SOBU', 'OTHER_CASH_PAY_RAL_INV_ACT', 'NET_INCR_PLEDGE_LOAN', 'STOT_CASH_OUTFLOWS_INV_ACT', 'NET_CASH_FLOWS_INV_ACT', 'CASH_RECP_CAP_CONTRIB', 'INCL_CASH_REC_SAIMS', 'CASH_RECP_BORROW', 'PROC_ISSUE_BONDS', 'OTHER_CASH_RECP_RAL_FNC_ACT', 'STOT_CASH_INFLOWS_FNC_ACT', 'CASH_PREPAY_AMT_BORR', 'CASH_PAY_DIST_DPCP_INT_EXP', 'INCL_DVD_PROFIT_PAID_SC_MS', 'OTHER_CASH_PAY_RAL_FNC_ACT', 'STOT_CASH_OUTFLOWS_FNC_ACT', 'NET_CASH_FLOWS_FNC_ACT', 'EFF_FX_FLU_CASH', 'NET_INCR_CASH_CASH_EQU', 'CASH_CASH_EQU_BEG_PERIOD', 'CASH_CASH_EQU_END_PERIOD', 'NET_PROFIT', 'UNCONFIRMED_INVEST_LOSS', 'PLUS_PROV_DEPR_ASSETS', 'DEPR_FA_COGA_DPBA', 'AMORT_INTANG_ASSETS', 'AMORT_LT_DEFERRED_EXP', 'DECR_DEFERRED_EXP', 'INCR_ACC_EXP', 'LOSS_DISP_FIOLTA', 'LOSS_SCR_FA', 'LOSS_FV_CHG', 'FIN_EXP', 'INVEST_LOSS', 'DECR_DEFERRED_INC_TAX_ASSETS', 'INCR_DEFERRED_INC_TAX_LIAB', 'DECR_INVENTORIES', 'DECR_OPER_PAYABLE', 'INCR_OPER_PAYABLE', 'OTHERS', 'IM_NET_CASH_FLOWS_OPER_ACT', 'CONV_DEBT_INTO_CAP', 'CONV_CORP_BONDS_DUE_WITHIN_1Y', 'FA_FNC_LEASES', 'END_BAL_CASH', 'LESS_BEG_BAL_CASH', 'PLUS_END_BAL_CASH_EQU', 'LESS_BEG_BAL_CASH_EQU', 'S_INFO_WINDCODE', 'WIND_CODE', 'ANN_DT', 'REPORT_PERIOD', 'STATEMENT_TYPE', 'CRNCY_CODE', 'CASH_RECP_SG_AND_RS', 'RECP_TAX_RENDS', 'IM_NET_INCR_CASH_CASH_EQU', 'FREE_CASH_FLOW', 'COMP_TYPE_CODE', 'ACTUAL_ANN_DT', 'SPE_BAL_CASH_INFLOWS_OPER', 'TOT_BAL_CASH_INFLOWS_OPER', 'SPE_BAL_CASH_OUTFLOWS_OPER', 'TOT_BAL_CASH_OUTFLOWS_OPER', 'TOT_BAL_NETCASH_OUTFLOWS_OPER', 'SPE_BAL_CASH_INFLOWS_INV', 'TOT_BAL_CASH_INFLOWS_INV', 'SPE_BAL_CASH_OUTFLOWS_INV', 'TOT_BAL_CASH_OUTFLOWS_INV', 'TOT_BAL_NETCASH_OUTFLOWS_INV', 'SPE_BAL_CASH_INFLOWS_FNC', 'TOT_BAL_CASH_INFLOWS_FNC', 'SPE_BAL_CASH_OUTFLOWS_FNC', 'TOT_BAL_CASH_OUTFLOWS_FNC', 'TOT_BAL_NETCASH_OUTFLOWS_FNC', 'SPE_BAL_NETCASH_INC', 'TOT_BAL_NETCASH_INC', 'SPE_BAL_NETCASH_EQU_UNDIR', 'TOT_BAL_NETCASH_EQU_UNDIR', 'SPE_BAL_NETCASH_INC_UNDIR', 'TOT_BAL_NETCASH_INC_UNDIR', 'S_INFO_COMPCODE', 'S_DISMANTLE_CAPITAL_ADD_NET', 'IS_CALCULATION', 'SECURITIE_NETCASH_RECEIVED', 'OTHER_IMPAIR_LOSS_ASSETS', 'CREDIT_IMPAIRMENT_LOSS', 'RIGHT_USE_ASSETS_DEP', 'OPDATE', 'OPMODE']]
            ...
            df = df[['OBJECT_ID', 'S_INFO_WINDCODE', 'S_PROFITNOTICE_DATE', 'S_PROFITNOTICE_PERIOD', 'S_PROFITNOTICE_STYLE', 'S_PROFITNOTICE_SIGNCHANGE', 'S_PROFITNOTICE_CHANGEMIN', 'S_PROFITNOTICE_CHANGEMAX', 'S_PROFITNOTICE_NETPROFITMIN', 'S_PROFITNOTICE_NETPROFITMAX', 'S_PROFITNOTICE_NUMBER', 'S_PROFITNOTICE_FIRSTANNDATE', 'S_PROFITNOTICE_ABSTRACT', 'S_PROFITNOTICE_REASON', 'S_PROFITNOTICE_NET_PARENT_FIRM', 'S_INFO_COMPCODE', 'OPDATE', 'OPMODE']]

        elif self.table_name in ['WIND_AShareEODDerivativeIndicator',
            ...
            df = s.get_factor_value(self.table_name, factors=['OBJECT_ID', 'S_INFO_WINDCODE', 'TRADE_DT', 'CRNCY_CODE',
               ...
               'S_DQ_ADJCLOSE', 'S_DQ_ADJFACTOR', 'S_DQ_AVGPRICE', 'S_DQ_TRADESTATUS',
                 'OPDATE', 'OPMODE'], trade_dt=[str(date)])
        ...
        #     sql_where = ' where TDATE='
            ...
            df = s.get_factor_value(self.table_name, EST_DT=[str(date)])
        else:
        ...
        if self.table_name == 'WIND_AShareMonthlyReportsofBrokers':
            ...
            if self.table_name in self.over_write_list:
                try:
                    ...
                    logger.warning('%s got exception as follows: ' % (self.table_name))
                    print(err)
#                    df2 = s.get_factor_value(self.table_name, OPDATE=['>=20210101']).append(
#                    s.get_factor_value(self.table_name, OPDATE=['>=20190101', '<20210101'])).append(
#                    s.get_factor_value(self.table_name, OPDATE=['>=20150101', '<20190101']))             
#                    df1 = df2.append(s.get_factor_value(self.table_name, OPDATE=['>=20120101', '<20150101']))
#                    df = df1.append(s.get_factor_value(self.table_name, OPDATE=['<20120101']))
                    # NSW
                    if self.table_name == 'WIND_AShareConseption':
                        df0 = s.get_factor_value(self.table_name,ENTRY_DT=['<20120101'])
                        df1 = s.get_factor_value(self.table_name,ENTRY_DT=['>=20120101','<20130101'])
                        df2 = s.get_factor_value(self.table_name,ENTRY_DT=['>=20130101','<20140101'])
                        df3 = s.get_factor_value(self.table_name,ENTRY_DT=['>=20140101','<20150101'])
                        df4 = s.get_factor_value(self.table_name,ENTRY_DT=['>=20150101','<20160101'])
                        df5 = s.get_factor_value(self.table_name,ENTRY_DT=['>=20160101','<20170101'])
                        df6 = s.get_factor_value(self.table_name,ENTRY_DT=['>=20170101','<20180101'])
                        df7 = s.get_factor_value(self.table_name,ENTRY_DT=['>=20180101','<20190101'])
                        df8 = s.get_factor_value(self.table_name,ENTRY_DT=['>=20190101','<20200101'])
                        df9 = s.get_factor_value(self.table_name,ENTRY_DT=['>=20200101','<20210101'])
                        df10 = s.get_factor_value(self.table_name,ENTRY_DT=['>=20210101','<20220101'])
                        df11 = s.get_factor_value(self.table_name,ENTRY_DT=['>=20220101','<20230101'])
                        df12 = s.get_factor_value(self.table_name,ENTRY_DT=['>=20230101','<20240101'])
                        df13 = s.get_factor_value(self.table_name,ENTRY_DT=['>=20240101'])
                        df = pd.concat([df0,df1,df2,df3,df4,df5,df6,df7,df8,df9,df10,df11,df12,df13]) 
                    else:
                        df0 = s.get_factor_value(self.table_name,OPDATE=['<20120101'])
                        df1 = s.get_factor_value(self.table_name,OPDATE=['>=20120101','<20130101'])
                        df2 = s.get_factor_value(self.table_name,OPDATE=['>=20130101','<20140101'])
                        df3 = s.get_factor_value(self.table_name,OPDATE=['>=20140101','<20150101'])
                        df4 = s.get_factor_value(self.table_name,OPDATE=['>=20150101','<20160101'])
                        df5 = s.get_factor_value(self.table_name,OPDATE=['>=20160101','<20170101'])
                        df6 = s.get_factor_value(self.table_name,OPDATE=['>=20170101','<20180101'])
                        df7 = s.get_factor_value(self.table_name,OPDATE=['>=20180101','<20190101'])
                        df8 = s.get_factor_value(self.table_name,OPDATE=['>=20190101','<20200101'])
                        df9 = s.get_factor_value(self.table_name,OPDATE=['>=20200101','<20210101'])
                        df10 = s.get_factor_value(self.table_name,OPDATE=['>=20210101','<20220101'])
                        df11 = s.get_factor_value(self.table_name,OPDATE=['>=20220101','<20230101'])
                        df12 = s.get_factor_value(self.table_name,OPDATE=['>=20230101','<20240101'])
                        df13 = s.get_factor_value(self.table_name,OPDATE=['>=20240101'])
                        df = pd.concat([df0,df1,df2,df3,df4,df5,df6,df7,df8,df9,df10,df11,df12,df13])
...
#    qtr_list = []    
#    first_daily_list = ['WIND_AShareConseption']           
#    qtr_list = [
#                'WIND_AShareTTMHis',
#                'WIND_AShareANNFinancialIndicator',
#                'WIND_AShareIssuingDatePredict', 'WIND_AShareDividend', 'WIND_AIndexFinancialderivative'
...
#                        ]
    ...
    for table_type in table_dict:
        for table_name in table_dict[table_type]:
            print(table_name)
            instantiate(table_name, sdate, edate, base_path='/data/group/800080/warehouse/prod/', operation='append',
...
# 重刷某张表
#tmp(20220117,20220117)

print("start running aimr tasks!")
param_str = AIMR.getParam()
...
print("task start: param = {}".format(param))
print(param[0])
print(param[1])
print(param[2])
print(param[3])
...
tables = param[0].split(',')
for table in tables:
    instantiate(table, int(param[1]), int(param[2]),param[3],param[4],table_dict)
    
print("All table saved! param={}".format(param))
