...
class WIND_DATABASE:
    def __init__(self, table_name, sdate, edate, 
        ...
        if self.dfreq=='DAILY':
            self.cdate_list = get_all_days(self.sdate,self.edate)
                ...
                if len(dat1)<=min_size:
                    ...
                    if self.table_name == 'WIND_AShareProfitNotice':
                        ...
                        h5_store.append(self.table_name,dat, data_columns=True,min_itemsize={'HOLDER_NAME':500,'TRANSACT_STOCK_SOURCE':200,'TRANSACT_SOURCE_FUNDS':200,
                                                                                            'HOLDER_STATUS':100,'TRANSACT_PERIOD_DESCRIPTION':200,'TRANSACTION_MODE':200,
    ...
    def retriever(self):
        ...
        if self.table_name == 'WIND_AShareMonthlyReportsofBrokers':
            ...
            if self.table_name in self.over_write_list:
                try:
                    df = FactorData().get_factor_value(self.table_name)
                    ...
                    logger.warning('%s is more than 50w rows'%(self.table_name))
                    df = FactorData().get_factor_value(self.table_name, S_CON_INDATE=['>=20130101']).append(FactorData().get_factor_value(self.table_name, S_CON_INDATE=['<20130101']))
                    df['date'] = self.cdate_list[-1]
                    save_name = os.path.join(self.table_csv_path,self.table_name + '.csv')
                    df.set_index('OBJECT_ID', inplace = True)
                    df.to_csv(save_name, sep=',', encoding='utf_8_sig')
...
def first_job(sdate=None,edate=None):
    ...
    first_daily_list = ['WIND_AShareEODDerivativeIndicator','WIND_AShareMonthlyReportsofBrokers','WIND_AShareIndustriesClassCITICS']
    ...
    for table_type in table_dict:
        for table_name in table_dict[table_type]:
            print (table_name)
            instantiate(table_name, sdate, edate, base_path = '/data/group/800080/warehouse/prod/', operation = 'append',table_dict=table_dict)
...
def every_day_job(sdate=None,edate=None):
    
    qtr_list = []

    first_daily_list = ['WIND_AShareManagementHoldReward'] 
...
every_day_job(20190926,20191007)
