...
from MDCDataProvider.HBaseProvider import HBaseProvider
...
data_type = "Tick"
for date in dates:
    start_date = date + ' 090000000'
    end_date = date + ' 153000000'
    for stock in stocks:
        print(stock)
        csv_path = os.path.join(root_path, date, stock)
        if not os.path.exists(csv_path):
            os.makedirs(csv_path)
        #new_df = mdc.get_data_by_time_frame('BondOrder', stock, start_date, end_date)
        t1 = time.time()
        new_df = bd.get_bond_data(stock, start_date, end_date, "Transaction")
        ...
        old_df = ma.getMDTransactionDataFrame(stock, start_date.replace(" ",""), end_date.replace(" ",""))
        #old_df = ma.getMDOrderDataFrame(stock, start_date.replace(" ",""), end_date.replace(" ",""))
        #old_df = ma.getMDSecurityTickDataFrame(stock, start_date.replace(" ",""), end_date.replace(" ",""), 1)
        print("old_time:", time.time()-t1)
        t1 = time.time()
        old_df = hp.get_data_by_time_frame("BondTransaction",stock, start_date, end_date)
        print("old_time:", time.time()-t1)
        ...
        for col in old_df.columns:
            ...
            if data_type == "Stock" or data_type == "Index" or data_type == 'Tick':
                ...
                if stock.endswith(".SH"):
                    merge_df = compare(new_df, old_df, ["ApplSeqNum"], [col], ["ApplSeqNum"], [col], sample_ratio = 0.05)
                else:
                    merge_df = compare(new_df, old_df, ["OrderIndex"], [col], ["OrderIndex"], [col], sample_ratio = 0.05)
