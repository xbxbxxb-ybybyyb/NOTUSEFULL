...
class SplitTaskParam:
    def __init__(self, value):
        ...
        if not isinstance(value, list):
            raise Exception("SplitTaskParam的值必须为list类型！")
        if len(value) == 0:
            raise Exception("TaskParam切分参数list不可为空!")
        self.value = value
    ...
    def __post_init__(self):
        ...
        if task_param.get("cpus_per_worker") != -1:
            if self.num_workers == -1 and self.cpus_per_worker == -1 and self.gpus_per_worker == -1:
                ...
                assert self.cpus_per_worker != 0, "RayParams的cpus_per_worker参数不可为0！"
                assert self.gpus_per_worker != 0, "RayParams的gpus_per_worker参数不可为0！"
                # 使用RayParams定义的进程池资源
...
def train_fun(config):
    if config['set_omp_num_threads']:
        from parallel_train.helper import set_omp_num_threads
        set_omp_num_threads()
...
class ParallelTrainer(Trainer):
    def __init__(
            ...
            max_retries: int = 3,
            set_omp_num_threads: Optional[bool] = True
    ):
        """
        Args:
        ...
        self._set_omp_num_threads = set_omp_num_threads
                ...
                try:
                    iterator = self.run_iterator(train_func=train_fun,
                                   config={'backend': self._backend_train_cls,
                                           ...
                                           'set_omp_num_threads': self._set_omp_num_threads
                                           },
                                   checkpoint=checkpoint)
