...
def _validate_ray_params(ray_params):
    ...
    if ray_params is None:
        ...
        raise ValueError(
            f"`ray_params` must be a `RayParams` instance or None, "
    ...
    if ray_params.num_workers <= 0:
        raise ValueError(
            "The `num_workers` parameter is auto set to 0. Please check parameter of"
            "`RayParams` to specify the resources of distributed workers you want to use.")
    elif ray_params.num_workers < 2:
        warnings.warn(
            f"`num_workers` in `ray_params` is smaller than 2 "
            f"({ray_params.num_workers}). Tasks will NOT be distributed!")
    return ray_params
...
def split_task_by_data_param(data_params: Dict[str, Any], num_actors: int, is_mode_actor=True):
    ...
    if total_task_nums < num_actors and not is_mode_actor:
        warnings.warn(f"LocalTrain模式资源闲置告警！切分的并行任务数{total_task_nums}小于当前设置并发度{num_actors}！请调小并发度，或者通过SplitTaskParam设置更多任务！")
...
def get_backend_config(backend) -> type:
    ...
    if issubclass(backend, DistTrainBackend):
        ...
        # LocalTrain模式传递模拟基类DistTrainBackend进BackendExecutor，不改变源代码情况下，backend_cfg必传，必须为Backend类型
        config = ConfigToBackend()
