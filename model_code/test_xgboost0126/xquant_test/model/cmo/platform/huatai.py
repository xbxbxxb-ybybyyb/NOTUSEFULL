...
class HuaTaiTrackingMiddleWare:
    ...
    def __init__(self, xquant_config_path, product_name, product_id,
        ...
        if os.path.exists(temp_outer_run_id_file):
            ...
            mlflow.set_tag("parent_pid", os.getpid())
        ...
        def auto_log_upload():
            ...
            parent_pid = int(run.data.tags.get("parent_pid"))

            # main process remove outer_run_id_file
            if os.path.exists(temp_outer_run_id_file) and parent_pid == os.getpid():
                os.remove(temp_outer_run_id_file)
            ...
            try:
                ...
                run_data_list = [run for run in run_data_list if
                    run.info.start_time > exp_start_time and
                                 (run.data.tags.get("pid", '0') == str(os.getpid()) or run.data.tags.get("pid", '0') == '0')]
        ...
        global g_original_exit
        if g_original_exit is None:
            g_original_exit = os._exit

        def os_hook_exit(n):
            auto_log_upload()
            g_original_exit(n)
        os._exit = os_hook_exit
...
def auto_log_params(run_data, run_id, order, get_parent_run_id):
    ...
    params = {param: value.replace('\'', '') if type(value)==str else value \
              for param, value in run_data.data.params.items() if param not in user_params_set}
    if user_params:
        base_info = {
            "exp_id": exp_id,
            "run_id": run_id,
...
def auto_log_metrics(run_data, run_id, order, get_parent_run_id):
    ...
    metrics = {metrics_mapping.get(metric, metric): value.replace('\'', '') if type(value)==str else value \
               for metric, value in run_data.data.metrics.items() if metric not in user_metrics_set}
    if user_metrics:
        base_info = {
            "exp_id": exp_id,
            "run_id": run_id,
