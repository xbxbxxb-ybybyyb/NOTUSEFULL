...
def auto_connect_ray():
    ...
    if not ray.is_initialized():
        try:
            #连接已有的ray集群
            ray.init("auto")
            logger.info("已检测到一组正在运行的Ray资源，自动连接成功！")
        except:
            ray.init()
            logger.info("未检测到正在运行的Ray资源， 在本地启动一组新的Ray资源成功！")

    logger.info("Ray集群的总资源为：{}，请确认此集群没有其他主程序在占用，否则会造成任务冲突。".format(ray.cluster_resources()))