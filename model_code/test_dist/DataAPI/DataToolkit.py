...
if platform_name == "Windows":  # 云桌面环境运行是Windows
    ...
    HQF_minute_data_path = '/data/group/800080/PanelMinDataForZT'
    running_platform = platform_name
    complete_stock_list_path = root_666889 + "/Apollo/AlphaDataBase/CompleteStockList.csv"
    future_dir = root_666889 + '/Apollo/FutureData/'
...
def get_panel_interval_pv_df(stock_list, start_date_int, end_date_int, pv_type='interval_twap_30_5', adj_type='NONE',
    ...
    if pv_type in ['interval_twap_30_5', 'interval_twap_15_5',
                   'interval_buy_twap_30_5', 'interval_buy_twap_15_5',
                   'interval_buy_twap_fill_rate_30_5', 'interval_buy_twap_fill_rate_15_5',
                   'interval_sell_twap_30_5', 'interval_sell_twap_15_5',
                   'interval_sell_twap_fill_rate_30_5', 'interval_sell_twap_fill_rate_15_5',
                   'interval_pre_close_30_0',
                   'interval_buyable_volume_30_5', 'interval_buyable_volume_15_5',
                   'interval_sellable_volume_30_5', 'interval_sellable_volume_15_5',
                   'interval_twap_60_5', 'interval_sell_twap_60_5', 'interval_buy_twap_60_5', 'interval_sellable_volume_60_5','interval_buyable_60_5', 'interval_buy_twap_fill_rate_60_5', 'interval_sell_twap_fill_rate_60_5',
                   'interval_vwap_30_5',
                   'interval_buy_vwap_30_5', 'interval_buy_vwap_15_5',
                   'interval_sell_vwap_30_5', 'interval_sell_vwap_15_5',
                   'interval_buy_vwap2_30_5','interval_sell_vwap2_30_5',
                   'interval_buy_twap_fill_rate2_30_5','interval_sell_twap_fill_rate2_30_5',
                   'interval_amt_30_0']:
        pass
    else:
        raise TypeError
...
def get_panel_daily_info(stock_list, start_date_int, end_date_int, info_type, output_index_type='date_int') \
    ...
    if info_type in ['turn', 'adjfactor', 'mkt_cap_ard', 'free_float_shares', 'total_shares', 'alpha_universe',
                     ...
                     'index_weight_sh50', 'index_weight_zz500','index_weight_hs300_raw','index_weight_zz500_raw',
                     'industry3', 'industry_sw','industry_swx','pb_lf', 'pcf_ocf_ttm',
                     'pe_ttm', 'ps_ttm', 'alpha_universe_b', 'limit_pctg', 'limit_pctg2', 'alpha_universe_c',
                     'alpha_universe_half_year','isvalid','adjfactor_raw','STPT','isvalid_raw_incl_st']:
        ...
        if info_type in ['index_50', 'index_300', 'index_500', 'index_weight_hs300',
            ...
            data_df = data_df.shift(-1)  # 在原始数据中t日的指数信息，key是t+1日的，因此指数要用下一天的数据，不包含_raw数据
...
def return_panel_info_complete_key_set():
    ...
    key_set = ('adjfactor', 'mkt_cap_ard', 'free_float_shares', 'total_shares', 'alpha_universe',
               'risk_universe', 'alpha_universe_b', 'limit_pctg','limit_pctg2','alpha_universe_c','alpha_universe_half_year',
               ...
               'index_weight_sh50', 'index_weight_zz500', 'risk_universe','index_weight_hs300_raw','index_weight_zz500_raw',

               'industry3','industry_sw','industry_swx',
               ...
               'isvalid', 'adjfactor_raw', 'STPT', 'isvalid_raw_incl_st'
               )
    return key_set
...
def back_fill(df_fill, df_qfa, df_ann, fill_na=True):
    ...
    for col_i, col in enumerate(columns):
        ...
        if col in df_ann and col in df_qfa:
            ...
            #如果财报发布的时间顺须反向了（如年报发布晚于一季报），在填充时采用最新一季报的数据
            for i in range(len(s_ann_temp) - 1):
                if s_ann_temp[i] > s_ann_temp[i+1]:
                    s_ann_temp[i] = s_ann_temp[i+1]
...
def get_panel_min_data(item, start_date, end_date, stock_list=None, dir_path=panel_minute_data_path, forward_adj=True,
    ...
    if item in ['open', 'high', 'low', 'close', 'volume', 'amt', 'limit_status', 'limit_status2'] + tick_item:
        pass
    else:
        raise TypeError
    if item in tick_item:
        dir_path = HQF_minute_data_path
    ...
    for i, i_date in enumerate(year_month_list):  # 逐月读取panel data
        ...
        try:
            ...
            pass
    trading_days = list(map(str, get_trading_day(start_date, end_date)))
    complete_index = pd.MultiIndex.from_product([trading_days, trading_mins])
    ...
    complete_index_datetime = pd.to_datetime(convert_date_or_time_int_to_datetime(complete_index))
    df = df.reindex(index=complete_index_datetime)
...
def get_future_daily_data(future_name,start_date:int,end_date:int,pv_type):
    '''
    用来获取股指期货当月连续的量价数据，即按照日期取到的数据为当日未交割最近的合约数据，可取的字段包括期货合约的高开低收成交量成交额以及结算价
    :param future_name: 期货合约的名称：IC，IH，IF
    :param start_date: 
    :param end_date: 
    :param pv_type: ['open','high','low','close','settle','volume','amt']
    :return: 
    '''
    if future_name in ['IC','IF','IH']:
        pass
    else:
        raise TypeError('{} incorrect'.format(future_name))
    if isinstance(pv_type,str):
        if pv_type in ['open','high','low','close','settle','volume','amt']:
            pass
        else:
            raise TypeError('pv_type incorrect')
    elif isinstance(pv_type,list):
        if (set(pv_type) - set(['open','high','low','close','settle','volume','amt'])).__len__() == 0:
            pass
        else:
            raise TypeError('pv_type incorrect')
    date_list = get_trading_day(start_date,end_date)
    fac = pd.read_pickle('{}{}.pkl'.format(future_dir,future_name))[['MDDate','OpenPx','ClosePx','HighPx','LowPx','TotalVolumeTrade','TotalValueTrade','SettlePrice']]
    fac.columns = ['date','open','close','high','low','volume','amt','settle']
    fac.date = fac.date.astype('int')
    fac.set_index('date',inplace=True)
    if isinstance(pv_type,str):
        ans_df = fac.reindex(date_list)[pv_type].to_frame()
    elif isinstance(pv_type,list):
        ans_df = fac.reindex(date_list)[pv_type]
    return ans_df


def get_panel_daily_hk_data(stock_list, start_date_int, end_date_int, fac_type='hk_pct_hold'):
    """
    北向资金数据接口
    :param stock_list:
    :param start_date_int:
    :param end_date_int:
    :param fac_type: hk_net_value:北向当日净买入（元）
                    hk_pct_hold:北向当日累计持股的流通市值占比（%）
                    hk_share_hold：北向当日累计持股数量（股）
    :return:
    """
    if fac_type in ['hk_net_value','hk_pct_hold','hk_share_hold']:
        pass
    else:
        raise TypeError
    query_date_list = get_trading_day(start_date_int, end_date_int)  # 获取起止日期list
    start_date_int = query_date_list[0]  # 确保start_date_int是交易日
    end_date_int = query_date_list[-1]  # 确保end_date_int是交易日，以利于后续计算BACKWARD2（前复权）时定位用
    data_file = "Data_" + fac_type + ".h5"
    data_full_path = os.path.join(database_dir, data_file)
    store = pd.HDFStore(data_full_path, mode='r')
    data_df = store.select("/factor")
    store.close()
    data_df = data_df.loc[start_date_int: end_date_int]
    data_df = data_df.reindex(index=query_date_list,columns=stock_list)
    return data_df


def get_factor_excess_ret_kline(start_date,end_date,factor_list):
    temp_root = '/data/user/015618/Structure/Logic_Excess_Kline/'
    trading_days = get_trading_day(start_date,end_date)
    year_month_list = sorted(np.unique([str(item)[:6] for item in trading_days]))
    df_list = []
    for year_month in year_month_list:
        df = pd.read_pickle('{}/{}_minute_ret.pkl'.format(temp_root, year_month))
        try:
            df_select = df[factor_list].copy()
        except:
            miss_factor = set(factor_list) - set(df.columns)
            raise Exception('{} not covered in database'.format(miss_factor))
        df_list.append(df_select)
    final_df = pd.concat(df_list,axis=0)
    ans_df = final_df.loc[str(start_date):str(end_date)].copy()
    return ans_df