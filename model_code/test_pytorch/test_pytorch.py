...
from xquant.model import auto_log, log_metrics
...
class MNISTDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        self.df_train = datasets.MNIST(
            "~/.keras/datasets", download=True, train=True, transform=self.transform
        )
        self.df_train, self.df_val = random_split(self.df_train, [55000, 5000])
        self.df_test = datasets.MNIST(
            "~/.keras/datasets", download=True, train=False, transform=self.transform
        )
...
if __name__ == "__main__":
    ...
    checkpoint_callback = ModelCheckpoint(
        filepath=os.getcwd(), save_top_k=1, verbose=True, monitor="val_loss", mode="min"
    )
    lr_logger = LearningRateMonitor()
    # auto_log('pytorch')

    trainer = pl.Trainer.from_argparse_args(
        args, callbacks=[lr_logger, early_stopping, checkpoint_callback], checkpoint_callback=checkpoint_callback
    )
