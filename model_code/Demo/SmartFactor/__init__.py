...
def get_custom_factor_class(BaseFactorClass, class_attrs):
    ...
    assert BaseFactorClass.__name__ == BaseFactorClass.factor_name, "因子名称冲突：因子类名{}与因子名{}不一致！".format(
        BaseFactorClass.__name__, factor_name)
    assert BaseFactorClass.__base__ == HFBaseFactor or BaseFactorClass.__base__ == Factor, "因子类{}的基类必须为HFBaseFactor或Factor！".format(
        BaseFactorClass)
    assert type
