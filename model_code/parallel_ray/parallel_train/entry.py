...
from parallel_train import get_actor_from_module
...
def _create_placement_group(cpus_per_actor, gpus_per_actor,
    ...
    if ready:
        logger.debug(f"Placement group创建成功. 策略为placement_strategy: {strategy}")
    else:
...
def _train_by_actor(backend_actor: str,
    ...
    local_actor_class = get_actor_from_module(backend_actor)
    try:
        remote_actor = ray.remote(local_actor_class)
    except:
        print(traceback.print_exc())
        raise Exception(f"backend_actor {backend_actor}注册成远程类失败：请确认backend_actor继承自base_actor基类, 并重载相应方法！")
    ...
    try:
        ...
        logger.debug("ready_states: {}".format(ready_states))
        ...
        while not_ready:
            ...
            if ray_params.elastic_training:
                _maybe_schedule_new_actors(
                    remote_actor = remote_actor,
                    data_params_groups = data_params_groups,
...
def train_func(backend_actor, rank_id, ray_params, data_params, model_params, _training_state):
    from parallel_train import get_actor_from_module
    from parallel_train.session import _session, get_session, shutdown_session
    import gc
    local_actor_class = get_actor_from_module(backend_actor)
    actor = local_actor_class()
    PrepareActorTask(
        rank_id,
        actor,
        # Maybe we got a new Queue actor, so send it to all actors.
    ...
    #必须删除全局变量，否则max_calls为1时，重启进程计算失败。
    shutdown_session()
    return
...
if __name__=="__main__":
    ...
    ray_params = RayParams(cpus_per_actor=1, gpus_per_actor=0, num_actors=4)
