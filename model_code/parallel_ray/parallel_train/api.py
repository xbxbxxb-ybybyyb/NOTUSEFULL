...
from parallel_train.helper import auto_connect_ray, get_xqconfig, get_total_ray_resources
...
class RayParams:
    ...
    mems_per_actor: int = -1#单位兆
    object_store_memory: int = -1#单位兆
    ...
    def __post_init__(self):
        ...
        if taskParam.get("cpus_per_actor") != -1:
            if self.num_actors == -1 and self.cpus_per_actor == -1 and self.gpus_per_actor == -1 and self.mems_per_actor == -1:
                ...
                num_actors =  taskParam.get("num_actors")
                ...
                #使用RayParams定义的进程池资源
                num_actors = min(int(total_resources['CPU']/self.cpus_per_actor),
                                      int(total_resources['GPU']/self.gpus_per_actor))

        else:
           self.mems_per_actor = 1#本地运行无需关注内存大小
           if self.gpus_per_actor >0:
               num_actors = min(int(total_resources['CPU']/self.cpus_per_actor),
                                          int(total_resources.get('GPU', 0)/self.gpus_per_actor))
           else:
               num_actors = int(total_resources['CPU'] / self.cpus_per_actor)
        if num_actors!=self.num_actors:
            self.num_actors = num_actors
            logger.warning(f"进程池最大并行度num_actors已自动赋值为{self.num_actors}，以保证资源合理利用！")                             
        ...
        logger.info(
            f"Ray计算进程池的资源为: [CPU: {float(self.cpus_per_actor)} * {self.num_actors}, GPU: {float(self.gpus_per_actor)} * {self.num_actors}]。")
