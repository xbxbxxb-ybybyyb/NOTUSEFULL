...
class TensorflowLocalBackendExample(LocalTrainBackend):
    ...
    def mnist_dataset(self, batch_size):
        (x_train, y_train), _ = tf.contrib.keras.datasets.mnist.load_data()
    ...
    def build_and_compile_cnn_model(self):
        model = models.Sequential([
            tf.contrib.keras.layers.InputLayer(input_shape=(28, 28)),
            tf.contrib.keras.layers.Reshape(target_shape=(28, 28, 1)),
            tf.contrib.keras.layers.Conv2D(32, 3, activation='relu'),
            tf.contrib.keras.layers.Flatten(),
            tf.contrib.keras.layers.Dense(128, activation='relu'),
...
if __name__ == "__main__":
    ...
    trainer = ParallelTrainer(backend=TensorflowLocalBackendExample, task_mode='LocalTrain',
                             ray_params=RayParams(cpus_per_worker=1, ))
    # step3: trainer开始处理，可以添加initial_hook函数
    trainer.start()

    # step4: 并行训练开始, 在tensorflow 1.x版本下运行
