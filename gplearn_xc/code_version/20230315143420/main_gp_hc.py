...
def load_data(index_code = '000906.SH',name = 'close',period = 63):
...
def train(label,X,y,score_df,metric='my_score',population_size = 100):
    ...
    # est_gp = SymbolicRegressor(generations=50, population_size=10000,
    #                            function_set=function_set,
    #                            ts_function_set=ts_function_set,
    #                            fixed_function_set=fixed_function_set,
    #                            const_range=None,
    #                            parsimony_coefficient='fixed_10_15_4.5_0.1',
    #                            max_samples=0.7, verbose=1,
    #                            stopping_criteria=999,
    #                            metric='my_score',
    #
    #                            random_state=0, n_jobs=-1, feature_names=label)
    est_gp = SymbolicRegressor(generations=10, population_size=population_size,tournament_size = round(population_size * 0.02),
                               function_set=function_set,
                               ts_function_set=ts_function_set,
                               fixed_function_set=fixed_function_set,
                               const_range=None,
                               # parsimony_coefficient='auto',
                               parsimony_coefficient='fixed_15_55_0.05_0.05',
                               max_samples=0.75, verbose=1,
                               stopping_criteria=999,
                               metric=metric,
                               random_state=0, n_jobs=-1, feature_names=label)
    
    # est_gp = SymbolicRegressor(generations=10, population_size=100,
    #                               function_set = function_set,
    #                               ts_function_set = ts_function_set,
    #                               fixed_function_set = fixed_function_set,
    #                               const_range = None,
    #                   parsimony_coefficient='fixed_15_20_4.5_0.1',
    #                   max_samples = 0.75, verbose=1,
    #                   stopping_criteria = 999,
    #                   metric=metric,
    #
...
# X_Train,Y_Train,label,info_dict = load_data('simple_close_63',period = 63)
# X_Train,Y_Train,label,info_dict = load_data('simple_close_63',period = 63)
# X_Train,Y_Train,label,info_dict = load_data('close',period = 63)
# est_gp,X,y,score_df = train(label,X_Train,Y_Train,Y_Train,metric = 'my_score_interval')
# est_gp,X,y,score_df = train(label,X_Train,Y_Train,Y_Train,metric = 'my_score_rl_only',population_size = 10000)
# est_gp,X,y,score_df = train(label,X_Train,Y_Train,Y_Train,metric = 'my_score')
# save_result([info_dict,est_gp])
# print(est_gp_programs.[0][0])

save_code()

# print(111)
# print(mf.get_list_trading_days(20220101,20220115))
