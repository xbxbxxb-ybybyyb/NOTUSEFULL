...
def _parallel_evolve(n_programs, parents, X, y, score_data,sample_weight, seeds, params):
    ...
    #     pickle.dump([ts_function_set,function_set],file)
    ...
    for i in range(n_programs):
        ...
        #设定训练集范围
        curr_sample_weight = range(max_samples)
        oob_sample_weight = range(max_samples,n_samples)
        
        program.raw_fitness_ = program.raw_fitness(X, y, score_data,curr_sample_weight,seeds[i])
        # program.raw_fitness_ = program.raw_fitness(X, y, score_data, curr_sample_weight)
        if max_samples < n_samples:
            # Calculate OOB fitness
            # program.oob_fitness_ = program.raw_fitness(X, y, score_data,oob_sample_weight)
            program.oob_fitness_ = program.raw_fitness(X, y, score_data,oob_sample_weight)
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def fit(self, X, y, score_data,sample_weight=None):
        ...
        # print(self._function_set,self._fixed_function_set,self._ts_function_set)
        ...
        if isinstance(self.metric, _Fitness):
            print(self.metric,'in _Fitness')
            self._metric = self.metric
        elif isinstance(self, RegressorMixin):
            if self.metric not in ('mean absolute error', 'mse', 'rmse',
                                    'pearson', 'spearman'):
                print('Unsupported metric1: %s' % self.metric)
            # if self.metric not in ('mean absolute error', 'mse', 'rmse',
                                   # 'pearson', 'spearman'):
            ...
            if self.metric != 'log loss':
                raise ValueError('Unsupported metric2: %s' % self.metric)
            self._metric = _fitness_map[self.metric]
        elif isinstance(self, TransformerMixin):
            if self.metric not in ('pearson', 'spearman'):
                raise ValueError('Unsupported metric3: %s' % self.metric)
            self._metric = _fitness_map[self.metric]
        ...
        for gen in range(prior_generations, self.generations):
            ...
            if self.test_mode == 0:
                ...
                for i in range(n_jobs):
                    ...
                    #     xxx = 1
                    #     break
                    population.append(_parallel_evolve(n_programs[i],
                                              parents,
                                              X,
                                              y,
                                              score_data,
            ...
            fitness2 = [program.oob_fitness_ for program in population]#更改样本外fitness算法
            # print(len(fitness),fitness)
            ...
            if self.parsimony_coefficient == 'auto':
                ...
                parsimony_coefficient = max(parsimony_coefficient,0)#修改,保持惩罚系数大于0
            elif isinstance(self.parsimony_coefficient,str):
            ...
            self.run_details_['best_oob_fitness'].append(np.max(fitness2))
