...
def _base_score_month(signal, close, fee=0.003, trade_delay=0):
    """
    策略回测打分,月频,当天收盘价成交，计算每笔平均收益
    signal: 每日信号,np.array,shapes:[n]
    close:每日收盘价,np.array,shapes:[n]
    fee:手续费，双边
    trade_delay:交易延迟天数，默认为1，当天计算信号，第二天以收盘价成交
    ...
    for i in range(1,len(r_array)):
        temp_r = r_array[i]  / r_array[i - 1] - 1
        if temp_r >= 0.0001:
            win_list.append(temp_r)
        elif temp_r <= -0.0001:
            lose_list.append(temp_r)

    n_win = len(win_list)
    n_lose = len(lose_list)
    n_all = len(r_array)
    if n_win + n_lose == 0:
        win_ratio = 0
    else:
        win_ratio = n_win / (n_win + n_lose)

    r_list = win_list + lose_list
    r_list.sort()

    if len(r_list) < 10:
        r_sum = len(r_list) * -0.1
    else:
        n_top = round(len(r_list) * 0.1)
        r_sum = np.nansum(r_list[n_top:-n_top])

    return r_sum * 12 / n_all,win_ratio
        # if r_array[i] > last_high:
        #     last_high = r_array[i]
        #     lose_index[0] = i
        # temp_r = r_array[i] / last_high - 1
...
def _base_score3(signal, close, fee=0.003, trade_delay=1):
    """
    策略回测打分，以夏普比为最终评分标准
    signal: 每日信号,np.array,shapes:[n]
    close:每日收盘价,np.array,shapes:[n]
    ...
    r_annual = (r_array[-1] / r_array[0]) ** (252 / n) - 1
    vol = np.nanstd(r_array,ddof = 1)
    if r_annual <= 0.03:
        adjust_sharp = r_annual
    else:
        adjust_sharp = (r_annual - 0.03) / (vol + 0.01)
    return adjust_sharp
...
def _my_score_return_only_month(y, y_pred, w):  # 只看每笔收益中位数
    """
    策略回测打分
    y_pred: X_Train经过遗传函数处理后的值,np.array,shapes:[n]
    y:和y_pred对应的目标值,默认为每日收盘价,np.array,shapes:[n]
    ...
    """
    y_pred = y_pred.reshape(-1, )
    signal = _create_signal_by_percentile(y_pred,period = 12)
    y_pred = y_pred[w]
    y = y[w]
    signal = signal[w]
    ...
    # print(y[0])
    close = y
    fee, trade_delay = 0.003, 0
    interval = 252
    # print(close[0])
    r_annual,win_ratio = _base_score_month(signal, close, fee=fee,trade_delay=trade_delay)

    final_score = r_annual
    return final_score
...
def _my_score_fixed_sharp(y, y_pred, w):
    """
    根据分位数生成买卖信号
    x: np.array,shapes:[n]
    up:分位数阈值上限,0-100
    ...
    fee, trade_delay = 0.003, 1
    interval = 252
    score = _base_score3(signal, close, fee=fee,trade_delay=trade_delay)
    return score
...
my_score_fixed_sharp = _Fitness(function=_my_score_fixed_sharp,greater_is_better=True)


my_score_return_only_month = _Fitness(function=_my_score_return_only_month,greater_is_better=True)
...
_fitness_map = {'pearson': weighted_pearson,
                ...
                'my_score_fixed_sharp':my_score_fixed_sharp,
                'my_score_return_only_month':my_score_return_only_month,
