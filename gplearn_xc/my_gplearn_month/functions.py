...
def _clear_inf(temp):
    index1 = np.where(temp == np.inf)[0]
    index2 = np.where(temp == -np.inf)[0]
    for x in index1:
        if x == 0:
            temp[x] = np.nan
        else:
            y = temp[max(0,x - 252):x]
...
def _fun_fillna(temp):
    index = np.where(np.isnan(temp))[0]
    for x in index:
        if x == 0:
            pass
        else:
...
def _protected_fun(x):
    return _fun_fillna(_clear_inf(x))
...
def _plus(a, b):
    return _protected_fun(np.add(a,b))


def _minus(a, b):
    return _protected_fun(np.subtract(a,b))

def _mul(a, b):
    return _protected_fun(np.multiply(a,b))


def _p_div(a, b):
    with np.errstate(divide='ignore', invalid='ignore'):
        temp = np.where((np.abs(a) == 0) & (np.abs(b) == 0), np.nan, np.divide(a,b))
        return _protected_fun(temp)
...
def _log(a):
    with np.errstate(divide='ignore', invalid='ignore'):
        temp = np.where(a < 0, -np.inf, np.log(a))
        return _protected_fun(temp)
...
def _mean2(a, b):
    return _plus(a,b) * 0.5

def _mean3(a, b, c):
    return _plus(_plus(a,b),c) / 3
...
def _ts_ta_beta(a,b,n):
    a1 = _fun_fillna(a)
    a2 = _fun_fillna(b)
    if np.all(np.isnan(a1)) or np.all(np.isnan(a2)):
        return np.ones_like(a) * np.nan
    return ta.BETA(a1,a2,n)

def _ts_ta_dema(a,n):
    if np.all(np.isnan(a)):
        return np.ones_like(a) * np.nan
    # return ta.DEMA(a,n)
    return ta.DEMA(a,max(5,n))

def _ts_ta_kama(a,n):
    if np.all(np.isnan(a)):
        return np.ones_like(a) * np.nan
    # return ta.KAMA(a,n)
    return ta.KAMA(a,max(5,n))
...
def _ts_ta_hma(a, n):
    ...
    temp = _protected_fun(2 * ta.WMA(a, int(n / 2))) - _protected_fun(ta.WMA(a, n))
    temp = _protected_fun(temp)
    if np.all(np.isnan(temp)):
        return np.ones_like(a) * np.nan
    return _protected_fun(ta.WMA(temp, int(n ** 0.5)))
...
def _ts_delta(a, n):
    b = np.ones_like(a) * np.nan
    for i in range(n,len(a)):
        b[i] = a[i] - a[i - n]
    return _protected_fun(b)

def _ts_corr_n(a, b, n):
    return _fun_fillna(pd.Series(a).rolling(n).corr(pd.Series(b)).values)
...
def _ts_prod(a, n):
    return _protected_fun(pd.Series(a).rolling(n).apply(lambda x:x.prod()).values)
...
def _ts_emals(a,b,n):
    ...
    beta = _p_div(_protected_fun(abw-aw*bw), _protected_fun(a2w-aw*aw))
    return _protected_fun(beta)
...
ts_ta_hma1 = _Function(function=_ts_ta_hma, name='ts_ta_hma', arity=1, is_ts=True)
ts_delay1 = _Function(function=_ts_delay, name='ts_delay', arity=1, is_ts=True)
ts_stddev1 = _Function(function=_ts_stddev, name='ts_stddev', arity=1, is_ts=True)
ts_delta1 = _Function(function=_ts_delta, name='ts_delta', arity=1, is_ts=True)
