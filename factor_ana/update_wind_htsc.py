            ...
            if self.table_name in self.over_write_list:
                try:
                    ...
                    if self.table_name == 'WIND_AIndexMembers':
                        ...
                        df = s.get_factor_value(self.table_name, S_ENDDATE=['>=20170101']) \
                        .append(s.get_factor_value(self.table_name, S_ENDDATE=['<20170101']))
                    elif self.table_name == 'WIND_IPOInquiryDetails':
                        df = s.get_factor_value(self.table_name, ANN_DT=['>=20190902']) \
                        ...
                        df = s.get_factor_value(self.table_name, OPDATE=['>=20190825']) \
...
def second_job(sdate = None, edate = None):
    ...
    daily_list =  ['WIND_AShareAnnInf','WIND_ASharePledgeproportion', 'WIND_AShareFFCalendar',
                     'WIND_CMFundAssetPortfolio', 'WIND_CMFOtherPortfolio', 'WIND_CMFundIndPortfolio','WIND_CMFundStockPortfolio',
                     'WIND_CMFundBondPortfolio', 'WIND_AShareMarginSubject','WIND_AShareBlockTrade', 
    ...
    qtr_list = ['WIND_FinNotesDetail','WIND_AShareIBrokerIndicator',
                ...
                'WIND_Top5ByLongTermBorrowing','WIND_AshareOthreceivables','WIND_AShareFinExpense',
                'WIND_AshareInventorydetails','WIND_AshareFinaccounts','WIND_Top5ByAccReceivable','WIND_AShareAuditOpinion',
                'WIND_AShareSalesSegment','WIND_Top5ByOperatingIncome']
               #daily_list + new_daily_list +  tech_daily_list
    table_dict = {'DAILY': daily_list + new_daily_list , 'QUARTERLY': qtr_list}


    for table_type in table_dict:
        for table_name in table_dict[table_type]:
            print (table_name)
            instantiate(table_name, sdate, edate, base_path = '/data/group/800080/warehouse/test/', operation = 'append',table_dict=table_dict,update_h5_flag=True)
    
    logger.info('===============2nd job upload finish======================')
    
def late_job(sdate = None, edate = None):
    sdate,edate,cdate_list = check_update_date(sdate = sdate, edate = edate)
    qtr_list = []
    daily_list =  ['WIND_AShareMarginTrade', 'WIND_AShareMarginTradeSum']
    ...
    logger.info('===============late_job upload finish======================')
...
def fix_problem_job(sdate = None, edate = None):
    sdate,edate,cdate_list = check_update_date(sdate = sdate, edate = edate)

    tech_daily_list = [ 'WIND_AShareAnnInf']
...
if not os.path.exists(flag_root):
    os.makedirs(flag_root)
flag_path_start = flag_root + str(edate) + '_' + 'tech_daily.start'
...
flag_path_success = flag_root + str(edate) + '_' + 'tech_daily.success'
with open(flag_path_success,'w') as file:
    pass 
    
# fix_problem_job(20190805, 20200107)
