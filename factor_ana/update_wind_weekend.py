...
class WIND_DATABASE:
    def __init__(self, table_name, sdate, edate, 
        ...
        if self.dfreq=='DAILY':
            # self.cdate_list = [self.sdate, self.edate]
            dayspd = pd.date_range(str(self.sdate),str(self.edate)).tolist()
            days = [str(x)[:10].replace('-','')  for x in dayspd]
            
            _,_,tradingdays = check_update_date(self.sdate, self.edate)
            
            tdays = [str(x) for x in tradingdays]
            
            c = list(set(days) - set(tdays))
            c.sort()
            
            # dayspd1 = pd.date_range('20200208','20200209').tolist()
            # days1 = [str(x)[:10].replace('-','')  for x in dayspd1]
            ...
            if self.table_name in self.over_write_list:
                try:
                    ...
                    if self.table_name == 'WIND_AIndexMembers':
                        ...
                        df = s.get_factor_value(self.table_name, ANN_DT=['>=20200515']) \
                        .append(s.get_factor_value(self.table_name, ANN_DT=['>=20200101','<20200515'])) \
...
def second_job_sunday(sdate = None, edate = None):
    # sdate,edate,cdate_list = check_update_date(sdate = sdate, edate = edate)


    daily_list =  ['WIND_AShareAnnInf','WIND_AShareFreeFloatCalendar', 'WIND_AShareMarginSubject', 'WIND_AShareStrangeTradedetail', 
                'WIND_AShareStrangeTrade', 'WIND_AShareHolderNumber', 'WIND_AShareFloatHolder', 'WIND_AShareEXRightDividendRecord', 
                'WIND_AShareCompRestricted', 'WIND_AShareEquityPledgeInfo', 'WIND_AShareInsideHolder', 'WIND_BOIndexWeightsWIND', 
                'WIND_AShareStyleCoefficient', 'WIND_AShareTradingSuspension', 'WIND_AShareIllegality', 'WIND_AShareMajorEvent', 
                'WIND_ASarePlanTrade']

    qtr_list = ['WIND_ChinaMutualFundAssetPortfolio', 'WIND_CMFOtherPortfolio', 'WIND_ChinaMutualFundIndPortfolio', 
                'WIND_ChinaMutualFundStockPortfolio', 'WIND_ChinaMutualFundBondPortfolio', 'WIND_CMMFPortfolioPTM', 
                'WIND_CFundPortfoliochanges']

    daily_list2 = ['WIND_AShareIndustriesCode', 'WIND_AShareCapitalization', 'WIND_AShareFreeFloat', 'WIND_AShareAgency', 
                'WIND_AShareCOCapitaloperation', 'WIND_AshareStockRepo', 'WIND_AShareCorporateFinance', 'WIND_AShareIssueCommAudit', 
                'WIND_AShareEquityDivision', 'WIND_IPOCompRFA', 'WIND_IECMemberList', 'WIND_AShareLeadUnderwriter', 'WIND_IPOInquiryDetails', 
                'WIND_AShareIncDescription', 'WIND_AShareIncQuantityPrice', 'WIND_AShareIncQuantityDetails', 'WIND_AShareIncExercisePct', 
                'WIND_AShareIncExecQtyPri', 'WIND_AShareEsopDescription', 'WIND_AShareEsopTradingInfo', 'WIND_AShareStaffStructure', 
                'WIND_AShareMajorHolderPlanHold', 'WIND_AShareTypeCode', 'WIND_htzqedbdzzbs', 'WIND_AShareMainandnoteitems']
                
    table_dict = {'DAILY': daily_list + daily_list2, 'QUARTERLY': []}
    ...
    logger.info('===============2nd sunday job upload finish======================')
    

    
# self.cdate_list 每次生成一周内的非交易日
edate = int(dt.datetime.now().strftime('%Y%m%d'))
sdate = int((dt.datetime.now() + dt.timedelta(days = -7)).strftime('%Y%m%d'))
