...
h5_path_listing_delisting = '/data/group/800080/warehouse/prod/ETC/CHINA_STOCK/WIND/STOCK_LISTING_DELISTING_DATE.h5'

#h5_path_listing_delisting = 'Z:\\warehouse\\prod\\ETC\\CHINA_STOCK\\WIND\\STOCK_LISTING_DELISTING_DATE.h5'

path_dict = {'wind':r'/data/group/800080/warehouse/prod/DATABASE/WIND',
             'derived':r'/data/group/800080/warehouse/prod/DATABASE',
             'suntime':'/data/group/800080/warehouse/prod/DATABASE/SUNTIME'}
flag_path_data = r'/data/group/800080/warehouse/prod/LOCAL_DATA/FLAG'

def flag_check(ftype, dt, flag_path=flag_path_data):
    if os.path.exists(flag_namer(ftype, dt, flag_path)):
        return True
    else:
        return False

def flag_namer(ftype, dt,flag_path=flag_path_data):
    dt = IO.str_date_parser(dt).strftime('%Y%m%d')
    flag_str = dt + '_' + ftype.upper() + '.success'
    return os.path.join(flag_path, dt, flag_str)

def flag_wait(ftypes, dt, flag_path=flag_path_data, gap=600, expiration=None):
    ...
    while True:
        check_list = list()
        for ftype in ftypes:
            check_list.append(flag_check(ftype, dt,flag_path))
        if all(check_list):
            break
...
def factor_tank_date_slice(factor_tank,cdate_list):
    print ('slice date range: %s - %s'%(cdate_list[0],cdate_list[-1]))
    ...
    for fac in factor_tank:
        ...
        if lack_num>0:
            print ('%s - has no data for %d days: %s - %s '%(fac,lack_num,date_diff[0],date_diff[-1]))
            raise Exception
    return factor_tank_sliced


def dict_slicer(factor_tank,sdate,edate):
    sdate,edate = IO.str_date_parser(sdate),IO.str_date_parser(edate)
    factor_tank_sliced = {}
    for fac in factor_tank:
...
def save_factor_tank(factor_tank,save_path,append,from_scratch,cdate_list=None):
    ...
    for i in range(factor_num):
        ...
        print ('%d/%d - %s'%(i+1,factor_num,factor_name))
        data = pd.DataFrame(factor_tank[factor_name].stack(), columns=[factor_name])
        data.index.names = ['dt', 'Ticker']
        IO.pd_hdf5_writer(data, save_path, factor_name,append=append)
    return
...
def standard_process(pd_raw, stock_filter=None, stock_industry=None, winsor=True, weight=None,date_complete=False):
    ...
    if lack_num>0:
        print('data empty for %d days: %s - %s'%(lack_num,date_lack[0],date_lack[-1]))
        if date_complete:
            raise Exception
...
def get_current_date(new_date_time=18):
    ...
    fdate_list_dt = IO.read_data([20000101, 20300101], ftype=FType.CALENDAR).index.get_level_values(0)
...
def multiprocess_wrapper(func,iter_list=None,logger=None,max_process=None,
                         collect_output=False,collect_status=False,**kwargs):
...
def calc_dividend_yield(sdate,edate,dividend_data,alpha_universe,mkt_cap_ard):
    print ('calc dividend yield: %d - %d'%(sdate,edate))
    start_date = IO.str_date_parser(sdate)
    end_date = IO.str_date_parser(edate)
    # calculate dividend
    div_widow = 400
    div_cnt_window = 252
    maks_fill_limit = 600
    ...
    sdate_prev = tdt.get_trading_day_offset(sdate, -1*min_window)[0]
    
    print ('calc dividend ex')
    div_dat_ex = easy_fill(dividend_data[['dividend_amount','EX_DT']], sdate_prev, edate,foward_date='EX_DT',fill_method=None)
...
def calc_df_ts_roll(func,roll_num,y_df,x_df=None,min_pct=0.5,verbose=False,handle_nan=True):
    ...
    for stk_name in stk_list:
        if verbose:
            ...
            if idx%100==0:
                toc = time.time()
                print ('%d/%d - %fs'%(idx,stk_num,round((toc-tic),2)))
        y_pd = y_df[stk_name]
        if x_type=='stk':
            x_pd = x_df[[stk_name]]
        elif x_type=='macro':
