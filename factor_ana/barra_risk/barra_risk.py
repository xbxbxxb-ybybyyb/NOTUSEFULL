...
def flag_touch(dtype,dt,flag_path):
    """ path\dt\dt_dtype.success"""
    dt = str(dt) if type(dt) is not str else dt
    path_dt = os.path.join(flag_path,dt,'%s_%s.success'%(dt,dtype.upper())) 
    if not os.path.exists(os.path.dirname(path_dt)):
        os.makedirs(os.path.dirname(path_dt))
    Path(path_dt).touch(exist_ok=False)
    return 
...
class UpdateBarraRisk:
    def __init__(self, sdate, edate, operation='append', 
                 ...
                 root_path=r'/data/group/800080/warehouse/prod',
                 parallel=True,
                 md_path=None,
                 fdd_path=None,
                 style_path=None,
        ...
        if style_path is None: 
            self.style_path = IO.path_assembler(self.mkttype, self.dtype, self.ftype, self.dfreq, dsource = self.dsource,
                                                dtable=None, alt=None, h5root=root_path)
        else:
            self.style_path = style_path
        
        if descriptor_path is None:         
            self.descriptor_path = IO.path_assembler(self.mkttype, self.dtype, self.ftype, self.dfreq, dsource=DSource.DESCRIPTOR,
                                                     dtable=None, alt=None, h5root=root_path)
        else:
            self.descriptor_path = descriptor_path

        for i in [self.root_path,self.style_path,self.descriptor_path,self.log_name,self.fdd_path,self.md_path,self.process_path]:
            path = os.path.dirname(i)
            if not os.path.exists(path):
                os.makedirs(path)

        # logger setup
        self.logger = logging.getLogger('BARRA_RISK')
        self.logger.setLevel(logging.DEBUG)
        file_handler = logging.FileHandler(self.log_name, mode='a')
        file_handler.setFormatter(logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s'))
        self.logger.addHandler(file_handler)
        self.logger.info('*'*40)
        self.logger.info('*** BARRA RISK UPDATE : %s - %s (%s days) ***'%(self.sdate,self.edate,len(self.cdate_list)))
        # stage variable
    ...
    def check_data_flag(self):
        self.logger.info('* Check Data Flag: %s *' % (self.edate))
        flag_wait(['MD','FDD','RDF','UNIV'], self.edate) 
        self.logger.info('* Check Data Flag Done *')
        return 
        
    def load_basic_data(self):
        # load data with long look back days
        self.logger.info('* Loading Basic Data: %s - %s *' % (self.sdate_prev, self.edate))
        tic = time.time()
        ...
        self.logger.info('Getting universe data')
        ...
        self.logger.info('Getting risk universe data')
        risk_univ = IO.read_data([self.sdate_prev, self.edate], columns=[risk_univ_name], ftype=FType.UNIV, dsource=DSource.OPTM)
        ...
        self.logger.info('Getting industry data')
        h5_ind = IO.read_data([self.sdate_prev, self.edate], ftype=FType.INDUSTRY, dsource=DSource.WIND, columns=[industry_name])
        ...
        self.logger.info('Getting benchmark data')
        index_lookup = {'zz500': '000905.SH', 'zz800': '000906.SH', 'sz50': '000016.SH', 'hs300': '000300.SH'}
        h5_index = IO.read_data([self.sdate_prev, self.edate], 'close', ftype=FType.MD, dtype=DType.INDEX,dsource=DSource.WIND)        
        ...
        toc = time.time()
        self.logger.info('* Load Basic data Done %s*'%(print_time(toc,tic)))

    # load fdd data
    def load_fdd_data(self):        

        
        self.logger.info('* Loading FDD Data: %s - %s *'%(self.sdate_qtr, self.edate_qtr))
        tic = time.time()
        ...
        self.logger.info('Getting backfill prep data')
        ...
        data_dividend['dividend_amount'] = data_dividend['CASH_DVD_PER_SH_PRE_TAX'] * data_dividend['S_DIV_BASESHARE'] *1e4

        #dividend_data = dividend_data[dividend_data['S_DIV_PROGRESS']==3]
        #div_dat = easy_fill(dividend_data[['CASH_DVD_PER_SH_PRE_TAX','EX_DT']],self.sdate_prev, self.edate,foward_date='EX_DT',fill_method=None)
        #dividend_per_share = (div_dat * total_shares.shift(1)).fillna(0).rolling(240).sum()
        ...
        self.logger.info('Getting fdd table data')
        # eod daily - no need for getting prev data
        data_eod_derive = IO.read_data([self.sdate, self.edate], columns=eod_derive_list, dsource=DSource.WIND, alt=path_dict['AShareEODDerivativeIndicator'])

        data_balance_full = IO.read_data([sdate_qtr, edate_qtr], columns=balance_list+['STATEMENT_TYPE'], dsource=DSource.WIND, alt=path_dict['AShareBalanceSheet'])
        data_income_full = IO.read_data([sdate_qtr, edate_qtr], columns=income_list+['STATEMENT_TYPE'], dsource=DSource.WIND, alt=path_dict['AShareIncome'])
        data_fin_ind = IO.read_data([sdate_qtr, edate_qtr], columns=ind_list, dsource=DSource.WIND, alt=path_dict['AShareFinancialIndicator'])
        data_ttm = IO.read_data([sdate_qtr, edate_qtr], columns=ttm_list, dsource=DSource.WIND, alt=path_dict['AShareTTMHis'])

        data_cashflow_full = IO.read_data([sdate_qtr, edate_qtr], columns=cashflow_list+['STATEMENT_TYPE','ANN_DT'], dsource=DSource.WIND, alt=path_dict['AShareCashFlow'])
        ...
        #fdd_data_qtr_dict = {k:{c:fdd_data_qtr_mi[k][c].unstack() for c in fdd_data_qtr_mi[k]} for k in fdd_data_qtr_mi}
        ...
        toc = time.time()
        self.logger.info('* Load FDD Data Done %s*'%(print_time(toc,tic)))
        return
        
    
    def read_data(self):
        self.logger.info('* Read Data From Existing Pickle *')
        self.logger.info('MD data: %s'%(self.md_path))
        self.logger.info('FDD data: %s'%(self.fdd_path))
        ...
        lack_list.sort()
        if len(lack_list)>0:
            self.logger.error('* read data wrong: lack - %s*'%(str(lack_list)))
            raise Exception
        self.boilerplate = boilerplate.reindex(index=dates_index)  # placeholder for newly generated factors
        self.iter_num = len(self.boilerplate.index)
        ...
        self.logger.info('* Read Data Done *')
        return 
    ...
    def baseline_process(self, pd_raw):
        ...
        if len(date_lack)>0:
            if self.operation == 'append':
                self.logger.warning('data empty for one day')
                raise Exception
            else:
                self.logger.warning('data not exist for %d days: %s - %s'%(len(date_lack),date_lack[0],date_lack[-1]))
    ...
    def boilerplate_pd(self, np_raw):
        ...
        if row_num_np!=row_num_bp or col_num_np!=col_num_bp:
            self.logger.info('boilerplate_pd: dimension not matching')
            raise Exception
    ...
    def risk_beta(self, half_life=252, look_back_days=504, smooth_num=4,ma_alpha=11,lag_alpha=11):
        self.logger.info('Update Risk Factor: Market Beta & Residual Volatility')
        take_idx = self.iter_num + look_back_days + ma_alpha + lag_alpha + 1
        if len(self.data['excess_return']) < take_idx:
            self.logger.warning('beta: data not enough')
            raise Exception
    ...
    def risk_book_to_price(self):
        ...
        self.logger.info('Update Risk Factor: Value')
    ...
    def risk_dividend_yield(self):
        self.logger.info('Update Risk Factor: Dividend Yield')
    ...
    def risk_earnings_quality(self):
        ...
        self.logger.info('Update Risk Factor: Earnings Quality')
    ...
    def risk_earnings_variability(self):
        self.logger.info('Update Risk Factor: Earnings Variability')
    ...
    def risk_earnings_yield(self):
        ...
        """
        self.logger.info('Update Risk Factor: Earnings Yield')
    ...
    def risk_growth(self):
        ...
        total_shares = self.data['total_shares']
        self.logger.info('Update Risk Factor: Growth')
    ...
    def risk_investment_quality(self):
        self.logger.info('Update Risk Factor: Investment Quality')
        lookback_qtr = self.fdd_lookback_qtr
        min_periods = int(self.fdd_lookback_qtr * self.fdd_min_pct)
    ...
    def risk_leverage(self):
        ...
        """
        self.logger.info('Update Risk Factor: Leverage')
    ...
    def risk_liquidity(self, window=21, look_back_terms=12):
        self.logger.info('Update Risk Factor: Liquidity')
        ...
        if date_num < window * look_back_terms + self.iter_num - 1:
            self.logger.warning('stock return date number less than required')
            raise AssertionError
    ...
    def risk_long_term_reversal(self, look_back_days=252, smooth_num=4, ma_window=11):
        self.logger.info('Update Risk Factor: Long Term Reversal')
        ...
        if len(self.data['excess_return']) < min_num:
            self.looger.warning('risk: long term reversal - data not enough')
            raise Exception
    ...
    def risk_mid_cap(self):
        self.logger.info('Update Risk Factor: Mid Capitalization')
        mkt_cap_ard = self.data['mkt_cap_ard']
    ...
    def risk_momentum(self, look_back_days=252, ma_window=11, lag=11, half_life=126):
        self.logger.info('Update Risk Factor: Momentum')
    ...
    def risk_profitability(self):
        self.logger.info('Update Risk Factor: Profitability')
    ...
    def risk_residual_volatility(self, half_life=42, look_back_days=252):
        self.logger.info('Update Risk Factor: Residual Volatility')
    ...
    def risk_size(self):
        self.logger.info('Update Risk Factor: Size')
        mkt_cap_ard = self.data['mkt_cap_ard']
    ...
    def update_descriptor(self):
        self.logger.info('*'*40)
        self.logger.info('* Update Descriptor *')
        tic = time.time()
        ...
        self.logger.info('Process Descriptor')
        des_list = list(self.raw.keys())
        des_num = len(des_list)
        
        if self.parallel and self.operation=='append':
            ...
            for des in self.raw:
                self.logger.info('%d/%d - %s'%(des_list.index(des)+1,des_num,des))
        ...
        toc = time.time()
        self.logger.info('* Descriptor Process Done %s*'%(print_time(toc,tic)))
        return 


    def update_style(self):        
        self.logger.info('*'*40)
        self.logger.info('* Update Style *')
        tic = time.time()
        ...
        self.logger.info('Process Style')
        ...
        if self.parallel and self.operation=='append':
            ...
            for s in self.style:
                self.logger.info('%d/%d - %s'%(style_list.index(s)+1,style_num,s))
        ...
        self.risk_industry()
        toc = time.time()
        self.logger.info('* Update Style Done %s*'%(print_time(toc,tic)))
        return 
        if self.operation=='create' and os.path.exists(self.descriptor_path):
            self.logger.info('Path existed already - remove first - %s'%(self.descriptor_path))
            os.remove(self.descriptor_path)
        
        if self.operation=='create' and os.path.exists(self.style_path):
            self.logger.info('Path existed already - remove first - %s'%(self.style_path))
            os.remove(self.style_path)        

    def save_factor(self):
        self.logger.info('*'*40)
        self.logger.info('* Save Factor *')
        tic = time.time()
        self.logger.info('Saving Descriptor to : %s'%(self.descriptor_path))
        save_factor_tank(self.descriptor,self.descriptor_path,self.append_mode,self.from_scratch,self.cdate_list)

        self.logger.info('Saving Style to : %s'%(self.style_path))
        save_factor_tank(self.style,self.style_path,self.append_mode,self.from_scratch,self.cdate_list)
        toc = time.time()
        self.logger.info('* Save Factor Done - %s *'%(print_time(toc,tic)))

        flag_touch('RISK',self.edate,self.flag_path)
        self.logger.info('* RISK Flag Generated : %s *'%(self.edate))
    ...
    def launch(self):
        ...
        self.logger.info('*** BARRA Risk descriptor Done - %s ***'%(print_time(destime,tic)))
        self.logger.info('*** BARRA Risk style Done - %s ***'%(print_time(stytime,destime)))
        self.logger.info('*** BARRA Risk calculate Done - %s ***'%(print_time(stytime,tic)))
        self.logger.info('*** BARRA Risk Save Done - %s ***'%(print_time(toc,stytime)))
        self.logger.info('*** BARRA Risk Update Done - %s ***'%(print_time(toc,tic)))
    ...
    def load_data(self):
        self.logger.info('*'*40)
        self.logger.info('* Prep Data: %s - %s *'%(self.sdate_prev, self.edate))
        tic = time.time()
        ...
        toc = time.time()
        self.logger.info('* Prep Data Done %s *'%(print_time(toc,tic)))
...
if __name__ == '__main__':
    ...
    # sdate,edate,cdate_list = check_update_date(20060101,20060201)
    sdate,edate,cdate_list = check_update_date()
      
    self = UpdateBarraRisk(sdate, edate, operation='append')
    
    print('check_data_flag(self)')
    self.check_data_flag()
    print('check finished')
    
    flag_root = '/data/group/800080/warehouse/prod/LOCAL_DATA/FLAG/' + str(edate) + '/'
    
    flag_path_start = flag_root + str(edate) + '_' + 'RISK.start'
    ...
    print('total time using: ', time.time() - start_time)
    print('load data time: ', read_start_time - load_start_time)
    print('read data time: ', launch_start_time - read_start_time)
