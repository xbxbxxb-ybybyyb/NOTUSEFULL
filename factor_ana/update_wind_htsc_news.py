...
class WIND_DATABASE:
    def __init__(self, table_name, sdate, edate, 
        ...
        if self.dfreq=='DAILY':
            # self.cdate_list = cdate_list
            c = pd.date_range(start=str(self.sdate),end=str(self.edate))
          
            self.cdate_list = [int(datetime.datetime.strftime(x, '%Y%m%d')) for x in c]
            ...
            for fname in update_list:
                ...
                if len(dat1)<=min_size:
                    ...
                    if self.table_name == 'WIND_AShareProfitNotice':
                        ...
                        h5_store.append(self.table_name,dat, data_columns=True,min_itemsize={'N_INFO_TITLE':1500,'N_INFO_FCODE':500,'N_INFO_STOCKID':20,'N_INFO_COMPANYID':20})
    ...
    def get_df(self, date):
        ...
        elif self.table_name in ['WIND_FinancialNews']:
            df = s.get_factor_value(self.table_name, PUBLISHDATE=[str(date)])
        else:
...
def news_job(sdate = None, edate = None):
    # c = pd.date_range(start=sdate,end=edate)
    # cdate_list = [int(datetime.datetime.strftime(x, '%Y%m%d')) for x in c]
    
    if sdate == None:
        sdate = int(str(datetime.datetime.now().date()).replace('-',''))
        edate = int(str(datetime.datetime.now().date()).replace('-',''))

    tech_daily_list = ['WIND_FinancialNews']
    ...
    for table_type in table_dict:
        for table_name in table_dict[table_type]:
            print (table_name)
            instantiate(table_name, sdate, edate, base_path = '/data/group/800080/warehouse/prod/', operation = 'append',table_dict=table_dict,update_h5_flag=False)
    
    logger.info('===============news_job finish======================')
    

# 20191226  20200108 20191227
news_job()
