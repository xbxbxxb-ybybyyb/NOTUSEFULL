...
class FactorGtjaAlpha057_n_tick10 : public Factor<> {
...
public:
  FactorGtjaAlpha057_n_tick10() { type_ = __func__; }
  ...
  Status caculate() override {
    ...
    auto distance =
        max_high_window == min_low_window ? 0.0 : (lastpx_now - min_low_window) / (max_high_window - min_low_window);
    distance_window_list.push(now_time, distance);

    value() = compute::mean(distance_window_list);
    return Status::OK();
  }
};
