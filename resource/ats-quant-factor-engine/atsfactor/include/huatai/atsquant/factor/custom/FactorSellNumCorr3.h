...
public:
  ...
  Status caculate() override {
    if (nonfac->num_asks_sec_list.size() < 5) {
      value() = 0;
    } else {
      // simple regression
      SimpleRegression regression;
      auto ask_num_list = nonfac->num_asks_sec_list.prev_n(lag1);
      for (int i = 1; i <= std::min(nonfac->num_asks_sec_list.size(), (size_t)lag1); ++i) {
        regression.addData(i, ask_num_list[i - 1]);
      }
