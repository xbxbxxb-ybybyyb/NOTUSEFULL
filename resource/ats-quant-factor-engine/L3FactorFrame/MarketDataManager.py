...
def merge_one_order_trade(symbol, order_df, trade_df, cancel_df, tick_df, use_pandas=False):
    if use_pandas:
        ...
        if symbol.endswith("SH"):
            ...
            # 过滤排除原始委托的SeqNo，只保留最后一笔成交的SeqNo
    ...
    # BUG: 一笔原始委托成交之后有剩余，还原原始委托groupby msg_buy_no只会保留一笔。但是Tick_df数据中同时有成交和委托多条记录。导致无法根据两条tick之间成交量的差推断该笔委托的成交；
...
# @profile
def get_l3_trade_order_data(symbol, date, merge_one_order = True, use_pandas=False, base_dir="/dfs/group/800657/library/l3_data", rename = True):
    try:
    ...
    if use_pandas:
        ...
        trade_df = trade_df.with_columns(
            Amount = pl.col("Price") * pl.col("Volume")
        )
        ...
        order_columns_map = {"msg_order_type": "OrderType",
            "msg_bsflag": "BSFlag", "msg_price": "Price",
            "msg_qty": "Volume", "last_seq_num": "SeqNo",
            "mdtime": "Timestamp", "msg_amt": "Amount"
            }
        trade_columns_map = {"msg_trade_type": "TradeType",
            "msg_bsflag": "BSFlag", "msg_price": "Price",
            "msg_qty": "Volume", "last_seq_num": "SeqNo",
            "mdtime": "Timestamp", "msg_amt": "Amount"
            }
        ...
        if merge_one_order:
            order_df, trade_df, tick_df = merge_one_order_trade(symbol, order_df, trade_df, cancel_df, tick_df, use_pandas)
        if not rename:
            reveser_order_columns_map = {value: key for key, value in order_columns_map.items()}
            reveser_trade_columns_map = {value: key for key, value in trade_columns_map.items()}
            reveser_tick_columns_map = {value: key for key, value in tick_columns_map.items()}

            order_df = order_df.rename(reveser_order_columns_map)
            cancel_df = cancel_df.rename(reveser_order_columns_map)
            trade_df = trade_df.rename(reveser_trade_columns_map)
            tick_df = tick_df.rename(reveser_tick_columns_map)
    return tick_df, order_df, trade_df, cancel_df


def get_l3_data_cpp(symbol, date, base_dir="/dfs/group/800657/library/l3_data"):
    tick_df, order_df, trade_df, cancel_df = get_l3_trade_order_data(symbol, date, merge_one_order = True, 
                                                            base_dir=base_dir, rename= False)
    order_df = order_df.drop("TradeVolume")
    if symbol.endswith("SH"):
        tick_df = tick_df.drop(["msg_order_type"])
    else:
        tick_df = tick_df.drop(["msg_trade_type", "msg_order_type"])
        order_df = order_df.with_columns(msg_trade_type = pl.lit(-1))
        cancel_df = cancel_df.with_columns(msg_order_type = pl.lit(-1))

    assert len(tick_df)==len(order_df)+len(cancel_df)
    order_cancel_df = pl.concat([order_df, cancel_df])
    tick_df = tick_df.join(order_cancel_df, on = 'last_seq_num')
    columns = [i for i in tick_df.columns if "right" not in i]
    tick_df = tick_df.select(columns).with_columns(pl.col("DateTime").dt.strftime("%Y%m%d%H%M%S%3f").alias("mdtime").cast(pl.Int64))
    tick_df = tick_df.sort(by = 'last_seq_num')
    tick_df = tick_df.with_columns(code_str = pl.lit(symbol))
    return tick_df
...
class MarketDataManager:
    ...
    def getPrevSampleFlag(self, seconds = 1):
        if seconds == 1:
            return self.tick_array[self.idx, self.tick_colidx["sample_1s_flag"]]
        else:
            raise Exception("当前仅支持1s采样的数据！")
    ...
    def getPrevSecTick(self, field, n_seconds, end_timestamp = None):
        """
        end_seconds: 时间区间末端, 若指定了将用end_timestamp替换self.last_timestamp作为截止时间往前搜索
        """
        if n_seconds >= 0:
            last_timestamp = self.last_timestamp if not end_timestamp else end_timestamp
            timestamp = self.tick_array[0:self.idx+1, self.tick_colidx["Timestamp"]]
            startIndex = np.searchsorted(timestamp, last_timestamp - n_seconds, side="right")
            endIndex = np.searchsorted(timestamp, last_timestamp, side="right") if end_timestamp else self.idx+1
            return self.tick_array[startIndex:endIndex, self.tick_colidx[field]]
        else:
            # 查询未来数据
    ...
    def getPrevSecOrder(self, field, n_seconds, end_timestamp = None):
        ...
        if n_seconds>=0:
            ...
            # 【？是否需要right】
            startIndex = np.searchsorted(timestamp, last_timestamp - n_seconds, side="right")
            endIndex = np.searchsorted(timestamp, last_timestamp, side="right") if end_timestamp else self.order_idx+1
            return self.order_array[startIndex:endIndex, self.order_colidx[field]]
        else:
            # 查询未来数据(不包括当前, 右闭)
    ...
    def getPrevSecTrade(self, field, n_seconds, end_timestamp = None):
        ...
        if n_seconds>=0:
            ...
            startIndex = np.searchsorted(timestamp, last_timestamp - n_seconds, side="right")
            endIndex = np.searchsorted(timestamp, last_timestamp, side="right") if end_timestamp else self.trade_idx+1
            return self.trade_array[startIndex:endIndex, self.trade_colidx[field]]
        else:
            # 查询未来数据(不包括当前)
    ...
    def getPrevSecCancel(self, field, n_seconds, end_timestamp = None):
        ...
        if n_seconds>=0:
            ...
            startIndex = np.searchsorted(timestamp, last_timestamp - n_seconds, side="right")
            endIndex = np.searchsorted(timestamp, last_timestamp, side="right") if end_timestamp else self.cancel_idx+1
            return self.cancel_array[startIndex:endIndex, self.cancel_colidx[field]]
        else:
            # 查询未来数据(不包括当前)
            timestamp = self.cancel_array[self.cancel_array+1:, self.cancel_colidx["Timestamp"]]
            endindex = np.searchsorted(timestamp, last_timestamp - n_seconds, side="right")
