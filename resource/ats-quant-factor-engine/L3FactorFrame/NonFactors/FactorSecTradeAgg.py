...
class FactorSecTradeAgg(FactorBase):
    ...
    def calculate(self):
        ...
        if sample_1s_flag == 1:
            ...
            trade_qtys = self.getPrevSecTrade("Volume", 1, end_timestamp=int(timestamps[-1]))
            trade_moneys = self.getPrevSecTrade("Amount", 1, end_timestamp=int(timestamps[-1]))
            trade_sides = self.getPrevSecTrade("BSFlag", 1, end_timestamp=int(timestamps[-1]))
