...
def analysis_total_trade_summray(daily_trade_summary, daily_market_df):
    result_list = []
    ...
    profit_10, profit_90 = backtest_result["回转盈亏"].sort_values().quantile([0.1, 0.9])
    profit_10_avg = backtest_result[backtest_result["回转盈亏"] <= profit_10]["回转盈亏"].mean()
    profit_90_avg = backtest_result[backtest_result["回转盈亏"] >= profit_90]["回转盈亏"].mean()

    profit_5, profit_95 = backtest_result["回转盈亏"].sort_values().quantile([0.05, 0.95])
    profit_5_avg = backtest_result[backtest_result["回转盈亏"] <= profit_5]["回转盈亏"].mean()
    profit_95_avg = backtest_result[backtest_result["回转盈亏"] >= profit_95]["回转盈亏"].mean()
    ...
    analysis_dict = {
                     ...
                     "日均收益_税后top5%": profit_95_avg,
                     "日均收益_税后bottom5%": profit_5_avg,
                     "日均收益_税后top10%": profit_90_avg,
                     "日均收益_税后bottom90%": profit_10_avg,
                     "最大回撤": mdd,
                     "最大回撤%": mdd_ratio * 100
