...
from ModelSystem.Util.CounterManager import CounterManager
...
class SignalEvaluate:
    ...
    def __init__(self, inputs: 'Input', signal_trigger_pairs: List[Tuple[pd.DataFrame, Dict[str, Dict[str, float]]]]):
        # 定义一些常量
        mockTradePara = inputs.mock_trade_para
        self.__STOP_LOSS = abs(mockTradePara["maxLoss"])  # 止损线参数（不考虑手续费）
        ...
        self.__freq_tick_dict = inputs.freq_tick_dict
        self.__signal_trigger_paris: List[Tuple[pd.DataFrame, Dict[str, Dict[str, float]]]] = signal_trigger_pairs
        for signal_data, trigger_dict in self.__signal_trigger_paris:
            ...
            elif len(signal_data.columns) != 4:
                raise Exception('There are NOT four columns in the data frame: {}'.format(signal_data.columns.tolist()))
        ...
        self.__counterMgr = CounterManager(self.__symbol)
        self.__exchangeHouse = ExchangeHouse(Data(self.__tick, self.__transaction), inputs.use_l2p)
        ...
        for freq in self.__freq_list:
            self.__executorDict[freq] = getattr(self.__modules, inputs.executor_str)(self.__positionManager, self.__riskMgr, self.__counterMgr)
            self.__executorDict[freq].set_data_dict(self.__data_dict.get(freq))
    ...
    def evaluate(self, show=None):
        for freq in self.__freq_list:
            self.__executorDict[freq].set_json_param_before_start(self.__order_capacity)
        ...
        if triggerTimes > 0:
            for order in TradingOrder["order"]:
                if show:
                    print(order)
                # 计算持仓时间(min)
                startTime = dt.datetime.strptime(order["startTime"], "%m/%d/%Y-%H:%M:%S:%f")
                endTime = dt.datetime.strptime(order["endTime"], "%m/%d/%Y-%H:%M:%S:%f")
    ...
    def runBackTest(self, show):
        ...
        for signals, trigger_dict in self.__signal_trigger_paris:
            ...
            for freq in self.__freq_list:
                self.__executorDict[freq].generateTriggerRatio(self.__symbol, trigger_dict[freq], self.__freq_tick_dict[freq])

            for index, row in signals.iterrows():
                # ["Timestamp", "Long", "Short", "TargetTimestamp"]
                ...
                if slice_data is not None:
                    ...
                    if self.__validTradingTime(slice_data.time):
                        self.__mockTrading(self.__symbol, freq, predictions, slice_data)
                    else:
    ...
    def __mockTrading(self, symbol, freq, predictions, slice_data):
        ...
        if self.__positionManager.hasNonFinished(symbol):
            if self.__isOrderValid(symbol, slice_data) and not self.__riskMgr.isStopLoss(symbol) and not self.__riskMgr.isInDanger(symbol):
                ...
                self.__onNewTick(symbol, freq, predictions, slice_data)
                return
            else:
                self.__driveInValidNonFinishedOrder(symbol)
        # do mock trading
        self.__onNewTick(symbol, freq, predictions, slice_data)
        self.__onPredictUpdated(symbol, freq, predictions, slice_data)
    ...
    def __onNewTick(self, symbol, freq, predictions, slice_data):
        self.__executorDict[freq].updatePredictInfo(predictions, slice_data)

    def __onPredictUpdated(self, symbol, freq, predictions, slice_data):
        if self.__positionManager.isPositionClosed(symbol):
            ...
            self.__allowOpenNew(symbol, freq, predictions, slice_data)
        elif self.__positionManager.isPositionPositive(symbol):
            self.__processPositivePosition(symbol, freq, predictions, slice_data)
        elif self.__positionManager.isPositionNegative(symbol):
            self.__processNegativePosition(symbol, freq, predictions, slice_data)

    def __allowOpenNew(self, symbol, freq, predictions, slice_data):
        if self.__isOpenLong(symbol, freq, predictions, slice_data):
            self.__processOpenSignal(symbol, OrderSide.Buy, slice_data)
        elif self.__isOpenShort(symbol, freq, predictions, slice_data):
            self.__processOpenSignal(symbol, OrderSide.Sell, slice_data)

    def __processPositivePosition(self, symbol, freq, predictions, slice_data):
        if self.__isCloseLong(symbol, freq, predictions, slice_data) or self.__riskMgr.isStopLoss(symbol) or self.__riskMgr.isInDanger(symbol):
            self.__processCloseSignal(symbol, OrderSide.Sell, slice_data)
        elif self.__isOpenLong(symbol, freq, predictions, slice_data):
            self.__processOpenSignal(symbol, OrderSide.Buy, slice_data)

    def __processNegativePosition(self, symbol, freq, predictions, slice_data):
        if self.__isCloseShort(symbol, freq, predictions, slice_data) or self.__riskMgr.isStopLoss(symbol) or self.__riskMgr.isInDanger(symbol):
    ...
    def __comingNewDay(self, symbol, slice_data):
        ...
        for freq in self.__freq_list:
            self.__executorDict[freq].resetNewDay()
    ...
    def __isOpenLong(self, symbol, freq, predictions, slice_data):
        result = self.__executorDict[freq].isOpenLong(predictions, slice_data)
        return self.__checkExecutorOutput(symbol, result)

    def __isOpenShort(self, symbol, freq, predictions, slice_data):
        result = self.__executorDict[freq].isOpenShort(predictions, slice_data)
        return self.__checkExecutorOutput(symbol, result)

    def __isCloseLong(self, symbol, freq, predictions, slice_data):
        result = self.__executorDict[freq].isCloseLong(predictions, slice_data)
        return self.__checkExecutorOutput(symbol, result)

    def __isCloseShort(self, symbol, freq, predictions, slice_data):
        result = self.__executorDict[freq].isCloseShort(predictions, slice_data)
        return self.__checkExecutorOutput(symbol, result)
    ...
    def __reset_init_amount(self, signals: pd.DataFrame):
        ...
        for i in range(try_times):
            ...
            if timestamp in self.__data_dict[freq]:
                base_price = self.__data_dict[freq][timestamp].previousClosingPrice
