...
from ExchangeHouse.Delay import get_order_delay
...
class ExchangeHouse:
    ...
    def __init__(self, data_class, use_l2p=False, update_gap=None):
        ...
        try:
            ...
            if order is None:
                ...
                if order.code not in self.__exchangeData.index:
                    # noinspection PyBroadException
                    senddelay = get_order_delay(order.code, "sendDelay")
                    backdelay = get_order_delay(order.code, "backDelay")

                    exchange = Exchange(order.code, senddelay, backdelay, self.__DataClass.get_dic(), self.__use_l2p)
