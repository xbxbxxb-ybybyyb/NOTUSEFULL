...
from DataAPI.SingletonMeta import Singleton
...
def get_panel_interval_pv_df(stock_list, start_date_int, end_date_int, pv_type='interval_twap_30_5', adj_type='NONE',
    ...
    if pv_type in ['interval_twap_30_5', 'interval_buy_twap_30_5', 'interval_buy_twap_fill_rate_30_5',
                   'interval_sell_twap_30_5', 'interval_sell_twap_fill_rate_30_5', 'interval_pre_close_30_0',
                    'interval_buyable_volume_30_5', 'interval_sellable_volume_30_5']:
        pass
    else:
        raise TypeError
...
def get_panel_daily_pv_df(stock_list, start_date_int, end_date_int, pv_type='close', adj_type='NONE') -> pd.DataFrame:
    ...
    if pv_type in ['close', 'open', 'high', 'low', 'pre_close', 'volume', 'amt', 'pct_chg', 'turn', 'twap',
                   ...
                   'sellable_volume', 'vwap', 'dealnum', 'buy_vwap', 'sell_vwap']:
        pass
    else:
        raise TypeError
    available_index_list = ["000001.SH", "000016.SH", "000300.SH", "000905.SH", "000906.SH", "399001.SZ",
...
def return_htsc_qt_pltfm_param(item):
    ...
    返回量化平台的字段，用于取量化平台数据
    """
    item_dict = {'close': xq.Factors.close, 'open': xq.Factors.open, 'high': xq.Factors.high, 'low': xq.Factors.low,
                 'pre_close': xq.Factors.pre_close, 'amt': xq.Factors.amt, 'volume': xq.Factors.volume,
                 'chg': xq.Factors.chg, 'pct_chg': xq.Factors.pct_chg, 'turn': xq.Factors.turn,
                 'free_turn': xq.Factors.free_turn, 'dealnum': xq.Factors.dealnum, 'swing': xq.Factors.swing,
                 'free_float_shares': xq.Factors.free_float_shares, 'float_a_shares': xq.Factors.float_a_shares,
                 'share_totala': xq.Factors.share_totala, 'mkt_cap_ard': xq.Factors.mkt_cap_ard,
                 'a_mkt_cap': xq.Factors.a_mkt_cap, 'adjfactor': xq.Factors.adjfactor, 'index_300': xq.IndexType.HS300,
                 'index_50': xq.IndexType.SH50, 'index_500': xq.IndexType.ZZ500,
                 'index_weight_hs300': xq.IndexType.HS300, 'index_weight_sh50': xq.IndexType.SH50,
                 'index_weight_zz500': xq.IndexType.ZZ500, 'pb_lf': xq.Factors.s_val_pb_new,
                 'pcf_ocf_ttm': xq.Factors.pcf_ocf_ttm,  'pe_ttm': xq.Factors.pe_ttm,
                 'ps_ttm': xq.Factors.ps_ttm, 'dividendyield2': xq.Factors.dyr_12}
...
def get_panel_min_data(item, start_date, end_date, stock_list=None, dir_path=panel_minute_data_path, forward_adj=True,
    ...
    :return: DataFrame
    """
    if item in ['open', 'high', 'low', 'close', 'volume', 'amt']:
        pass
    else:
        raise TypeError
    ...
    if daily_bar_num == 240:  # 如每日bar数为240，则需要将原始数据（每日242根bar）做一定调整
        ...
        df = df.loc[df_exc_925].copy()
        df_exc_1500 = df.index.time != dt.time(15, 00)
        df = df.loc[df_exc_1500].copy()
