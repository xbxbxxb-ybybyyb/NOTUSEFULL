...
def combine_trade_and_capacity(portfolio, start_date, end_date, curr_date):
    ...
    df = pd.read_excel("/data/user/666888/AlgoModelCmp/portfolios/" + file_name)
    print("Combining {}'s trade and capacity".format(portfolio))
    codes = list([_code for _code in df.iloc[:, 0]])
    print(codes)
    volumes = list([_code for _code in df.iloc[:, 1]])
...
#    portfolio = 'TEST'
    ...
    py = Pyfile()
    py.upload(
        'AlgoModelCmp/bt_info/{}-{}/{}/'.format(start_date, end_date, portfolio),
