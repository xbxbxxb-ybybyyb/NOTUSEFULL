...
def datesSegment(startDate, endDate):
    ...
    if divmod(daysDiff, 60)[1] > 0 or daysDiff == 0:
        datesPeriod = divmod(daysDiff, 60)[0] + 1
    else:
        datesPeriod = divmod(daysDiff, 60)[0]
    if datesPeriod == 1:
        startEndDatesList = [[startDate, endDate]]
    else:
        startEndDatesList = [None for _ in range(datesPeriod)]
        for i in range(datesPeriod - 1):
            startEndDatesList[i] = [startDate, startDate + dt.timedelta(60)]
            startDate = startDate + dt.timedelta(61)
