...
class SignalManager:
    ...
    def __pre_process_single(self, filename: str) -> pd.DataFrame:
        ...
        if True:
            try:
            # if True:
                # remote_print(self.__symbol, filename[10:18])
                # t1 = dt.datetime.now()
            ...
            if not self.__py.exists(testname):
                remote_print(testname + ' file does not exist. Pass this file.')
                return None
        ...
        # data["ave_long"] = (data['2minLong'] + data['5minLong']) / 2
        # data["ave_short"] = (data['2minShort'] + data['5minShort']) / 2
