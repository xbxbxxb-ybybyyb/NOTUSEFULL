...
class SignalManager:
    ...
    def __pre_process_single(self, filename: str) -> pd.DataFrame:
        ...
        data2 = self.__factor_data.get_factor_value("big_20191101_universe_20200414_simple_131", self.__symbol, filename[10:18], self.__prediction_names)
        data2 = data2.rename(columns=self.__rename_dict)

        column_long = data2['1minLong'] + data2['2minLong'] + data2['5minLong']
        column_long /= 3
        data2 = data2.assign(ave_long=column_long)

        column_short = data2['1minShort'] + data2['2minShort'] + data2['5minShort']
        column_short /= 3
        data2 = data2.assign(ave_short=column_short)

        data2['Ticktime'] = data2['Ticktime'].apply(lambda x: dt.datetime.strptime(x, '%H:%M:%S').time())
        data2 = SignalManager.average_special_time(data2, dt.time(11, 25, 0), dt.time(11, 28, 0), dt.time(11, 30, 0))
        data2 = SignalManager.average_special_time(data2, dt.time(14, 52, 0), dt.time(14, 55, 0), dt.time(14, 57, 0))

        data["ave_long"] = (data["ave_long"] + data2["ave_long"]) / 2
        data["ave_short"] = (data["ave_short"] + data2["ave_short"]) / 2
