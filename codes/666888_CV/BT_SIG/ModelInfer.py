...
from BT_SIG.DeepNNDataSetLongShort import DeepNNDataSet
import math
import os
import re
import math
    ...
    def infer(self):
        ...
        with tf.Session() as sess:
            ...
            for d in timestamp["timestamp"].keys():
                d_key = d[1:]
                d_key_split = d_key.split('-')
                cvt_d_key = ""
                for k in d_key_split:
                    cvt_d_key = cvt_d_key + k
                ...
                for index in range(len(window_size)):
                    ...
                    # print("factor", factor_data[d].shape)
                ...
                for index in range(1, len(predict_slice_datas)):
                    # print(index, predict_slice_datas[index][0].shape)
...
def preprocess_test_data(data, scale_mean, scale_std, up_limit, down_limit):
    for dim in range(data.shape[1]):
        dim_data = data[:, dim]
        dim_data[dim_data > up_limit[dim]] = up_limit[dim]
        dim_data[dim_data < down_limit[dim]] = down_limit[dim]
        data[:, dim] = dim_data

    return normalizeData(data, scale_mean, scale_std)
...
def inference_tensor(eval_tensor, feed_dict_tensors, batch_data, batch_size):
    ...
    if res > 0:
        ...
        if eval_tensor != None:
            ...
            eval_data = eval_tensor.eval(
                feed_dict={tensor: value for tensor, value in zip(feed_dict_tensors, feed_dict_data)})
            # print("pure inference cost", time.time()-start)
