...
from xquant.xqutils.perd_profile import profile
...
def FutureBias(today,pf_code):
    ...
    df = ma.getMDSecurityTickDataFrame("%s.CF" % contact,"%s140000" % pre_date,"%s150000" % pre_date,0)
...
def format_weight(w, pf_code, today, time, open_position=False):
    ...
    qpool = pd.read_excel(pool_stock_path+pool_file, sheet_name=0, header=0, squeeze=False, converters={'证券代码':str})    
    assert np.sum(w<0)==0
...
def pf_generator(today_act, time, last_position, valid_position, valid_stk, pf_code, turn_ad, hedge_index, capital, cash, \
    barra_limit_dict, industry_loose, dupl_industry, split_fin, amt_limit_,single_stock_max_weight_,asset=None,mode=1,benchmark='500'):
