...
import sys
sys.path.insert(0,'/data/user/013546/rubbish/am_update_pf/')
...
def get_trade_price(basic_daily_path, basic_minute_path, start_date='', end_date='',
    ...
    if price_type == 'minute':
        ...
        if os.path.exists(minute_trade_price_path + time_saver + '.pkl'):
        ...
        if not os.path.exists(minute_trade_price_path + time_saver + '.pkl') or add_trade_price_dates:
            ...
            price.to_pickle(minute_trade_price_path + time_saver + '.pkl')
...
def pm_update_depart_feature(depart_factor_list,own_factor_list,start_date,end_date,own=True, check=True, silent=False):
    is_valid = pd.read_pickle(basic_daily_path+'is_valid.pkl')
    flag = True
    neu_sample = get_neu_sample(basic_daily_path, depart_factor_list, start_date, end_date, 0)
...
def am_update_depart_feature(sample_path,hsample_path,day_factor_list,fix_factor_list, start_date, end_date, check=True, silent=False):
    is_valid = pd.read_pickle(basic_daily_path+'is_valid.pkl')
    flag = False
    neu_sample = get_neu_sample(basic_daily_path, fix_factor_list, start_date, end_date, 1)
...
def update_sample(start_date, end_date=None, mode=0):
    ...
    factor_info = pd.read_pickle(factor_info_path_tmp)
