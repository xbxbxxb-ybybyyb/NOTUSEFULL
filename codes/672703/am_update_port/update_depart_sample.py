...
fix_1300_pkl_path = '/data/user/013160/FactorFactory/PICKLE/'
...
def pm_update_label(label_path,start_date, end_date,winsard_flag=True, norm_flag=True):
    ...
    label = get_future_return(basic_data_path, trade_price, price_type='1300_1459', lag_list=[1, 3, 5], shift_lag=1,
                            winsard_flag=winsard_flag,norm_flag=norm_flag)
    for date in label_date_list:
        print(date)
        date = date.strftime('%Y%m%d')
        tmp = label.loc[date].reset_index()
        tmp = get_label_bin(tmp, '1300_1459_re_1d')
        tmp.rename(columns={'level_0': 'date', 'level_1': 'columns'}, inplace=True)
        pd.to_pickle(tmp.sort_index(by='stock').reset_index(drop=True), label_path + date + '.pkl')
        
def pm_update_vwap_label(label_path, start_date, end_date,winsard_flag=True, norm_flag=True):
    ...
    trade_price_vwap = get_trade_price(basic_data_path, minute_data_path, label_start_date, label_end_date,
                                       price_type='vwap')
    label_vwap = get_future_return(basic_data_path, trade_price_vwap, price_type='vwap', lag_list=[1, 3, 5],
                                   shift_lag=0,winsard_flag=winsard_flag,norm_flag=norm_flag)
    trade_price_0930_1129 = trade_price = get_trade_price(basic_data_path, minute_data_path, label_start_date,
                                                          label_end_date, transaction_time='0930',
                                                          transaction_period=120)
    label_0930_1129 = get_future_return(basic_data_path, trade_price, price_type='0930_1129', lag_list=[1, 3, 5],
                                        shift_lag=0,winsard_flag=winsard_flag,norm_flag=norm_flag)
    ...
    for date in label_date_list:
        ...
        tmp = get_label_bin(tmp, 'vwap_re_1d')
        tmp = get_label_bin(tmp, '0930_1129_re_1d')
        date = date.strftime('%Y%m%d')
        pd.to_pickle(tmp.sort_index(by='stock').reset_index(drop=True), label_path + date + '.pkl')
...
def update_sample(start_date, end_date=None, mode=0):
    ...
    factor_info = pd.read_pickle('/data/group/800020/AlphaDataCenter/Department/DepartSample/factor_info.pkl')
    ...
    if mode == 0:
        ...
        pm_update_vwap_label(day_depart_label_path, start_date, end_date,winsard_flag, norm_flag)
    elif mode == 5:
        pm_update_label(fix_depart_label_path,start_date, end_date,winsard_flag, norm_flag)