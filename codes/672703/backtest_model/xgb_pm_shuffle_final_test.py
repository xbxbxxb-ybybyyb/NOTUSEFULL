...
class XgboostReg_Model():
    ...
    def get_model(self, sample, factor_list):
        ...
        filename = self._model_path  +'model_'+str(0)+'.pkl'
        ...
        xgb_model.fit(X=train_x, y=train_y, verbose=False, eval_set=[(train_x, train_y),(validation_x, validation_y)],
                 eval_metric=[ 'mae'], early_stopping_rounds = 100)
        filename = self._model_path  +'model_'+str(2)+'.pkl'
        with open(filename, 'wb') as f:
            pickle.dump(obj =xgb_model,file= f)
        print('scores:', xgb_model.best_score, xgb_model.best_iteration)
        np.random.seed(self._random_seed)
        ...
        xgb_model.fit(X=train_x, y=train_y, verbose=False, eval_set=[(train_x, train_y),(validation_x, validation_y)],
                 eval_metric=[ 'mae'], early_stopping_rounds = 100)
        filename = self._model_path  +'model_'+str(3)+'.pkl'
        with open(filename, 'wb') as f:
            pickle.dump(obj =xgb_model,file= f)
        print('scores:', xgb_model.best_score, xgb_model.best_iteration)
        return
...
root_model_path = '/data/user/013546/AlphaDataCenter/Models/'+t+'/'+'weekday_t/shuffle_final_test/'
root_predict_path = '/data/user/013546/AlphaDataCenter/DailyPrediction/'+t+'/'+'weekday_t/shuffle_final_test/'
