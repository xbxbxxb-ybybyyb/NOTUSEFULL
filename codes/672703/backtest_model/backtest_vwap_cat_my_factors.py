...
model_dict={'XgboostReg_Model':('vwap_re_1d',1),
            ...
            'CatboostMultiClass_Model':('bin_vwap_re_1d',1),
            'Lr_Model':('vwap_re_5d',5)}
model_dict = {'CatboostMultiClass_Model':('bin_vwap_re_1d',1)}
factor_list = pd.read_pickle('/data/group/800020/AlphaDataCenter/Sample/factor_list.pkl')
factor_list1= pd.read_excel('/data/group/800020/AlphaExperiment/Test/myfactor.xlsx',header=None).fillna(0)
factor_list1= factor_list1.values.reshape((1,factor_list1.size))
factor_list1 = factor_list1[factor_list1!=0].astype('str').tolist()
factor_list = list(set(factor_list1)&set(factor_list))
