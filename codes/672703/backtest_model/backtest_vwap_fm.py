...
count = 0
lookback_period = 240
root_model_path = '/data/user/013546/AlphaDataCenter/Models/vwap/'
root_predict_path = '/data/user/013546/AlphaDataCenter/DailyPrediction/vwap/'
model_dict={'XgboostReg_Model':('vwap_re_1d',1),
            'DeepFM_Model':('vwap_re_1d',1),
            'Lr_Model':('vwap_re_5d',5)}
model_dict = {'DeepFM_Model':('vwap_re_1d',1)}
factor_list = np.load('/data/user/013546/rubbish/use_factor_list.npy').tolist()['all_am']
ori_factor_list = ori_factor_list = pd.read_pickle('/data/group/800020/AlphaDataCenter/Sample/factor_list.pkl')
left_factor_list = sorted(list(set(factor_list)-set(ori_factor_list)))
path_sample = '/data/group/800020/AlphaDataCenter/Sample/NormSample/'#
path2_sample = '/data/group/800020/AlphaExperiment/Factor/DepartmentSample/Sample/'  
...
for today_date in dates:
    ...
    if retrain_flag == True:
        ...
        for day in sample_required_dates:
            if day<train_start:
                continue
            df1 = pd.read_pickle(path_sample+ day +'.pkl')
            df2 = pd.read_pickle(path2_sample+ day +'.pkl')[['date','stock']+left_factor_list]
            df = df1.merge(df2,on=['date','stock'],how='left')
    ...
    df1 = pd.read_pickle(path_sample+ today_date +'.pkl')
    df2 = pd.read_pickle(path2_sample+ today_date +'.pkl')[['date','stock']+left_factor_list]
    sample_daily = df1.merge(df2,on=['date','stock'],how='left')
