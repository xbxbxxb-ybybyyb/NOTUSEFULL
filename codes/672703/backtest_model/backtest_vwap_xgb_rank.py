...
class XgboostRank_Model():
    ...
    def train(self,train_x,train_y,valid_x,valid_y):  
        ...
        xgb_model = xgb.train(params, train_dmatrix,maximize=True,early_stopping_rounds=30,
            evals=[(valid_dmatrix, 'validation')],num_boost_round=100,verbose_eval=1)
        return xgb_model
    def get_model(self, sample, factor_list):
        ...
        for i in range(4):
            ...
            train_x,train_y = self.unifier_sample_set(sample[sample['date'].isin(train_dates)],factor_list,self._label_name+'_pct',self._label_name+'_pct',0.5)
            valid_x,valid_y = self.unifier_sample_set(sample[sample['date'].isin(valid_dates)],factor_list,self._label_name+'_pct',self._label_name+'_pct',0.5)
...
root_model_path = '/data/user/013546/AlphaDataCenter/Models/'+t+'/'+'depart/max8/'
root_predict_path = '/data/user/013546/AlphaDataCenter/DailyPrediction/'+t+'/'+'depart/max8/'
model_dict = {'XgboostRank_Model':(need_label,w)}
print(model_dict)
