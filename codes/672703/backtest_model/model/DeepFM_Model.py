...
class DeepFM_Model():
    ...
    def get_model(self, sample, factor_list):
        ...
        if nn_layer1_num % 2 != 0:
            nn_layer1_num+=1
        nn_layer1_num = max(nn_layer1_num,2)
        ...
        dfm.fit(Xv_train_all_, y_train_all_, Xv_valid_all_, y_valid_all_,i,clf_str, early_stopping=True, refit=True)
        # save model
        dfm.saver.save(dfm.sess,self._model_path,global_step=i)
        print('########################################### training use time: ', time.time()-t )
        return
    ...
    def label_predict(self,sample_daily, factor_list):    
        ...
        saver = tf.train.import_meta_graph(self._model_path + '-0.meta')
        saver.restore(sess,self._model_path + '-0') # 注意路径写法
