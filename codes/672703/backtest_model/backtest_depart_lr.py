...
def update_model_predict(root_path='',path_sample=[],factor_list=[],
    ...
    if retrain_flag == True:
        all_trading_days = sorted([e[:-4] for e in os.listdir(path_sample[0])])
        ...
        for day in sample_required_dates:
            feature = pd.read_pickle(path_sample[0]+ day + '.pkl')[['date','stock']+factor_list]
            label = pd.read_pickle(path_sample[1] + day + '.pkl')
            df = label.merge(feature,on=['date','stock'],how='left')
            sample_list.append(df)
        training_sample = pd.concat(sample_list)
        print('......................')   
    sample_daily = pd.read_pickle(path_sample[0]+today_date+'.pkl')
...
factor_list = pd.read_pickle('/data/group/800020/AlphaDataCenter/Department/DepartSample/factor_info.pkl')['day_factors']
print('factor list num:',len(factor_list))
feature_path = '/data/group/800020/AlphaDataCenter/Department/DepartSample/Sample/feature/'#
label_path = '/data/group/800020/AlphaDataCenter/Department/DepartSample/Sample/label/'
...
for today_date in dates:
    ...
    count+=1  
    update_model_predict(root_path=root_path,path_sample=(feature_path,label_path),factor_list=factor_list,
