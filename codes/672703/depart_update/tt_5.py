...
def update_model_predict(root_path='',path_sample=[],factor_list=[],
    ...
    day_factors = pd.read_pickle('/data/group/800020/AlphaDataCenter/Department/DepartSample/factor_info_new.pkl')['day_factors']
    if retrain_flag == True:
        ...
        for day in sample_required_dates:
            pre_date = s.tradingday(day,-2)[0]
            df0 = pd.read_pickle(path_feature+ day + '.pkl')
            df1 = pd.read_pickle(path_daily+ pre_date + '.pkl')[['stock']+day_factors]
    ...
    pre_date = s.tradingday(today_date,-2)[0]
    df = pd.read_pickle(path_daily+pre_date+'.pkl')[['stock']+day_factors]
    sample_daily = sample_daily.merge(df,on=['stock'],how='left')
...
fix_factors = pd.read_pickle('/data/group/800020/AlphaDataCenter/Department/DepartSample/factor_info_new.pkl')['fix_factors']
factor_list = sorted(list(set(factor_list+fix_factors)))
print('factor list num:',len(factor_list))
path_sample = '/data/group/800020/AlphaDataCenter/Department/DepartSample/HFSample/'#
