...
#        sample = sample[sample[self._label_name].abs()<=3]
        ...
        count = int(len(idx)/3)+1
        for j in range(3):
            this_sample = sample[sample['date'].isin(dates[j*80:(j+1)*80])]
#            this_sample = sample.iloc[idx[j*count:(j+1)*count]]
            ...
            learning_rate=0.2, n_estimators=2000, silent=False,metric='rmse',
            ...
            model.fit(train_x, train_y, callbacks=[lgb.reset_parameter(learning_rate=lambda n: 0.15 * np.exp(-n/600) + 0.001)],    
...
#    label = pd.read_pickle('/data/user/013546/rubbish/vwap_ret_fill.pkl')
...
suffix = 'depart_depart_own300_5step_shuffle_date_'+date_start+'_'+date_end+'/'
model_config_list= [
