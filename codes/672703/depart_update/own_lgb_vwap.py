...
#    label = pd.read_pickle('/data/user/013546/rubbish/rank_gauss_vwap.pkl')
...
#        training_sample = training_sample[['date','stock']+factor_list].merge(label,on=['date','stock'],how='left')
    ...
    for config in model_config_list:
        ...
        if retrain_flag == True:
            ...
            time_date = all_trading_days[-gap_period-1]
            
            may_dates = pd.read_pickle('/data/user/013546/rubbish/ret_corr.pkl')
            may_dates = may_dates.loc[:today_date].tail(80).append(may_dates.loc[today_date:].head(80))
            may_dates = [d.strftime('%Y%m%d') for d in may_dates.index]
            model_retrain_date = sorted(list(set(model_retrain_date)&set(may_dates)))
            print(len(model_retrain_date))
...
step = 20
count = 0
lookback_period_max = 400
retrain_flag = True  
week_flag = True 
root_path = '/data/user/013546/AlphaDataCenter/'
suffix = 'own_rankgauss_nocorr_'+date_start+'_'+date_end+'/'
model_config_list= [
...
#rubbish+=['GTJA_096', 'Min30_PRchange_5d', 'RankedReversion', 'DownSpace', 'VolumePriceKurt60d', 'IlliqCv20d', 'CloseLoc', 'MinuteEODRetVolMultiple', 'MinSkipCorrRatio', 'MinuteAmPmReturnDiff', 'MinuteLastHourMtm', 'Min_CloseSignAutoCorrZScore', 'DownDayReturn', 'Max2Min5d', 'PB_Hist2_5D', 'GTJA_063', 'GTJA_009', 'MinuteReturnVolumeMultiple', 'Min_CloseAutoCorrZScore', 'DeltaTurn', 'ClosePercentHighLow', 'Min30_PRchange_1d', 'WQ_054', 'DeltaVolume', 'ClosePercentIndus', 'CloseDistance2Journey', 'GTJA_028', 'GTJA_013', 'Min_Range_Ratio_1d', 'MinuteCloseDuoKong', 'MinuteVWAPSortino', 'MinUpDownRetAcc', 'LongBear', 'MinExcessIndusPercent', 'GTJA_124', 'MinuteCorrReturnVolumeResampled', 'VolumeRatio5d', 'DeltaVwapAdjValid', 'GTJA_022', 'VolumeReversionMultiple', 'GTJA_011', 'GTJA_021', 'MinWR_20_80', 'GTJA_085', 'GTJA_038', 'DeltaCloseAdj1d', 'Min_CorrCloseVolume', 'MinuteEODRetVolMultipleBias', 'MinuteCloseNetural', 'GTJA_002']
