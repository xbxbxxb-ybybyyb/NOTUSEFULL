...
class Lr_Model():
    ...
    def get_model(self, sample, factor_list):
        ...
        print('nan',y_train[np.isnan(y_train)])
        print('inf',y_train[np.isinf(y_train)])
...
def update_model_predict(root_path='',path_sample=[],factor_list=[],
    ...
    path_own_feature = '/data/group/800020/AlphaDataCenter/HFSample/NormSample/'
    label = pd.read_pickle('/data/group/800020/AlphaExperiment/Test/pm_ret_rank_norm.pkl')
    if retrain_flag == True:
        ...
        for day in sample_required_dates:
            ...
            df0 = pd.read_pickle(path_feature+ pre_day + '.pkl')[['stock']+depart_factors]
            df1 = pd.read_pickle(path_own_feature+ day + '.pkl')[['date','stock']+own_factors]
    ...
    df0 = pd.read_pickle(path_feature+ pre_day + '.pkl')[['stock']+depart_factors]
    df1 = pd.read_pickle(path_own_feature+ today_date + '.pkl')[['date','stock']+own_factors]
...
model_config_list= [
            ('Lr_Model','pm','1300_1459_re_5d',5,240)
             ]
own_factors = pd.read_pickle('/data/user/013546/陈卓/factor300.pkl')
day_factors = pd.read_pickle('/data/group/800020/AlphaDataCenter/Sample/factor_list.pkl')
fix_factors = pd.read_pickle('/data/group/800020/AlphaDataCenter/HFSample/hfactor_list.pkl')
fix_factors = sorted(list(set(fix_factors)-set(day_factors)))
own_factors = sorted(list(set(own_factors+fix_factors)))
...
factor_list = sorted(list(set(own_factors+depart_factors)))
