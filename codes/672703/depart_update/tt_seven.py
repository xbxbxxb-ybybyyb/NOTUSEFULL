...
class XgbRegTs():
    ...
    def get_model(self, sample, factor_list):
        ...
        dates = sorted(dates)
        count_date = int(len(dates)/self._n_splits)
        num = 0
        for i in range(self._n_splits):
            valid_dates = dates[i*count_date:(i+1)*count_date]
            other_split = sorted(list(set(range(self._n_splits))-set([i])))
            valid_sample = sample[sample['date'].isin(valid_dates)]
            valid_x,valid_y = valid_sample[factor_list],valid_sample[self._label_name]
            for j in other_split:
                train_dates = dates[j*count_date:(j+1)*count_date]
                train_sample = sample[sample['date'].isin(train_dates)]
                train_x,train_y = train_sample[factor_list],train_sample[self._label_name]
               
                xgb_model = xgb.XGBRegressor (
                            n_estimators=1000, nthread=100, min_child_weight = 0.1,reg_alpha = 1, gamma =1.0,
                            tree_method='gpu_hist', gpu_id=0,
                          colsample_bytree= 0.3, colsample_bylevel= 0.3 ,subsample=0.8 , seed=self._random_seed,  max_depth=8, learning_rate=0.1 )

                xgb_model.fit(X=train_x, y=train_y, verbose=False, eval_set=[(train_x, train_y),(valid_x, valid_y)],
                         eval_metric=[ 'mae'], early_stopping_rounds = 20)
                filename = self._model_path  +'model_'+str(num)+'.pkl'
...
class XgbRegTsSeven():
    ...
    def get_model(self, sample, factor_list):
        ...
        dates = sample['date'].unique().tolist()
        dates = sorted(dates)
        map_date = dict(zip(dates,np.tile(np.arange(self._n_splits),int(len(dates)/self._n_splits+1))[:len(dates)]))
        num = 0
        for i in range(self._n_splits):
            valid_dates = [k for k,v in map_date.items() if v==i]
            other_split = sorted(list(set(range(self._n_splits))-set([i])))
            valid_sample = sample[sample['date'].isin(valid_dates)]
            valid_x,valid_y = valid_sample[factor_list],valid_sample[self._label_name]
            for j in other_split:
                train_dates = [k for k,v in map_date.items() if v==j]
                train_sample = sample[sample['date'].isin(train_dates)]
                train_x,train_y = train_sample[factor_list],train_sample[self._label_name]
...
#             model_retrain_date = pd.read_pickle('/data/user/013546/rubbish/model_train_date.pkl')
#             print(model_retrain_date[-5:])
...
suffix = 'twelve_'+date_start+'_'+date_end+'/'
model_config_list= [
            ('XgbRegTs','vwap','vwap_re_5d',5,240)
             ]
model_path = ''
