...
class LgbRegTs():
    ...
    def get_model(self, sample, factor_list):
        ...
        for j in range(self._count):
            train_dates = dates[j*num_dates:(j+1)*num_dates]
            ...
            learning_rate=0.1, n_estimators=2000, silent=False,metric='rmse',
                 random_state=1605, min_child_weight = 0.1,reg_alpha = 100, 
                gamma =1.0,first_metric_only=True,
                 feature_fraction=0.5, feature_fraction_seed=1504, 
...
suffix = 'select_sample_'+date_start+'_'+date_end+'/'
model_config_list= [
    ('LgbRegTs','vwap','vwap_re_5d',5,240)
             ]
model_path = ''
factor_list = pd.read_pickle('/data/group/800020/AlphaDataCenter/Department/DepartSample/factor_info_shipan.pkl')['day_factors']
