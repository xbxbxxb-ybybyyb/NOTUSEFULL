...
class Xgb4Model():
    ...
    def get_model(self, sample, factor_list):
        ...
        date_list = []
        for i in range(4):
            date_list.append(dates[i*60:(i+1)*60]) 
        num = 0
        for i in range(4):
            for j in range(i+1,4):  
                train_dates = sorted(list(set(date_list[i]+date_list[j])))
...
suffix = 'select_factor_'+date_start+'_'+date_end+'/'
model_config_list= [
            ('Xgb4Model','vwap','vwap_re_5d',5,240)
             ]
model_path = ''
...
factor_list = pd.read_pickle('/data/user/013546/rubbish/factor_list.pkl')
