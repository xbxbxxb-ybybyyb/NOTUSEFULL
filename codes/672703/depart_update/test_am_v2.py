...
class Xgb_liu():
    ...
    def get_model(self, sample, factor_list):
        ...
        tmp = validation_set.groupby('date')['stock'].count()
        invalid_validation_dates = tmp[tmp<1500].index.tolist()
        validation_set_valid = validation_set[~ validation_set['date'].isin(invalid_validation_dates)]
        validation_set_valid = validation_set_valid[np.logical_and(validation_set_valid[self._label_name] >= -3,
                                                                   validation_set_valid[self._label_name] <=3 )]
        ...
        for j in range(4):
            ...
            tmp = train_set.groupby('date')['stock'].count()
            invalid_train_dates = tmp[tmp<1500].index.tolist()
            train_set = train_set[~ train_set['date'].isin(invalid_train_dates)]
            train_set = train_set[np.logical_and(train_set[self._label_name] >= -3,train_set[self._label_name] <=3 )]
           ...
           'gamma': 1.0,'nthread': 100,
              'subsample':0.8,'colsample_bytree':0.8,'reg_alpha':10,'reg_lambda':10,
               'min_child_weight': 0.1, 'max_depth': 10,'tree_method':'gpu_hist'}
            maximize = False
            this_params = {}
            this_params = {'objective':'reg:linear','eval_metric':'mae'}
            params.update(this_params)
            xgb_model = xgb.train(params, train_dmatrix, num_boost_round=1000,maximize=maximize,early_stopping_rounds=20,
...
suffix = 'test_xgb_ts_4/'
model_config_list= [
            ('Xgb_liu','am','0930_1129_re_5d',5,240)
             ]
...
# factor_list = factor_list+['OprProfitToNP_qfa_tsrank8', 'GTJA_059', 'UpDownVol5d', 'GTJA_093', 'GTJA_012', 'DebtToAsset_tsrank4', 'DedProfitToNP_qfa_tsrank12', 'Minute15mReTo5D', 'GTJA_013', 'EbitdaTTM', 'CurDebtToDebt_tsrank8', 'OCFToSales_qfa_surprise', 'MinuteMADistance', 'Re300ReturnScore5D', 'ClosePercentUp5d', 'GTJA_028', 'RankedReversion', 'MinuteLTSkew', 'alpha53', 'PDPS_Hist1_120D', 'GTJA_096', 'EbitTTM', 'RoeNeuPb', 'MinuteSkew5d', 'RhoSwingAmt60d', 'GTJA_098', 'GTJA_054', 'NetOprCash_surprise', 'MinuteSignedAbnormalSmartMean', 'DownDayReturn', 'MinuteCorrCloseVolume', 'GTJA_004', 'DeltaTo5DMin', 'DedProfitToNP_ttm_tsrank12', 'CurDebtToDebt_yoy', 'DebtToAsset_tsrank8', 'MinuteHighPricePos5d', 'PDPS_Hist2_120D', 'GTJA_045', 'LongShortPower', 'GTJA_057', 'ShortBull', 'MinuteCloseAmtReHHI', 'MinuteLastMtmRank5d', 'OCFToSales_qfa_tsrank8', 'GTJA_015', 'Min5LastHalfHourVolStd5d', 'DedProfitToNP_ttm_std_3y', 'GTJA_048', 'GTJA_091']
