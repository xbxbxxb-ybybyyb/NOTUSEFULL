    ...
    def get_model(self, sample, factor_list):
        ...
        for i in range(3):
            ...
            xgb_model = xgb.XGBRegressor (
                        n_estimators=1000, nthread=100, min_child_weight = 0.1,reg_alpha = 100, gamma =1.0,
                        tree_method='gpu_hist', gpu_id=0,
                      colsample_bytree= 0.5, colsample_bylevel= 0.5, subsample=0.8 , seed=self._random_seed,  max_depth=6, learning_rate=0.1 )
...
class XgbRegTSDrop():
    ...
    def get_model(self, sample, factor_list):
        ...
        sample = sample[(sample['rank']-0.5).abs()>=0.3]
        dates = sample['date'].unique().tolist()
        dates = sorted(dates)
        for i in range(3):
            train_dates = dates[i*80:(i+1)*80]
            validation_dates = sorted(list(set(dates)-set(train_dates)-set(dates[(i+1)*80:][:20])))
            validation_set = sample[sample['date'].isin(validation_dates)].copy()  
            validation_x = validation_set[factor_list]
            validation_y = validation_set[self._label_name]
            train_set = sample[sample['date'].isin(train_dates)].copy()  
            ...
            xgb_model = xgb.XGBRegressor (
                        n_estimators=1000, nthread=100, min_child_weight = 0.1,reg_alpha = 100, gamma =1.0,
                        tree_method='gpu_hist', gpu_id=0,
                      colsample_bytree= 0.3, colsample_bylevel= 0.3, subsample=0.8 , seed=self._random_seed,  max_depth=6, learning_rate=0.1 )
...
class XgbRegTSDropA1():
    ...
    def get_model(self, sample, factor_list):
        ...
        for i in range(3):
            ...
            train_y = train_set[self._label_name]
            xgb_model = xgb.XGBRegressor (
                        n_estimators=1000, nthread=100, min_child_weight = 0.2,
                        reg_alpha = 100,
                        tree_method='gpu_hist', gpu_id=0,
                      colsample_bytree= 0.5, colsample_bylevel= 0.5, subsample=0.8 , seed=self._random_seed,  max_depth=6, learning_rate=0.1 )
...
def update_model_predict(root_path='',path_sample=[],factor_list=[],
    ...
    t = time.time()
    path_feature = '/data/group/800020/AlphaExperiment/long_sample_all/'
#    path_label = '/data/group/800020/AlphaExperiment/Test/label_sharp_vwap.pkl'
    label = pd.read_pickle('/data/group/800020/AlphaExperiment/Test/label_sharp_vwap.pkl')
...
class XgboostRegression4():
    ...
    def get_model(self, sample, factor_list):
        sample['rank'] = sample.groupby(by='date')[self._label_name].rank(pct=True)
        sample = sample[(sample['rank']-0.5).abs()>=0.1]
        sample[self._label_name] =  (sample.groupby(by='date')[self._label_name].apply(lambda x:(x-x.mean())/x.std())).values
        all_dates = pd.to_datetime(sample['date'].unique().tolist())
        
        date_list = [0,1,2,3]*int(len(all_dates))
        date_list = date_list[:len(all_dates)]
        date_map = dict(zip(all_dates,date_list))
        
        for j in range(4):
            dates = [k for k,v in date_map.items() if v==j] 
 
            np.random.seed(self._random_seed)
            np.random.shuffle(dates)
            validation_set = sample[sample['date'].isin(dates[50:])].copy()  
            validation_x = validation_set[factor_list]
            validation_y = validation_set[self._label_name ]
            train_set = sample[sample['date'].isin(dates[:50])]  
            train_x = train_set[factor_list]
            train_y = train_set[self._label_name]
           ...
           'gamma': 1.0,'nthread': 100,
              'subsample':0.8,'colsample_bytree':0.5,'reg_alpha':100,
            ...
            xgb_model = xgb.train(params, train_dmatrix, num_boost_round=1000,maximize=maximize,early_stopping_rounds=100,
                                  evals=[(valid_dmatrix, 'validation')],verbose_eval=500)
...
suffix = 'longterm1_'+date_start+'_'+date_end+'/'
model_config_list= [
    ('XgboostRegression4','vwap','vwap_re_20d',20,240)
             ]
model_path = ''
factor_list = list(set(pd.read_pickle('/data/group/800020/AlphaExperiment/long_sample_all/20180102.pkl').columns)-set(['date','stock']))
