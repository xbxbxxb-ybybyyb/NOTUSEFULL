...
class XgbReg():
    ...
    def get_model(self, sample, factor_list):
        ...
        stock_num = int(len(stocks)/5)
        validation_stocks = stocks[stock_num*4:]
        
        validation_set = sample[sample['stock'].isin(validation_stocks)].copy()  
        validation_x = validation_set[factor_list]
        validation_y = validation_set[self._label_name ]

        for j in range(4):
            train_stocks = stocks[j*stock_num:(j+1)*stock_num]
            train_set = sample[sample['stock'].isin(train_stocks)]  
...
suffix = 'depart_xgbts_stock_'+date_start+'_'+date_end+'/'
model_config_list= [
            ('XgbReg','am','0930_1129_re_5d',5,240)
             ]
