...
from CatboostMultiClass_Hybrid_Model import *
...
test_end_date = '20191231'
test_date_list = s.tradingday(test_start_date,test_end_date)
lookback_period_max = 240
retrain_flag = True
root_path = '/data/user/013546/AlphaDataCenter/'
model_config_list= [
            ('CatboostMultiClass_Hybrid_Model','vwap','bin_vwap_re_1d',1,240)
...
for today_date in test_date_list:
    if (count==0) or (datetime.datetime.strptime(today_date,'%Y%m%d').weekday()==0):
        retrain_flag = True 
    else:
        retrain_flag = False 
    count+=1
    valid_dates = s.tradingday('20111102',today_date)
    # check_dates = valid_dates[-oos-1-interval:-oos-1]
    check_dates_1 = valid_dates[-1-1-interval:-1-1]
    check_dates_3 = valid_dates[-3-1-interval:-3-1]
    check_dates = {}
    check_dates['r1d'] = check_dates_1
    check_dates['r3d'] = check_dates_3
    ...
    update_model_predict(root_path=root_path,path_sample=path_sample,factor_list=factor_list,
                ...
                model_config_list = model_config_list,check_dates = check_dates)
