...
def get_trade_price(transaction_price='vwap', transaction_time='0930', transaction_period=120):
    ...
    os.mkdir(run_path+'temp_for_check_prediction_minute_trade_price') if not os.path.exists(
        run_path+'temp_for_check_prediction_minute_trade_price') else None
    time_saver = transaction_price + '_' + transaction_time + '_' + str(transaction_period)
    if os.path.exists(run_path+'temp_for_check_prediction_minute_trade_price/' + time_saver + '.pkl'):
        trade_price_0 = pd.read_pickle(
            run_path+'temp_for_check_prediction_minute_trade_price/' + time_saver + '.pkl').dropna(how='all', axis=0)
        minute_close_0 = pd.read_pickle(
            run_path+'temp_for_check_prediction_minute_trade_price/' + time_saver + '_minute_close.pkl')
    ...
    if not os.path.exists(
            run_path+'temp_for_check_prediction_minute_trade_price/' + time_saver + '.pkl') or add_trade_price_dates:
        ...
        if not os.path.exists(run_path+'temp_for_check_prediction_minute_trade_price/' + time_saver + '.pkl'):
        ...
        price.to_pickle(run_path+'temp_for_check_prediction_minute_trade_price/' + time_saver + '.pkl')
        close.to_pickle(run_path+'temp_for_check_prediction_minute_trade_price/' + time_saver + '_minute_close.pkl')
...
def check_prediction(activation, transaction_price='close', transaction_time='0930', transaction_period=1,
    ...
    if len(set(adjfactor.loc[act.index[0]:].index)-set(act.index)) > 0:
        print('warning:act lack of some dates',set(adjfactor.loc[act.index[-1]:].index).difference(act.index))
    ...
    time_saver = transaction_price + '_' + transaction_time + '_' + str(transaction_period)
    trade_price = pd.read_pickle(run_path+'temp_for_check_prediction_minute_trade_price/' + time_saver + '.pkl').loc[date_list[0]:date_list[-1]]
    minute_close = pd.read_pickle(run_path+'temp_for_check_prediction_minute_trade_price/' + time_saver + '_minute_close.pkl').loc[date_list[0]:date_list[-1]]
...
def save_data(df_update,path):
    if os.path.exists(path):
        ...
        if isinstance(df_update,dict):
            for k,v in df_update.items():
                df_update[k] = v.astype(np.float64)
        else:
            df_update = df_update.astype(np.float64)
...
def cal_performance(net):
    ...
    return df
def get_stats_in_out(cum_exc_ret,split_in_out_sample_date=None,
    save_path='',name='High_freq_model'):
    ...
    plt.savefig(save_path+name+'.png')
    plt.show()
def get_pct2(score,ret):
    ret['full_market_mean'] = ret.mean(axis=1)
    ret['portfolio'] = score[1]
    ret_rank = ret.rank(axis=1,pct=True,ascending=False)
    return ret_rank
def get_pct1(score,ret):
    ret['full_market_mean'] = ret.mean(axis=1)
    ret['portfolio'] = score[1]
    ret_rank = ret.rank(axis=1,pct=True,ascending=False)
    return ret_rank
def get_pct0(score,ret):
...
need_model = sorted(['DeepFM', 'Xgboost', 'LinearRegression', 'All','Catboost'])
act = {}
...
quantile1 = pd.DataFrame(quantile1)
ave_return = pd.Series(ave_return).to_frame('ave_return').T
pd.to_pickle(quantile_mean,run_path+'performance/quantile_mean.pkl')
pd.to_pickle(quantile1,run_path+'performance/quantile1.pkl')
...
save_perform_path = run_path+ 'performance/'
...
def single(k,v):
    ...
    last_weight_hf = w[end_time].loc[last_date]
...
save_perform_path = run_path+'performance/benchmark_'+benchmark+'/'
last_date = close.index[-2].strftime('%Y%m%d')
print('last_date',last_date)
print('today',today)
...
w_path_dict = {
                '5d_control_size,turnover10%':(0.3,0.5)
              }
append_map = {0.3:0.4}
...
import pandas as pd
a = pd.read_pickle('/data/user/013546/DailyReport/performance/vwap_5d_no_control_size,turnover10%_capital8e8_amt0.025_turn0.5_w_stats.pkl')
a.to_pickle('/data/user/013546/DailyReport/performance/shipan/own_shipan_stats/shipan_5160503_stats.pkl')
a = pd.read_pickle('/data/user/013546/DailyReport/performance/hf_5d_no_control_size,turnover20%_capital8e8_amt0.025_turn0.6_w_stats.pkl')
a.to_pickle('/data/user/013546/DailyReport/performance/shipan/own_shipan_stats/shipan_5160803_stats.pkl')
a = pd.read_pickle('/data/user/013546/DailyReport/performance/benchmark_300/vwap_5d_control_size,turnover10%_capital4e8_amt0.025_turn0.3_w_stats.pkl')
a.to_pickle('/data/user/013546/DailyReport/performance/shipan/own_shipan_stats/shipan_5160304_stats.pkl')
...
w_path_dict = {
                '5d_control_size,turnover20%':(0.3,0.5)
              }
...
def single(k,v):
    turn_ad={'0930':append_map[v[0]],'1300':append_map[v[0]]}
    barra_limit_dict.update({'Size':(v[1],v[1])})
    print(k+' optimize')
    w_hf_path = save_perform_path+t+'_'+k+'_capital'+str_cap+'_amt'+str(amt_limit)+'_turn'+str(v[0])+'_w.pkl'
    w_hf = pd.read_pickle(w_hf_path)
...
#实盘权重
benchmark = '500'
hf_shi_w = pd.read_pickle('/data/user/013546/DailyReport/performance/hf_5d_no_control_size,turnover20%_capital8e8_amt0.025_turn0.6_w.pkl')
vwap_shi_w =pd.read_pickle('/data/user/013546/DailyReport/performance/vwap_5d_no_control_size,turnover10%_capital8e8_amt0.025_turn0.5_w.pkl')
...
#实盘权重
benchmark = '300'
hf_shi_w = pd.read_pickle('/data/user/013546/DailyReport/performance/benchmark_'+benchmark+'/'+'hf_5d_control_size,turnover20%_capital5e8_amt0.025_turn0.3_w.pkl')
vwap_shi_w =pd.read_pickle('/data/user/013546/DailyReport/performance/benchmark_'+benchmark+'/'+'vwap_5d_control_size,turnover10%_capital4e8_amt0.025_turn0.3_w.pkl')
simulation_w_am_path = '/data/group/800020/AlphaDataCenter/DailyWeight/benchmark_'+benchmark+'/simulation_w_am.pkl'
simulation_w_pm_path = '/data/group/800020/AlphaDataCenter/DailyWeight/benchmark_'+benchmark+'/simulation_w_pm.pkl'
simulation_w_vwap_path = '/data/group/800020/AlphaDataCenter/DailyWeight/benchmark_'+benchmark+'/simulation_w_vwap.pkl'
hf_shi_w['0930'].to_pickle(simulation_w_am_path)
...
benchmark = '500'
pf_code = '5160803'
open_o32_date = '20200630'
o32_path = '/data/user/011477/order/O32/afternoon/'
capital,open_position_weight = get_open_position_w(open_o32_date,pf_code)

hf_w_start_date = '20200701'
vwap_w_start_date = '20200701'
vwap300_w_start_date = '20200709'
shipan_track={}
weight_path = '/data/group/800020/AlphaDataCenter/DailyWeight/benchmark_'+benchmark+'/'
# old_w_vwap = pd.read_pickle('/data/user/013546/DailyReport/performance/vwap_5d_no_control_size,turnover10%_capital6e8_amt0.025_turn0.5_w.pkl')
# old_w_hf = pd.read_pickle('/data/user/013546/DailyReport/performance/hf_5d_no_control_size,turnover20%_capital5e8_amt0.025_turn0.5_w.pkl')
# old_w_vwap300 = pd.read_pickle('/data/user/013546/DailyReport/performance/benchmark_300/vwap_5d_control_size,turnover10%_capital2e8_amt0.025_turn0.4_w.pkl')

new_w_vwap = pd.read_pickle('/data/user/013546/DailyReport/performance/vwap_5d_no_control_size,turnover10%_capital8e8_amt0.025_turn0.5_w.pkl')
new_w_hf = pd.read_pickle('/data/user/013546/DailyReport/performance/hf_5d_no_control_size,turnover20%_capital8e8_amt0.025_turn0.6_w.pkl')
new_w_vwap300 = pd.read_pickle('/data/user/013546/DailyReport/performance/benchmark_300/vwap_5d_control_size,turnover10%_capital4e8_amt0.025_turn0.3_w.pkl')
# w_am0 = old_w_hf['0930']
# w_pm0 = old_w_hf['1300']
# w_vwap0 = old_w_vwap['0930']
# w_vwap0300 = old_w_vwap300['0930']
w_am = new_w_hf['0930'].loc[hf_w_start_date:]
w_pm = new_w_hf['1300'].loc[hf_w_start_date:]
w_vwap = new_w_vwap['0930'].loc[vwap_w_start_date:]
w_vwap300 =new_w_vwap300['0930'].loc[vwap300_w_start_date:]
def temp(df0,df1):
    v = df0.append(df1)
...
# w_vwap300 = temp(w_vwap0300,w_vwap300)
stats_path = run_path+'performance/'
...
open_date = '20200630'
next_open_date = '20200701'
capital = 80e7
capital_dict = {capital:('80e7','8亿')}
v = capital_dict[capital]
e = 'hf'
t = e+'_capital'+v[0]
print(t)
w = {'0930':w_am,'1300':w_pm}
w['0930'].loc[pd.to_datetime(open_date)] = open_position_weight
w['1300'].loc[pd.to_datetime(open_date)] = open_position_weight
start_time='1300'
end_time='1300'
transaction_period=120
hedge_index = 'ZZ500'
btf = BacktestHF({k:v.sort_index().loc[open_date:] for k,v in w.items()}, start_time=start_time,end_time=end_time,transaction_period=transaction_period, 
...
transaction_period=240
hedge_index = 'ZZ500'
btf = BacktestHF({k:v.sort_index().loc[open_date:].sort_index() for k,v in w.items()}, start_time=start_time,end_time=end_time,transaction_period=transaction_period, 
                 benchmark=hedge_index, capital=capital, commission=0.0005,simulation=True)
btf.run()
...
# # 300日频实盘跟踪
pf_code = '5160304'
open_o32_date = '20200709'
o32_path = '/data/user/011477/order/O32/afternoon/'
capital,open_position_weight = get_open_position_w(open_o32_date,pf_code)
open_date = '20200709'
next_open_date = '20200710'
capital = 40e7
capital_dict = {capital:('40e7','4亿')}
e = 'vwap300'
v = capital_dict[capital]
print('*'*10,' shipan track QuantMachine_'+v[0],'open date:',open_date)
t = e+'_capital'+v[0]
...
#     add_img(os.path.join(save_path, bt+'_capital10e7_bt.png'), story, fig_height*1, fig_width*0.8)
    ...
    number = 6
    if bt=='vwap':
        story.append(Paragraph('6. '+excess_map_dict[bt]+'300实盘跟踪表现（对冲300 规模4亿）', stylesheet['Song']))
    #     story.append(Paragraph('3. '+excess_map_dict[bt]+' 1亿规模过优化器回测表现', stylesheet['Song']))
        name=bt+'300_shipan'
...
pdf_maker(save_path, run_path+'compare/' + today + '_QuantMachine日报.pdf','vwap',backtest['vwap'],backtest300['vwap'],
          shipan_track['vwap'],need_model,['vwap'])
pdf_maker(save_path, run_path+'compare/' + today + '_AlphaHunter日报.pdf','hf',backtest['hf'],backtest300['hf'],
...
a = pd.read_pickle('/data/user/013546/DailyReport/performance/shipan_hf_stats.pkl')
a.to_pickle('/data/user/013546/DailyReport/performance/shipan/own_shipan_stats/shipan_5160803_stats.pkl')
a = pd.read_pickle('/data/user/013546/DailyReport/performance/shipan_vwap_stats.pkl')
a.to_pickle('/data/user/013546/DailyReport/performance/shipan/own_shipan_stats/shipan_5160503_stats.pkl')
a = pd.read_pickle('/data/user/013546/DailyReport/performance/shipan_vwap300_stats.pkl')
a.to_pickle('/data/user/013546/DailyReport/performance/shipan/own_shipan_stats/shipan_5160304_stats.pkl')
...
ftp.cwd('/XQuant/516/QuantMachine回测跟踪/实盘日报/')
upload(ftp, today + '_QuantMachine日报.pdf', run_path+'compare/' + today + '_QuantMachine日报.pdf')
upload(ftp, today + '_AlphaHunter日报.pdf', run_path+'compare/' + today + '_QuantMachine日报.pdf')
ftp.quit()
