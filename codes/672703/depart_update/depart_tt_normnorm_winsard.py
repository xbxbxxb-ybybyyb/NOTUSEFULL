...
class LgbReg():
    ...
    def get_model(self, sample, factor_list):
        ...
        all_dates = sample['date'].unique().tolist()
        import numpy as np
        for j in range(3):
            this_sample = sample.copy()
            stocks = this_sample['stock'].unique().tolist()
            dates = this_sample['date'].unique().tolist()
            ...
            train_sample = this_sample[(this_sample['date'].isin(train_dates))&(this_sample['stock'].isin(train_stocks))]
            valid_sample = this_sample[(this_sample['date'].isin(valid_dates))&(this_sample['stock'].isin(valid_stocks))]
            train_x,train_y = train_sample[factor_list],train_sample[self._label_name]
            validation_x,validation_y = valid_sample[factor_list],valid_sample[self._label_name]
            print(len(train_x),len(train_y),len(validation_x),len(validation_y))
            model = LGBMRegressor(n_jobs=24, num_leaves=64, max_depth=6, learning_rate=0.1, 
                 n_estimators=2000, silent=False,
                 reg_lambda=10,random_state=1605, first_metric_only=True,
                 feature_fraction=0.4, feature_fraction_seed=1504, bagging_fraction=0.8, bagging_fraction_seed=1504)
...
suffix = 'depart_lgb_ts__'+date_start+'_'+date_end+'/'
model_config_list= [
...
path_sample = '/data/group/800020/AlphaDataCenter/Department/DepartSample/Sample/'#
