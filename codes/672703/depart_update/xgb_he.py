...
class XGBMulti_Model():
    ...
    def __init__(self,params):
        ...
        self.label_range = {'1': [0, 0.1], '2': [0.1, 0.2], '3': [0.2, 0.3],  ## short 收益
         '4': [0.7, 0.8], '5': [0.8, 0.9], '6': [0.9, 1]}  ## long 收益
    ...
    def init_model(self):
        ...
        model = XGBClassifier(n_estimators=n_estimators, nthread=30, colsample_bylevel=colsample_bylevel,
                              colsample_bytree=colsample_bytree, subsample=subsample, min_child_weight=min_child_weight,
                              reg_lambda=reg_lambda, scale_pos_weight=scale_pos_weight, learning_rate=learning_rate,
                              max_depth=max_depth, silent=False, objective=objective, 
                            eval_metric=eval_metric,tree_method=tree_method)
        return model

    def get_train_samples(self,all_data, factor_name_list):
    ...
    def get_model(self, sample, factor_list):
        ...
        X_train,y_train,X_valid,y_valid = self.get_train_samples(sample,factor_list)
    ...
    def label_predict(self, sample_daily, factor_list):
        ...
        label_pred = predict_result[6]-predict_result[1]
        return label_pred
...
suffix = 'hezq_'+date_start+'_'+date_end+'/'
model_config_list= [
            ('XGBMulti_Model','vwap','vwap_re_1d',1,240)
             ]
