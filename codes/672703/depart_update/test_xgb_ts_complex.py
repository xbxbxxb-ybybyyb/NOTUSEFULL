...
class Xgb_Reg_TS_Complex():
    ...
    def get_model(self, sample, factor_list):
        ...
        dates = sorted(dates)
        train_dates = []
        train_dates.append(dates[:25])
        train_dates.append(dates[:50])
        train_dates.append(dates[:75])
        train_dates.append(dates[:100])
        train_dates.append(dates[:125])
        train_dates.append(dates[:150])
        train_dates.append(dates[:175])
        train_dates.append(dates[:200])
        
        validation_dates = []
        validation_dates.append(dates[25:50])
        validation_dates.append(dates[50:75])
        validation_dates.append(dates[75:100])
        validation_dates.append(dates[100:125])
        validation_dates.append(dates[125:150])
        validation_dates.append(dates[150:175])
        validation_dates.append(dates[175:200])
        validation_dates.append(dates[200:])
        


        for j in range(8):
            
            validation_set = sample[sample['date'].isin(validation_dates[j])].copy()  
            validation_x = validation_set[factor_list]
            validation_y = validation_set[self._label_name ]
            
            train_set = sample[sample['date'].isin(train_dates[j])].copy()  
...
suffix = 'drop_factor_ori_'+date_start+'_'+date_end+'/'
model_config_list= [
            ('Xgb_Reg_TS_Complex','vwap','vwap_re_5d',5,240)
             ]
