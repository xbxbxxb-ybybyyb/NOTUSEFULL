...
class LgbReg():
    ...
    def get_model(self, sample, factor_list):
        ...
        df = sample.groupby(by='date')['stock'].count()
        valid_dates = df[df>1500].index.tolist()
        sample = sample[sample['date'].isin(valid_dates)]
...
def update_model_predict(root_path='',path_sample=[],factor_list=[],
    ...
    label = pd.read_pickle('/data/user/013546/rubbish/ret_two_updown.pkl')
...
suffix = 'depart_depart_own300_5step_ret_two_updown_'+date_start+'_'+date_end+'/'
model_config_list= [
