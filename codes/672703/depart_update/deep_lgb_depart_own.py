...
def update_model_predict(root_path='',path_sample=[],factor_list=[],
    ...
    own_factors = factor_info['own_day_factors']
    depart_factors = factor_info['depart_day_factors']
#    label = pd.read_pickle('/data/group/800020/AlphaExperiment/Test/label_vwap_am_keepna.pkl')
    label = pd.read_pickle('/data/user/013546/rubbish/vwap_ret_drop_median_norm.pkl')
#    label['vwap_re_com'] = label[['vwap_re_'+str(i)+'d' for i in [5,10,20]]].mean(axis=1)
...
suffix = 'depart_norm_dropmedian_'+date_start+'_'+date_end+'/'
model_config_list= [
