...
#         encoder_inputs = self.model.get_layer('encoder_inputs')
    ...
    def get_model(self, train_sample, valid_sample, factor_list):
        ...
        y_valid = valid_sample[self._label_name].values
        x_valid = valid_sample[factor_list]
        x_valid = ((x_valid - mu) / std).values
        x_valid_np = np.array(x_valid)
        prev_idx_valid = valid_sample['prev_idx'].values
        x_valid_stack = [x_valid_np]
        
        for i in range(4):
            x_train_stack.insert(0,self.prev(x_train_stack[0], prev_idx_train))
            x_valid_stack.insert(0,self.prev(x_valid_stack[0], prev_idx_valid))
        
        x_train_stack=np.transpose(np.asarray(x_train_stack),(1,0,2))
        x_valid_stack=np.transpose(np.asarray(x_valid_stack),(1,0,2))
        
        print(y_valid.shape, x_valid_stack.shape)
        ...
        valid_decoder_input_data = np.zeros((x_valid.shape[0], 1, 1))
        
        epochs=int(1e7/x_train.shape[0])
...
#         early_stopping = EarlyStopping(monitor='val_loss', patience=5)
        ...
        self.model.fit([x_train_stack, train_decoder_input_data], y_train, batch_size=512, epochs=epochs, 
                  validation_data=([x_valid_stack, valid_decoder_input_data], y_valid), callbacks=callback_lists, verbose=2) 
    ...
    def label_predict(self, sample_daily, factor_list):        
        ...
        for i in range(4):
            x_test_stack.insert(0,self.prev(x_test_stack[0], prev_idx_test))
...
def update_model_predict(root_path='',path_sample=[],factor_list=[],
    ...
    for config in model_config_list:
        ...
        if retrain_flag == True:
            ...
            retrain_samples = training_sample[training_sample['date'].isin(model_train_date)]
            ...
            model.get_model(retrain_samples.copy(), valid_sample.copy(), factor_list)
...
suffix = 'depart128_'+date_start+'_'+date_end+'/'
