...
class XgbV1():
    ...
    def __init__(self,params):
        ...
        self._test_splits = [200,160,120,80]
    ...
    def unifier_sample_set(self,sample_set,factor_list,limit=3):
        ...
        sample_set_valid = sample_set_valid[(sample_set_valid[self._label_name]>=-limit)&(sample_set_valid[self._label_name]<=limit)]
        X = sample_set_valid[factor_list]
        y = sample_set_valid[self._label_name]
        return X,y
    def get_model(self, sample, factor_list):
        
        dates = sorted(sample['date'].unique().tolist())
        n = 5
        import numpy as np
        ...
        for j in range(len(self._test_splits)):
            ...
            temp_dates = dates[self._test_splits[j]:].copy()
            # temp_dates = sorted(list(set(temp_dates)-set(existed_date)))
            np.random.shuffle(temp_dates)
            validation_dates = temp_dates
            existed_date+=validation_dates
            train_set = sample[sample['date'].isin(train_dates)]  
            train_x,train_y = self.unifier_sample_set(train_set,factor_list)
            validation_set = sample[sample['date'].isin(validation_dates)] 
            validation_x,validation_y = self.unifier_sample_set(validation_set,factor_list)
           ...
           'gamma': 1.0,'nthread': 100,
            ...
            'reg_alpha':10,'reg_lambda':10, 'max_depth': 10,'tree_method':'gpu_hist'}
            maximize = False
            ...
            maximize=maximize,early_stopping_rounds=40,
...
suffix = 'just_test/'
model_config_list= [
            ('XgbV1','am','0930_1129_re_5d',5,240)
             ]
