...
model_name_map = {
    'am':['XgbRegTSFourModel_0930_1129_re_5d','LinearRegression_0930_1129_re_5d','DeepFM_0930_1129_re_5d','All'],
 'pm':['XgbRegTSFourModel_1300_1459_re_5d','LinearRegression_1300_1459_re_5d','DeepFM_1300_1459_re_5d','All'],
 'vwap':['XgbRegTSFourModel_vwap_re_5d','DeepFM_vwap_re_5d','LinearRegression_vwap_re_5d','All']}
...
for k in excess_hf_model_dict:
    print(k)
    draw_custom_style(excess_hf_model_dict[k].cumsum(),save_path,k,'20191231',split=False,name='hf_'+k+'_in_out_sample')
for k in excess_vwap_model_dict:
    draw_custom_style(excess_vwap_model_dict[k].cumsum(),save_path,k,'20191231',split=False,name='vwap_'+k+'_in_out_sample')
if show_flag:
    plt.show()
...
for k in quantile_hf_model_dict:
    draw_custom_style(quantile_hf_model_dict[k].cumsum(),save_path,k,'20200101',split=False,name='hf_quantile_'+k+'_in_out_sample')
for k in quantile_vwap_model_dict:
    draw_custom_style(quantile_vwap_model_dict[k].cumsum(),save_path,k,'20200101',split=False,name='vwap_quantile_'+k+'_in_out_sample')
if show_flag:
    plt.show()
...
draw_custom_style(excess_bench500[['All_0930','All_1300']].fillna(0).cumsum(),save_path,k,None,split=False,name='hf_excess_bench500')
draw_custom_style(excess_bench500[['All_vwap']].fillna(0).cumsum(),save_path,k,None,split=False,name='vwap_excess_bench500')
...
config_dict['5160803'] = {
    ...
    'w_path_dict': 
    {
        '5d_no_control_size,turnover20%':(0.6,1000,True),
        '5d_control_size,turnover20%':(0.6,0.01,False),
        '5d_no_control_size,turnover40%':(0.3,1000,False),
        '5d_control_size,turnover40%':(0.3,0.01,False)
    },
    'save_perform_path':'/data/user/013546/AlphaDataCenter/Department/png/benchmark_500/',
    'backtest_date':['20200101'],
    'bt_conf':
...
def update_weight_bt_stats(k,v,pf_code,conf,weight_flag=True):
    ...
    for open_date in config['backtest_date']:
        btf = BacktestHF({k:v.loc[open_date:] for k,v in w.items()}, 
