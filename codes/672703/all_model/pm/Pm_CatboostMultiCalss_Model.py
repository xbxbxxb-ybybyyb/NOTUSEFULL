...
class CatboostMultiCalss_Model():
    ...
    def __init__(self):
        ...
        self._label_name = 'bin_1300_1459_re_1d'
    ...
    def get_model(self, sample, factor_list):
        ...
        factor_daily_cut = pd.read_excel(file_path+'daily_10_cut.xlsx',header=None)[0].tolist()
        factor_daily = list(set(factor_daily_all) - set(factor_daily_cut))

        factor_13 = pd.read_excel(self._myfile_path + '/factor_13_new.xlsx',header=None)[0].tolist()
        ...
        factor_list_all = list(set(factor_list_all)&set(factor_exist))

        sample = sample[sample[self._label_name].values!=0]

        X_in_sample = data_train.loc[:,factor_list_all]
    ...
    def label_predict(self, sample_daily, factor_list):
        ...
        X = sample_daily[factor_list_all]
        weight = [-6, 0, 0, 0, 2, 10]
