...
def single(k,v):
    ...
    print(k+' optimize')
    last_weight = None
    w_path = save_path+t+'_'+k+'_'+str(capital_dict[capital])+'_'+str(amt_limit)+'_'+str(v[0])+'_w1.pkl'
    w = optimize_hf({k:v.loc[open_date:close_date] for k,v in hf_model.items()},turn_ad, hedge_index, capital, barra_limit_dict=barra_limit_dict, industry_loose=industry_loose, amt_limit=amt_limit, 
