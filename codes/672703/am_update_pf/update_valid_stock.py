...
def update_valid_stock(today,time,invalid_stk=None,pool_stock_path=''):
    s = FactorData()
    ...
    # quant_pool
    q_pool = pd.read_excel(pool_stock_path+'quant_pool_20190905.xls', sheet_name=0, header=0, squeeze=False, converters={'证券代码':str})
    ...
    if time == '0930':
        ...
        close = pd.read_pickle('/data/group/800020/AlphaDataCenter/Basic/minute/Close/%s.pkl' % today_date)
    ...
    valid_stk = {'trade_stk':valid_stk, 'maxup_stk':maxup_stk, 'maxdown_stk':maxdown_stk, 'valid_pre_close':valid_pre_close}
    np.save('/data/group/800020/AlphaTask/valid_stock/valid_stock/valid_stock_' + today_date + '_' + time + '.npy', valid_stk)
