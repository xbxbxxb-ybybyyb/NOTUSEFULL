...
def update_basic_data(start_date,end_date):
    ...
    exmp = pd.read_pickle('/data/group/800020/AlphaDataCenter/Basic/minute/Close/'+start_date+'.pkl')
    print(exmp.tail())
    assert exmp.iloc[-1,:].isnull().sum()!=len(exmp.columns),'minute data not prepared!'

    m_status = pd.read_pickle('/data/group/800020/AlphaDataCenter/Basic/minute/Minute_Status.pkl').loc[start_date]
