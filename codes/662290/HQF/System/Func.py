...
def execute(taskMeta, broadcast, libraryName):
    """
    ...
    strategy = taskMeta.getStrategyBase().toStrategy()
    preDayTicks = []
    for i in range(broadcast.getLenForLoop()):
        ...
        # Previous Day Ticks with Filling Ticks
        if i == 0:
            sliceData = broadcast.getSliceData()
            preDayTicks.append(sliceData)
        ...
        # if len(preDayTicks) != 0:
            ...
            try:
                ...
                for j in range(sliceNum - 1):
                    ...
                    # 补的原则是时间+3秒、量和额为0、逐笔成交为空，盘口信息等不变
                        ...
                        while tickNeedFillingNum > 0:
                            ...
                            # t4 = dt.datetime.now()
                            # print("onMarketData time:", (t4 - t3).total_seconds())
                            factor, factorName, timestamp, tag = executeStrategy.getOutput()
                            factors.append(factor)
                            timestamps.append(timestamp)
                            tags.append(tag)
                ...
                Utils.output2HBase(factors, subTags, broadcast.getTradingUnderlyingCode(), factorName, libraryName, taskMeta)
            except:
                py = Pyfile()
                s = taskMeta.getTradingUnderlyingCode()[0]
                d = "".join(taskMeta.getDays()[1].split("/"))
                with py.open("015618/jiangweiyinzi_problemstocks/" + d + "_" + s + ".csv", "wb"):
                    pass
