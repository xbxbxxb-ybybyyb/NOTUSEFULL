...
from System import Func as Func
...
class StrategyManagement:
    def __init__(self):
        self.__MAX_EXECUTOR_TASK_NUM = 100
        self.__MAX_EXECUTOR_INSTANCES = 200
        self.__MAX_TASKMETAS_BATCH_SIZE = 100000
    ...
    def start(self):
        ...
        print(len(taskMetas))
        middleTimeStamp = dt.datetime.timestamp(dt.datetime.now())
        print('splitTask time: ' + str(round((middleTimeStamp - startTimeStamp) / 60, 2)) + " min")
        ...
        try:
            print("start")
            taskMetasList = self.__splitTaskMetas(taskMetas)
            for tms in taskMetasList:
                self.__runTasks(tms)
            # self.__mergeOutput()
        finally:
            try:
    ...
    def __initEnv(self):
        ...
        sparkConf.set('spark.network.timeout', '1800')
        # sparkConf.set('spark.dynamicAllocation.enabled', 'false')
        sparkConf.set('spark.dynamicAllocation.enabled', 'true')
        sparkConf.set('spark.yarn.queue', 'root.app.xquant.StrategyTrading')
        sparkConf.set('spark.dynamicAllocation.initialExecutors', str(self.__MAX_EXECUTOR_INSTANCES))
        # sparkConf.set('spark.dynamicAllocation.minExecutors', str(self.__MAX_EXECUTOR_INSTANCES))
        sparkConf.set('spark.dynamicAllocation.maxExecutors', str(self.__MAX_EXECUTOR_INSTANCES))
        sparkConf.set('spark.dynamicAllocation.initialExecutors', str(self.__MAX_EXECUTOR_INSTANCES))
        if 'ENTRYPOINTS_JSON_FILE' in os.environ:
            ...
            with open(port_file_path, 'r') as port_file:
                ...
                for i in range(0, port_num, 4):
                    if i + 3 < port_num and is_free_port(int(port_list[i]['port'])):
                        self.__free_port_list = port_list[i:i + 4]
                        break
                ...
                sparkConf.set('spark.yarn.rmProxy.enabled', 'false')
                sparkConf.set('spark.ui.port', self.__free_port_list[3]['port'])
...
#        print('daysList = {}'.format(daysList))
        print("yy", len(codeGroups), len(daysList), daysList[0])
    ...
    def __splitDays(self, strategy):
        ...
        tradeDatesInt = getTradingDay(startDateInt, endDateInt)
        tradeDatesStr = list(map(str, tradeDatesInt))
        datesStr = [date[:4] + "/" + date[4:6] + "/" + date[6:] for date in tradeDatesStr]
        ...
        allSplitDays = [splitDays] * len(codeGroups)

        return allSplitDays

    def __splitTaskMetas(self, taskMetas):
        batch_num = len(taskMetas) // self.__MAX_TASKMETAS_BATCH_SIZE + 1
        taskMetasList = [taskMetas[i * self.__MAX_TASKMETAS_BATCH_SIZE: (i + 1) * self.__MAX_TASKMETAS_BATCH_SIZE] for i
                         in range(batch_num)]
        if len(taskMetasList[-1]) < self.__MAX_TASKMETAS_BATCH_SIZE // 2 and len(taskMetasList) > 1:
            lastTaskMetas = taskMetasList.pop()
            taskMetasList[-1].extend(lastTaskMetas)
        return taskMetasList
    ...
    def __runTasks(self, taskMetas):
        ...
        # partition_num = executor_instances
        dic = {'partition_num':partition_num,'pickledTaskMetas':pickledTaskMetas}
...
def zipPythonFiles():
    ...
    with zipfile.ZipFile(writer, 'w', zipfile.ZIP_DEFLATED) as ziph:
        for subDir in ['Factor', 'NonFactor', 'System', 'Tag', 'Rpc']:  # , 'xquant1']:
        ...
        dataprovider_prop_file = '__dataprovider_config.py'
        ...
        if path.exists(path.join(prop_base_dir, dataprovider_prop_file)):
                        ziph.write(path.join(prop_base_dir, dataprovider_prop_file),
                                    arcname=dataprovider_prop_file)
...
def makeTaskFunc(javaHome, hadoopHome, libhdfsDir, func, srcDir, dstDir, tmpEnvDir, libraryName, rpcHost, rpcPort):
    ...
    def f(it):
        ...
        for pickledTaskMeta in it:
            ...
            exec('Worker.work(dfs, taskMeta, func0, srcDir0, dstDir, libraryName0)')
...
def getEnvFunctions():
    ...
    def prepareEnv(dfs, dstDir, tmpEnvDir):
        ...
        # sys.path.append(path.join(localTmpDir, 'xquant1'))
...
def generate_prop_file():
    try:
        ...
        exec("import MDCDataProvider.DataProvider_ini as DPI")
        mdcdataprovider_prop = eval("DPI.get_dataprovider_config()")
        mdcdataprovider_prop_str = 'dataprovider_ini_dict = ' + json.dumps(mdcdataprovider_prop)
        with open(path.join(prop_file_base_dir, '__dataprovider_config.py'), 'w') as dataprovider_prop_writer:
            fcntl.flock(dataprovider_prop_writer, fcntl.LOCK_EX)
            dataprovider_prop_writer.write(mdcdataprovider_prop_str + '\n')
            fcntl.flock(dataprovider_prop_writer, fcntl.LOCK_UN)
    except:
        pass
