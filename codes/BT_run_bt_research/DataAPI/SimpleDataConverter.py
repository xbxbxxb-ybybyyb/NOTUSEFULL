from DataAPI.SliceData import SliceData
            ...
            if stockData is not None:
                ...
                for i in range(len(stockData['TimeStamp'])):
                    ...
                    if i != len(stockData['TimeStamp']) - 1:
                        next_timestamp = stockData['TimeStamp'][i + 1]
                    else:
                        next_timestamp = stockData['TimeStamp'][-1]
                    time = stockData['Time'][i]
                    slice_data = SliceData(code=code, timestamp=timestamp, next_timestamp=next_timestamp, time=time,
