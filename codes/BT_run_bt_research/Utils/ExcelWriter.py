...
def OutputSpreadSheet(detailedOrders, result, dirPath, funStr, result_name=""):
    ...
    py = Pyfile()
    if py.exists(dirPath):
        py.delete(dirPath)
    with py.open(dirPath, 'wb') as f:
        wb.save(f)
            ...
            for j in range(len(detailedOrders[i])):
                for k in range(len(DetailedOrdersTitleList)):
                    if k == 0:
                        ...
                        if DetailedOrdersTitleList[k] == "orderAmount" or DetailedOrdersTitleList[k] == "cumAmount" or \
                                DetailedOrdersTitleList[k] == "quantity" or DetailedOrdersTitleList[k] == "cumQty":
...
def OutputDate(wb, orders_data, funStr, result):
    ...
    if funStr is None:
        ...
        for i in range(len(dateList)):
            ...
            EarnLossRate = round(-afterCostProfit_earn / afterCostProfit_loss,
                                 2) if afterCostProfit_loss != 0 else "nan"
