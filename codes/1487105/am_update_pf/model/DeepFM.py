...
class DeepFM():
    ...
    def get_model(self, sample, factor_list):
        ...
        # for factor_name in factor_list:
        #     boundary = boundaries[factor_name]
        #     data_recode_train = self.Layer_equally_boundary(data_train[factor_name].copy(),factor_name,layer_num,boundary)
        ...
        for i, (train_idx, valid_idx) in enumerate(folds):
            ...
            dfm_params = {
                ...
                "learning_rate": 0.0009,#0.001  # can use a bigger as lr0,then decay
                "optimizer_type": "adam",
                "batch_norm": 1,
                "batch_norm_decay": 0.995,
                "l2_reg": 0.9, #0.01
                "l1_reg": 0.000001, #0.01#0.000001
                "verbose": True,
                "loss_type":'mse',
    ...
    def transform_norm(self,data):
        value = data.values
        t_ = (value-value.mean())/(value.std())
        data = pd.Series(t_,index=data.index)
        return data
