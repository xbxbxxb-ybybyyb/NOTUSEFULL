...
def get_mkt_indus(path, start_date, end_date, shift_lag=1,barra_factors = []):
    ...
    for f in barra_factors:
        neu_basic[f] = pd.read_pickle(barra_data_path + f+'.pkl').shift(shift_lag).loc[start_date:end_date].stack(dropna=False)
    return neu_basic
...
def get_future_return(basic_daily_path, trade_price, price_type='1300_1459', lag_list=[1, 3, 5], shift_lag=1,
                      winsard_flag=True, norm_flag=True,sharp_flag=True,
                        roll=True,fill_value=np.nan,fake_team=True,neu_flag=True):
    industry_code_all = pd.read_pickle(basic_daily_path + 'industry_code_all.pkl').shift(shift_lag).loc[
        trade_price.index]
    is_valid = pd.read_pickle(basic_daily_path + 'is_valid.pkl').shift(shift_lag).loc[trade_price.index]
    adjfactor = pd.read_pickle(basic_daily_path + 'adjfactor.pkl').loc[trade_price.index]
    ...
    future_return_df = future_return_df[future_return_df['is_valid'] == 1]
    if neu_flag:
        data = future_return_df[ori_label_list]
        size_flag =True
        tool_name = ['is_valid','industry_code_all']
        for t in tool_name:
            tmp = pd.read_pickle(basic_daily_path + t + '.pkl').shift(shift_lag).loc[date_list[0]:date_list[-1]]
            data[t] = tmp.stack(dropna=False)
        barra_factors= []
        neu_basic = get_mkt_indus(basic_daily_path, date_list[0], date_list[-1], shift_lag=shift_lag,barra_factors=barra_factors)
        data[neu_basic.columns] = neu_basic
        data = data[data['is_valid'] == 1]
        data.index = data.index.remove_unused_levels()
        data = data.reset_index().rename(columns={'level_0': 'date', 'level_1': 'stock'})
        ...
        data = data.set_index(['date', 'stock'])
        if not size_flag:
            norm_list = [f for f in neu_basic.columns if f!='log_mkt']
            how_norm=None
        else:
            norm_list = neu_basic.columns
            how_norm=['log_mkt']
        data = neutral_label(data[ori_label_list], data[norm_list], data_norm=ori_label_list,how_norm=how_norm)
        future_return_df[['neu_'+l for l in ori_label_list]] = data 
        all_label_list+=['neu_'+l for l in ori_label_list]
    if winsard_flag:
...
def pm_update_label(label_path,start_date, end_date,winsard_flag=True, norm_flag=True,sharp_flag=False,lag_list=[1,3,5],
                    fill_value=0,transaction_time='1300', transaction_period=120):
    ...
    trade_price = get_trade_price(basic_daily_path, minute_data_path, label_start_date, label_end_date,
                                  transaction_time=transaction_time, transaction_period=transaction_period)
    price_type = get_price_type(transaction_time,transaction_period)
    label = get_future_return(basic_daily_path, trade_price, price_type = price_type, lag_list=lag_list, shift_lag=1,
    ...
    for date in label_date_list:
        ...
        for l in lag_list:
            tmp = get_label_bin(tmp, price_type+'_re_'+str(l)+'d')
        tmp.rename(columns={'level_0': 'date', 'level_1': 'columns'}, inplace=True)
...
def pm_update_vwap_label(label_path, start_date, end_date,winsard_flag=True, norm_flag=True,sharp_flag=False,lag_list=[1,3,5],fill_value=0):
    ...
    label_vwap_team = get_future_return_team(basic_daily_path, trade_price_vwap, price_type='vwap', 
                lag_list=lag_list, shift_lag=0,winsard_flag=True, norm_flag=True)
...
def load_sample(factor_list,sample_dates=[],strategy_type='lf',label_name='vwap_re_5d',train_flag=True):
    ...
    for day in sample_dates:
        ...
        for k,v in factor_info.items():
            if len(v)>0:
                print(factor_day_path_dict[k][0],v[0])
                if hf_flag:
                    feature_day = fa.tradingday(day,-2)[0]
                df = pd.read_pickle(factor_day_path_dict[k][0]+feature_day+'.pkl')
                if sample_day is None:
                    sample_day = df[['stock']+v]
                else:
                    sample_day = sample_day.merge(df[['stock']+v],on=['stock'],how='left')
        for k,v in factor_fix_info.items():
            if len(v)>0:
             
                df = pd.read_pickle(feature_fix_path+strategy_type+'/feature/'+day+'.pkl')
                if sample_day is None:
                    sample_day = df[['stock']+v]
                else:
                    sample_day = sample_day.merge(df[['stock']+v],on=['stock'],how='left')
        if (train_flag):
            ...
            for l in label_name:
                sample_day = sample_day[~np.isnan(sample_day[l])]
        sample_day['date'] = pd.to_datetime([day]*len(sample_day))
...
def update_sample(start_date, end_date=None, mode=0):
    ...
    print('depart_day_factors:',len(depart_day_factors),'own_day_factors:',len(own_day_factors))
    fix_dict_1000 = pd.read_pickle(fix_dict_path)
    fix_dict = {'0930':[]}
    fix_dict.update(fix_dict_1000)
    if mode == 0:
        ...
        for timepoint in sorted(fix_dict.keys()):
            print(timepoint)
            this_fix_path = feature_fix_path+timepoint+'/'
            this_fix_depart_feature_path = this_fix_path+'feature/'
            this_fix_depart_label_path = this_fix_path+'label/'
            need_path = [this_fix_depart_feature_path,this_fix_depart_label_path]
            for p in need_path:
                if not os.path.exists(p):
                    os.makedirs(p)
            if timepoint!='0930':
                fix_factors = fix_dict_1000[timepoint]
                am_update_depart_feature(this_fix_depart_feature_path,fix_factors, start_date, end_date, check=True, silent=False)
            pm_update_label(this_fix_depart_label_path,start_date, end_date,winsard_flag, norm_flag,sharp_flag,lag_list,fill_value,
                        transaction_time=timepoint, transaction_period=30)
        lm.sendMessage('fix feature and label update finish')
