...
def update_portfolio(capital_fake,reb_path,today,time = '0930',pf_code = '5160803',reb = True,close_after_open = False,
            index_price = 0.,first_open = False,open_capital = 1e9,mode=1,benchmark='500',test_label=False,version='V0'):
    
    files = os.listdir(pool_stock_path)
    file_today = 'trading_pool_%s.xls' % str(today)
    print('O32 file:',file_today)
    assert file_today in files,print('Error! No O32 file:',file_today)
    year=today[:4]
    ratio_=0.8
    ...
    if reb:
        ...
        amt_limit,single_stock_max_weight,short_capital,mode=mode,benchmark=benchmark,test_label=test_label)
        
        # save daily weight
        if test_label is True:#@
            try:
                os.makedirs(save_weight_path)
            except:
                pass
            w.to_pickle(save_weight_path+'%s.pkl' % today_dt)                                        
        if time in ['0930vwap','0930vwap300','0930lf']:
            ...
            if test_label is False:
                t0_xlsx.to_excel(order_path+'T0_file/'+'T0可用额度_' + pf_code + today_dt + '_' + time + '_%s.xlsx' % version)
...
#            reb_xlsx.to_excel(reb_path + pf_code + '_rebalance_' + today_dt + '_' + time + '.xlsx')
            reb_xlsx.to_excel(order_path+'Rebalance_file/' + pf_code + '_rebalance_' + today_dt + '_' + time + '_%s.xlsx' % version)
        print(reb_path + pf_code + '_rebalance_' + today_dt + '_' + time + '.xlsx')

        # calculate buy and sell amount
        if time[:4]=='0930':
            close = pd.read_pickle(config_path.basic_data_path + 'daily/pre_close.pkl').loc[today_dt]
            #'/daily/pre_close.pkl'
        else:
