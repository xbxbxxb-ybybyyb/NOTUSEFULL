...
class Neutralize(object):
    ...
    def neutralize_help(self,factor_data):
        ...
        model=linear_model.LinearRegression(fit_intercept=False,n_jobs=num_threads)
...
class Demean(object):
    ...
    class to stardardize 
    """
    def __init__(self,data,inf2nan=True,axis=1):
        self.data = data
        self.data= self.data.astype(np.float64)
        if inf2nan==True:
            self.data[np.isinf(self.data)] = np.nan
    ...
    def demean(self):
        return self.data.apply(lambda x:self.demean_help(x),axis=self.axis)
        
    
    def demean_help(self,arr):
        if len(arr[~np.isnan(arr)]):
            mu = np.nanmean(arr)
            arr = (arr - mu)   
        return arr
...
def demean(data,inf2nan=True, axis=1):
    return Demean(data,inf2nan,axis).demean()
def standard_winsor(data,axis=1):
    return standardlize(winsorize(data,axis=axis),axis=axis)
def apply_parallel(dfGrouped, func):
    retLst = Parallel(n_jobs=int(multiprocessing.cpu_count()/2))(delayed(func)(group,axis=0) for name, group in dfGrouped)
...
def neutralize(data,how,data_norm=None,how_norm=None):
    ...
    data[data_norm]= apply_parallel(data[data_norm].groupby(level=0),standard_winsor)
    feature_list = data.columns
    how[how_norm]= apply_parallel(how[how_norm].groupby(level=0),standard_winsor)
    data = pd.concat(Parallel(n_jobs=int(multiprocessing.cpu_count()/2))(delayed(neu_help)(data[x],how) for x in feature_list),axis=1)
    data.columns = feature_list
    return data

def neutral_label(data,how,data_norm=None,how_norm=None):
    if data_norm is None:
        data_norm = data.columns
    if how_norm is None:
        how_norm = how.columns
    data[data_norm]= apply_parallel(data[data_norm].groupby(level=0),demean)#对标签中性化前只做demean操作，zscore好像会变差
    feature_list = data.columns
    how[how_norm]= apply_parallel(how[how_norm].groupby(level=0),standard_winsor)
    data = pd.concat(Parallel(n_jobs=int(multiprocessing.cpu_count()/2))(delayed(neu_help)(data[x],how) for x in feature_list),axis=1)
    data.columns = feature_list
