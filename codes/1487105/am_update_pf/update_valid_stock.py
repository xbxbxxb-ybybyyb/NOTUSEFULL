...
#    black_list = ['000415.SZ','000564.SZ','000585.SZ','000616.SZ','000796.SZ','300052.SZ','300350.SZ','300437.SZ','301089.SZ','600112.SH','600218.SH','600221.SH','600306.SH','600387.SH','600515.SH','600555.SH','600751.SH','600968.SH','603032.SH','605286.SH'] # 20211108
    black_list = ['000415.SZ','000564.SZ','000585.SZ','000616.SZ','000796.SZ','300052.SZ','300350.SZ','300437.SZ','301089.SZ','600112.SH','600218.SH','600221.SH','600306.SH','600387.SH','600515.SH','600555.SH','600751.SH','600968.SH','603032.SH','605286.SH','605333.SH'] # 20211117
...
#        df = s.get_factor_value('Basic_factor', stock = stk_list, mddate=[today_date], factor_names=['mdc_trade_status'])
...
#                        & np.array(adj_today['mdc_trade_status']!='')].index.tolist()             
    ...
    pre_close_today=df['mdc_pre_close'][today_date].to_frame()
    if pre_close.index[-1] == pd.to_datetime(today_date):
        pass
    else:
        pre_close_today = pre_close_today.reindex(index=pre_close.columns).T
    ...
    # adjfactor
    pre_close=pd.read_pickle(config_path.basic_data_path+'/daily/adjfactor.pkl')
    pre_close_today=df['mdc_adjfactor'][today_date].to_frame()
    if pre_close.index[-1] == pd.to_datetime(today_date):
        pass
    else:
        pre_close_today = pre_close_today.reindex(index=pre_close.columns).T
        pre_close_today.index=[pd.to_datetime(today_date)]
        pre_close = pre_close.append(pre_close_today)
        pd.to_pickle(pre_close,config_path.basic_data_path+'/daily/adjfactor.pkl')        
