...
class MinuteUpdateXquant(object):
    ...
    def update_one_date(self, date):
        ...
        for var, save_var in self.field_map.items():
            if not os.path.exists(self.minute_store_path + save_var + '/'):
                os.makedirs(self.minute_store_path + save_var + '/')
...
if __name__ == '__main__':
    ...
    minute_store_path = data_center_path+'Basic/minute/'
    universe_path='/data/user/012620/AlphaDataCenter/PoolManagement/'
    mk_path_list = [minute_store_path,universe_path]
    for path in mk_path_list:
        if not os.path.exists(path):
            os.makedirs(path)
    instance = MinuteUpdateXquant(date_list,minute_store_path,universe_path)
