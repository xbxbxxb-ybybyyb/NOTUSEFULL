...
class DailyDataUpdateXquant(object):
    ...
    """
    def __init__(self,start_date='',end_date='',save_raw_flag=False,hf_flag=True,data_center_path=''):
        ...
        self.xquant = DataIOFromXquant(start_date=self.start_date, end_date=self.end_date, save_flag=save_raw_flag,
                universe_path=self.data_center_path+'PoolManagement/',
                save_path=self.data_center_path+'Raw/daily/')
...
#        np.save('/data/user/013417/temp/invalid_today.npy', invalid_data)
        if invalid_data:
    ...
    def wrangle_up_down_limit(self, price, pre_close, is_valid_raw, limit=0.1):
        cyb_reform_pre_date = '20200821'
        cyb_reform_date = '20200824'
        is_valid = is_valid_raw.copy()
        is_valid0 = is_valid.loc[:cyb_reform_pre_date]
        is_valid1 = is_valid.loc[cyb_reform_date:]
        all_stocks = is_valid_raw.columns
        cyb_stocks = sorted([s for s in all_stocks if s.startswith('3')])
        cyb_limit = limit+0.1
        other_stocks = sorted(list(set(all_stocks)-set(cyb_stocks)))
        cond0 = (pre_close*(1-limit) >= price)|(pre_close*(1+limit) <= price)
        cond1 = (pre_close*(1-cyb_limit) >= price)|(pre_close*(1+cyb_limit) <= price)
        is_valid0[cond0.loc[:cyb_reform_pre_date]] = 0
        is_valid0[cond0.loc[:cyb_reform_pre_date]] = 0
        for date in is_valid1.index:
            other_stock_limit = set(other_stocks)&set(cond0.loc[date].loc[cond0.loc[date]==True].index)
            is_valid1.loc[date,other_stock_limit] = 0
            cyb_stock_limit = set(cyb_stocks)&set(cond1.loc[date].loc[cond1.loc[date]==True].index)
            is_valid1.loc[date,other_stock_limit] = 0
        is_valid = is_valid0.append(is_valid1)
        return is_valid
...
#            index_data = np.array(index_data)
    ...
    def update_index_industry_data(self):
        ...
        if not raw_index_industry:
            ...
            print('Industry code done.')
#            pd.to_pickle(raw_index_industry_, '/data/user/013417/temp/idx_code.pkl')
#            pd.to_pickle(self.universe, '/data/user/013417/temp/stocks.pkl')
...
#     end_date_list = ['20151231','20141231','20131231','20121231','20111231','20101231']  
    ...
    data_center_path = '/data/user/012620/AlphaExperiment/temp/AlphaDataCenter/'
    if start_date < '20150101': hf_flag = False
    instance = DailyDataUpdateXquant(start_date=start_date,end_date=end_date,hf_flag=hf_flag,data_center_path=data_center_path)
