...
market_list = pd.read_excel(transit_path+'market_data.xlsx',header=None)[0].tolist()

# Xquant数据读入
from xquant.factordata import FactorData
factorData = FactorData()
stock_list = pd.read_pickle(basic_data_path+'daily/is_valid.pkl').columns.tolist()
...
for p in market_list[:6]:
    ...
    if True in (temp > 3000):
        print('Invalid Price Data. Price Bigger than 3000: ', p)
#        print(database[(temp > 1500)])
#        error.append('Invalid Price Data. Price Bigger than 2000: '+ p)
...
# 涨跌幅是否在20%内
flag = (database['high_badj'].values -  1.205 * database['pre_close_badj'].values > 0.0)
temp=database[flag]
...
if temp.size>0:
    ...
    print('Invalid Price Data. Up Range Over 20%.')
    error.append('Invalid Price Data. Up Range Over 20%.')


flag = (database['low_badj'].values -  (1-0.205) * database['pre_close_badj'].values < 0.0)
temp=database[flag]
temp=temp[(temp['trade_status']=='交易')&(temp['close']>1)]
if temp.size>0:
    print(temp[['high','low','close','pre_close','pct_chg','high_badj','low_badj','close_badj','pre_close_badj','adjfactor','trade_status','stpt']])
    print('Invalid Price Data. Down Range Over 20%.')
    error.append('Invalid Price Data. Down Range Over 20%.')
