...
def update_portfolio(capital_fake,reb_path,today,time = '0930',pf_code = '5160803',reb = True,close_after_open = False,
    ...
    print('time:',time,'last:',last_dt,'today',today_dt,'model_list:',model_list)
    excess_type = config_path.name_map[time]
    save_weight_path = config_path.data_root_path+'/AlphaDataCenter/Department/DailyWeight/benchmark_'+benchmark+'/'+excess_type+'/'
    order_path = config_path.order_path
    all_name = 'All_'+benchmark
    act_path = config_path.data_root_path+ '/AlphaDataCenter/Department/DailyPrediction/'+excess_type+'/'+all_name+'/'
    print(pf_code,excess_type,act_path)
#    if '0930' in time:
#        get_act(last_dt,model_list,excess_type,all_name,mode=1)
#        # o32_path = '/data/user/011477/order/O32/morning/综合信息查询_组合证券' + today_dt + '_516.xls'
#    elif time == '1300':
...
#        # o32_path = '/data/user/011477/order/O32/noon/综合信息查询_组合证券_' + today_dt + '_1130.xls'
    ...
    if pf_code in ['']:#['5160803']:
        predict_path = config_path.data_root_path + '/AlphaDataCenter/DailyPrediction/'
    else:
        predict_path = config_path.data_root_path + '/AlphaDataCenter/Department/DailyPrediction/'
...
#        o32_path = '/data/user/011477/order/O32/noon/综合信息查询_组合证券_' + today_dt + '_1130.xls'        
    ...
    o32_path = '/data/user/011477/order/O32/afternoon/综合信息查询_组合证券_' + last_dt + '.xls'
    o32_path_ = o32_path
    # prepare
    if not first_open:
        try:
        ...
        last_position, valid_position = (long['持仓'], long['持仓'])
    ...
    valid_stk_ = np.load(config_path.valid_stock_path+'/valid_stock_' + today_dt + '_' + '1500' + '.npy').item()
    ...
    if pf_code=='5160304':
        ...
        turn_ad = 0.4#0.4
        amt_limit = 0.025   
        single_stock_max_weight=0.0075            
        hedge_index = 'HS300'
        barra_limit_dict = {
        ...
        index_w = pd.read_pickle(config_path.data_root_path+'/AlphaDataCenter/Basic/daily/HS300_data.pkl').iloc[-1]
    ...
    if close_after_open:
        ...
        index_price = pd.read_pickle(config_path.data_root_path+'/AlphaDataCenter/Basic/daily/close_000905SH.pkl').loc[last_dt].iloc[0]

    # check the percentage of valid stocks
    is_valid_raw = pd.read_pickle(config_path.data_root_path+'/AlphaDataCenter/Basic/daily/is_valid_raw.pkl').iloc[-1]
...
#        reb_xlsx.to_excel(order_path+'Rebalance_file/' + pf_code + '_rebalance_' + today_dt + '_' + time + '.xlsx')
        ...
        if time[:4]=='0930':
            close = pd.read_pickle(config_path.data_root_path+'/AlphaDataCenter/Basic/daily/close.pkl').loc[last_dt]
        else:
            close = pd.read_pickle(config_path.data_root_path+'/AlphaDataCenter/Basic/minute/Close/' + today_dt + '.pkl').iloc[119]
        ...
        if time == '0930vwap':        
            lm.sendMessage("{0} 调仓文件和T0文件已上传，调仓股票数量{1},T0股票数量{2}, 换手率{3}%,   空头 (万元): {4},多头 (万元):{5}, 买入金额(万元):{6},卖出金额(万元):{7},{8}".\
            ...
            lm.sendMessage("{0} 调仓文件已上传，调仓股票数量{1}, 换手率{2}%,   空头 (万元): {3},多头 (万元):{4}, 买入金额(万元):{5},卖出金额(万元):{6},{7}"\
