...
def FutureBias(today,IC_contact,open_capital_hf=None,open_capital_vwap=None):
    result1 = factor_tool.tradingday(today,-2)
    pre_date = result1[-2]
    date = result1[-1]

    df = ma.getMDSecurityTickDataFrame("%s.CF" % IC_contact,"%s140000" % pre_date,"%s150000" % pre_date,0)
    IC05 = df['LastPx'].iloc[-1]
    print(date,pre_date,IC05)
    point_price = 200
    # stats=pd.DataFrame(index=['QuantMachine','AlphaHunter'],columns=['新增期货手数','期货合约市值（万）','现货中属于指数成份股的市值（万）'])
    zz500_w = pd.read_pickle('/%s/Basic/daily/ZZ500_data.pkl' % config_path.data_center_path).loc[pre_date]
    zz500_w = zz500_w[zz500_w>0]
    zz500_w.index=[c[:6] for c in zz500_w.index]
    info_dict = {}
    if open_capital_hf is not None:
        info_dict['AlphaHunter'] = (open_capital_hf,'5160803')
    if open_capital_vwap is not None:
        info_dict['QuantMachine'] = (open_capital_vwap,'5160503')

    stats=pd.DataFrame(index=pd.MultiIndex.from_product(([date],info_dict.keys())),columns=['新增期货手数','期货合约市值（万）','现货中属于指数成份股的市值（万）'])
    for k,v in info_dict.items():
        capital = v[0]
        if k == 'QuantMachine':
            real_w = pd.read_excel('/data/user/013547/建仓权重文件/5160503/'+v[1]+'_zz500_'+date+'_percentage_am.xls',header=1,converters={'证券代码':str})
        else:
            real_w = pd.read_excel('/data/user/013547/建仓权重文件/5160803/'+v[1]+'_zz500_'+date+'_percentage_am.xls',header=1,converters={'证券代码':str})
        real_w.set_index('证券代码',inplace=True)
        real_w = real_w['设置比例']/100
        print(k,'weight sum：',real_w.sum())
        stats.loc[(date,k),'新增期货手数']= math.ceil(capital/point_price/IC05)
        stats.loc[(date,k),'期货合约市值（万）'] = round(stats.loc[(date,k),'新增期货手数']*point_price*IC05/1e4,2)
        stats.loc[(date,k),'现货中属于指数成份股的市值（万）'] = round((real_w.loc[zz500_w.index]*capital).sum()/1e4,2)
...
def Open_file_save(today_open_fw,time,pf_code,today_dt):
    template_path = '/%s/AlphaTask/Noon_update_process/template.xlsx' % config_path.data_root_path
    if time[:4] == '0930':
        day = 'am'
    else:
        day = 'pm'
    tmp_path = '/%s/AlphaTask/Noon_update_process/temp.xlsx' % config_path.data_root_path
    output_path = '/data/user/013547/建仓权重文件/%s/%s_zz500_%s_percentage_%s.xls'%(pf_code,pf_code,today_dt,day)
...
def pf_generator(today_act, time, last_position, valid_position, valid_stk, pf_code, turn_ad, hedge_index, capital, cash, \
    ...
    last_dt = result1[-2]      
    close = pd.read_pickle('/%s/AlphaDataCenter/Basic/daily/close.pkl' % config_path.data_root_path)
...
#    print('lower_limit',lower_limit)
    ...
    if time in ['0930','1300']:
        w = optInd.optimize_hf({time[:4]:today_act}, {time[:4]:turn_ad}, hedge_index, capital, barra_limit_dict=barra_limit_dict, industry_loose=industry_loose, 
                        ...
                        single_stock_max_weight=single_stock_max_weight_,simulate_label=True)
    ...
    if abs(w[time[:4]].loc[today] - last_weight).sum()==0:
