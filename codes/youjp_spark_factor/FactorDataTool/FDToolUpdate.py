...
class FDToolUpdate:
        ...
        for trading_day in trading_day_list:
            for market in MARKET_TYPE:
                ...
                if not stock_set.empty:
                    self.fa.update_factor_value(self.lib_name, stock_set, market, trading_day)
    ...
    def update_hset_index(self, start_date, end_date):
        ...
        for trading_day in trading_day_list:
            for index in INDEX_TYPE:
                ...
                stock_set.columns = HSET_HBASE_COLUMNS
                if not stock_set.empty:
                    self.fa.update_factor_value(self.lib_name, stock_set, index, trading_day)
    ...
    def update_hset_industry(self, start_date, end_date):
        ...
        for trading_day in trading_day_list:
            ...
            for indus in CITICS_I_CODE + CITICS_II_CODE:
                ...
                stock_set.columns = HSET_HBASE_COLUMNS
                if not stock_set.empty:
                    self.fa.update_factor_value(self.lib_name, stock_set, indus, trading_day)
            ...
            for indus in SW_I_CODE + SW_II_CODE:
                ...
                if not stock_set.empty:
                    ...
                    self.fa.update_factor_value(self.lib_name, stock_set, indus_shenwan, trading_day)
    ...
    def update_hsi(self, start_date, end_date):
        ...
        for trading_day in trading_day_list:
            ...
            self.fa.update_factor_value(self.lib_name, indus_df, trading_day, "20200102")
    ...
    def update_cbond_stock_map(self, start_date, end_date):
        ...
        for trading_day in trading_day_list:
            ...
            self.fa.update_factor_value(self.lib_name, cbond_stock_df, trading_day, "20200102")
        ...
        for trading_day in trading_day_list:
            for future in INDEX_FUTURE_CODE:
                ...
                if not index_future_set.empty:
                    self.fa.update_factor_value(self.lib_name, index_future_set, "{}_IdF".format(future), trading_day)

def addFactors(lib_name):
    factor_names = HSET_HBASE_COLUMNS + HSI_HBASE_COLUMNS + CBOND_STOCK_HBASE_COLUMNS + INDEX_FUTURE_HBASE_COLUMNS
    for factor in factor_names:
        try:
            FactorData().add_factor(lib_name, [factor])
        except Exception as e:
            print(e)

def run_meta_task(lib_name, start_date, end_date):
    instance = FDToolUpdate(lib_name)
...
def execute_task(task):
    run_meta_task(task["lib_name"], task["start_date"], task["end_date"])
...
if __name__=="__main__":
    ...
    date_groups = split_calc_date_into_group(trading_days, 10)

    if mode == "Local":
        for group in date_groups:
            sub_start_date = group[0]
            sub_end_date = group[-1]
            run_meta_task(lib_name, sub_start_date, sub_end_date)
        ...
        for group in date_groups:
            sub_start_date = group[0]
            sub_end_date = group[-1]
            task = (lib_name, sub_start_date, sub_end_date, )
