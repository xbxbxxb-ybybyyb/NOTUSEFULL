...
from HFDataLoader.Config import TICK_SUFFIX, INDEX_TARGET_TICK_COLUMNS, TICK_INDEX_HBASE_COLUMNS
...
class IndusSynthetizeData:
    def __init__(self, lib_name, indus_code, tick_play_lag=0):
    ...
    def synthetize_industry_tick_data(self, start_date, end_date):
        """ 获取一段交易日区间内行业指数合成数据 """
        self.my_print("Start Synthetize Industry Tick Data: {}-{}-{}".format(self.indus_code, start_date, end_date))
        t1 = dt.datetime.now()
        ...
        if not full_stock_list:
            self.my_print("Empty Stock List: {}-{}-{}".format(self.indus_code, trading_day_list[0], trading_day_list[-1]))
            return pd.DataFrame(columns=INDEX_TARGET_TICK_COLUMNS + ["IsMock"])

        weight_df = self.load_weight_data(pre_trading_day_list, full_stock_list)   ### MultiIndex: date, stock, FreeFloatShares
        ...
        for pre_trading_day, trading_day in zip(pre_trading_day_list, trading_day_list):
            ...
            daily_weight = weight_df.loc[pre_trading_day].loc[stock_list, "FreeFloatShares"]   ### pd.Series, stock, FreeFloatShares
            ...
            if not daily_stock_tick_df_dict:
                ...
                self.my_print("Empty Stock Data: {}-{}".format(self.indus_code, trading_day))
            else:
                sub_indus_tick_df = self.synthetize_daily_tick_data(daily_fill_tick_df, daily_stock_tick_df_dict)
                if sub_indus_tick_df.empty:
                    sub_indus_tick_df = pd.DataFrame(columns=INDEX_TARGET_TICK_COLUMNS)
                    self.my_print("Empty Synthetize Industry Data: {}-{}".format(self.indus_code, trading_day))
            indus_tick_df_list.append(sub_indus_tick_df)
        indus_tick_df = pd.concat(indus_tick_df_list, axis=0)
        indus_tick_df["IsMock"] = 0
        t2 = dt.datetime.now()
        self.my_print("{} Synthetize Industry Data Time Cost: {}".format(self.indus_code, (t2 - t1).total_seconds()))
    ...
    def load_stock_tick_df(self, trading_day, stock_list, weight):
        ...
        for stock in stock_list:
            ...
            if not stock_tick_df.empty:
                ...
                self.my_print("Empty Data: {}-{}".format(stock, trading_day))
    ...
    def get_stock_tick_data(self, stock, date, map_col=True):
        ...
        try:
            tick_df = self.fa.get_factor_value(self.lib_name, "{0}_{1}".format(stock, TICK_SUFFIX), date, HBASE_COLUMNS)
        except:
    ...
    def synthetize_daily_tick_data(self, fill_tick_df, stock_tick_df_dict):
        ...
        for tick, this_tick_timestamp in enumerate(daily_indus_df["Timestamp"]):
            curr_slice_dict = {}  ### 保存当前TICK所有股票数据信息
