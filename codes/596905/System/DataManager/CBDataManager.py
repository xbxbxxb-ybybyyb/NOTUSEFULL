        ...
        for ua in self.__UASet:
            if self.__historicalTickDataLengthUA > 0:
                ...
                if "Tick" in self.__dataTypeUA:
                    ...
                    if self.__splitAdjustedNeeded:
                        self.__tickDataDict1SplitAdjusted[ua] = pd.DataFrame(
                            columns=self.__tickColumnNames1UA, dtype=np.float64
                        )
                        self.__tickDataDict2SplitAdjusted[ua] = pd.DataFrame(columns=self.__tickColumnNames2UA)

                    continue

            self.__tickDataDict1[ua], self.__tickDataDict2[ua] = self.__preprocessUATickData(
                ua, startDate, endDate
            )

            if self.__splitAdjustedNeeded:
                (self.__tickDataDict1SplitAdjusted[ua],
                 self.__tickDataDict2SplitAdjusted[ua]) = self.__preprocessUATickData(
                    ua, startDate, endDate
                )
    ...
    def __loadSplitAdjustedDataFirstTime(self):
        ...
        for stock, tickData in self.__tickDataDict2SplitAdjusted.items():
            if stock in self.__UASet:
                priceColumnIndex3 = self.__priceColumnIndex3UA
            else:
