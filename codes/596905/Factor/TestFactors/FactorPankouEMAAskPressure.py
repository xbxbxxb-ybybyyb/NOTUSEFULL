...
class FactorPankouEMAAskPressure(Factor):
    def __init__(self, config, factorManager):
        ...
        self._addIntermediate("askPressure", [])
    ...
    def calculate(self):
        ...
        askPressure = np.sum(np.diff(np.log1p(askEqualPriceList)) * self.__weightRatio) * 1000

        askPressureList = self.getIntermediate("askPressure")
        askPressure = self._EMA_calculate(askPressure, askPressureList, self.__lag)
        askPressureList.append(askPressure)
