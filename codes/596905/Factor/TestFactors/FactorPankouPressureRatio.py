...
class FactorPankouPressureRatio(Factor):
    ...
    def calculate(self):
        ...
        bidPressure = np.sum(np.diff(np.log1p(bidEqualPriceList)) * self.__weightRatio) * 1000

        if bidPressure > 0 and askPressure > 0:
            value = np.log(bidPressure / askPressure)
        else:
            value = 0.
