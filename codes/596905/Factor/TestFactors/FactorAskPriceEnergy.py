...
class FactorAskPriceEnergy(Factor):
    ...
    def calculate(self):
        ...
        if len(askPriceList) < self.__lag:
            factorValue = 0
        else:
            askPriceSlice = np.array(askPriceList[-self.__window:])
            imfs = EMD().emd(askPriceSlice, np.arange(askPriceSlice.shape[0]))
