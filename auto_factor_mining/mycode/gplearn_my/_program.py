...
class _Program(object):
    ...
    arities : dict
        一个字典映射表：artity的个数 -> 到具体的function
    ...
    def build_program(self, random_state):
        ...
        program = [function]
        terminal_stack = [function.arity] # function.arity为个数
        # len(program) == len(terminal_stack)
        # program[i].arity = terminal_stack[i]
        # 即第 i 个 program 需要的 输入参数个数 = 第 i 个 terminal_stack 

        while terminal_stack: # start 时候： len(terminal_stack) == 1
            depth = len(terminal_stack)  # == 1???
            # N个feature + N个算子
            ...
            if (depth < max_depth) and (method == 'full' or
                ...
                # stack的长度在计算，只用到了arity， 但还没有确定实参是谁
                ...
                if terminal == self.n_features:
                    ...
                    if self.const_range is None:
                        ...
                        raise ValueError('A constant was produced with '
                                         'const_range=None.')
                # 这里的program append的不再是一个function , 而是一个数字
                # terminal是一个数字，terminal: random_state.uniform
                # 把对应的倒数第一个terminal stack minus 1, 替换为现在的terminal对应的feature
    ...
    def execute_norm(self, X, y, decap_df, data_index):
        """
        calculate y_pred_raw
        """
    ...
    def raw_fitness(self, X, y, sample_weight, decap_df, data_index, parsimony_coefficient=None):
        ...
        tt = self.execute_norm(X, y, decap_df, data_index)
        
        # 使用valid进行过滤
        raw_fitness = self.metric(tt)
