...
def _average_pearson(tt_df, column_name="y_pred_raw", mode="avg"):
    """Calculate the average Pearson correlation coefficient."""

    # 先还原构造DataFrame，以便按天groupby
    # 合并真实标签

    def _pearson(df):
        w = np.ones((df.shape[0],))
        y, y_pred = df["1Day_return"].copy(), df[column_name].copy()
        with np.errstate(divide='ignore', invalid='ignore'):
            y_pred_demean = y_pred - np.average(y_pred, weights=w)
            y_demean = y - np.average(y, weights=w)
            corr = ((np.sum(w * y_pred_demean * y_demean) / np.sum(w)) /
    ...
    def _pearson2(df):
        w = np.ones((df.shape[0],))
        rk = pd.DataFrame()
        rk['y_pred_raw'] = df['y_pred_raw'].rank(ascending=False)
        rk['1Day_return'] = df['1Day_return'].rank(ascending=False)
        y, y_pred = rk["1Day_return"].copy(), rk["y_pred_raw"].copy()
        with np.errstate(divide='ignore', invalid='ignore'):
            y_pred_demean = y_pred - np.average(y_pred, weights=w)
            y_demean = y - np.average(y, weights=w)
            corr = ((np.sum(w * y_pred_demean * y_demean) / np.sum(w)) /
    ...
    value = tt_df.fillna(0).groupby("mddate").apply(_pearson)
    if mode == "avg":
        return np.abs(value.mean())
    else:
        return value


def _average_spearman(tt_df, column_name="y_pred_raw", mode="avg"):
    def _spearman(df):
        w = np.ones((df.shape[0],))
        y, y_pred = df["1Day_return"].copy(), df[column_name].copy()
        y_pred = np.apply_along_axis(rankdata, 0, y_pred)
    ...
    value = tt_df.fillna(0).groupby("mddate").apply(_spearman)
    if mode == "avg":
        return np.abs(value.mean())
    else:
        return value
...
average_pearson = _Fitness(function=_average_pearson,
                            greater_is_better=True)
weighted_spearman = _Fitness(function=_weighted_spearman,
                             greater_is_better=True)
average_spearman = _Fitness(function=_average_spearman,
                             greater_is_better=True)
...
_fitness_map = {'pearson': weighted_pearson,
                'apearson': average_pearson,
                'spearman': weighted_spearman,
                'aspearman': average_spearman,
