...
from .function_map import _user_function_map

from .utils import _partition_estimators
from .utils import check_random_state
import pandas as pd

from .customized_utils import get_sample_flag, gplearn_offical_alpha_seq
import logging
...
def _parallel_evolve(n_programs, parents, X, y, sample_weight, seeds, params, decap_df, data_index, restrict_type=False, terminal_types=[]):
    ...
    if "min_expression_len" in params.keys():
        min_expression_len = params['min_expression_len']
    else:
        min_expression_len = 1000

    max_samples = int(max_samples * n_samples)

    def _tournament(parents, output_type=None):
        """Find the fittest individual from a sub-population."""
        if output_type is None or output_type=="ALL":
            contenders = random_state.randint(0, len(parents), tournament_size)
            fitness = [parents[p].fitness_ for p in contenders]
        else:
            # condidates = [i for i in range(len(parents)) if parents[i].parameter]
            condidates = []
            for i, p in enumerate(parents):
                for node in p.program:
                    if isinstance(node, _Function):
                        if node.parameter_pair[-1] == output_type:
                            condidates.append(i)
                            break
                    else:
                        if terminal_types[node] == output_type:
                            condidates.append(i)
                            break
            contenders_indx = random_state.randint(0, len(condidates), min(tournament_size,len(condidates)))
            contenders = [condidates[p] for p in contenders_indx]
            # print(contenders)
            fitness = [parents[p].fitness_ for p in contenders]
            # print(condidates)
        if True:
    ...
    for i in range(n_programs):
        ...
        if parents is None:
            ...
            parent, parent_index = _tournament(parents)

            if method < method_probs[0]:
                # crossover
                donor, donor_index = _tournament(parents)
                if not restrict_type:
                    program, removed, remains = parent.crossover(donor.program,random_state)
                else:
                    program, removed, remains = parent.crossover(donor.program, random_state, parents, _tournament)
        ...
        # Draw samples, using sample weights, and then fitGen
        ...
        #try:
        logger.info(program)
        program.raw_fitness_ = max(program.raw_fitness(X, y, curr_sample_weight, decap_df, data_index), -999)
        if program.raw_fitness_ == np.nan:
            program.raw_fitness_ == -999
        #except:
        #logger.error("计算raw_fitness出现问题")
        #program.raw_fitness_ = -999

        if program._length() <= min_expression_len:
            program.raw_fitness_ = -999

        # if max_samples < n_samples:
        #     # Calculate OOB fitness
        #     program.oob_fitness_ = program.raw_fitness(X, y, oob_sample_weight, decap_df, data_index)
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        self.decap_file = pd.read_parquet(decap_file)
        ...
        if run_details is None:
            ...
            try:
                print(line_format.format(run_details['generation'][-1],
                                         ...
                                         oob_fitness,
                                         remaining_time))
                logger.info("gen: {}, avg_len: {}, avg_fitness: {}, best_length: {}, best_fitness: {}, oob_fitness: {}, remaining_time: {}".format(
                                         str(run_details['generation'][-1]),
                                         str(run_details['average_length'][-1]),
                                         str(run_details['average_fitness'][-1]),
                                         str(run_details['best_length'][-1]),
                                         str(run_details['best_fitness'][-1]),
                                         str(oob_fitness),
                                         str(remaining_time)))
            except:
                logger.error('report error')

    def fit(self, X, y, sample_weight=None, more_params=dict()):
        ...
        if n_components > hall_of_fame or n_components < 1:
            raise ValueError('n_components (%d) must be less than or equal to '
                             'hall_of_fame (%d).' % (self.n_components,
                                                     self.hall_of_fame))
        # 更新用户自定义的字典
        _function_map.update(_user_function_map)
        # print(_function_map)
        self._function_set = []
        ...
        params['_metric'] = _fitness_map["apearson"]
        ...
        data_index = more_params['data_index']
        
        # self.decap_file = self.decap_file.loc[data_index]
        # print("decap file len = {}".format(len(self.decap_file)))

        assert len(data_index) == len(X)

        # 定义自定义参数长度判断
        for key in more_params.keys():
            params[key] = more_params[key]
        ...
        for gen in range(prior_generations, self.generations):
            ...
            logger.info("GEN {} STARTS, n_jobs:{}, n_programs:{}, starts:{}".format(gen, n_jobs, n_programs, starts))
            if self.restrict_type:
                print("++++++++++", self.restrict_type)
            ...
            for sub_population in population:
                for individual in sub_population:
                    # if str(individual) in flag_population:
                    #     continue
                    flag_population.add(str(individual))
            ...
            # 记录best_program, 只记录了best program的fitness, 以及对应的oob的fitness
            self._top_best_programs = best_program
        ...
        if isinstance(self, TransformerMixin):
            ...
            print("Evaluate the Steadiness for alpha sequence")

            def _get_steady_utils(gp, X, y, decap_file:pd.DataFrame):
                y_pred_raw = gp.execute(X)
                alpha_seq = gplearn_offical_alpha_seq(y, y_pred_raw, None, decap_file, data_index)
                # steady = get_sample_flag(alpha_seq['top_10_alpha']
                return alpha_seq.values.reshape(-1)

            def _get_executed_value_utils(gp, X):
                tmp = gp.execute(X)
                return tmp 


            tmp = Parallel(n_jobs=n_jobs, verbose=int(self.verbose>1))(delayed(_get_executed_value_utils)(gp, X)  for gp in [self._programs[-1][i] for i in hall_of_fame])
            evaluation = np.array(tmp)
            ...
            length = min(self.hall_of_fame, len(evaluation))
            components = list(range(length))
