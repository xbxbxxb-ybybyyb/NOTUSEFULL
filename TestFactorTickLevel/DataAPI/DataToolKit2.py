...
def get_query_day_div_info(stock_code: str, query_date: int) -> dict:
    ...
    if query_date_month <= 6:  # 如查询日期发生在上半年（例如20180531），那么分红的报告期仅可能是上年年报（例如20171231）
        ...
        factor_data = xqf.get_factor_value("Basic_factor",
                                           ...
                                           [str(report_date)],
                                           ["per_div_trans", "div_cashpaidaftertax", "ex_dt"])
        if math.isnan(factor_data["per_div_trans"][0])==True and math.isnan(factor_data["div_cashpaidaftertax"][0])==True and type(factor_data["ex_dt"][0])==float:  # 如当年半年报期无分红信息，则查询上年年报期
            report_date = (query_date_year - 1) * 10000 + 1231
        factor_data = xqf.get_factor_value("Basic_factor",
                                           [stock_code],
                                           [str(report_date)],
                                           ["per_div_trans", "div_cashpaidaftertax", "ex_dt"])
    if math.isnan(factor_data["per_div_trans"][0])==True and math.isnan(factor_data["div_cashpaidaftertax"][0])==True and type(factor_data["ex_dt"][0])==float:  # 如无分红送转信息则返回全为0，ex_dt 为空时类型为float
        return {'per_cashpaidaftertax': 0, 'per_div_trans': 0, 'ex_dt': 0}
    elif math.isnan(factor_data["per_div_trans"][0])==False and math.isnan(factor_data["div_cashpaidaftertax"][0])==False and type(factor_data["ex_dt"][0])==float:   #如只公告了分红送转信息，但没有公告除权除息日
        return {'per_cashpaidaftertax': 0, 'per_div_trans': 0, 'ex_dt': 0}
    elif math.isnan(factor_data["per_div_trans"][0])==False and math.isnan(factor_data["div_cashpaidaftertax"][0])==False and type(factor_data["ex_dt"][0])==str: #如信息完整，则按信息填写
        div_info_report_date = {}
        if int(factor_data["ex_dt"][0])> query_date:  # 如除权除息日晚于查询日
            return {'per_cashpaidaftertax': 0, 'per_div_trans': 0, 'ex_dt': 0}
        else:
            div_info_report_date['per_cashpaidaftertax'] = float(factor_data["div_cashpaidaftertax"][0])
            div_info_report_date['per_div_trans'] = float(factor_data["per_div_trans"][0])
            div_info_report_date['ex_dt'] = int(factor_data["ex_dt"][0])   # 得到的日期是str, 要转为int
            if div_info_report_date["ex_dt"] == query_date:
...
def get_report_day_div_info(stock_code: str, report_date: int) -> dict:
    ...
    使用时务必小心
    :param stock_code: string, eg. '600549.SH' —— 股票代码
    :param report_date: int, eg. 20181231 —— 查询分红送转信息的日期（需要为年中或者年末）
...
def get_stock_latest_info(stock_code_list, basic_info_type) -> dict:
    ...
    factor_data = xqf.get_factor_value('Basic_factor', stock_code_list, [str(today_int)], [requested_basic_info])
    ...
    if basic_info_type == 'Listing_date':
        output_list = [int(item) for item in factor_data.values]
...
def read_h5_gogoal_data_original(table_name, key_name, start_date_int, end_date_int, stock_list, max_contype=2):
    ...
    if platform.system() == "Windows":
        ...
        gogoal_data_path = root_group + "/wdb_h5/WIND/"
        if table_name == 'FCD_CHINA_STOCK_DAILY_SUNTIME':
...
def get_auction_min_data(item, start_date, end_date, stock_list=None, forward_adj=True):
    ...
    if stock_list is not None:  # 如指定了输入的stock_list，则按其输出
        df = df.reindex(columns=stock_list)
    for i in range(df.shape[0]):  # 如有na则用当天的前值填充
        if  divmod(i, 10)[1] != 0:
            if i != 0:
                na_index = np.isnan(df.iloc[i, :])
                df.loc[df.index[i], na_index] = df.iloc[(i-1), :][na_index].values
