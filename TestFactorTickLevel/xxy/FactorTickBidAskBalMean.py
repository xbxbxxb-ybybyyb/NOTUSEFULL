...
class FactorTickBidAskBalMean(FactorBase):
...
#        print(Tick_data.keys())
        #################
        #以下为因子计算逻辑
        ans_bid = Tick_data['T_BidVolume'][-1][-1]/Tick_data['T_BidVolume'][-1][-8:-1].mean()

        ans_ask = Tick_data['T_AskVolume'][-1][-1]/Tick_data['T_AskVolume'][-1][-8:-1].mean()
        ans = Tick_data['T_Volume'].rolling(3, min_periods=3).sum()/(ans_ask/ans_bid)
