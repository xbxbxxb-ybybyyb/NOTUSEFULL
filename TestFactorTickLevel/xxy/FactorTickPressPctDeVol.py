...
class FactorTickPressPctDeVol(FactorBase):
...
#        print(Tick_data.keys())
        ...
        ans_shift = Tick_data['T_LastPrice'].rolling(20, min_periods=3).mean()/Tick_data['T_LastPrice'].rolling(3, min_periods=3).mean()
        ans = ans_bid/ans_ask * ans_shift * Tick_data['T_Volume'].rolling(20, min_periods=3).mean()/Tick_data['T_Volume'].rolling(3, min_periods=3).mean()
