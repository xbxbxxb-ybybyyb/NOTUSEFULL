...
class FactorTickBidAskBal(FactorBase):
...
#        print(Tick_data.keys())
        #################
        #以下为因子计算逻辑
        ans_bid = Tick_data['T_BidVolume'][-1][-1]/Tick_data['T_BidVolume'][-1][-3:-1].sum()

        ans_ask = Tick_data['T_AskVolume'][-1][-1]/Tick_data['T_AskVolume'][-1][-3:-1].sum()
        ans = Tick_data['T_Volume'].rolling(5, min_periods=3).sum()/(ans_bid/ans_ask)
