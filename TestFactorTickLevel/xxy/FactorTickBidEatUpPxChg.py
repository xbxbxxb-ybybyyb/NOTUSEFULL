...
class FactorTickBidEatUpPxChg(FactorBase):
...
#        print(Tick_data.keys())
        #################
        #以下为因子计算逻辑
        ans_vol = Tick_data['T_BidVolume'][-1][-3:-1].sum()/Tick_data['T_Volume'].rolling(20, min_periods=3).mean()
        ans_PxChg = Tick_data['T_Volume'].rolling(20, min_periods=3).max()/Tick_data['T_Volume'].rolling(20, min_periods=3).min()-1
